...
'comentario_desarrolladores "172.16.100.128", titulado, 0
...
if carrera <>"" then
   ...
   filtro_plan = " and pn.plan_ccod=(SELECT plan_ccod FROM SALIDAS_CARRERA WHERE saca_ncorr="&saca_ncorr&")"
end if

if peri_ccod = "" OR peri_ccod = "1" then
	periodo_por_defecto = "240"
end if
...
sql = " select top 1 protic.obtener_rut(a.pers_ncorr) as rut, protic.obtener_nombre_completo(a.pers_ncorr,'n')+ ',' as nombre, "& vbCrLf &_
 ...
 if peri_ccod <> "" and  peri_ccod <> "1" then
	if carrera <> "" then
	   sql = sql &  "     protic.es_alumno_nueva_version_prueba(" & pers_nrut & "," & peri_ccod & ",'" & carrera & "',1, "&saca_ncorr&") as CARRERA, "& vbCrLf &_
			        "     protic.es_alumno_nueva_version_prueba(" & pers_nrut & "," & peri_ccod & ",'" & carrera & "',2, "&saca_ncorr&") as DUAS_TDESC, "
	else
	   sql = sql &  "     protic.es_alumno_nueva_version_prueba(" & pers_nrut & "," & peri_ccod & ",'0',1, "&saca_ncorr&") as CARRERA, "& vbCrLf &_
			        "     protic.es_alumno_nueva_version_prueba(" & pers_nrut & "," & peri_ccod & ",'0',2, "&saca_ncorr&") as DUAS_TDESC, "
    end if 
 else
	if carrera <> "" then
	   sql = sql &  "     protic.es_alumno_nueva_version_prueba(" & pers_nrut & ","&periodo_por_defecto&",'" & carrera & "',1, "&saca_ncorr&") as CARRERA, "& vbCrLf &_
					"     protic.es_alumno_nueva_version_prueba(" & pers_nrut & ","&periodo_por_defecto&",'" & carrera & "',2, "&saca_ncorr&") as DUAS_TDESC, "
	else
	   sql = sql &  "     protic.es_alumno_nueva_version_prueba(" & pers_nrut & ","&periodo_por_defecto&",'0',1,"&saca_ncorr&") as CARRERA, "& vbCrLf &_
					"     protic.es_alumno_nueva_version_prueba(" & pers_nrut & ","&periodo_por_defecto&",'0',2,"&saca_ncorr&") as DUAS_TDESC, "
	end if
 end if
...
'if f_encabezado.obtenerValor("rut") <> "1" OR f_encabezado.obtenerValor("rut") <> "2" then 
'	titulado = "no"
'end if
...
'-----------------------------------------------------------------------------------------------------------------------------------------------
'-----------------------------------------------------------------------------------------------------------------------------------------------
sql22 = "SELECT * FROM SALIDAS_CARRERA WHERE SACA_NCORR="&saca_ncorr

set datos_variados = new CFormulario
datos_variados.Carga_Parametros "tabla_vacia.xml", "tabla"
datos_variados.Inicializar conexion
datos_variados.Consultar sql22
datos_variados.siguiente
carr_ccod22 = datos_variados.obtenerValor("carr_ccod")
'response.Write("<pre>"&datos_variados.obtenerValor("tsca_ccod")&"</pre>")
'response.end

if pers_nrut="18162121" then
carr_ccod22="800"
end if

if NOT(datos_variados.obtenerValor("tsca_ccod") = "1") AND NOT(datos_variados.obtenerValor("tsca_ccod") = "2") AND NOT(datos_variados.obtenerValor("tsca_ccod") = "5") AND NOT(datos_variados.obtenerValor("tsca_ccod") = "3") then
	titulado = "NO"
end if

'
'
'IF datos_variados.obtenerValor("tsca_ccod") = "4") OR datos_variados.obtenerValor("tsca_ccod") = "6") OR datos_variados.obtenerValor("tsca_ccod") = "7") THEN
'	HACER ALGO
'END IF

'
'if NOT((datos_variados.obtenerValor("tsca_ccod") = "1") OR (datos_variados.obtenerValor("tsca_ccod") = "2") OR (datos_variados.obtenerValor("tsca_ccod") = "3") OR (datos_variados.obtenerValor("tsca_ccod") = "5"))
'else
'	HACER ALGO
'END IF
'
'
'if carr_ccod22 = "840" then 'Este if estara mientras se actualiza la base de datos asignaturas_salidas_carrera
...
'								"		and a.emat_ccod=1 and cast(a.pers_ncorr as varchar)=protic.obtener_pers_ncorr(" & pers_nrut & ")")
'	
'	sql2 = " select distinct cast(horas as numeric(4)) as T,a.asig_ccod,b.asig_tdesc,case when c.sitf_ccod <> 'RI' then nota_final else NULL end as carg_nnota_final,anos_ccod as peri_ccod, "& vbCrLf &_
...
'	   " cantidad "& vbCrLf &_
'	   " from concentracion_notas a, asignaturas b,situaciones_finales c, asignaturas_salidas_carrera asca, MALLA_CURRICULAR mc, "& vbCrLf &_
'	   "(SELECT cast(e.asig_ccod as varchar)AS asignatura_origen, " & vbcrlf &_
'				"       cast(a.asig_ccod as varchar) AS asignatura_destino " & vbcrlf &_
...
'				"union " & vbcrlf &_
'				"	select " & vbcrlf &_
'				"		  cast(i.asig_ccod as varchar)as asignatura_origen ,   " & vbcrlf &_
'				"					cast(j.asig_ccod as varchar)as asignatura_destino " & vbcrlf &_
'				"	from " & vbcrlf &_
...
'				"		,asignaturas_salidas_carrera ascas " & vbcrlf &_
'				"	where " & vbcrlf &_
'				"		ascas.MALL_CCOD=k.MALL_CCOD " & vbcrlf &_
...
'				"		 and ascas.SACA_NCORR="&saca_ncorr&" ) as omega" & vbcrlf &_
...
'	   " and mc.PLAN_CCOD = a.plan_ccod"& vbCrLf &_
'	   " and asca.SACA_NCORR="&saca_ncorr&" -- ESTE ES VARIABLE"& vbCrLf &_
'	   " and asca.MALL_CCOD=mc.MALL_CCOD"& vbCrLf &_
'		" and (mc.ASIG_CCOD = b.ASIG_CCOD OR a.asig_ccod = omega.asignatura_destino)"& vbCrLf &_
...
'else
	sql2 = " select cast(horas as numeric(4)) as T,a.asig_ccod,b.asig_tdesc,case when c.sitf_ccod <> 'RI' then nota_final else NULL end as carg_nnota_final,anos_ccod as peri_ccod, "& vbCrLf &_
	   ...
	   " order by peri_ccod asc,b.asig_tdesc asc "
'end if
'comentario_desarrolladores "172.16.100.128",sql2, 1
...
valor_salida_sql = "SELECT COUNT(SACA_TDESC) FROM SALIDAS_CARRERA WHERE saca_ncorr IN (SELECT SACA_ncorr FROM alumnos_salidas_carrera WHERE pers_ncorr = protic.obtener_pers_ncorr("&pers_nrut&")) AND LINEA_1_CERTIFICADO LIKE '%LICENC%' AND saca_ncorr = "&saca_ncorr 
'response.write valor_salida_sql
valor_salida = conexion.consultaUno(valor_salida_sql)

'response.write valor_salida


if tiene_salida_intermedia <> 0 OR valor_salida > 0 then
	consulta_salida = "SELECT SACA_TDESC FROM SALIDAS_CARRERA WHERE saca_ncorr IN (SELECT SACA_ncorr FROM alumnos_salidas_carrera WHERE pers_ncorr = protic.obtener_pers_ncorr("&pers_nrut&"))  AND LINEA_1_CERTIFICADO LIKE '%LICENC%' AND saca_ncorr = "&saca_ncorr				 
	'response.write consulta_salida
	texto_salida = conexion.consultaUno(consulta_salida)
end if 
espacio="                                       "
espacio2="    "
...
pdf.Cell 180,0,jornada,"","","L"
	pdf.ln(10)
pdf.SetFont "times","",12
pdf.Cell 180,1,"La Secretaria General que suscribe certifica que el (la) Sr.(ta).","","","L" 
	pdf.ln(3)
pdf.SetFont "times","",12
'response.write tiene_salida_intermedia & " " & valor_salida
'response.end
'if tiene_salida_intermedia <> 0 OR valor_salida > 0 then
'	pdf.MultiCell 180,5,nombre&" obtuvo el Grado Académico de "&texto_salida&", "&carrera&" "&duas_tdesc&" calificaciones de acuerdo a la escala de uno a siete, siendo cuatro el mínimo de aprobación:","","","J" 
'else
...
wend
	...
	if titulado <> "SI" then
		...
		'comentario_desarrolladores "172.16.100.128", "OK", 1
		pdf.ln(7)
		pdf.SetFont "times","B",10
		pdf.SetFillColor(230) 
		pdf.SetX(30)
...
pdf.SetX(130)
pdf.Cell 50,0,"GLORIA FRANCISCA VARGAS UGALDE","","","C"   
pdf.SetY(-46)
pdf.SetFont "times","B",12
pdf.SetX(130)
pdf.Cell 50,0,"Secretaria General","","","C"   
	pdf.ln(1)
pdf.SetY(-40)
pdf.SetFont "times","",12
