...
botonera.carga_parametros "editar_evaluacion.xml", "btn_editar_evaluacion"
...
fecha_eval=request.QueryString("cali_fevaluacion")
...
'sql="select cali_ncorr from calificaciones_alumnos where cali_ncorr='"&cali_ncorr&"'"
'existe=conexion.consultauno(sql)

sql = "select count(*) " & vbCrLf &_
      "from calificaciones_seccion a, cargas_academicas b " & vbCrLf &_
	  "where a.secc_ccod = b.secc_ccod " & vbCrLf &_
	  "  and cast(a.cali_ncorr as varchar)= '"&cali_ncorr&"' " & vbCrLf &_
	  "  and b.sitf_ccod is not null" & vbCrLf &_
	  "  and b.estado_cierre_ccod = 2 "
existe=CInt(conexion.consultauno(sql))
		'if existe <> "" then
		'if existe > 0  and ip_usuario <> "172.16.11.67" or ip_usuario="172.16.11.205" and ip_usuario <> "172.16.11.61" and ip_usuario="172.16.11.147" then' and ip_usuario<>"172.16.11.143" and ip_usuario <> "172.16.11.147"  or ip_usuario="172.16.11.249" then
		%>
			<script language="JavaScript">
			//alert('No puede editar. \nExisten alumnos con situación final para esta sección . ');
...
f_asignatura.Carga_Parametros "editar_evaluacion.xml", "f_datos_asignaturas"
f_asignatura.Inicializar conexion

set f_evaluacion = new CFormulario
f_evaluacion.Carga_Parametros "editar_evaluacion.xml", "f_datos_evaluacion_cambio"
f_evaluacion.Inicializar conexion

dotos_asignatura=   " select a.asig_ccod,a.secc_tdesc," & _
...
SQL_plan = "  select  plan_ccod " & _
		   " from cargas_academicas a, alumnos b " & _
		   " where a.matr_ncorr = b.matr_ncorr " & _
		   " and cast(secc_ccod as varchar)='"&secc_ccod&"' " 

v_plan_ccod = conexion.consultauno(SQL_plan)

v_asig_ccod = conexion.consultauno("select asig_ccod from secciones where cast(secc_ccod as varchar)= '"&secc_ccod&"'")
sql = " select mall_nevaluacion_minima " & _
	  " from malla_curricular " & _
	  " where cast(plan_ccod as varchar)='"&v_plan_ccod &"'" & _
	  " and cast(asig_ccod as varchar)= '"&v_asig_ccod&"'"
	  
PMinima=conexion.consultauno(sql)
texto="Evaluaciones Minimas a Ingresar :"&PMinima&".  Segun El Reglamento Academico"
	
datos_eval= " select a.cali_nevaluacion,a.cali_nevaluacion AS cali_nevaluacionD,a.cali_nponderacion,protic.trunc(a.cali_fevaluacion) as cali_fevaluacion, b.teva_ccod,b.teva_tdesc " & _
			" from calificaciones_seccion a, tipos_evaluacion b " & _
			" where a.teva_ccod=b.teva_ccod and" &_
	  		" cast(a.cali_ncorr as varchar)='"&cali_ncorr&"' " 
...
'response.Write("suma "&suma &" ponderacion "&f_evaluacion.obtenervalor("cali_nponderacion"))
suma= cdbl(suma) - cdbl(f_evaluacion.obtenervalor("cali_nponderacion"))
'response.Write("<br>suma "&suma)
suma=conexion.consultaUno("select replace('"&suma&"',',','.')")
f_evaluacion.agregaCampoCons "cali_nponderacion",conexion.consultaUno("select replace('"&f_evaluacion.obtenervalor("cali_nponderacion")&"',',','.')")
'response.Write("<br>suma "&suma)
...
FPruebas=" select convert(datetime,cali_fevaluacion,103) as cali_fevaluacion " & _
		...
		"and cast(a.cali_ncorr as varchar) <>'"&cali_ncorr&"'" & _
...
sql=" select protic.trunc(min(cali_fevaluacion)) as cali_fevaluacion " & vbCrLf &_
	" from calificaciones_seccion " & vbCrLf &_
	" where secc_ccod='"&secc_ccod&"' " & vbCrLf &_
	" and  cali_fevaluacion>convert(datetime,'"&fecha_eval&"',103) " & vbCrLf &_
	" and teva_ccod<>'N' "
'response.Write("<pre>"&sql&"</pre>")
menordemayor=conexion.consultauno(sql)

'response.Write("1"&menordemayor&"<br>")

sql2=" select protic.trunc(max(cali_fevaluacion)) as cali_fevaluacion " & vbCrLf &_
	 " from calificaciones_seccion " & vbCrLf &_
	 " where cast(secc_ccod as varchar)='"&secc_ccod&"' " & vbCrLf &_
	 " and  cali_fevaluacion < convert(datetime,'"&fecha_eval&"',103) " & vbCrLf &_
	 " and teva_ccod<>'N' "
'response.Write("<pre>"&sql2&"</pre>")
mayordemenor=conexion.consultauno(sql2)				
	
'response.Write("2"&mayordemenor&"<br>")

if isnull(mayordemenor) then
	mayordemenor=conexion.consultaUno("select protic.trunc(secc_finicio_sec) from secciones where cast(secc_ccod as varchar)='"&secc_ccod&"'")
	'response.Write("select protic.trunc(secc_finicio_sec) from secciones where cast(secc_ccod as varchar)='"&secc_ccod&"'")
	'mayordemenor="01/01/2003"
end if

if isnull(menordemayor) then
	menordemayor= conexion.consultaUno("select protic.trunc(secc_ftermino_sec) from secciones where cast(secc_ccod as varchar)='"&secc_ccod&"'")
	'response.Write("select protic.trunc(secc_ftermino_sec) from secciones where cast(secc_ccod as varchar)='"&secc_ccod&"'")
end if
'response.Write("3"&mayordemenor&"<br>")
'response.Write("4"&menordemayor&"<br>")
...
'consulta_terminados = conexion.consultaUno("select count(*) from cargas_Academicas where isnull(sitf_ccod,'1')<> '1' and isnull(cast(carg_nnota_final as varchar),'N') <> 'N' and cast(secc_ccod as varchar)='"&secc_ccod&"'")
...
StrMAYME="<%=mayordemenor%>"
StrMEMAY="<%=menordemayor%>"
StrIng=formulario.elements["m[0][cali_fevaluacion]"].value
//alert("StrMAYME "+StrMAYME+" StrMEMAY "+StrMEMAY+" StrIng "+StrIng)
arrMAYMEBD=StrMAYME.split('/');
arrMEMAY=StrMEMAY.split('/');
arrFechaIng=StrIng.split('/');
var fechaMAYME= new Date(arrMAYMEBD[2],arrMAYMEBD[1],arrMAYMEBD[0])
var fechaMEMAY= new Date(arrMEMAY[2],arrMEMAY[1],arrMEMAY[0])
var fechaIng  = new Date(arrFechaIng[2],arrFechaIng[1],arrFechaIng[0])

if ((fechaMAYME < fechaIng ) && (fechaMEMAY>fechaIng)){
	return true;
}
else {return false;}
}
		...
		if (buscarfecha(formulario)==true){
			if (buscaevaluacion(formulario)==true){
				if(compfecha(formulario)==true){
					...
					NRegistros=parseInt("<%=nroRegistros%>") 
				...
				else{alert("imposible Guardar , La fecha Ingresada debe ser \n mayor a: "+a+" y menor a :"+b)}
			}
...
<table width="632" height="93%" border="0" cellpadding="0" cellspacing="0">
  <tr>
  </tr>
  <tr>
          ...
          <tr>
            <td><div align="center"><br>
                ...
                <table width="98%"  border="0" align="center" cellpadding="0" cellspacing="0">
                  <tr>
                    <td><div align="center"><strong><%=texto%>
                          <br>
                          </strong></div>
                      <table width="100%" border="0">
                            <tr> 
                            ...
                            <tr> 
                              ...
                              <td><%=f_evaluacion.dibujacampo("cali_fevaluacion")%> 
                                  ...
                                  <%calendario.DibujaImagen "fecha_oculta_fevaluacion","1","buscador" %>(dd/mm/yyyy)</td>
                            </tr>
							<tr> 
                              <td align="right">Número Resolución</td>
                              <td align="left"><%=f_evaluacion.dibujacampo("cali_nresolucion")%></td>
                            </tr>
							<tr> 
                              <td align="right">Observación de Cambio</td>
                              <td align="left"><%=f_evaluacion.dibujacampo("cali_tobservacion")%></td>
                            </tr>
                          </table>
                          <p><strong>N&uacute;mero de Evaluaciones Ya Registradas 
                            : <%=nroRegistros%></strong></p>
                          
                            <input name="secc_ccod" type="hidden" value="<%=secc_ccod%>">
                        <input name="cali_ncorr" type="hidden" value="<%=cali_ncorr%>">            </td>
                  </tr>
                </table>
                          <br>
            </form></td></tr>
            ...
            <td width="38%" height="20"><div align="center">
              <table width="90%"  border="0" cellspacing="0" cellpadding="0">
                <tr>
                  ...
                  <td><div align="center"><% if cint(total_alumnos) = cint(consulta_terminados) and secc_ccod <> "40847" then
