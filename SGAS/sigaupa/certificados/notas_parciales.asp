...
alumnos.carga_parametros			"notas.xml","cambio_alumnos"
...
'response.Write("seccion " & secc_ccod &" periodo "&periodo)
...
consulta_alumnos="select  " & vbCrlf & _
					...
					" replace(case cala_nnota when null then '1.0' when '1' then '1.0' when '2' then '2.0' when '3' then '3.0' when '4' then '4.0' when '5' then '5.0' when '6' then '6.0' when '7' then '7.0' else cala_nnota end,',','.') as cala_nnota_respaldo,  " & vbCrlf & _
					...
					"	where  -- b.emat_ccod in (1,2,4) and" & vbCrlf & _
					"		 c.carg_nsence is null  " & vbCrlf & _
					...
					"		and c.matr_ncorr not in (select matr_ncorr from convalidaciones where matr_ncorr=c.matr_ncorr and cast(asig_ccod as varchar)='"&asig_ccod&"') " 
...
consulta_nota	="  select cali_ncorr, " & vbCrlf & _
				"	cast(case cali_nevaluacion when null then 'PN' else cali_nevaluacion end as varchar) +' - '+ cast(convert(datetime,cali_fevaluacion,103)as varchar)+' - '+ teva_tdesc  as cali_nevaluacion  " & vbCrlf & _
				...
				"  	and cast(c.peri_ccod as varchar)='"&periodo&"'"
...
function dibujar(formulario){
	formulario.action='notas_parciales.asp';
	formulario.submit();
}
...
function cala_nnota_blur(objeto)
{
	...
	//alert(indice)
	//alert("not["+indice+"][cala_nnota_respaldo]");
	...
	{
		if (objeto.value != document.lista.elements["not["+indice+"][cala_nnota_respaldo]"].value)
			{ 
			  document.lista.elements["not["+indice+"][cala_nresolucion]"].disabled = false;
			  document.lista.elements["not["+indice+"][cala_tobservacion]"].disabled = false;
			  document.lista.elements["not["+indice+"][cala_nresolucion]"].id = "TO-S";
			  document.lista.elements["not["+indice+"][cala_tobservacion]"].id = "TO-N";
			}
	...
	<table width="88%"  border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#D8D8DE">
      ...
      <tr>
        ...
        <td><table width="100%"  border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td><%				
				pagina.DibujarLenguetas lenguetas_evaluacion, 2
				%></td>
          </tr>
          <tr>
                    ...
                    <tr> 
                      <td align="left"><p> 
                        ...
                        <p>Calificacion Seleccionada:<strong> 
                          <%
					  response.Write(conectar.consultauno("select 'Nº '+ cast(cali_nevaluacion as varchar)+ ' - ' + cast(protic.trunc(a.cali_fevaluacion)as varchar)+' - '+ teva_tdesc as evaluacion from calificaciones_seccion a, tipos_evaluacion b where a.teva_ccod=b.teva_ccod and cast(cali_ncorr as varchar)='"&cali_ncorr&"' "))
					  %>
                          </strong> 
                          <%end if%>
                        </p>
            ...
            <td width="38%" height="20"><div align="center">
              <table width="90%"  border="0" cellspacing="0" cellpadding="0">
                <tr>
                  ...
                  <td><div align="center"><%
				                         if no_permite > 0 and ponderacion=100 then%>
