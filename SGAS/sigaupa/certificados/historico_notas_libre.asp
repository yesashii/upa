...
botonera.Carga_Parametros "historico_notas_libre.xml", "botonera"
...
"where  cast(pers_nrut as varchar) = '"&rut&"'                              " & vbCrLf &_
"       and cast(c.peri_ccod as varchar) = '"&peri_ccod_temporal&"'         " 
...
f_busqueda.Carga_Parametros "historico_notas_libre.xml", "busqueda"
...
combo_b.carga_parametros	"historico_notas_libre.xml","combo"
...
resumen_he.carga_parametros		"historico_notas_libre.xml","resumen"		
...
'if plan_ccod <> "" then 
'		f_asignaturas.Carga_Parametros "historico_notas_libre.xml", "asignaturas"
'else
		f_asignaturas.Carga_Parametros "historico_notas_libre.xml", "asignaturas2"
'end if
...
sql_todas_asignaturas= " select distinct asig.asig_ccod,asig.asig_tdesc, " & vbcrlf &_
						...
						"	 2003, " & vbcrlf &_
						"	 isnull(cast(d.anos_ccod as varchar)+ ' 0'+ cast(d.plec_ccod as varchar),' ') as anos_ccod,e.duas_tdesc as duracion, " & vbcrlf &_
						"    case cast(b.sitf_ccod as varchar) when 'A' then 'Apr' when 'R' then 'Repr' when 'C' then 'Conv' when 'SP' then 'S.P' when 'H' then 'Hom' when 'S' then 'Suf' when 'RS' then 'RS' when 'RC' then 'RC' when 'RI' then 'RI' when 'CR' then 'ECR' end as sitf_ccod,  " & vbcrlf &_
						" b.secc_ccod,b.matr_ncorr,'"&rut&"' as pers_nrut, '"&plan&"' as plan_ccod, '"&especialidad &"' as espe_ccod,'"&carrera&"' as carr_ccod" & vbcrlf &_
						" From    asignaturas asig, secciones c, cargas_academicas b,periodos_academicos d,duracion_asignatura e" & vbcrlf &_
						" where asig.asig_ccod = c.asig_ccod" & vbcrlf &_
						" and b.secc_ccod=c.secc_ccod and isnull(b.carg_noculto,0) <> 1 " & vbcrlf &_
						" and c.peri_ccod=d.peri_ccod and asig.duas_ccod = e.duas_ccod " & vbcrlf &_
						...
						" union all " & vbcrlf &_           
						" select b.asig_ccod,b.asig_tdesc, " & vbcrlf &_
						...
						" 2003,isnull(cast(d.anos_ccod as varchar)+ ' 0'+ cast(d.plec_ccod as varchar),' ') as anos_ccod,e.duas_tdesc as duracion, " & vbcrlf &_
						" case cast(a.sitf_ccod as varchar) when 'A' then 'Apr' when 'R' then 'Repr' when 'C' then 'Conv' when 'SP' then 'S.P' when 'H' then 'Hom' when 'S' then 'Suf' when 'RS' then 'RS' when 'RC' then 'RC' when 'CR' then 'ECR' end as sitf_ccod,  " & vbcrlf &_
						" 0 as secc_ccod,al.matr_ncorr,'"&rut&"' as pers_nrut, '"&plan&"' as plan_ccod, '"&especialidad&"' as espe_ccod,'"&carrera&"' as carr_ccod " & vbcrlf &_
...
'response.Write("select plan_nresolucion from planes_estudio where cast(plan_ccod as varchar)='"&plan&"'")

fecha_actual = conectar.consultaUno("select protic.trunc(getDate())")

v_mes_actual	= 	Month(now())
if v_mes_actual <= 3 then
  ano_consulta = conectar.consultaUno("select datepart(year,getDate()) - 1 ")
else
  ano_consulta = conectar.consultaUno("select datepart(year,getDate())")
end if

es_cae = conectar.consultaUno("select case count(*) when 0 then 'NO' else 'SI' end  from ufe_alumnos_cae ttt where cast(ttt.anos_ccod as varchar)='"&ano_consulta&"' and esca_ccod=1 and cast(ttt.rut as varchar)= '"&rut&"'")
...
function enviar(formulario){
		if(!(valida_rut(formulario.elements["busqueda[0][pers_nrut]"].value + '-' + formulario.elements["busqueda[0][pers_xdv]"].value))){
			...
			formulario.action = 'historico_notas_libre.asp';
			return true
		}
		return false
}
			  ...
			  <table width="98%"  border="0" align="center" cellpadding="0" cellspacing="0">
                  <tr>
                    <td><%pagina.DibujarSubtitulo "Resultado de la Búsqueda"%>
                      <br>
					  <form name="editar" action="historico_notas_libre.asp">
                      <%if rut <> "" then %>
                      <table width="50%" cellspacing="0" cellpadding="0">
						...
						<%if es_cae="SI" then%>
								<tr><td nowrap>&nbsp;</td></tr>
								<tr><td nowrap align="center" bgcolor="#FF9900"><font face="Times New Roman, Times, serif" size="+1" color="#0033CC">ALUMNO(A) BENEFICIO CAE <%=ano_consulta%></font></td></tr>
						<%end if%>
                        <tr>
                          <td nowrap>&nbsp;</td>
                        </tr>
...
<br>
				...
				<td align="left">
				 <table width="60%" border="0" align="left" cellpadding="0" cellspacing="0">
                            ...
                            <tr> 
                              <td  width="3%" align="center"><strong>ERC</strong></td>
							  <td  width="5%" align="center"><strong>:</strong></td>
							  <td  align="left"> Asignatura Aprobada por Conocimientos Relevantes.</td>
                            </tr>
						</table>
						</td>
					</tr>
                ...
                <tr>
                  <td width="13%"><div align="center">
				   ...
				   <td width="14%"><div align="center"><%
					                       botonera.agregabotonparam "excel", "url", "notas_alumnos_excel.asp?rut="&rut
