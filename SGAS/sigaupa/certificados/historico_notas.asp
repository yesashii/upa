...
consulta_actual_plan=" select  distinct cast(f.espe_ccod as varchar)+ '-' + cast(f.plan_ccod as varchar)+ '-' + cast(d.carr_ccod as varchar) as parametro " & vbcrlf &_
						 ...
						 "	ON a.pers_ncorr = b.pers_ncorr AND cast(pers_nrut as varchar) ='"&rut&"'" & vbcrlf &_
						 "	INNER JOIN ofertas_academicas c " & vbcrlf &_
						 "	ON b.ofer_ncorr = c.ofer_ncorr AND cast(c.peri_ccod as varchar) ='"&peri_ccod_temporal&"'" & vbcrlf &_
...
sql_todas_asignaturas= " select distinct asig.asig_ccod,asig.asig_tdesc, " & vbcrlf &_
						...
						"	 2003, " & vbcrlf &_
						"	 case ('('+ cast(d.anos_ccod as varchar) + '/' + case plec_ccod  when 1 then 'O' when 2 then 'I' when 3 then 'P' when 4 then 'V' end +'/'+ cast(b.sitf_ccod as varchar)+')') " & vbcrlf &_
						"    when ('('+ cast(d.anos_ccod as varchar) + '/' + case plec_ccod  when 1 then 'O' when 2 then 'I' when 3 then 'P' when 4 then 'V' end +'/)') then ' ' " & vbcrlf &_
						"    when '(//)' then ' '" & vbcrlf &_
						"    else ('('+ cast(d.anos_ccod as varchar) + '/' + case plec_ccod  when 1 then 'O' when 2 then 'I' when 3 then 'P' when 4 then 'V' end +'/'+ cast(b.sitf_ccod as varchar)+')') end as anos_ccod,  " & vbcrlf &_
...
ultimo_estado =	conectar.consultauno("select top 1 emat_tdesc  " & vbcrlf & _
									"	from alumnos a, ofertas_academicas b,estados_matriculas c  " & vbcrlf & _
									"	where cast(a.pers_ncorr as varchar)='" & pers_ncorr &"' and a.emat_ccod=c.emat_ccod " & vbcrlf & _
									"	and a.ofer_ncorr = b.ofer_ncorr and cast(plan_ccod as varchar)='"&plan&"' and cast(b.peri_ccod as varchar)='"&ultimo_peri_ccod&"' order by a.audi_fmodificacion desc")
...
function enviar(formulario){
		if(!(valida_rut(formulario.elements["busqueda[0][pers_nrut]"].value + '-' + formulario.elements["busqueda[0][pers_xdv]"].value))){
			...
			formulario.action = 'historico_notas.asp';
			return true
		}
		return false
}
			  ...
			  <table width="98%"  border="0" align="center" cellpadding="0" cellspacing="0">
                  <tr>
                    <td><%pagina.DibujarSubtitulo "Resultado de la Búsqueda"%>
                      <br>
					  <form name="editar" action="historico_notas.asp">
                      <%if rut <> "" then %>
