...
if not esVacio(q_peri_ccod) then
	...
	'response.Write("<pre>"&consulta&"</pre>")
	consulta_carrera="(select distinct e.plan_ccod as carr_ccod, " & vbCrLf &_
					 " case (select count(*) from alumnos_salidas_carrera tt, salidas_carrera t1 " & vbCrLf &_
					 " where tt.pers_ncorr=a.pers_ncorr and tt.saca_ncorr=t1.saca_ncorr " & vbCrLf &_
					 " and t1.plan_ccod=e.plan_ccod ) when 0 then '' else '* ' end + " & vbCrLf &_
					 " ltrim(rtrim(d.carr_tdesc)) + ' - ' + ltrim(rtrim(e.plan_tdesc)) as carr_tdesc" & vbCrLf &_
					 ...
					 " and b.espe_ccod=c.espe_ccod  and a.plan_ccod = e.plan_ccod and emat_ccod not in (6,11)" & vbCrLf &_
...
consulta2 = " select tabla2.asig_ccod,tabla2.asig_tdesc,tabla3.carg_nnota_final,tabla2.peri_ccod,tabla3.anos_ccod,tabla3.plec_ccod,tabla3.sitf_ccod, " & vbCrLf &_
			...
			" (select case count(*) when 0 then 0 else 1 end from concentracion_notas cn where cn.asig_ccod=tabla2.asig_ccod and cast(cn.pers_ncorr as varchar)='"&pers_ncorr_temporal&"' and tabla3.anos_ccod = cn.anos_ccod and tabla3.sitf_ccod=cn.sitf_ccod " & vbCrLf &_
							  		...
							  		<table border="1" width="80%">
										...
										<tr>
											<td width="100%" align="center" bgcolor="#FFFFFF"><font color="#0033FF"><% 
											                                   if alumno_titulado = "0" and alumno_egresado = "0" then 'and alumno_si="0" then
                          ...
                          <td scope="col" colspan="2" align="center">
							...
							<tr>
								<td width="30" align="center"><div align="center">
								                                       <%if moroso = "Sí" or notas_guardadas = "0"   then
								...
								<td width="40" align="center">          <%if moroso = "Sí" or cantidad_notas = "0" or notas_guardadas = "0" then
																			'response.Write("es_moroso "&moroso&" cantidad_notas "&cantidad_notas&" notas_guardadas "&notas_guardadas)
