...
'ENTRADA		:NA
'SALIDA			:NA
'MODULO QUE ES UTILIZADO: MANTENEDOR ACADÉMICO 
'
'--ACTUALIZACION--
'
...
num_resolucion = conectar.consultaUno("select plan_nresolucion from planes_estudio where cast(plan_ccod as varchar)='"&plan&"'")
...
tablas=" select distinct " & vbCrLf & _
		...
		" isnull(g.cred_valor,0) as creditos  " & vbCrLf & _
		" from asignaturas a join malla_curricular b " & vbCrLf & _
		"    on a.asig_ccod = b.asig_ccod " & vbCrLf & _
		" join planes_estudio c " & vbCrLf & _
		"    on b.plan_ccod=c.plan_ccod " & vbCrLf & _
		" join  especialidades e " & vbCrLf & _
		"    on e.ESPE_CCOD=c.ESPE_CCOD" & vbCrLf & _
		" join  duracion_asignatura f " & vbCrLf & _
		"    on a.duas_ccod=f.duas_ccod  " & vbCrLf & _
		" left outer join creditos_asignatura g " & vbCrLf & _
		"    on a.cred_ccod = g.cred_ccod    " & vbCrLf & _
		" where b.mall_npermiso=0 " & vbCrLf & _
...
tabla_remedial=" select distinct " & vbCrLf & _
		" b.nive_ccod as nivel, b.mall_ccod,b.plan_ccod as plan_ccod, c.espe_ccod as espe_ccod " & vbCrLf & _
		" ,a.asig_ccod as asig_ccod, a.asig_tdesc as asignatura ,a.asig_nhoras as asig_nhoras,e.carr_ccod as carr_ccod,f.duas_tdesc as regimen " & vbCrLf & _
...
'---------------------------------------------------------remediales--------------------------------------------------
set fo_remedial 		= 		new cFormulario
fo_remedial.carga_parametros	"adm_mallas_curriculares3.xml",	"tabla_remedial"
fo_remedial.inicializar		conectar
fo_remedial.consultar 		tabla_remedial
'----------------------------------------------------------------------------------------------------------------------
...
'----------------------------------------------------------------------------------------------------------------
 set f_busqueda = new CFormulario
 f_busqueda.Carga_Parametros "adm_mallas_curriculares3.xml", "buscador"
 f_busqueda.inicializar conectar
...
if carrera <> "" and nivel <> "" then
	filtro = " cast(A.nive_ccod as varchar)='" & nivel & "' and b.plan_ccod=isnull(" & plan & ",0)"  
else
	filtro = " 1=2 "
end if
...
' "          and b.espe_ccod=c.espe_ccod " & vbCrLf & _
' "          and a.mall_npermiso=0 " & vbCrLf & _
' "          and " & filtro & vbCrLf & _
' "   ) a " & vbCrLf & _
...
' "    , a.carr_ccod " & vbCrLf
 ...
 "      from  " & vbCrLf & _
 "           malla_curricular a  " & vbCrLf & _
 "           INNER JOIN planes_estudio b  " & vbCrLf & _
 "           ON a.plan_ccod = b.plan_ccod and a.mall_npermiso = 0 " & vbCrLf & _
 "          and " & filtro & vbCrLf & _
 "           INNER JOIN especialidades c  " & vbCrLf & _
 "           ON b.espe_ccod = c.espe_ccod " & vbCrLf & _
 "   ) a " & vbCrLf & _
 "   INNER JOIN asignaturas b  " & vbCrLf & _
 "   ON a.asig_ccod = b.asig_ccod " & vbCrLf & _
 "   LEFT OUTER JOIN secciones c " & vbCrLf & _
 "   ON a.asig_ccod = c.asig_ccod   " & vbCrLf & _
 "    and C.sede_ccod = " & sede & vbCrLf & _
 "    and C.peri_ccod = " & peri & vbCrLf & _
 "  group by sede_ccod, b.asig_ccod, asig_tdesc,a.carr_ccod,a.nive_ccod,a.plan_ccod,a.espe_ccod " & vbCrLf & _
 "    , a.carr_ccod " & vbCrLf
'response.Write("<pre>"&consulta&"</pre>")
                  ...
                  <tr>
                    <td><%if carrera <> "" and especialidad  <> "" and plan <> "" then %>
                          <table width="627">
							...
							<tr> 
                              <td>Resolución</td>
                              <td>:<strong><%=num_resolucion%></strong></td>
                            </tr>
                          </table>
...
%>
					  ...
					  <br><%pagina.DibujarSubtitulo "Asignaturas Remediales"%>
                      <br>
                      <table width="100%" border="0">
                        <tr>
                          <td>&nbsp;</td>
