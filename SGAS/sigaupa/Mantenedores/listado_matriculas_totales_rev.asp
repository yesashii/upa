...
if autoriza_puntaje="S" then 			
		consulta = consulta &	" cast( isnull(cast(pos.post_npaa_verbal as varchar),'--')as varchar) as puntaje_verbal_real, "& vbCrLf &_
								" cast( isnull(cast(pos.post_npaa_matematicas as varchar),'--') as varchar) as puntaje_matematicas_real, "& vbCrLf &_
...
if agrega_carga <> "" then 			
			consulta = consulta & "	(select case count(*) when 0 then 'No' else 'Sí' end from cargas_academicas carg where carg.matr_ncorr=d.matr_ncorr) as con_carga, "& vbCrLf &_
...
consulta = consulta & "   isnull(cast(pos.post_nano_paa as varchar),'--') as ano_rindio_prueba,isnull(pos.post_tinstitucion_anterior,'--') as institucion_anterior, "& vbCrLf &_
			"   pai.pais_tdesc as pais,e.carr_ccod as cod_carrera,f.carr_tdesc as Carrera,e.espe_tdesc as especialidad,pl.plan_tdesc as plan_est,case c.post_bnuevo when 'N' then 'ANTIGUO' else 'NUEVO' end as tipo,case when talu_ccod=1 then '' when talu_ccod=2 then 'ALUMNO UPA DE INTERCAMBIO' when talu_ccod='ALUMNO EXTRANJERO DE INTERCAMBIO' end as tipo_intercambio, "& vbCrLf &_
			"   g.jorn_tdesc as jornada ,h.sede_tdesc as sede,ARA.ARAN_MMATRICULA,ARA.ARAN_MCOLEGIATURA,protic.ano_ingreso_carrera_egresa2(a.pers_ncorr,e.carr_ccod) as ano_ingreso,isnull(cast(protic.PROMEDIO_MEDIA(a.pers_ncorr) as varchar),'') as promedio_media, "& vbCrLf &_
			"   ( select cast(pers_nnota_ens_media as varchar) from personas tt where tt.pers_nrut=a.pers_nrut) as promedio_media2, "& vbCrLf &_
		    ...
		    "           from alumnos mm,observaciones_estado_matricula om,ofertas_academicas ccc,periodos_academicos ddd, especialidades eee "& vbCrLf &_ 
			...
			consulta = consulta & "	,(select case count(*) when 0 then 'No' else 'Sí' end from documentos_postulantes aa where aa.pers_ncorr=a.pers_ncorr and aa.doma_ccod = 1) as Ced_identidad, "& vbCrLf &_
			 "	(select case count(*) when 0 then 'No' else 'Sí' end from documentos_postulantes aa where aa.pers_ncorr=a.pers_ncorr and aa.doma_ccod = 2) as Lic_Enseñanza_Media,"& vbCrLf &_
			 "	(select case count(*) when 0 then 'No' else 'Sí' end from documentos_postulantes aa where aa.pers_ncorr=a.pers_ncorr and aa.doma_ccod = 3) as Conc_de_notas_Enseñanza_Media, "& vbCrLf &_
			 "	(select case count(*) when 0 then 'No' else 'Sí' end from documentos_postulantes aa where aa.pers_ncorr=a.pers_ncorr and aa.doma_ccod = 4) as Puntaje_PSU, "& vbCrLf &_
			 "	(select case count(*) when 0 then 'No' else 'Sí' end from documentos_postulantes aa where aa.pers_ncorr=a.pers_ncorr and aa.doma_ccod = 6) as Fotografias,	"& vbCrLf &_
			 "	(select case count(*) when 0 then 'No' else 'Sí' end from documentos_postulantes aa where aa.pers_ncorr=a.pers_ncorr and aa.doma_ccod = 7) as Certificado_Residencia,"& vbCrLf &_
			 "	(select case count(*) when 0 then 'No' else 'Sí' end from documentos_postulantes aa where aa.pers_ncorr=a.pers_ncorr and aa.doma_ccod = 8) as Seguro_Salud"
			end if
			...
			" and d.emat_ccod not in (9) and f.tcar_ccod=1 and isnull(d.alum_nmatricula,0) <> 7777"& vbCrLf &_
			" and i.tdir_ccod = 1 "& vbCrLf &_
			" and exists (select 1 from datos_globales_bi_alumnos tt where tt.rut=(cast(a.pers_nrut as varchar)+'-'+a.pers_xdv) and MATRICULADO_31_ABRIL=1 and convert(datetime,protic.trunc(d.alum_fmatricula),103)=tt.alum_fmatricula) "& vbCrLf &_
			" "& filtro_periodo & vbCrLf &_
			" and exists (select 1 from [sigaupa_abril_2009].[protic].[contratos] cont1, [sigaupa_abril_2009].[protic].[compromisos] comp1 where d.post_ncorr=cont1.post_ncorr and d.matr_ncorr=cont1.matr_ncorr and cont1.cont_ncorr=comp1.comp_ndocto and tcom_ccod in (1,2) ) "
...
if post_nano_paa <> "" then	
		 	consulta = consulta & " AND cast(pos.POST_NANO_PAA as varchar) = '" & post_nano_paa & "' "& vbCrLf	
end if
...
'response.Write("<pre>"&consulta&" order by sede,carrera,AP_Paterno,AP_Materno,Nombre </pre>")
'response.End()
...
<table width="100%" border="1" cellpadding="0" cellspacing="0">
  <tr>
	...
	<td bgcolor="#FFFFCC"><div align="center"><strong>PROMEDIO ENS. MEDIA 2</strong></div></td>
  ...
  <tr>
	...
	<td><%=f_listado.ObtenerValor("promedio_media2")%></td>
