	 ...
	 if RUT <> "" then
	   ...
	   if es_alumno <> "0" then
		'############################################################################################
			...
			sql_carr_ccod = "SELECT ltrim(rtrim(carr_ccod)) FROM alumnos a, ofertas_academicas b, especialidades c WHERE a.ofer_ncorr=b.ofer_ncorr and b.espe_ccod=c.espe_ccod and cast(a.pers_ncorr as varchar)='"&v_pers_ncorr&"' and b.peri_ccod in (230) and a.emat_ccod in (1,4)"
            carr_ccod = conexion2.consultaUno(sql_carr_ccod)
			sql_sede_ccod = "SELECT sede_ccod FROM alumnos a, ofertas_academicas b WHERE a.ofer_ncorr=b.ofer_ncorr and cast(a.pers_ncorr as varchar)='"&v_pers_ncorr&"' and b.peri_ccod in (230) and a.emat_ccod=1"
			...
			'end if
			'if sede_ccod <> "4" then 
				if (v_mes_actual = 2 and v_dia_actual >= 01)  or  (v_mes_actual = 3 and v_dia_actual <= 28)  then
						cerrar_proceso  = false
				end if
