...
if matr_ncorr <> "" then
	...
	if tipo_plan = "0" then
		...
		filtro_plan = " and isnull(plan_tcreditos,0)=0"
		pagina.Titulo = "Toma de Carga Extraordinaria (Secciones)"
	end if	
...
asig_disponibles= "select distinct c.asig_ccod, mall_ccod, cast(c.asig_ccod as  varchar)+ ' - ' + c.asig_tdesc as asignatura, asig_nhoras, b.secc_ccod, " & vbCrLf & _
				  ...
				  "                  and isnull(ac.sitf_ccod,'1')= '1' and cast(se.peri_ccod as varchar)= '" & peri_ccod & "')" & vbCrLf & _
...
cons_resumen="select "& vbCrLf & _
			...
			"	 and exists ( select 1 from cargas_academicas aa where aa.matr_ncorr=c.matr_ncorr and aa.secc_ccod=c.secc_ccod and acse_ncorr=1) "& vbCrLf & _
