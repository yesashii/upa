...
filtro = "(select distinct datepart(year,co.comp_fdocto) ano, datepart(year,co.comp_fdocto) ano2 "& vbCrLf &_
			...
			" where co.comp_ndocto=ab.comp_ndocto "& vbCrLf &_
			" and ab.peri_ccod='"&Periodo&"'"& vbCrLf &_
			"  and convert(datetime,co.comp_fdocto,103)>=convert(datetime,'01/12/2003',103)) a	"
...
consulta = "select cast(pp.pers_nrut as varchar)  + '-' + pp.pers_xdv pers_nrut,pp.pers_nrut pers_nrut2,"& vbCrLf &_
			...
			if mes<>"" and ano<>"" then 			   
			   if mes=01 then	
			     consulta=consulta & "and convert(datetime,co.comp_fdocto,103) like '%/01/%' and datepart(year,co.comp_fdocto)='"&ano&"'"
			   end if 
			   if mes=02 then				 				      		     
				 consulta=consulta & "and convert(datetime,co.comp_fdocto,103) like '%/02/%' and datepart(year,co.comp_fdocto)='"&ano&"'"			    
			   end if
			   if mes=03 then				 						     		     
				 consulta=consulta & "and convert(datetime,co.comp_fdocto,103) like '%/03/%' and datepart(year,co.comp_fdocto)='"&ano&"'"				 				 
			   end if 
			   if mes=04 then				 				 
				 consulta=consulta & "and convert(datetime,co.comp_fdocto,103) like '%/04/%' and datepart(year,co.comp_fdocto)='"&ano&"'"				 				 
			   end if
			   if mes=05 then				 				 		     		     
				 consulta=consulta & "and convert(datetime,co.comp_fdocto,103) like '%/05/%' and datepart(year,co.comp_fdocto)='"&ano&"'"				 				 
			   end if
			   if mes=06 then				 				 
				 consulta=consulta & "and convert(datetime,co.comp_fdocto,103) like '%/06/%' and datepart(year,co.comp_fdocto)='"&ano&"'"				 				 
			   end if
			   if mes=07 then				 				 
				 consulta=consulta & "and convert(datetime,co.comp_fdocto,103) like '%/07/%' and datepart(year,co.comp_fdocto)='"&ano&"'"				 				 
			   end if
			   if mes=08 then				 				 
				 consulta=consulta & "and convert(datetime,co.comp_fdocto,103) like '%/08/%' and datepart(year,co.comp_fdocto)='"&ano&"'"				 				 
			   end if
			   if mes=09 then				 				 
				 consulta=consulta & "and convert(datetime,co.comp_fdocto,103) like '%/09/%' and datepart(year,co.comp_fdocto)='"&ano&"'"				 				 
			   end if
			   if mes=10 then				 				 
				 consulta=consulta & "and convert(datetime,co.comp_fdocto,103) like '%/10/%' and datepart(year,co.comp_fdocto)='"&ano&"'"				 				 
			   end if
			   if mes=11 then				 				 
				 consulta=consulta & "and convert(datetime,co.comp_fdocto,103) like '%/11/%' and datepart(year,co.comp_fdocto)='"&ano&"'"				 				 
			   end if
			   if mes=12 then				 				 
				 consulta=consulta & "and convert(datetime,co.comp_fdocto,103) like '%/12/%' and datepart(year,co.comp_fdocto)='"&ano&"'"				 				 
			   end if
			end if
			...
			if mes<>"" and ano="" then 			   
			   if mes=01 then	
			     consulta=consulta & "and convert(datetime,co.comp_fdocto,103) like '%/01/%'"
			   end if 
			   if mes=02 then				 				      		     
				 consulta=consulta & "and convert(datetime,co.comp_fdocto,103) like '%/02/%'"			    
			   end if
			   if mes=03 then				 						     		     
				 consulta=consulta & "and convert(datetime,co.comp_fdocto,103) like '%/03/%'"				 				 
			   end if 
			   if mes=04 then				 				 
				 consulta=consulta & "and convert(datetime,co.comp_fdocto,103) like '%/04/%'"				 				 
			   end if
			   if mes=05 then				 				 		     		     
				 consulta=consulta & "and convert(datetime,co.comp_fdocto,103) like '%/05/%'"				 				 
			   end if
			   if mes=06 then				 				 
				 consulta=consulta & "and convert(datetime,co.comp_fdocto,103) like '%/06/%'"				 				 
			   end if
			   if mes=07 then				 				 
				 consulta=consulta & "and convert(datetime,co.comp_fdocto,103) like '%/07/%'"				 				 
			   end if
			   if mes=08 then				 				 
				 consulta=consulta & "and convert(datetime,co.comp_fdocto,103) like '%/08/%'"				 				 
			   end if
			   if mes=09 then				 				 
				 consulta=consulta & "and convert(datetime,co.comp_fdocto,103) like '%/09/%'"				 				 
			   end if
			   if mes=10 then				 				 
				 consulta=consulta & "and convert(datetime,co.comp_fdocto,103) like '%/10/%'"				 				 
			   end if
			   if mes=11 then				 				 
				 consulta=consulta & "and convert(datetime,co.comp_fdocto,103) like '%/11/%'"				 				 
			   end if
			   if mes=12 then				 				 
				 consulta=consulta & "and convert(datetime,co.comp_fdocto,103) like '%/12/%'"				 				 
			   end if
			end if
			...
			if mes="" and ano<>"" then 			   			   
			     consulta=consulta & "and datepart(year,co.comp_fdocto)='"&ano&"'"			   
			end if
