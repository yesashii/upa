...
if estado_alumno = "" and sede_ccod <> "" then
	estado_alumno = "2"
end if
...
f_botonera.Carga_Parametros "seguimiento_matriculas.xml", "botonera"
...
lista.carga_parametros "seguimiento_matriculas.xml", "list_alumnos_matriculados"
...
lista.carga_parametros "seguimiento_matriculas.xml", "list_alumnos"
...
consulta=" select distinct protic.FORMAT_RUT(cast(a.pers_nrut as varchar(10))) as rut,protic.trunc(c.audi_fmodificacion) as fecha," & vbCrLf &_ 
		 ...
		 " and d.espe_ccod in (Select espe_ccod from sis_especialidades_usuario where pers_ncorr='"&pers_ncorr_encargado&"')" & vbCrLf &_
 		 " and not exists (select 1 from alumnos ccc where ccc.post_ncorr=b.post_ncorr) " & vbCrLf &_
         " and d.espe_ccod = e.espe_ccod "
		 if carr_ccod <> "" and carr_ccod <> "-1" then
         	consulta= consulta & " and cast(f.carr_ccod as varchar)= '"&carr_ccod&"'" 
		 end if
		 ...
		 consulta=consulta & " and isnull(c.eepo_ccod,1) = 2 "&vbCrlf &_
         " and b.epos_ccod = 1 " 
end if
...
if lista.nroFilas > 0 then
	...
	cantidad_encontrados=conexion.consultaUno("Select Count(distinct rut) from ("&consulta&")a")
...
'----------------------------------------------------------------------- 
 ...
 f_busqueda.Carga_Parametros "seguimiento_matriculas.xml", "f_busqueda"
...
if pers_ncorr_encargado <> "24289" then 
...
else 'Para que pueda acceder al sistema Don Arturo Rodriguez
	consulta_carreras = "Select distinct c.carr_ccod,c.carr_tdesc,a.sede_ccod" & vbCrLf &_ 
						" from ofertas_academicas a, especialidades b,carreras c" & vbCrLf &_ 
						" where a.post_bnuevo in ('S','N')" & vbCrLf &_ 
						" and cast(a.peri_ccod as varchar)='"&v_peri_ccod&"'" & vbCrLf &_ 
						" and a.espe_ccod=b.espe_ccod" & vbCrLf &_
						" and a.espe_ccod in (Select espe_ccod from sis_especialidades_usuario where pers_ncorr='"&pers_ncorr_encargado&"')" & vbCrLf &_
						" and b.carr_ccod=c.carr_ccod" 
...
formulario.action="seguimiento_matriculas.asp";
formulario.submit();
}
...
document.buscador.action="seguimiento_matriculas.asp";
...
function graficos()
{
   self.open('http://216.72.170.71/postulacion/www/graficos_admision.php','gráficos','width=800px, height=550px, scrollbars=yes, resizable=yes')
}
                    ...
                    <table width="100%"  border="0" cellspacing="0" cellpadding="0">
					  ...
					  <tr>
                        ...
                        <td colspan="2"><select name='estado_ccod'>
									...
									<%end if%>
							    	<%if estado_ccod="9" then%>
									<option value='9' selected>POSTULADO SIN RENDIR</option>
									<%else%>
									<option value='9' >POSTULADO SIN RENDIR</option>
									<%end if%>
									<%if estado_ccod="6" then%>
									<option value='6' selected>APROBADO SIN COMPLETAR</option>
									<%else%>
									<option value='6' >APROBADO SIN COMPLETAR</option>
									<%end if%>
									<%if estado_ccod="7" then%>
									<option value='7' selected>FICHA COMPLETA, SIN MATRICULAR</option>
									<%else%>
									<option value='7' >FICHA COMPLETA, SIN MATRICULAR</option>
									<%end if%>
									<%if estado_ccod="4" then%>
									...
									<%end if%>
									<%if estado_ccod="2" then%>
									<option value='2' selected>ALUMNO UPA DE INTERCAMBIO</option>
									<%else%>
									<option value='2' >ALUMNO UPA DE INTERCAMBIO</option>
									<%end if%>
									<%if estado_ccod="3" then%>
									<option value='3' selected>ALUMNO EXTRANJERO DE INTERCAMBIO</option>
									<%else%>
									<option value='3' >ALUMNO EXTRANJERO DE INTERCAMBIO</option>
									<%end if%>
                           </select> 
						 </td>  
          ...
          <tr>
            <td><div align="center"> 
			  ...
			  <input type="hidden" name="paso" value="<%=busqueda%>">
                  ...
                  <tr valign="middle">
                    <td><!--<div align="center">
					               <table width="40%" cellpadding="0" cellspacing="0" bgcolor="#ccd080" border="1" bordercolor="#666666">
								   <tr>
								   		<td width="100%" align="center"><br><%'f_botonera.dibujaboton "impr_graficos"%><br></td>
								   </tr>
								   </table>
						</div>-->
					    <strong>Cantidad Encontrados :&nbsp;&nbsp;</strong><%=cantidad_encontrados%>&nbsp; Alumnos
              ...
              <table width="90%"  border="0" cellspacing="0" cellpadding="0">
                <tr>
				  ...
				  <td width="29%"> <div align="center">  <% if cantidad_encontrados = 0 then
					                       ...
					                       f_botonera.agregabotonparam "excel", "url", "seguimiento_matriculas_excel.asp?sede_ccod="&sede_ccod&"&jorn_ccod="&jorn_ccod&"&carr_ccod="&carr_ccod&"&estado_ccod="&estado_ccod&"&estado_alumno="&estado_alumno&"&inicio="&inicio&"&termino="&termino
