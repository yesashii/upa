...
sql_info= "select top 1 a.paie_ncorr,cast(pers_nrut as varchar)+'-'+pers_xdv as rut,e.dpie_quiere_curso_espanol,p.pers_tpasaporte,p.pers_tape_paterno,p.pers_tape_materno,p.PERS_TNOMBRE,p.PERS_FNACIMIENTO,d.diai_tfono,p.PERS_TEMAIL,case p.SEXO_CCOD when 1 then 'MASCULINO' else 'FEMENINO' end  as SEXO,"& vbCrLf &_
...
sql_lenguaje="select m.paie_ncorr,i.idio_tdesc, n.nidi_tdesc ,m.maes_semestres_espanol "& vbCrLf &_
			...
			" and cast(m.idio_ccod as varchar) = cast(i.idio_ccod as varchar) "& vbCrLf &_
			" and cast(m.nidi_ccod as varchar) = cast(n.nidi_ccod as varchar) "& vbCrLf &_
			"and po.pers_ncorr ="&q_pers_ncorr&""
'response.Write("<pre>"&sql_lenguaje&"</pre>")
f_antec_extranjeros2.Consultar sql_lenguaje
f_antec_extranjeros2.siguiente
'-- Fin (datos alumno ) -------------------------------------------
'response.write("b[0]periodo")
%>
<html>
<head>
