...
Response.AddHeader "Content-Disposition", "attachment;filename=listado_alumnos_upa_al_extranjero.xls"
...
anos_ccod =Request.form("c[0][anos_ccod]")
peri_ccod =Request.form("c[0][peri_ccod]")
carr_ccod =Request.form("c[0][carr_ccod]")
univ_ccod =Request.form("c[0][univ_ccod]")
pais_ccod =Request.form("c[0][pais_ccod]")
facu_ccod =Request.form("c[0][facu_ccod]")
anos_ccod_fin =Request.form("c[0][anos_ccod_fin]")
'response.End()
'---------------------------------------------------------------------------------------------------
...
'if  peri_ccod <>""  then
'filtro_doc=filtro_doc&" and c.peri_ccod="&peri_ccod&""
'end if
...
if  carr_ccod <>"" then
filtro_doc=filtro_doc&" and protic.obtener_nombre_carrera((select top 1 OFER_NCORR from alumnos x where x.pers_ncorr = tabla.pers_ncorr), 'CC')="&carr_ccod&""
end if
...
if univ_ccod<>"" then
filtro_doc=filtro_doc&" and tabla.univ_ccod="&univ_ccod&""
end if
 '********************OK 
if pais_ccod<>"" then
filtro_doc=filtro_doc&" and tabla.pais_ccod="&pais_ccod&""
end if
...
if facu_ccod = "1" then
filtro_doc=filtro_doc&" and protic.obtener_nombre_carrera((select top 1 OFER_NCORR from alumnos x where x.pers_ncorr = tabla.pers_ncorr), 'CC') in (51)"
end if
if facu_ccod = "2" then
filtro_doc=filtro_doc&" and protic.obtener_nombre_carrera((select top 1 OFER_NCORR from alumnos x where x.pers_ncorr = tabla.pers_ncorr), 'CC') in (17,21,22)"
end if
if facu_ccod = "3" then
filtro_doc=filtro_doc&" and protic.obtener_nombre_carrera((select top 1 OFER_NCORR from alumnos x where x.pers_ncorr = tabla.pers_ncorr), 'CC') in (14,32,41,45,47,800,970)"
end if
if facu_ccod = "4" then
filtro_doc=filtro_doc&" and protic.obtener_nombre_carrera((select top 1 OFER_NCORR from alumnos x where x.pers_ncorr = tabla.pers_ncorr), 'CC') in (43,860,880,950,870,49)"
end if
if facu_ccod = "8" then
filtro_doc=filtro_doc&" and protic.obtener_nombre_carrera((select top 1 OFER_NCORR from alumnos x where x.pers_ncorr = tabla.pers_ncorr), 'CC') in (830,840,850,108)"
end if
...
'filtro_doc=filtro_doc&" and c.peri_ccod BETWEEN "&anos_perio_a&" AND "&anos_perio_b&""
'end if

if  anos_ccod_fin <>"" and anos_ccod_fin <>""  then
filtro_doc=filtro_doc&" and tabla.peri_ccod BETWEEN "&anos_perio_a&" AND "&anos_perio_b&""
end if
'**********************************************************************************
...
sql_descuentos= " select *, (select top 1 anos_ccod from periodos_academicos pac where pac.peri_ccod=tabla.peri_ccod) as anos_ccod,"& vbCrLf &_
				" (select  peri_tdesc from periodos_academicos pac where pac.peri_ccod=tabla.peri_ccod) as peri_tdesc "& vbCrLf &_
				" From ( "& vbCrLf &_
				"select protic.obtener_rut(a.pers_ncorr) as rut,pers_tnombre+' '+pers_tape_paterno+' '+pers_tape_materno as nombre,a.pers_ncorr,"& vbCrLf &_
				"pais_tdesc,ciex_tdesc,univ_tdesc,e.univ_ccod,f.pais_ccod,"& vbCrLf &_
				"case when c.peri_ccod is null then (select peri_ccod from ofertas_academicas where ofer_ncorr in ((select top 1 OFER_NCORR from alumnos x where x.pers_ncorr = a.pers_ncorr))) else c.peri_ccod end as peri_ccod, "& vbCrLf &_
				"(select top 1 lower(email_nuevo) from cuentas_email_upa tt where tt.pers_ncorr=a.pers_ncorr) as email_upa,pers_temail as email_personal,"& vbCrLf &_
				"protic.obtener_nombre_carrera((select top 1 OFER_NCORR from alumnos x where x.pers_ncorr = a.pers_ncorr and x.EMAT_CCOD=1), 'C') as carrera "& vbCrLf &_
				"from personas a join (select distinct pers_ncorr from alumnos where matr_ncorr in ("& vbCrLf &_
				...
				"and talu_ccod in (2)) as b"& vbCrLf &_
				"    on a.pers_ncorr=b.pers_ncorr"& vbCrLf &_
				"left outer join rrii_postulacion_alumnos_intercambio_upa c"& vbCrLf &_
				...
				"    on g.pais_ccod=f.pais_ccod"& vbCrLf &_
...
<table width="100%" border="1">
  ...
  <tr>
    ...
    <td width="23%" bgcolor="#99CC33"><div align="center"><strong>Carrera Destino</strong></div></td>
    <td width="15%" bgcolor="#99CC33"><div align="center"><strong>Periodo Academico</strong></div></td>  
    <td width="15%" bgcolor="#99CC33"><div align="center"><strong>Anio Pasantia</strong></div></td>         
    <td width="23%" bgcolor="#99CC33"><div align="center"><strong>Universidad Destino</strong></div></td>
	<td width="17%" bgcolor="#99CC33"><div align="center"><strong>Ciudad Destino</strong></div></td>
	<td width="19%" bgcolor="#99CC33"><div align="center"><strong>Pais Destino</strong></div></td>			    
  ...
  <tr> 
    ...
    <td valign="top"><div align="center"><%=f_resumen_convenio.ObtenerValor("anos_ccod")%></div></td> 
