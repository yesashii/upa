...
formulario.Carga_Parametros "Envios_Banco.xml", "f_enviar"
...
for fila = 0 to formulario.CuentaPost - 1
   ...
   if envio <> "" then
      SQL = "select count(a.envi_ncorr) as total_doc from envios a,  detalle_envios b where a.envi_ncorr = b.envi_ncorr "&_  
	   ...
	   if cantidad <> 0 then
	 	  consulta = "UPDATE detalle_envios SET edin_ccod = 4, AUDI_TUSUARIO='" & audi_tusuario  & "', audi_fmodificacion = getDate() WHERE edin_ccod not in (6,11) and cast(envi_ncorr as varchar)='" & envio & "'"
          conexion.EstadoTransaccion conexion.EjecutaS(consulta)
		  consulta = "UPDATE detalle_ingresos SET edin_ccod = 4, AUDI_TUSUARIO='" & audi_tusuario  & "', audi_fmodificacion = getDate() WHERE edin_ccod not in (6,11) and cast(envi_ncorr as varchar)='" & envio & "'"
          conexion.EstadoTransaccion conexion.EjecutaS(consulta)	      			  
	   else
