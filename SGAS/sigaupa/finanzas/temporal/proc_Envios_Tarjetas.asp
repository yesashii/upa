...
formulario.Carga_Parametros "Envios_Tarjetas.xml", "f_enviar"
formulario.Inicializar conexion
formulario.ProcesaForm
formulario.AgregaCampoPost "eenv_ccod" , 2
'formulario.ListarPost
'ACTUALIZO LOS DETALLES DEL INGRESO A 'EN NOTARIA'
...
for fila = 0 to formulario.CuentaPost - 1
   ...
   if envio <> "" then
      ...
      if cantidad <> 0 then
	     consulta = "UPDATE detalle_ingresos SET edin_ccod = 6, AUDI_TUSUARIO='" & audi_tusuario  & "', audi_fmodificacion = getdate()  WHERE envi_ncorr='" & envio & "'"
         conexion.EstadoTransaccion conexion.EjecutaS(consulta)	
	     consulta = "UPDATE detalle_envios SET edin_ccod = 6, AUDI_TUSUARIO='" & audi_tusuario  & "', audi_fmodificacion = getdate()  WHERE envi_ncorr='" & envio & "'"
         conexion.EstadoTransaccion conexion.EjecutaS(consulta)			
	  else
