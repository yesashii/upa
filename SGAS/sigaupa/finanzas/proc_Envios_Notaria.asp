...
formulario.Carga_Parametros "Envios_Notaria.xml", "f_enviar"
...
for fila = 0 to formulario.CuentaPost - 1
   ...
   if envio <> "" then
      ...
      if cantidad <> 0 then
	     consulta = "UPDATE detalle_ingresos SET edin_ccod = 2, AUDI_TUSUARIO='" & audi_tusuario  & "', audi_fmodificacion = getdate()  WHERE envi_ncorr='" & envio & "'"
         conexion.EstadoTransaccion conexion.EjecutaS(consulta)	
	     consulta = "UPDATE detalle_envios SET edin_ccod = 2, AUDI_TUSUARIO='" & audi_tusuario  & "', audi_fmodificacion = getdate()  WHERE envi_ncorr='" & envio & "'"
         conexion.EstadoTransaccion conexion.EjecutaS(consulta)			
	  else
