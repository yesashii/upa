...
f_consulta.Inicializar conexion	
	...
	SQL = " select envi_ncorr,eenv_ccod,envi_fenvio from envios " & vbcrlf & _
		  " where eenv_ccod in (2,4) and tenv_ccod=5 and envi_ncorr=480 "		  
		  
	SQL = " select envi_ncorr,eenv_ccod,envi_fenvio from envios " & vbcrlf & _
		  " where eenv_ccod in (2,4) and tenv_ccod=5 and envi_fenvio > '31-01-2005' and envi_fenvio<'04-03-2005' "		  
		  
	SQL = " select envi_ncorr,eenv_ccod,envi_fenvio from envios " & vbcrlf & _
		  " where eenv_ccod in (2,4) and tenv_ccod=5 and envi_fenvio > '02-01-2005' and envi_fenvio<'01-02-2005' "		  		  
	
	SQL = "select envi_ncorr,eenv_ccod,envi_fenvio from envios where eenv_ccod=4 and tenv_ccod=5 "

	SQL = " select envi_ncorr,eenv_ccod,envi_fenvio from envios " & vbcrlf & _
		  " where eenv_ccod in (2,4) and tenv_ccod=5 and envi_fenvio >= convert(datetime,'01-03-2005',103) and envi_fenvio < convert(datetime,'16-09-2005',103) "		  		  

	SQL2 = " select envi_ncorr,eenv_ccod,envi_fenvio from envios " & vbcrlf & _
		   " where eenv_ccod in (2,4) and tenv_ccod=5 and envi_ncorr=8527"	

	f_consulta.Consultar SQL2
...
Function TablaAArchivoSoftland(envi_ncorr, p_conexion)
	...
	'sql_banco = "SELECT   c.banc_ccod   FROM envios a, estados_envio b, instituciones_envio c,tipos_depositos d,cuentas_corrientes e  " & vbcrlf & _
	...
	'banc_ccod = p_conexion.consultauno(sql_banco)
	...
	archivo_salida 		= fecha_envio&"_deposito_tarjeta_reprocesada_"&envi_ncorr&".txt"

	set fso = Server.CreateObject("Scripting.FileSystemObject")
	set o_texto_archivo = fso.CreateTextFile(RUTA_ARCHIVOS_SALIDA_MASIVA & "\tarj_repro\" & archivo_salida)
