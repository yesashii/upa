...
<%
'---------------------------------------------------------------------------------------------------------------------------------
Response.AddHeader "Content-Disposition", "attachment;filename=reporte_pagare_upa_texto.txt"
...
'---------------------------------------------------------------------------------------------------------------------------------
set f_pagare_upa = new CFormulario
f_pagare_upa.Carga_Parametros "Reporte_pagare_upa.xml", "f_pagare_upa_excel"
f_pagare_upa.Inicializar conexion
...
consulta = 	" Select  replace(convert(varchar,a.ding_fdocto,103),'/','') as fecha_docto, "& vbCrLf &_
				...
				"    and b.eing_ccod <> 3  "
					...
					if num_doc <> "" then                   
				      consulta = consulta &  "AND case when len(isnull(a.ding_ndocto,0))<=4 then protic.obtener_numero_pagare_upa_pagado(a.ingr_ncorr) else cast(a.ding_ndocto as varchar) end = '" & num_doc & "' "& vbCrLf
					end if
...
f_pagare_upa.Consultar consulta


while f_pagare_upa.Siguiente
	Response.Write(Ac(f_pagare_upa.ObtenerValor("rut_apoderado"),10,"D"))
	Response.Write(Ac(f_pagare_upa.ObtenerValor("nombre_apoderado"),50,"D"))
	Response.Write(Ac(f_pagare_upa.ObtenerValor("cod_banco"),15,"D"))
	Response.Write(Ac(f_pagare_upa.ObtenerValor("rut_alumno"),11,"D"))
	Response.Write(Ac(f_pagare_upa.ObtenerValor("saldo_pagare"),16,"D"))
	Response.Write(Ac(f_pagare_upa.ObtenerValor("fecha_docto"),8,"D"))
