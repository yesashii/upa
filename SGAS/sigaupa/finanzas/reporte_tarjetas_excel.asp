...
Response.AddHeader "Content-Disposition", "attachment;filename=Reporte_Tarjetas.xls"
...
f_letras.Carga_Parametros "Reporte_Tarjetas.xml", "f_tarjetas_excel"
f_letras.Inicializar conexion

					
consulta = 	"select    substring(j.ting_tdesc,12,7) as ting_tdesc,b.mcaj_ncorr, a.envi_ncorr, d.edin_tdesc, convert(varchar,b.ingr_fpago,103) as ingr_fpago, " & vbCrLf &_
				" 		case when len(isnull(a.ding_ndocto,0))<=3 then Right(substring(a.ding_tcuenta_corriente,1,12),4) else cast(a.ding_ndocto as varchar) end as ding_ndocto, " & vbCrLf &_
				"       convert(varchar,a.ding_fdocto,103) as ding_fdocto, isnull(a.ding_mdocto,a.ding_mdetalle) as ding_mdocto, " & vbCrLf &_
				...
				" 	join tipos_ingresos j " & vbCrLf &_
				" 		on a.ting_ccod=j.ting_ccod " & vbCrLf &_
				...
				"    and a.ding_ncorrelativo > 0 " 
					...
					if v_ting_ccod <> "" then
						...
						consulta = consulta &  " AND a.ting_ccod in (13,51) "& vbCrLf
					end if				
					...
					if estado_letra <> "" then
					 ...
					 consulta = consulta & "order by ting_tdesc, ding_ndocto asc, a.ding_fdocto asc, b.ingr_fpago asc"
...
<table width="75%" border="1">
  <tr>
	...
	<td width="11%"><div align="center"><strong>Tipo Tarjeta</strong></div></td>
  ...
  <tr> 
	...
	<td><font><%=f_letras.ObtenerValor("ding_ndocto")%>&nbsp;</font></td>
