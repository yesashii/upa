...
for fila = 0 to f_solicitud.CuentaPost - 1
	...
	if v_cod_solicitud<>"" then
		...
		if v_aprueba="2" then
			...
			' Rechaza la solicitud, Valores asgi_nestado (1= Aprobado, 5 = Rechazado, 10 = Observado)
			...
			' Aprueba la solicitud,"vibo_ccod=1" visto bueno jefe directo, estado finanzas 1 = aprobado
			vibo_ccod=1
			f_solicitud.AgregaCampoFilaPost fila,"vibo_ccod", vibo_ccod
			f_solicitud.AgregaCampoFilaPost fila,"asgi_nestado", asgi_nestado
			f_solicitud.AgregaCampoFilaPost fila,"asgi_fautorizado", fecha_actual
		...
		'RESPONSE.WRITE("1. sql_update : "& conexion.ObtenerEstadoTransaccion &" "&sql_update&"<BR>")
		estado=asgi_nestado
	end if
next
...
'response.Write("<br/><b> 2: "&estadotran&"</b>")
'response.End()
