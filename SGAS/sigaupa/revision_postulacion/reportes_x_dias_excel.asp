...
<%
Response.AddHeader "Content-Disposition", "attachment;filename=reportes_diarios.xls"
...
'************************'
	if tipo = "3" then 	
	'**************************************'
	'* pregrado y postgrado + matriculados*'
	'**************************************'	
	tituloPag = "Alumnos con Contratos realizados en fecha consultada"
	consulta = " select d.sede_tdesc as sede,c.cont_ncorr as n_contrato,e.econ_tdesc as estado,protic.trunc(c.cont_fcontrato) as fecha,c.cont_fcontrato as fecha2, " & vbCrLf &_
			...
			" h.carr_tdesc as carrera, i.jorn_tdesc as jornada,protic.ano_ingreso_carrera(a.pers_ncorr,h.carr_ccod) as promocion,f.pers_tfono as fono, f.pers_tcelular as celular, lower(f.pers_temail) as email, " & vbCrLf &_
			...
			" and b.jorn_ccod=i.jorn_ccod and b.post_bnuevo = 'S' and protic.ano_ingreso_carrera(a.pers_ncorr,h.carr_ccod) = '"&anos_ccod&"' " & vbCrLf &_
	...
	elseif tipo = "2" then
	'***************************************'
	'* pregrado y postgrado + en entrevista*'
	'***************************************'	
	tituloPag = "Postulantes con test o entrevista realizada en fecha consultada"
	consulta = " select cast(h.pers_nrut as varchar)+'-'+h.pers_xdv as rut,protic.initcap(h.pers_tnombre + ' ' + h.pers_tape_paterno + ' ' + h.pers_tape_materno) as nombre,sede_tdesc as sede, carr_tdesc as carrera, jorn_tdesc as jornada, " & vbCrLf &_
			" protic.trunc(dpos_fexamen) as fecha,dpos_fexamen as fecha2,eepo_tdesc as estado,h.pers_tfono as fono, h.pers_tcelular as celular, lower(h.pers_temail) as email, " & vbCrLf &_
			" protic.obtener_direccion_letra(a.pers_ncorr,1,'CNPB') as direccion,protic.obtener_direccion_letra(a.pers_ncorr,1,'C-C') as comuna  "& vbcrlf & _
			" from postulantes a, detalle_postulantes b, ofertas_academicas c, sedes d, especialidades e, carreras f, " & vbCrLf &_
			" jornadas g, personas_postulante h,estado_examen_postulantes i " & vbCrLf &_
			...
			" and cast(a.peri_ccod as varchar)='"&periodo&"' and a.post_bnuevo = 'S' and isnull(b.eepo_ccod,1)=i.eepo_ccod"
				if esVacio(termino) then
	...
	'*********************************************************'
	'* pregrado y postgrado + postulaciones o ninguna opción *'
	'*********************************************************'
	tituloPag = "Postulaciones realizadas en fecha consultada"
	consulta = " select cast(h.pers_nrut as varchar)+'-'+h.pers_xdv as rut,protic.initcap(h.pers_tnombre + ' ' + h.pers_tape_paterno + ' ' + h.pers_tape_materno) as nombre,sede_tdesc as sede, carr_tdesc as carrera, jorn_tdesc as jornada, " & vbCrLf &_
			" protic.trunc(fecha_asignacion_carrera) as fecha,fecha_asignacion_carrera as fecha2,h.pers_tfono as fono, h.pers_tcelular as celular, lower(h.pers_temail) as email, " & vbCrLf &_
			" protic.obtener_direccion_letra(a.pers_ncorr,1,'CNPB') as direccion,protic.obtener_direccion_letra(a.pers_ncorr,1,'C-C') as comuna  "& vbcrlf & _
			" from postulantes a, detalle_postulantes b, ofertas_academicas c, sedes d, especialidades e, carreras f, " & vbCrLf &_
...
'*************'
		...
		'*****************************'	
         ...
         "       d.pers_tfono                                           as fono,      " & vbCrlf & _
         "       d.pers_tcelular                                        as celular,   " & vbCrlf & _
         "       lower(d.pers_temail)                                   as email,     " & vbCrlf & _
         "       g.dire_tcalle                                          as direccion, " & vbCrlf & _
         "       protic.obtener_direccion_letra(a.pers_ncorr, 1, 'C-C') as comuna     " & vbCrlf & _
	...
	ElseIf tipo = "2" then	
		...
		"                + ', ' + pers_tnombre                                  as nombre " & vbCrlf & _
		"                ,                                                                " & vbCrlf & _
		"                f.sede_tdesc                                                     " & vbCrlf & _
		"                as sede,                                                         " & vbCrlf & _
		"                e.dcur_tdesc                                           as        " & vbCrlf & _
		"                carrera,                                                         " & vbCrlf & _
		"                'DIURNO'                                               as        " & vbCrlf & _
		"                jornada,                                                         " & vbCrlf & _
		...
		"                ,                                                                " & vbCrlf & _
		"                d.pers_tfono                                                     " & vbCrlf & _
		"                as fono,                                                         " & vbCrlf & _
		"                d.pers_tcelular                                        as        " & vbCrlf & _
		"                celular,                                                         " & vbCrlf & _
		"                lower(d.pers_temail)                                   as email, " & vbCrlf & _
		"                h.dire_tcalle                                          as        " & vbCrlf & _
		"                direccion,                                                       " & vbCrlf & _
		"                protic.obtener_direccion_letra(a.pers_ncorr, 1, 'C-C') as comuna " & vbCrlf & _
		"from   postulacion_otec as a                                                     " & vbCrlf & _
		"       inner join datos_generales_secciones_otec as b                            " & vbCrlf & _
		...
		"       inner join direcciones as h                                               " & vbCrlf & _
		"               on d.pers_ncorr = h.pers_ncorr                                    " & vbCrlf & _
	...
	ElseIf tipo = "3" then
		...
		"       d.pers_tcelular                                         				as celular, " & vbCrlf & _
		"       lower(d.pers_temail)                                    				as email,   " & vbCrlf & _
		"       h.DIRE_TCALLE 														as direccion,   " & vbCrlf & _
		"       protic.obtener_direccion_letra(a.pers_ncorr, 1, 'C-C')  				as comuna   " & vbCrlf & _
		"from   postulacion_otec as a                                                               " & vbCrlf & _
		"       inner join datos_generales_secciones_otec as b                                      " & vbCrlf & _
		"               on a.dgso_ncorr = b.dgso_ncorr                                              " & vbCrlf & _
...
'response.End()
f_matriculados.Consultar consulta & " order by fecha2 asc "
...
<table width="100%" border="0">
 <tr> 
  ...
  <tr> 
    <td width="11%" height="22"><strong>Reporte día </strong></td>
...
<p>&nbsp;</p><table width="100%" border="1">
  <tr> 
    <td bgcolor="#0066FF" style="color:#FFF"><div align="center"><strong>N°</strong></div></td>
    <td bgcolor="#0066FF" style="color:#FFF"><div align="center"><strong>Sede</strong></div></td>
	<td bgcolor="#0066FF" style="color:#FFF"><div align="center"><strong>Carrera</strong></div></td>
    <td bgcolor="#0066FF" style="color:#FFF"><div align="center"><strong>Jornada</strong></div></td>
    <td bgcolor="#0066FF" style="color:#FFF"><div align="center"><strong>Rut</strong></div></td>
	<td bgcolor="#0066FF" style="color:#FFF"><div align="center"><strong>Nombre</strong></div></td>
	<td bgcolor="#0066FF" style="color:#FFF"><div align="center"><strong>Fecha proceso</strong></div></td>
	<%if tipo="2" then%>
	<td bgcolor="#0066FF" style="color:#FFF"><div align="center"><strong>Estado</strong></div></td>
	<%end if%>
	<td bgcolor="#0066FF" style="color:#FFF"><div align="center"><strong>Fono</strong></div></td>
	<td bgcolor="#0066FF" style="color:#FFF"><div align="center"><strong>Celular</strong></div></td>
	<td bgcolor="#0066FF" style="color:#FFF"><div align="center"><strong>Email</strong></div></td>
	<td bgcolor="#0066FF" style="color:#FFF"><div align="center"><strong>Dirección</strong></div></td>
	<td bgcolor="#0066FF" style="color:#FFF"><div align="center"><strong>Comuna</strong></div></td>
  </tr>
  <% fila = 1 
  ...
  <tr> 
	...
	<td><div align="left"><%=f_matriculados.ObtenerValor("fecha")%></div></td>
	<%if tipo="2" then%>
	<td><div align="left"><%=f_matriculados.ObtenerValor("estado")%></div></td>
	<%end if%>
	<td><div align="left"><%=f_matriculados.ObtenerValor("fono")%></div></td>
	<td><div align="left"><%=f_matriculados.ObtenerValor("celular")%></div></td>
	<td><div align="left"><%=f_matriculados.ObtenerValor("email")%></div></td>
	<td><div align="left"><%=f_matriculados.ObtenerValor("direccion")%></div></td>
	<td><div align="left"><%=f_matriculados.ObtenerValor("comuna")%></div></td>
  </tr>
  <% fila = fila + 1  
  wend %>
</table>
