...
pagina.Titulo = "Solicitudes de Soporte Anteriores"
...
"where a.sede_ccod in ("&sedes&") and pers_ncorr_responsable is null and convert(datetime,protic.trunc(FECHA_INCIDENTE),103)<convert(datetime,protic.trunc(getdate()),103) "&filtro&" "&filtro3&""

'sql_descuentos= "select a.inci_ccod,"&sedes&" as pers_ncorr_responsable,
'a.inci_ccod as inci_ccod2,protic.trunc(FECHA_INCIDENTE)as FECHA_INCIDENTE,
'folio,(select sede_tdesc from sedes aaa where aaa.sede_ccod=a.sede_ccod)as sede, 
'(select pers_tnombre+' '+pers_tape_paterno from personas aa where aa.pers_ncorr=a.pers_ncorr )as persona_solicitante,
'peso_tdescripcion as solicitud 
'from peticion_soporte a, incidentes b 
'where  a.inci_ccod=b.inci_ccod 
'and  a.sede_ccod in ("&sedes&") 
'and pers_ncorr_responsable is null 
'and convert(datetime,protic.trunc(FECHA_INCIDENTE),103)<convert(datetime,protic.trunc(getdate()),103) "&filtro&""
...
function filtrar()
{
	document.forms['filtrado'].action = "peticiones_pendientes.asp";
