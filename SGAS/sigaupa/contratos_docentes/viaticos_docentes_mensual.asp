...
<%
'*******************************************************************
'DESCRIPCION				  :	Obtiene la sumatoria mensual de los dias que un profesor tiene clase en la semana
...
sql_listado_viaticos = "Select (select mes_tdesc from meses where mes_ccod=month(inicio_semana)) as mes,   " & vbcrlf & _
					" rut_docente,nombre_docente,direccion, comuna_ciudad,tipo_docente,  " & vbcrlf & _
					" sum(cast(viaticos_lunes as integer)) as lunes,  " & vbcrlf & _
					" sum(cast(viaticos_Martes as integer)) as martes,  " & vbcrlf & _
					" sum(cast(viaticos_Miercoles as integer)) as miercoles,  " & vbcrlf & _
					" sum(cast(viaticos_Jueves as integer)) as jueves,  " & vbcrlf & _
					" sum(cast(viaticos_Viernes as integer)) as viernes,   " & vbcrlf & _
					" sum(cast(viaticos_Sabado as integer)) as sabado,   " & vbcrlf & _
					" sum(cast(viaticos_Domingo as integer)) as domingo   " & vbcrlf & _
					" from (  " & vbcrlf & _
					" select semanas.inicio as inicio_semana, semanas.fin as fin_semana, case when count(Lunes)>=1 then '1' else '0' end  as viaticos_lunes,case when count(Martes)>=1 then '1' else '0' end  as viaticos_Martes,case when count(Miercoles)>=1 then '1' else '0' end  as viaticos_Miercoles,case when count(Jueves)>=1 then '1' else '0' end  as viaticos_Jueves,case when count(Viernes)>=1 then '1' else '0' end  as viaticos_Viernes," & vbcrlf & _
					" case when count(Sabado)>=1 then '1' else '0' end  as viaticos_Sabado,case when count(Domingo)>=1 then '1' else '0' end  as viaticos_Domingo,rut_docente,nombre_docente,direccion, comuna_ciudad,tipo_docente  " & vbcrlf & _
					" from (  " & vbcrlf & _
					" select distinct case when isnull(a.TPRO_CCOD,1) = 1 then 'Profesor' else 'Ayudante' end tipo_docente,protic.obtener_direccion(a.pers_ncorr,1,'CNPB') as direccion,protic.obtener_direccion(a.pers_ncorr,1,'C-C') as comuna_ciudad,protic.obtener_nombre_completo(a.pers_ncorr, 'n') as nombre_docente,protic.obtener_rut(a.pers_ncorr) as rut_docente,carr_tdesc as carrera, jorn_tdesc as jornada,  " & vbcrlf & _
					...
					" group by tabla.rut_docente,semanas.inicio,semanas.fin, nombre_docente,direccion, comuna_ciudad,tipo_docente  " & vbcrlf & _
					" ) as por_mes  " & vbcrlf & _
					" group by month(inicio_semana),rut_docente,nombre_docente,direccion, comuna_ciudad,tipo_docente  " & vbcrlf & _
					" order by month(inicio_semana), rut_docente asc  " 
...
<table width="75%" border="1">
  ...
  <tr> 
	<td><div align="left"><%=f_valor_viatico.ObtenerValor("mes")%></div></td>
    ...
    <td><div align="left"><%=f_valor_viatico.ObtenerValor("lunes")%></div></td>
    <td><div align="left"><%=f_valor_viatico.ObtenerValor("martes")%></div></td>
    <td><div align="left"><%=f_valor_viatico.ObtenerValor("miercoles")%></div></td>
    <td><div align="left"><%=f_valor_viatico.ObtenerValor("jueves")%></div></td>
    <td><div align="left"><%=f_valor_viatico.ObtenerValor("viernes")%></div></td>
    <td><div align="left"><%=f_valor_viatico.ObtenerValor("sabado")%></div></td>
	<td><div align="left"><%=f_valor_viatico.ObtenerValor("domingo")%></div></td>
  </tr>
  <%  wend %>
</table>
</body>
