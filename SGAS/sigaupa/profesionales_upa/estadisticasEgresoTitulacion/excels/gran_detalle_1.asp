...
<html>
<head>
<title>ESTADÍSTICAS EGRESADOS, TITULADOS Y GRADUADOS 1</title>
<style type="text/css">
.cabecera
{
	text-align:center;
...
'** PRIMERA fILA  **'
'*******************'
v_sede_ccod 	= 1'cambia por sede

while v_sede_ccod < 10 
sede_tdesc  = conexion.ConsultaUno("select sede_tdesc from sedes where sede_ccod = '"&v_sede_ccod&"'")
'******************************'
'** 	 TROZO SELECT 1 	 **'
'******************************'--------------------------
select_1 = ""
...
select_inter = Cstr(selectUnoInter(institucion, sede_tdesc, v_anio_egreso, v_anio_titula))
...
if masculino = "1" then  			
			...
			consulta = select_inter& vbCrLf &_
						conAux_1
...
conAux_1 = CStr(UPAPostgradoGraduadosp1(v_sede_ccod, v_sexo_ccod, v_facu_ccod, v_carr_ccod, v_anio_promo, v_anio_egreso,v_anio_titula))			
			consulta = Cstr(selectUnoInter(institucion, sede_tdesc, v_anio_egreso, v_anio_titula))& vbCrLf &_	
			conAux_1
