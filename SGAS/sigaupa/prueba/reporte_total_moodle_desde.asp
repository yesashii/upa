...
consulta_grl =  " select distinct a.secc_ccod,a.sede_ccod,a.carr_ccod,a.jorn_ccod,a.asig_ccod,substring(a.secc_tdesc,1,1) as seccion,"& vbCrLf &_
				...
				" from secciones a, cargas_academicas b (nolock), alumnos c (nolock), moodle_course cc"& vbCrLf &_
				" where a.secc_ccod=b.secc_ccod and b.matr_ncorr=c.matr_ncorr"& vbCrLf &_
				" and a.peri_ccod in (226) "& vbCrLf &_
...
if total_1 <> "0" then
	...
	while formulario_grl.siguiente
		...
		consulta =  " select replace(replace(replace(replace(replace(replace(c.susu_tlogin,'á','A'),'é','E'),'í','I'),'ó','O'),'ú','U'),'ñ','N') as username,matr_ncorr,secc_ccod, "& vbCrLf &_
		            " b.pers_ncorr, replace(replace(replace(replace(replace(replace(upper(c.susu_tclave),'á','A'),'é','E'),'í','I'),'ó','O'),'ú','U'),'ñ','N') as password, "& vbCrLf &_
