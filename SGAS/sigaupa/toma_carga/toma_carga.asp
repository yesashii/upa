...
tresumen_otrasede.carga_parametros	"tabla_resumen.xml","tabla_resumen_otra_sede"
 ...
 if matr_ncorr<>"" then 
		...
		sql_jorn_ccod = " select jorn_ccod from alumnos a, ofertas_academicas b " & _
		 ...
		 'response.Write("<br>espe_ccod "&v_espe_ccod&" jorn_ccod "&v_jorn_ccod)
		...
		' response.Write(sql_pers_ncorr)
		 ...
		 'response.Write("<br>acceso usuario "&acceso_usuario)
end if 
...
'end if 
 ...
 'response.Write("<br>pers_ncorr "&pers_ncorr&" nombre "&nombre&" carrera "&carrera&" carr_ccod "&v_carr_ccod)
 if isnull(pers_ncorr) then
	...
	'response.Write("<br> sede_ccod "&sede_ccod&" plan_ccod "&plan_ccod&" es_nuevo "&es_nuevo& " area_ccod "&area_ccod)
 end if
...
" and asig_ccod NOT IN (select asig_ccod " & vbCrlf & _
"  	   			 	 	from homologacion_destino  " & vbCrlf & _
"						where homo_ccod in (select homo_ccod  " & vbCrlf & _
" 	  			        	  			   from homologacion_fuente " & vbCrlf & _
"				                		   where asig_ccod in (select b.asig_ccod from  " & vbCrlf & _
"	  			   		  			       		 		   	   cargas_academicas a, secciones b, " & vbCrlf & _
"															   asignaturas c, alumnos d " & vbCrlf & _
...
"  order by c.asig_ccod, asig_ccod_paso,asignatura, asig_nhoras, b.secc_ccod," & vbCrLf & _
"             matr_ncorr , a.nive_ccod,reprobado " & vbCrLf

'response.Write("<br><pre>"& asignaturas_disponibles_cons&"</pre>")
...
"	  case when cast(a.carr_ccod as varchar) = '"&v_carr_ccod&"'  " & vbCrLf & _
"	  then '(*)'+ cast(a.asig_ccod as varchar) + '-' + a.secc_tdesc + ' -> ' +  cast(protic.horario(a.secc_ccod) as varchar)" & vbCrLf & _
"	  else cast(a.asig_ccod as varchar) + '-' + a.secc_tdesc + ' -> ' + cast(protic.horario(a.secc_ccod) as varchar) " & vbCrLf & _
...
cons_resumen="select " & _
			"    c.secc_ccod,cast(e.asig_ccod as varchar) + ' ' + e.asig_tdesc as a_plan ,    " & _
			"    cast(a.asig_ccod as varchar) + ' ' + a.asig_tdesc as a_destino, " & _
			"    'Secc. ' + b.secc_tdesc + ' -> ' + protic.horario(c.secc_ccod) seccion, " & _
			...
			"	 and cast(matr_ncorr as varchar)='"&matr_ncorr&"'" & _
			"	 and cast(sede_ccod as varchar)='"&sede_ccod&"'" & _
			"	 and cast(b.peri_ccod as varchar)='"&peri_ccod&"'" 
...
Secciones =	" SELECT 	isnull(C.ASIG_CCOD,A.ASIG_CCOD) asig_ccod, b.secc_ccod,case  when cast(a.carr_ccod as varchar)= '"&v_carr_ccod&"' then " & _
	 	   	"	 	  '(*)' + cast(a.asig_ccod as varchar) + '-' + a.secc_tdesc + ' -> ' + protic.horario(a.secc_ccod) " & _
		   	" else  " & _
		   	" 	   cast(a.asig_ccod as varchar) + '-' + a.secc_tdesc + ' -> ' + protic.horario(a.secc_ccod)   " & _
		 	" end    horario " & _
