...
'response.Write(Extraer_rut(arch))
'response.End()

Response.AddHeader "Content-Disposition", "attachment;filename=bd_respaldados_cae.csv"
...
'			"r.jorn_ccod, t.aran_mcolegiatura " & vbCrlf & _
...
'				"and a.pers_nrut in ("&rut&")"
	 ...
	 consulta ="select distinct a.pers_nrut, a.pers_xdv, a.pers_tape_paterno,a.pers_tape_materno,a.pers_tnombre,"& vbCrlf & _ 
				"protic.obtener_tipo_ies('ing') as Tipo_ies ,protic.obtener_codigo_institucion_ies('ing') as cod_IES, "& vbCrlf & _
				" case when r.sede_ccod = 2 and s.carr_ccod in ('51','110') then '002'"& vbCrlf & _
				"	 when r.sede_ccod = 2 and s.carr_ccod not in ('51','110') then '005'"& vbCrlf & _
				"	 else m.seie_ing_ccod end as Sede,h.cod_carrera_ing as Carrera, r.jorn_ccod,"& vbCrlf & _ 
				"isnull(protic.ANO_INGRESO_CARRERA_EGRESA2(a.pers_ncorr,i.CARR_CCOD), "& vbCrlf & _
				"protic.ANO_INGRESO_UNIVERSIDAD(a.pers_ncorr))as ano_ingreso, "& vbCrlf & _
				"r.jorn_ccod,"& vbCrlf & _
				"protic.ufe_obtener_arancel_ano_siguiente(s.CARR_CCOD,r.SEDE_CCOD,r.JORN_CCOD,t.ARAN_NANO_INGRESO,"&ano&")as aran_mcolegiatura "& vbCrlf & _
				"from personas a "& vbCrlf & _
				"join alumnos o"& vbCrlf & _
				...
				"on s.CARR_CCOD=n.carr_ccod COLLATE Modern_Spanish_CI_AS"& vbCrlf & _
				"join ufe_carreras_ingresa h"& vbCrlf & _
				"on n.car_ing_ncorr=h.car_ing_ncorr"& vbCrlf & _
				"join carreras i"& vbCrlf & _
				"on s.CARR_CCOD=i.CARR_CCOD"& vbCrlf & _
				"join sedes l"& vbCrlf & _
				"on r.SEDE_CCOD=l.SEDE_CCOD "& vbCrlf & _
				...
				"where p.PERI_CCOD in(select peri_ccod from periodos_academicos where anos_ccod in ("&ano&")) "& vbCrlf & _
				"and emat_ccod=1"& vbCrlf & _
				"and pers_nrut not in (select distinct rut from ufe_alumnos_cae where esca_ccod=1) "& vbCrlf & _
				"and a.pers_nrut in ("&rut&")"
			
'response.Write(consulta)
'response.End()
fecha=now()
descripcion="bd_respaldados_cae"
...
'response.End()
'------------------------------------------------------------------------------------
encabezados="RUT;DV;APELLIDO_PATERNO;APELLIDO_MATERNO;NOMBRES;COD_T_IES;COD_IES;COD_SEDE;COD_CARRERA;ANO_INGRESO;JORNADA;ARANCEL_REAL"
