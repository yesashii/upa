...
consulta= " select distinct  a.pers_nrut, a.pers_xdv, a.pers_tape_paterno,a.pers_tape_materno, a.pers_tnombre, " & vbCrlf & _
			...
			" and isnull(tr.mall_npermiso,0) = 0   " & vbCrlf & _
			" and isnull(protic.estado_ramo_alumno(o.pers_ncorr,tr.asig_ccod,s.carr_ccod,tr.plan_ccod,r.peri_ccod),'') = ''  " & vbCrlf & _
			" order by tr.nive_ccod asc) when 1  then '1'  when 2  then '1'  when 3  then '2'  when 4  then '2'  when 5  then '3'when 6  then '3'  when 7  then '4'  when 8  then '4'  when 9 then '5'  else 'SNR' end as nivel_estudio, '' as arancel_solicitado, t.aran_mcolegiatura, c.CONTRATO   " & vbCrlf & _
			...
			"             and com.tcom_ccod in (1,2)) " & vbCrlf & _
			" and t.aran_mcolegiatura > 1 " & vbCrlf & _
			" and c.econ_ccod=1 " & vbCrlf & _
 			" and p.post_ncorr=c.post_ncorr " & vbCrlf & _
			" and a.pers_nrut in ("&rut&")"	
...
'response.End()
'------------------------------------------------------------------------------------
encabezados="RUT;DV;APELLIDO_PATERNO;APELLIDO_MATERNO;NOMBRES;SEXO;FECHA_NACIMIENTO;DIRECCION;CIUDAD;COMUNA;REGION;COD_AREA;FONO_FIJO;CELULAR;EMAIL;COD_T_IES;COD_IES;COD_SEDE;COD_CARRERA;JORNADA;ANO_ING_CARRERA;NIVEL_ESTUDIOS;ARANCEL_SOLICITADO;ARANCEL_REAL;CONTRATO"
...
while tabla.Siguiente
	...
	detalle=""&tabla.ObtenerValor("pers_nrut")&";"&tabla.ObtenerValor("pers_xdv")&";"&tabla.ObtenerValor("pers_tape_paterno")&";"&tabla.ObtenerValor("pers_tape_materno")&";"&tabla.ObtenerValor("pers_tnombre")&";"&tabla.ObtenerValor("sexo")&";"&tabla.ObtenerValor("fecha_nacimiento")&";"&tabla.ObtenerValor("direccion")&";"&tabla.ObtenerValor("codigo_ciudad")&";"&tabla.ObtenerValor("codigo_comuna")&";"&tabla.ObtenerValor("codigo_region")&";"&tabla.ObtenerValor("Codigo_area")&";"&tabla.ObtenerValor("pers_tfono")&";"&tabla.ObtenerValor("pers_tcelular")&";"&tabla.ObtenerValor("pers_temail")&";"&tabla.ObtenerValor("Tipo_ies")&";"&tabla.ObtenerValor("cod_IES")&";"&tabla.ObtenerValor("Sede")&";"&tabla.ObtenerValor("Carrera")&";"&tabla.ObtenerValor("jorn_ccod")&";"&tabla.ObtenerValor("ano_ingreso")&";"&tabla.ObtenerValor("nivel_estudio")&";"&tabla.ObtenerValor("arancel_solicitado")&";"&tabla.ObtenerValor("aran_mcolegiatura")&";"&tabla.ObtenerValor("contrato")&""
