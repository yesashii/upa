...
'en caso contrario, consultar por todas las becas
'filtro2=filtro2&"and a.tdet_ccod in (910,1390,1446,1537,1538,1539,1912)"  	

'en caso contrario, consultar por todas las becas
'A partir del 26-06-2015
filtro2=filtro2&" and   (  g.tdet_tdesc like '%BECA%ARTICULACIÓN%' or g.tdet_tdesc like '%BECA%ARTICULACION%' "& vbCrLf &_
      " or g.tdet_tdesc like '%BECA%EXCELENCIA%TECNICA%' or g.tdet_tdesc like '%BECA%EXCELENCIA%TÉCNICA%' "& vbCrLf &_
      " or g.tdet_tdesc like '%BECA%EXCELENCIA%ACADEMICA%' or g.tdet_tdesc like '%BECA$EXCELENCIA%ACADÉMICA%' "& vbCrLf &_
      " or g.tdet_tdesc like '%BECA%EXCELENCIA%ACADEMICA%MINEDUC%' or g.tdet_tdesc like '%BECA%EXCELENCIA%ACADÉMICA%MINEDUC%' "& vbCrLf &_
      " or g.tdet_tdesc like '%BECA%JUAN%GOMEZ%MILLAS%' or g.tdet_tdesc like '%BECA%JUAN%GÓMEZ%MILLAS%' "& vbCrLf &_
      " or g.tdet_tdesc like '%BECA%NIVELACIÓN%ACADÉMICA%' or g.tdet_tdesc like '%BECA%NIVELACION%ACADEMICA%' or g.tdet_tdesc like '%BECA%NIVELACION%ACADÉMICA%' "& vbCrLf &_
	  " or g.tdet_tdesc like '%BECA%NIVELACIÓN%ACADEMICA%' "& vbCrLf &_
      " or g.tdet_tdesc like '%BECA%NUEVO%MILENIO%' "& vbCrLf &_
      " or g.tdet_tdesc like '%BECA%TITULAR%VALECH%' "& vbCrLf &_
      " or g.tdet_tdesc like '%BECA%TRASPASO%VALECH%' "& vbCrLf &_
      " or g.tdet_tdesc like '%DESTACADOS%PEDAGOGIA%' or g.tdet_tdesc like '%DESTACADOS%PEDAGOGÍA%' "& vbCrLf &_
      " or g.tdet_tdesc like '%HIJOS%PROFESIONALES%EDUCACIÓN%' or g.tdet_tdesc like '%HIJOS%PROFESIONALES%EDUCACION%' ) "

end if

'para consultar por sede
if q_sede_ccod <> "" then
filtro3=filtro3&"and d.sede_ccod='" &q_sede_ccod&"'"
end if

'para consultar por periodo academico (año)
if q_anos_ccod <> "" then
filtro4=filtro4&"and d.peri_ccod in (select peri_ccod from periodos_academicos a1 where a1.anos_ccod="&q_anos_ccod&")"
end if

'para consultar por facultad
if q_facu_ccod<> "" then
filtro5=filtro5&"and i.facu_ccod ="&q_facu_ccod&""
end if

'para consultar por carrera
if q_carr_ccod<> "" then
filtro6=filtro6&"and f.carr_ccod ="&q_carr_ccod&""
end if

'para consultar por año de adjudicacion
...
'--SOLO PARA LA FICALIZACION DEL MINEDUC PARA 2015
'--los que no estan en la lista de Jacqueline Oyanedel
filtro8=filtro8&" and pers_nrut not in ('18447226','18490020','18778333','19068992','19411131') "
