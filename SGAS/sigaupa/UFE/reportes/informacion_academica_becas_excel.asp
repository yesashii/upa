...
'
'
'sem1_actual = conexion.consultaUno("select peri_ccod from periodos_academicos WITH(NOLOCK) where cast(anos_ccod as varchar)='"&v_ano_actual&"' and plec_ccod=1")
'sem2_actual = conexion.consultaUno("select peri_ccod from periodos_academicos WITH(NOLOCK) where cast(anos_ccod as varchar)='"&v_ano_actual&"' and plec_ccod=2")
'sem1_anterior = conexion.consultaUno("select peri_ccod from periodos_academicos WITH(NOLOCK) where cast(anos_ccod as varchar)='"&v_ano_actual-1&"' and plec_ccod=1")
'sem2_anterior = conexion.consultaUno("select peri_ccod from periodos_academicos WITH(NOLOCK) where cast(anos_ccod as varchar)='"&v_ano_actual-1&"' and plec_ccod=2")
'
'c_dato_promedio = ""
'mensaje_aclaratorio = ""
'if v_mes_actual >= 9 and v_mes_actual <= 11 then
'c_dato_promedio = " ,(select count(*) " & vbCrLf & _
'                  "   from alumnos a WITH(NOLOCK), ofertas_academicas b WITH(NOLOCK), especialidades c WITH(NOLOCK), cargas_academicas d WITH(NOLOCK),situaciones_finales e WITH(NOLOCK) " & vbCrLf & _
...
'
' consulta = "select top 5 pers_ncorr,pers_nrut as rut,pers_xdv as dv,nombre,carr_ccod,anos_adicionales,arancel_solicitado,rut_banco, carrera,sede,jornada, plan_ccod,ingreso,ano_obtencion,estado,periodo,nivel_estudio,nivel_estudio_max, "& vbCrLf &_
...
'            " (select max(espe_nduracion) from especialidades ttt WITH(NOLOCK) where ttt.carr_ccod=tra2.carr_ccod) as duracion_carrera"& vbCrLf &_
'			" from "& vbCrLf &_
'			"( "& vbCrLf &_
...
'		    "    (select top 1 nive_ccod from malla_curricular tr WITH(NOLOCK) where tr.plan_ccod = tra.plan_ccod   " & vbCrLf & _
...
'			"	 from alumnos c WITH(NOLOCK), ofertas_academicas d WITH(NOLOCK), especialidades e WITH(NOLOCK), carreras f WITH(NOLOCK),  "& vbCrLf &_
'			"		  estados_matriculas g WITH(NOLOCK) ,periodos_academicos h WITH(NOLOCK)  "& vbCrLf &_
...
'			"	 from alumnos c WITH(NOLOCK), ofertas_academicas d WITH(NOLOCK), especialidades e WITH(NOLOCK), carreras f WITH(NOLOCK),  "& vbCrLf &_
'			"		  estados_matriculas g WITH(NOLOCK),periodos_academicos h WITH(NOLOCK),sedes i WITH(NOLOCK)  "& vbCrLf &_
'			"	 where c.ofer_ncorr=d.ofer_ncorr and d.espe_ccod=e.espe_ccod and e.carr_ccod=f.carr_ccod  "& vbCrLf &_
'			"	 and c.pers_ncorr=b.pers_ncorr and c.emat_ccod <> 9 and c.emat_ccod=g.emat_ccod  "& vbCrLf &_
...
'			"	 from alumnos c WITH(NOLOCK), ofertas_academicas d WITH(NOLOCK), especialidades e WITH(NOLOCK), carreras f WITH(NOLOCK),  "& vbCrLf &_
'			"		  estados_matriculas g WITH(NOLOCK),periodos_academicos h WITH(NOLOCK),jornadas i WITH(NOLOCK)  "& vbCrLf &_
'			"	 where c.ofer_ncorr=d.ofer_ncorr and d.espe_ccod=e.espe_ccod and e.carr_ccod=f.carr_ccod  "& vbCrLf &_
'			"	 and c.pers_ncorr=b.pers_ncorr and c.emat_ccod <> 9 and c.emat_ccod=g.emat_ccod  "& vbCrLf &_
'			"	 and d.peri_ccod=h.peri_ccod and d.jorn_ccod=i.jorn_ccod order by d.peri_ccod desc) as jornada,  "& vbCrLf &_
'			"	 (select top 1 anos_adicionales from ufe_alumnos_cae  ttt WITH(NOLOCK) where ttt.rut=b.pers_nrut and ttt.anos_ccod=a.anos_ccod) as anos_adicionales,  "& vbCrLf &_
'			"	 (select isnull((select arancel_solicitado from ufe_alumnos_cae ttt WITH(NOLOCK) where ttt.rut=b.pers_nrut and ttt.anos_ccod=a.anos_ccod),0)) as arancel_solicitado,  "& vbCrLf &_
'			"	 (select baca_tdesc from ufe_alumnos_cae ttt WITH(NOLOCK),ufe_bancos_cae fff WITH(NOLOCK) where ttt.rut=b.pers_nrut and ttt.anos_ccod=a.anos_ccod and ttt.rut_banco=fff.baca_nrut) as rut_banco,  "& vbCrLf &_
'			" (select top 1 anos_ccod from ufe_alumnos_cae ggg WITH(NOLOCK) where ggg.taca_ccod =1 and ggg.rut=b.pers_nrut ) as ano_obtencion, "& vbCrLf &_
...
'			"	 from alumnos c WITH(NOLOCK), ofertas_academicas d WITH(NOLOCK), especialidades e WITH(NOLOCK),  "& vbCrLf &_
'			"		  carreras f WITH(NOLOCK), estados_matriculas g WITH(NOLOCK), periodos_academicos h WITH(NOLOCK)  "& vbCrLf &_
'			"	 where c.ofer_ncorr=d.ofer_ncorr and d.espe_ccod=e.espe_ccod and e.carr_ccod=f.carr_ccod  "& vbCrLf &_
'			"	 and c.pers_ncorr=b.pers_ncorr and c.emat_ccod <> 9 and c.emat_ccod=g.emat_ccod  "& vbCrLf &_
...
'			"	 from ufe_alumnos_cae a WITH(NOLOCK), personas b WITH(NOLOCK) "& vbCrLf &_
...
f_alumnos.Carga_Parametros "tabla_vacia.xml", "tabla"
f_alumnos.Inicializar conexion
consulta=  " Select pers_ncorr,rut,dv,nombre,carr_ccod,anos_adicionales,arancel_solicitado, " & vbCrLf &_
		   " rut_banco,carrera,sede,jornada,plan_ccod,ingreso,ano_obtencion,estado,periodo , " & vbCrLf &_
		   " nivel_estudio,nivel_estudio_max,total_carga,total_aprobados,avance,duracion_carrera " & vbCrLf &_
		   " from ufe_inf_academica_cae"
...
<html>
<head>
<title>Informaci&oacute;n acad&eacute;mica Alumnos cae <%=v_ano_actual%></title>
<meta http-equiv="Content-Type" content="text/html;">
</head>
<body >
<table width="100%" border="0">
 <tr> 
    <td colspan="4"><div align="center"><font size="+2" face="Arial, Helvetica, sans-serif">Informaci&oacute;n acad&eacute;mica Alumnos cae<%=v_ano_actual%></font></div></td>
 </tr>
 <tr> 
    <td colspan="4">&nbsp;</td>
 </tr>
