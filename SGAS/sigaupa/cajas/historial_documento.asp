...
sql_historial=  " select distinct ingr_ncorr_origen,a.ding_ndocto_origen, " & vbCrLf &_
				...
				"			else f.sede_tdesc end)  as estado_destino, " & vbCrLf &_
				" protic.trunc(a.ding_fdocto_origen) as fecha_origen,protic.trunc(a.ding_fdocto_destino) as fecha_destino, " & vbCrLf &_
				" a.ding_mdocto_origen,a.ding_mdocto_destino,d.mcaj_ncorr " & vbCrLf &_
				" ,( " & vbCrLf &_
				" select top 1 pers_tnombre+' '+pers_tape_paterno as nombre " & vbCrLf &_
				" from cajeros caj " & vbCrLf &_
				" INNER JOIN MOVIMIENTOS_CAJAS mc " & vbCrLf &_
				" ON mc.caje_ccod = caj.caje_ccod " & vbCrLf &_
				" INNER JOIN personas p " & vbCrLf &_
				" ON caj.pers_ncorr = p.pers_ncorr where mcaj_ncorr in (d.mcaj_ncorr) " & vbCrLf &_
				" )as nombre_cajero,  " & vbCrLf &_
				" edin_ccod_origen,edin_ccod_destino,audi_tusuario_origen,audi_tusuario_destino,dist_ncorr, a.dist_fhistorial, protic.trunc(a.dist_fhistorial) as fecha_mod, " & vbCrLf &_
				" envi_ncorr_origen,envi_ncorr_destino " & vbCrLf &_
				" from detalle_ingresos_historial a " & vbCrLf &_
				" INNER JOIN ingresos d " & vbCrLf &_
				" ON a.ingr_ncorr_origen = d.ingr_ncorr and a.ding_ncorrelativo_destino = 1 and cast(a.ding_ndocto_origen as varchar) = '"&q_ding_ndocto&"' and cast(a.ting_ccod_origen as varchar) = '"&q_ting_ccod&"' " & vbCrLf &_ 
				" INNER JOIN movimientos_cajas e " & vbCrLf &_
				" ON d.mcaj_ncorr = e.mcaj_ncorr " & vbCrLf &_
				" INNER JOIN sedes f " & vbCrLf &_
				" ON e.sede_ccod = f.sede_ccod " & vbCrLf &_
				" LEFT OUTER JOIN sedes g " & vbCrLf &_
				" ON a.sede_actual_destino = g.sede_ccod " & vbCrLf &_
				" INNER JOIN estados_detalle_ingresos b " & vbCrLf &_
				" ON a.edin_ccod_origen = b.edin_ccod " & vbCrLf &_
				" INNER JOIN estados_detalle_ingresos c " & vbCrLf &_
				" ON a.edin_ccod_destino = c.edin_ccod " & vbCrLf &_
				" 	order by dist_ncorr asc, ingr_ncorr_origen desc"
