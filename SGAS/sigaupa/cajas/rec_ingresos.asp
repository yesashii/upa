...
<!-- #include file = "../biblioteca/nclases/ClaAlumno.asp" -->
<%
	...
	consulta = "select pers_tnombre + ' ' + pers_tape_paterno " & vbCrLf &_
		"from personas"
	if q_pers_nrut<>"" then
		consulta = consulta + " where cast(pers_nrut as varchar)='" & q_pers_nrut& "' and cast(pers_xdv as varchar)= '"&q_pers_xdv&"'"
	end if
...
'/********************************************************************/
'/**	INICIO DE CONDICIÓN PARA SABER SI ES ÚLTIMO AÑO DE CARRERA	**/
'/****************************************************************/
rut_alumno		= q_pers_nrut
v_anio_actual	= conexion.ConsultaUno("select year(getdate())")

if rut_alumno<>"" then

	set alumno1 	= new ClaAlumno
	alumno1.Inicializa conexion, rut_alumno
	codcarrera		= alumno1.getCodCarrera
	aPers_ncorr		= alumno1.getPersNCorr

  if codcarrera<>"" then

		anioSalidaCarrera		= alumno1.getUltimoAnioCarrera(codcarrera, aPers_ncorr)
		if IsNull(anioSalidaCarrera) then anioSalidaCarrera = "0" end if
		if IsNull(v_anio_actual) then v_anio_actual = "0" end if
		'response.write("v_anio_actual: "&v_anio_actual)
		'response.write("<br/>anioSalidaCarrera: "&anioSalidaCarrera)
		'response.end()
		if(CStr(v_anio_actual) = CStr(anioSalidaCarrera)) then
				%><script>alert("¡Atención! El alumno está por titularse.\nVerifique si el alumno debe pagar/documentar Arancel de Título.");</script><%
		end if
...
'/****************************************************************/
'/**	FIN DE CONDICIÓN PARA SABER SI ES ÚLTIMO AÑO DE CARRERA	**/
'/************************************************************/
if q_pers_nrut<>"" then
...
if q_pers_nrut<>"" then
sql=conexion.consultaUno("SELECT protic.ES_Moroso((Select pers_ncorr from personas_postulante where cast(pers_nrut as varchar)='"&q_pers_nrut&"'), GETDATE())")
end if
...
function valida_pagos(miformulario) {
   ...
   for( i = 0; i < nro; i++ ) {
	  ...
	  if(str=="cc_compromisos_pendientes["+aux+"][dcom_ncompromiso]"){

		valor=comp.value;
	  }
	  if((comp.value == 4) && (str == "cc_compromisos_pendientes["+aux+"][tcom_ccod]") && (document.getElementById("cc_compromisos_pendientes["+aux+"][dcom_ncompromiso]").checked) )
	  {
		estado=true;
	  }
	...
	formulario = document.getElementById("moro");

	if( num > 0 ) {
			if(estado && formulario.innerHTML =="S"){
				alert('Posee Morosidad con la Universidad, no se puede pagar titulación hasta regular el pago');
				return false;
...
<div id="moro" name="moro" style="display: none;"><% response.write(sql) %></div>
