...
if caja_abierta <>"" then
  ...
  formulario.ProcesaForm
	...
	for fila = 0 to formulario.CuentaPost - 1
		...
		v_pepu_ccod		= formulario.ObtenerValorPost (fila, "pepu_ccod")
		...
		v_fecha_vencimiento= formulario.ObtenerValorPost (fila, "v_fecha_venc")
	...
	v_pagado = conexion.consultaUno("select protic.total_recepcionar_cuota("&tcom_ccod&",1,"&comp_ndocto&","&dcom_ncompromiso&") as saldo")
	   ...
	   if v_num_letra<>"" and v_monto_letra <>"" and v_pers_ncorr <>"" and Clng(v_pagado)=Clng(v_monto_letra) then
			  ...
			  ' ------------------------------------------------------------------		  
			   ...
			   sql = "INSERT INTO abonos (ingr_ncorr, tcom_ccod, inst_ccod, comp_ndocto, dcom_ncompromiso, abon_fabono, abon_mabono, pers_ncorr, peri_ccod,  audi_tusuario, audi_fmodificacion) "& vbCrLf &_
				 ...
				 sql = "INSERT INTO detalle_ingresos (ingr_ncorr, ting_ccod, ding_ndocto, ding_nsecuencia, ding_ncorrelativo, ding_fdocto, ding_mdetalle, ding_mdocto, edin_ccod, audi_tusuario, audi_fmodificacion) "& vbCrLf &_
					   "(SELECT " & nuevo_ingr_ncorr & ", 63,LEFT('" & v_num_letra & "',LEN('" & v_num_letra & "')-2) , "&ding_nsecuencia&",1, getdate() ,'" &  v_monto_letra & "','" & v_monto_letra & "', 6 ," & usuario & ", getdate())"
			 ...
			 sql_actualiza="update pago_electronico_pagare_upa set ingr_nfolio_referencia="&nuevo_folio_referencia&", pepu_fpago=getdate() where pepu_nidentificacion="&v_num_letra&" and pepu_ccod="&v_pepu_ccod
			 ...
			 sql_actualiza_estado="update detalle_ingresos set edin_ccod=6, audi_tusuario='PAGO ELECTRONICO' where ding_ndocto= LEFT('" & v_num_letra & "',LEN('" & v_num_letra & "')-2) and edin_ccod=1 and ding_fdocto=convert(datetime,'"&v_fecha_vencimiento&"',103) and ting_ccod= 66"
			 'response.Write("<BR><BR><PRE>" & sql_actualiza_estado & "</PRE><BR><BR>")
			 'response.End()
	...
	if nuevo_ingr_ncorr <>"" or nuevo_folio_referencia<>"" then
		sql_actualiza_estado="update pago_electronico_pagare_upa set epel_ccod=4 where  pepu_ccod="&v_pepu_ccod
...
response.Redirect("comprobante_archivo_pagare_upa_electronico.asp?q_leng=4&pepu_ccod="&v_pepu_ccod)
%>