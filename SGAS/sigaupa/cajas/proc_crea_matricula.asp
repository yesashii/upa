...
f_mantiene_carreras.Carga_Parametros "matricula_anticipada.xml", "tabla_valores"
f_mantiene_carreras.Inicializar conexion
f_mantiene_carreras.ProcesaForm
for fila = 0 to f_mantiene_carreras.CuentaPost - 1
	...
	if v_ofer_ncorr <> ""  and v_post_ncorr <> "" then
		...
		' si no se ha generado Matricula para esta oferta academica
			if(cint(v_existe_compromiso) = 0)  then
				...
				session("mensajeError")="Ya existe un compromiso creado para la carrera seleccionada."
...
if conexion.ObtenerEstadoTransaccion then
	session("mensajError")="Los datos han sido guardados correctamente"
else
	session("mensajError")="No se pudo realizar la accion solicitada"
end if
response.Redirect(request.ServerVariables("HTTP_REFERER"))
