...
if v_mcaj_ncorr="" then
 ...
 f_cargados.Carga_Parametros "archivo_pagare_upa_electronico.xml", "archivos_cargados"
 f_cargados.Inicializar conexion

sql_existentes= " SELECT protic.trunc(getdate()) as fecha,count(*) as cantidad, sum (pepu_mmonto_recaudado) as  total , "&_ 
				" min (pepu_nidentificacion) as  desde , max(pepu_nidentificacion) as hasta, isnull(epel_ccod,1) as estado, "&_
				" '<a href=""javascript:UltimoEstado('+ cast(pepu_ccod as varchar)+ ', '+ cast(isnull(epel_ccod,1) as varchar)+ ' )"">'+ 'Ver' + '</a>' as revisar " &_
				" FROM pago_electronico_pagare_upa "&_
				" group by pepu_ccod, epel_ccod "
...
function UltimoEstado(pepu_ccod,estado)
{
	var url;
	if (estado==1){
		location.href="pagar_archivo_pagare_upa_electronico.asp?q_leng=2&pepu_ccod="+pepu_ccod;
	}else if(estado==4){
		location.href="comprobante_archivo_pagare_upa_electronico.asp?q_leng=4&pepu_ccod="+pepu_ccod;
	}
}
      ...
      <tr>
        ...
        <td><table width="100%"  border="0" cellspacing="0" cellpadding="0" >
          ...
          <tr>
			...
			<br/>
			<%
			pagina.DibujarSubtitulo "Resumen con cargas de Pagare UPA"
			f_cargados.DibujaTabla()
			%>
			<form enctype="multipart/form-data" method="post" action="cargar_archivo_pagare_upa_proc.asp" name="datos" >
              <br>
			<%pagina.DibujarSubtitulo "Cargar archivo de Pagare UPA"%>			  
