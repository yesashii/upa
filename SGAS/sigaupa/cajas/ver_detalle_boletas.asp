...
'					"order by b.tdet_ccod asc"
	...
	consulta	 =  " Select a.bole_ncorr,a.tbol_ccod,isnull(dbol_mtotal,0) dbol_mtotal, isnull(dbol_mtotal,0) as c_dbol_mtotal , isnull(bole_nboleta,0) bole_nboleta , ISNULL(b.tdet_ccod,0) as tdet_ccod, d.tdet_tdesc " & vbCrLf &_
					" from boletas a " & vbCrLf &_
					" LEFT OUTER JOIN detalle_boletas b " & vbCrLf &_
					" ON a.bole_ncorr = b.bole_ncorr " & vbCrLf &_
					" LEFT OUTER JOIN personas c " & vbCrLf &_
					" ON a.pers_ncorr = c.pers_ncorr " & vbCrLf &_
					" left outer join TIPOS_DETALLE d " & vbCrLf &_
					" ON b.TDET_CCOD = d.TDET_CCOD " & vbCrLf &_
					" WHERE a.bole_ncorr = '" & q_bole_ncorr & "'" & vbCrLf &_
					"order by b.tdet_ccod asc"
					...
					<br>
                         ...
                         <table width="96%"  border="0" cellspacing="0" cellpadding="0">
							 ...
							 <tr>
								...
								<td><%f_datos_alumnos.DibujaCampo("rut_alumno")
								f_contrato.primero
								total = 0
								'cont = 0
								'dim arr_detalle()
								while f_contrato.siguiente
									'redim preserve arr_detalle(cont) 
									total = total + cdbl(f_contrato.ObtenerValor("c_dbol_mtotal"))
									
									detalle = detalle &f_contrato.ObtenerValor("tdet_tdesc")&","
									montos = montos &f_contrato.ObtenerValor("dbol_mtotal")&","
									'cont = cont + 1
									
								wend
								carrera = f_datos_alumnos.obtenervalor("carrera")
								
								'response.Write("carrera: "&carrera)
								%></td>
							 </tr>
            ...
            <td width="35%" height="20"><div align="center">
              <table width="90%"  border="0" cellspacing="0" cellpadding="0">
                <tr>
                  ...
                  <td><%'f_botonera.AgregaBotonParam "imprimir", "url", "imprimir_boleta.asp?bole_ncorr="&q_bole_ncorr&"&pers_ncorr="&q_pers_ncorr&"&pers_ncorr_aval="&q_pers_ncorr_aval&" "
				  'f_botonera.DibujaBoton("imprimir") %></td>
				  <td><%f_botonera.AgregaBotonParam "boleta_electronica", "url", "imprimir_boleta_electronica.asp?monto="&total&"&detalle="&detalle&"&f_emision="&date&"&usuario="&v_usuario&"&bole_ncorr="&q_bole_ncorr&"&pers_ncorr="&q_pers_ncorr&"&tipo_boleta="&v_tipo_boleta&"&montos="&montos&"&pers_ncorr_aval="&q_pers_ncorr_aval&"&carrera="&carrera
				  f_botonera.DibujaBoton("boleta_electronica")%></td>
                  </tr>
              </table>
            </div></td>
