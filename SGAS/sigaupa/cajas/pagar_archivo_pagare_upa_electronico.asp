...
if session("nombre_archivo") = "" and pepu_ccod="" then
...
sql_subido=" select tcom_ccod,comp_ndocto, dcom_ncompromiso,protic.obtener_nombre_completo(b.pers_ncorr,'n') as nombre,protic.trunc(pepu_fvencimiento) as  fvencimiento, "&_
			...
			" protic.trunc(pepu_frecaudacion) as  frecaudacion, pepu_nidentificacion,pepu_mvalor_cuota,pepu_mmonto_recaudado,protic.trunc(pepu_fvencimiento) as  v_fecha_venc, "&_
			...
			" where pepu_ccod="&pepu_ccod&" "&_
			" and d.eing_ccod=4 "&_
			" and a.pepu_bpagada in ('N') "&_
...
'v_num_filas=1
 ...
 f_pagadas.Carga_Parametros "archivo_pagare_upa_electronico.xml", "tabla_letras_pagada"
 f_pagadas.Inicializar conexion

	sql_pagadas=" select tcom_ccod,comp_ndocto, dcom_ncompromiso,protic.obtener_nombre_completo(b.pers_ncorr,'n') as nombre,protic.trunc(pepu_fvencimiento) as  fvencimiento, "&_
				" pepu_ccod,edin_ccod, b.pers_ncorr, pepu_nidentificacion as num_letra,pepu_mmonto_recaudado as monto_letra, protic.obtener_rut(b.pers_ncorr) as rut_alumno, "&_
        ...
        <td><table width="100%"  border="0" cellspacing="0" cellpadding="0" >
          ...
          <tr>
            <td><form method="post" action="../archivos asp/pagar_archivo_pagare_upa_proc.asp" name="datos" >
			  ...
			  <%end if%>
	  			<p>&nbsp;</p>	
			  <%pagina.DibujarSubtitulo "Pagare UPA disponibles para pagar"%>		
						<%f_formulario.dibujaTabla()%>
				<p>&nbsp;</p>		
			  <%pagina.DibujarSubtitulo "Pagare UPA ya pagadas"%>
