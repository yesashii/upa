...
sql="select top 1" & vbCrLf &_
	"CASE protic.ES_MOROSO(a.pers_ncorr, GETDATE())" & vbCrLf &_
	   		"when 'S' THEN 'SI'" & vbCrLf &_
	   		"WHEn 'N' Then  'NO'" & vbCrLf &_
	   		"END AS moroso" & vbCrLf &_
	"From alumnos a" & vbCrLf &_
	"WHERE PERS_NCORR = " & q_pers_ncorr
 
 morosidad = conexion.consultaUno(sql)
 
 'response.write morosidad
v_inst_ccod = "1"
'---------------------------------------------------------------------------------------------------
...
if EsVacio(persona.ObtenerMatrNCorr(negocio.ObtenerPeriodoAcademico("CLASES18"))) then
	...
	'response.write f_datos.obtenerdatos("moroso")

if v_pers_ncorr_cajera="7108375" then
	filtro=" and a.tdet_ccod in (1338) "
	f_cargo.AgregaCampoParam "tdet_ccod", "filtro", " tdet_ccod in (1338)"
end if

sql_existe_postulacion= "select count(*) from alumnos a, ofertas_academicas b, especialidades c, carreras d, periodos_academicos e " & vbCrLf &_
						" where a.ofer_ncorr=b.ofer_ncorr" & vbCrLf &_
						" and b.peri_ccod =e.peri_ccod " & vbCrLf &_
						" and e.plec_ccod in (1,2) " & vbCrLf &_
						"  and e.anos_ccod in (year(getdate()), year(getdate())+1)" & vbCrLf &_
						" and pers_ncorr="&q_pers_ncorr&" " & vbCrLf &_
						" and emat_ccod in (1,4,8) " & vbCrLf &_
						" and b.espe_ccod=c.espe_ccod " & vbCrLf &_
						" and c.carr_ccod=d.carr_ccod " & vbCrLf &_
						" and tcar_ccod=1 "
						
existe_matricula=conexion.consultaUno(sql_existe_postulacion)	

sql_existe_magister= "select count(*) from alumnos a, ofertas_academicas b, especialidades c, carreras d, periodos_academicos e " & vbCrLf &_
						" where a.ofer_ncorr=b.ofer_ncorr" & vbCrLf &_
						" and b.peri_ccod =e.peri_ccod " & vbCrLf &_
						" and e.plec_ccod in (1,2) " & vbCrLf &_
						" and e.anos_ccod in (year(getdate()),year(getdate())-1)" & vbCrLf &_
...
'existe_magister=conexion.consultaUno(sql_existe_magister)	
'response.Write("<pre>"&sql_existe_magister&"</pre>")	


if	existe_matricula=0 then
	filtro=" and a.tdet_ccod not in (1224) "
	bloqueo="1224"
end if		

if	existe_magister=0 then
	if existe_matricula=0 then
		filtro=" and a.tdet_ccod not in (1224,1898)"
		bloqueo="1224,1898"
	else
		filtro=" and a.tdet_ccod not in (1898) "
		bloqueo="1898"
	end if
	
end if	
if morosidad = "SI" then
	if	existe_magister=0 then
		if existe_matricula=0 then
			filtro=" and a.tdet_ccod not in (1224,1898,1230)"
			bloqueo="1224,1898,1230"
		else
			filtro=" and a.tdet_ccod not in (1898,1230) "
			bloqueo="1898,1230"
		end if
	end if
else
	filtro = " and tdet_ccod not in (1230)"
	bloque="1230"
end if
'f_cargo.AgregaCampoParam "tdet_ccod", "filtro", " tdet_bcargo='S' and isnull(tdet_bdescuento, 'N')='N' and tdet_bvigente = 'S' and tdet_ccod not in (1230)"
if existe_matricula=0 or existe_magister=0 or morosidad ="SI" then
	f_cargo.AgregaCampoParam "tdet_ccod", "filtro", " tdet_bcargo='S' and isnull(tdet_bdescuento, 'N')='N' and tdet_bvigente = 'S' and tdet_ccod not in ("&bloqueo&")"
end if		
...
sql_tipos_detalle = "select a.tcom_ccod, a.tdet_ccod, a.tdet_tdesc, a.tdet_mvalor_unitario " & vbCrLf &_
					...
					" and a.tcom_ccod not in (7) "& vbCrLf &_
					" "&filtro&" "& vbCrLf &_
					"order by a.tdet_tdesc, a.tcom_ccod"
'f_cargo.Consultar sql_tipos_detalle
'response.Write("<pre>"&bloque&"</pre>")	
