...
if session("nombre_archivo") ="" then
	session("mensaje_error")="Aun no se ha cargado un archivo valido para realizar el pago de Pagare UPA"
	response.Redirect("cargar_archivo_pagare_upa_electronico.asp")
end if
...
strFileName = Server.MapPath("..\archivos_pagare_upa_electronico\"&nombre_archivo)
	...
	<table width="90%"  border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#D8D8DE">
      ...
      <tr>
        ...
        <td><table width="100%"  border="0" cellspacing="0" cellpadding="0" >
          ...
          <tr>
            <td><form method="post" action="../archivos asp/guardar_archivo_pagare_upa_proc.asp" name="datos" >
              <br/>
			  ...
			  <br/>
			  <br/>
			  <%pagina.DibujarSubtitulo "Listado de Pagare UPA cargadas"%>
				...
				ind=0
					...
					if instr(strLine,",") and len(strLine)>0 then
						...
						<tr bgcolor="#FFFFFF">
							...
							<input type="hidden" name="letras[<%=ind%>][pepu_nidentificacion]" value="<%=arreglo(1)%>">
							<input type="hidden" name="letras[<%=ind%>][pepu_tmoneda]" value="<%=arreglo(3)%>">
							</th>
							<td><%=arreglo(0)%></td>
							<td><%=arreglo(1)%></td> 
							<td><%=arreglo(2)%></td>
							<td><%=arreglo(3)%></td>
							<td><input type="text" name="letras[<%=ind%>][pepu_mvalor_cuota]" value="<%=arreglo(4)%>" size="8"></td>
							<td><input type="text" name="letras[<%=ind%>][pepu_mvalor_mora]" value="<%=arreglo(5)%>" size="6"></td>
							<td><input type="text" name="letras[<%=ind%>][pepu_mmonto_recaudado]" value="<%=arreglo(6)%>" size="8"></td>
							<td><input type="text" name="letras[<%=ind%>][pepu_fvencimiento]" value="<%=arreglo(7)%>" size="12"></td>
							<td><input type="text" name="letras[<%=ind%>][pepu_frecaudacion]" value="<%=arreglo(8)%>" size="12"></td>
					   </tr>
					  <% 
						  ind=ind+1
