...
sql_elimina="delete from pago_electronico_letras where ingr_nfolio_referencia is null and protic.trunc(audi_fmodificacion) > protic.trunc(getdate())"
...
Set f   = fso.OpenTextFile(server.mappath("..") & "\archivos_pago_electronico\"&FileName, ForWriting, True)
f.Write strFileData
Set f = nothing
Set fso = nothing
...
response.Redirect("revisar_archivo_pago_electronico.asp?q_leng=2")
