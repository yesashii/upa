...
if tiene_grabado = "S" then
	 consulta = " select pers_ncorr,plan_ccod,nombre_empresa,ubicacion_empresa,telefono_empresa,email_empresa,nombre_encargado,asca_nregistro,asca_nfolio,protic.trunc(fecha_proceso) as fecha_proceso, "& vbCrLf &_
				...
				" where tt.asig_ccod=t2.asig_ccod and tt.plan_ccod = a.plan_ccod and t2.asig_tdesc = '%PR%CTICA PROFESIONAL') ) as horas_practica, "& vbCrLf &_
				...
				"  where tt.asig_ccod=t2.asig_ccod and tt.plan_ccod = a.plan_ccod and t2.asig_tdesc = '%PR%CTICA PROFESIONAL')) as asig_ccod,  "& vbCrLf &_
				" (Select top 1 t3.peri_ccod from alumnos tt, cargas_academicas t2, secciones t3, asignaturas t4 "& vbCrLf &_
				"  where tt.matr_ncorr=t2.matr_ncorr and t2.secc_ccod=t3.secc_ccod and t3.asig_ccod=t4.asig_ccod and t4.asig_tdesc like '%PR%CTICA profesional%' "& vbCrLf &_
				"  and tt.pers_ncorr=a.pers_ncorr and t3.carr_ccod=a.carr_ccod  ORDER BY peri_ccod DESC) as peri_ccod,isnull(informar_cae,0) as informar_cae,observaciones_cae, isnull(protic.trunc(fecha_cae),protic.trunc(getDate())) as fecha_cae"& vbCrLf &_
     ...
     consulta = " select '"&plan_consulta&"' as plan_ccod, '"&q_pers_ncorr&"' as pers_ncorr, '"&registro&"' as salu_nregistro, '"&folio&"' as salu_nfolio,'"&carr_ccod_informar&"' as carr_ccod, "& vbCrLf &_
				...
				"  and cast(tt.pers_ncorr as varchar)='"&q_pers_ncorr&"' and t3.carr_ccod='"&carr_ccod_informar&"' ORDER BY peri_ccod DESC),(select t2.asig_ccod from malla_curricular tt, asignaturas t2 "& vbCrLf &_
 				"  where tt.asig_ccod=t2.asig_ccod and cast(tt.plan_ccod as varchar) = '"&q_plan_ccod&"' and t2.asig_tdesc = '%PR%CTICA PROFESIONAL') ) as asig_ccod,  "& vbCrLf &_
				"  (Select top 1 t3.peri_ccod from alumnos tt, cargas_academicas t2, secciones t3, asignaturas t4 "& vbCrLf &_
				"  where tt.matr_ncorr=t2.matr_ncorr and t2.secc_ccod=t3.secc_ccod and t3.asig_ccod=t4.asig_ccod and t4.asig_tdesc like '%PR%CTICA profesional%' "& vbCrLf &_
				...
				"  where tt.asig_ccod=t2.asig_ccod and cast(tt.plan_ccod as varchar) = '"&q_plan_ccod&"' and t2.asig_tdesc = '%PR%CTICA PROFESIONAL') )as horas_practica, protic.trunc(getDate()) as fecha_cae "
end if
