...
SQL = "  select carr_ccod,carrera, '"&pers_ncorr&"' as pers_ncorr,tsca_ccod,case a.tsca_ccod when 1 then '<font color=#073299><strong>' "& vbCrLf &_ 
...
if pers_ncorr <> "" then
	...
	SQL = " select isnull(ACADEMICA,'N') as ACADEMICA,isnull(FINANCIERA,'N') as FINANCIERA,isnull(BIBLIOTECA,'N') as BIBLIOTECA,isnull(AUDIOVISUAL,'N') as AUDIOVISUAL,"& vbCrLf & _
	...
	'response.Write(bloqueo_para_titulos)
	'response.Write(bloqueo_para_grados)
