...
pagina.Titulo = "DETALLES CANDIDATO"
...
f_encabezado.Carga_Parametros "predictivo_tit_grados.xml", "encabezado_vb"
...
f_salidas.Carga_Parametros "predictivo_tit_grados.xml", "salidas_vb"
f_salidas.Inicializar conexion

SQL = " select a.tsca_ccod, a.saca_ncorr, (select isnull(ttt.SACA_BMUESTRA_ESCUELA,'') from salidas_carrera ttt where ttt.saca_ncorr=a.saca_ncorr) as escuela, "& vbCrLf & _
      " a.tsca_tdesc as tsca_tdesc,"+q_pers_ncorr+" as pers_ncorr,'"+q_carr_ccod+"' as carr_ccod, "& vbCrLf & _
	  " a.saca_tdesc, a.plan_ccod, (select plan_tdesc from planes_estudio tt where tt.plan_ccod=a.plan_ccod) as plan_e,"& vbCrLf & _
	  " (select espe_tdesc from planes_estudio tt,especialidades t2 where tt.plan_ccod=a.plan_ccod and tt.espe_ccod=t2.espe_ccod) as espe_e,"& vbCrLf & _
      ...
      " case PROTIC.PREDICTIVO_EGRESO_ESCUELA_VB("+q_pers_ncorr+",'"+q_carr_ccod+"',"+q_plan_ccod+",a.saca_ncorr,a.asignaturas) when 1 then 'SI' else 'NO' end as para_escuela "& vbCrLf & _
	  " from (  "& vbCrLf & _
      ...
      "			WHERE cast(a.carr_ccod as varchar) = '" & q_carr_ccod & "' and a.tsca_ccod in (1,2,4,5) "& vbCrLf & _
	  ...
	  " WHERE PROTIC.PREDICTIVO_EGRESO_TGRADOS_VB("+q_pers_ncorr+",'"+q_carr_ccod+"',"+q_plan_ccod+",a.saca_ncorr,a.asignaturas,"&anos&") = 1 "& vbCrLf & _
...
while f_salidas.siguiente
   if f_salidas.obtenerValor("escuela") = "1" then
...
grabado = conexion.consultaUno("select count(*) from EXCEPCIONES_EGRESO where cast(pers_ncorr as varchar)='"&q_pers_ncorr&"' and cast(plan_ccod as varchar)='"&q_plan_ccod&"' and carr_ccod='"&q_carr_ccod&"'")
...
lenguetas = Array(Array("Solicitud de egreso", "predictivo_tit_grados_especiales.asp?pers_ncorr=" & q_pers_ncorr&"&carr_ccod="& q_carr_ccod & "&plan_ccod="&q_plan_ccod))
%>
                ...
                <tr>
                  <td width="90%">
				      <table width="100%" cellpadding="0" cellspacing="0">
						...
						<tr>
						    <td colspan="3" height="25" align="left"><b>Salidas y Menciones a las que puede acceder el alumno</b></td>
						</tr>
						<tr>
						...
						<% if total_carrera > 0 and grabado = "0" then%>
						  <tr>
						    <td colspan="3"><div align="right">* La generación de excepción permitirá a la escuela el registro del candidato aunque el período de egreso no se encuentre habilitado.</div></td>
						  </tr>
						<%end if%>
						</form>
					  </table>	
            ...
            <td width="20%" height="20">
			   <table width="90%">
			   		<tr>
						...
						<td width="25%"><div align="center"><% if total_carrera > 0 and grabado = "0" then
						                                         f_botonera.DibujaBoton "reg_excepcion"
															   end if%></div></td>
