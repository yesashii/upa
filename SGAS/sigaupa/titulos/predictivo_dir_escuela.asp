...
pagina.Titulo = "Predictivo de egreso escuela"
...
'---------------------------------------------------------------------------------------------------
 ...
 f_busqueda.Carga_Parametros "predictivo_dir_escuela.xml", "f_busqueda"
...
f_salidas.Carga_Parametros "predictivo_dir_escuela.xml", "salidas"
...
c_periodo = " select count(*)  "& vbCrLf & _
			" from PRE_PERIODOS_EGRESO where epeg_ccod=1 and getdate() >= pegr_finicio "& vbCrLf & _
			" and getdate() <= pegr_ftermino "
periodo_activo = conexion.consultaUno(c_periodo)

if q_carga_alumnos = "1" then
	set f_candidatos = new CFormulario
	f_candidatos.Carga_Parametros "predictivo_dir_escuela.xml", "candidatos"
...
set f_excepcion = new CFormulario
	f_excepcion.Carga_Parametros "predictivo_dir_escuela.xml", "candidatos"
	f_excepcion.Inicializar conexion
	
	c_excepcion  = " select sede, jornada,pers_ncorr,      "& vbCrLf & _
				   " rut, ap_paterno + ' ' + ap_materno + ' ' + nombres as nombre_completo,    "& vbCrLf & _
				   " ano_ingreso_carrera, ultimo_estado, ultimo_periodo, ultimo_plan  as plan_ccod,'"&q_carr_ccod&"' as carr_ccod, "& vbCrLf & _
				   " isnull((select case isnull(tt.CEGR_NVB_ESCUELA,'0') when '0' then '' else 'SI' end from CANDIDATOS_EGRESO tt where tt.pers_ncorr=table1.pers_ncorr and tt.plan_ccod=table1.ultimo_plan and tt.carr_ccod='"&q_carr_ccod&"'),'') as V_B_Escuela, "& vbCrLf & _
				   " isnull((select case isnull(tt.CEGR_NVB_TITULOS,'0') when '0' then '' when '3' then 'NO' else 'SI' end from CANDIDATOS_EGRESO tt where tt.pers_ncorr=table1.pers_ncorr and tt.plan_ccod=table1.ultimo_plan and tt.carr_ccod='"&q_carr_ccod&"'),'') as V_B_Titulos, "& vbCrLf & _
				   ...
				   " )table1   "& vbCrLf & _ 
				   " where protic.PREDICTIVO_EGRESO_ESCUELA(table1.pers_ncorr,'"&q_carr_ccod&"',table1.ultimo_plan) = 1  "& vbCrLf & _
				   " AND EXISTS (SELECT 1 FROM EXCEPCIONES_EGRESO EE WHERE EE.PERS_NCORR = table1.pers_ncorr and EE.CARR_CCOD = '"&q_carr_ccod&"' AND EE.PLAN_CCOD = table1.ultimo_plan) "& vbCrLf & _ 
				   " order by sede, jornada, nombre_completo asc "
	'response.Write("<pre>"&c_excepcion&"</pre>")
	f_excepcion.Consultar c_excepcion
%>
  ...
  <tr>
	...
	<table width="90%"  border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#D8D8DE">
      ...
      <tr>
        ...
        <td><table width="100%"  border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td><%pagina.DibujarLenguetas Array("Salidas asociadas a la escuela"), 1 %></td>
          </tr>
          <tr>
            <td height="2" background="../imagenes/top_r3_c2.gif"></td>
          </tr>
            ...
            <td><div align="center"><br>
              ...
              <form name="edicion" method="get">
                <table width="98%"  border="0" align="center" cellpadding="0" cellspacing="0">
                  <tr>
                    <td align="center"><%pagina.DibujarSubtitulo "Salidas de la escuela"%>
                      <table width="98%"  border="0" align="center">
                        <tr>
				  ...
				  <%if periodo_activo = "0" then%>
				  <tr>
				     <td align="center">
					    <font color="#0033CC">EL PROCESO DE EGRESO SE ENCUENTRA CERRADO</font>
					 </td>
				  </tr>
				  <%else%>
				  <tr>
				  ...
				  <%end if%>
						  ...
						  <tr>
							<td align="center"><%pagina.DibujarSubtitulo "Alumnos informados por títulos y grados como excepción"%>
							  <table width="98%"  border="0" align="center">
								<tr>
								  <td scope="col"><div align="center"><%f_excepcion.DibujaTabla%></div></td>
								</tr>
							  </table>
							</td>
						  </tr>
