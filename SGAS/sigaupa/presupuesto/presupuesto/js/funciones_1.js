...
function inicio()
{
	//activaDetalleInicio();
	var x = $("#conceptoP");
	//esta acción desactiva el combo "Concepto Presupuestario"
...
//-------------------------------------------------
function activaDetalleInicio()
{
	var valor =  document.getElementsByName('busqueda[0][codcaja]').value;
	alert(valor);
}
...
function traeNuevoDetalle(valor_a)
{
	if(valor_a != 0)
	{
		var concepto	= document.busca_codigo.elements['busqueda[0][codcaja]'].value
...
{	
 ...
 $.ajax({
			  ...
			  type: "POST",
			  //dataType: "asp",
			  data: $("form#busca_codigo").serialize(),
...
// esta funcion trae la tabla de los meses al costado.
function traeTablaMeses(cod_pre)
{
	var area_ccod	= document.busca_codigo.elements['busqueda[0][area_ccod]'].value
	var eje_ccod  	= "";//document.busca_codigo.elements['selCombo'].value
	var foco_ccod	= "";//document.busca_codigo.elements['selCombo2'].value
	var prog_ccod   = "";//document.busca_codigo.elements['selCombo3'].value
	var proye_ccod 	= "";//document.busca_codigo.elements['selCombo4'].value
	var obje_ccod   = "";//document.busca_codigo.elements['selCombo5'].value
	var cpre_ncorr  = document.busca_codigo.elements['selCombo6'].value	
	//alert("prueba="+cod_pre);// +"&eje_ccod="+eje_ccod
	urlAux="presupuesto/trozosHtml/tablaMeses.asp?valor="+cod_pre+"&eje_ccod="+eje_ccod+"&foco_ccod="+foco_ccod

+"&prog_ccod="+prog_ccod+"&proye_ccod="+proye_ccod+"&obje_ccod="+obje_ccod+"&area_ccod="+area_ccod

+"&cpre_ncorr="+cpre_ncorr;
	$.ajax({
			async:true,
			type: "GET",			  
			url: urlAux,
...
{
//alert("aca");
var v_area_ccod		= document.busca_codigo.elements['busqueda[0][area_ccod]'].value //v_area_ccod
var codcaja			= document.busca_codigo.elements['busqueda[0][codcaja]'].value //codcaja
var v_nuevo_detalle = document.busca_codigo.elements['busqueda[0][nuevo_detalle]'].value//v_nuevo_detalle
var v_dpre_ncorr	= document.busca_codigo.elements['selCombo6'].value

	urlAux="presupuesto/proc/proc_agrega_detalle.asp?v_area_ccod="+v_area_ccod+"&codcaja="+codcaja

+"&v_nuevo_detalle="+v_nuevo_detalle+"&v_dpre_ncorr="+v_dpre_ncorr;
	//alert(urlAux);
	$.ajax({
			async:true,
			type: "GET",			  
...
{	
	...
	var codcaja			= document.busca_codigo.elements['busqueda[0][codcaja]'].value
...
{
	var eje			= document.busca_codigo.elements['selCombo'].value //codcaja
	var programa	= document.busca_codigo.elements['selCombo2'].value
	if( eje == 100)
	{
		var x = $("#conceptoP");
...
function eliminaTabla()
{
	var h = $("#tablaTotal");
	h.css("display","none");
	document.busca_codigo.elements['busqueda[0][codcaja]'].value="";
	sumaTotal();
}
//------------------------------------------------------------>>funciones calculo anual
function totalAnual(objetoDom)
{
	seleccionaTodo(objetoDom);
}

function sumaEnCambio()
{
	var total = sumaTotal();
	document.getElementById('total_solicitud').value = total;
}

function seleccionaTodo(objetoDom)
{
	objetoDom.select();
}
function sumaTotal()
{
	...
	for(var i = 0; i < 12; i++)
	{
		sumaTotal = sumaTotal + parseInt(document.getElementById('_test['+i+'][solicitado]').value);
	}
