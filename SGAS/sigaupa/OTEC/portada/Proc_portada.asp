...
'////////////////////////////////////// Revisamos si la persona que ingresa tiene los roles para el permiso solicitado //////////////////////////////////////
		...
		if tipo_usuario="D.Docencia" then
			 c_permiso = "select case count(*) when 0 then 'N' else 'S' end from sis_roles_usuarios rus, sis_roles rol where rus.srol_ncorr = rol.srol_ncorr and rol.srol_tdesc in ('OTEC_DIRDOCENCIA','DIRDOCENCIA') and cast(rus.pers_ncorr as varchar) = '"&v_pers_ncorr&"'"

		'Para Dirección de Extensión
		elseif tipo_usuario = "D.Extensión" then
			 c_permiso = "select case count(*) when 0 then 'N' else 'S' end from sis_roles_usuarios rus, sis_roles rol where rus.srol_ncorr = rol.srol_ncorr and rol.srol_tdesc in ('OTEC_DIREXTENSION','DIREXTENSION') and cast(rus.pers_ncorr as varchar) = '"&v_pers_ncorr&"'"

		'Para Call Center
		elseif tipo_usuario = "Call Center" then
			 c_permiso = "select case count(*) when 0 then 'N' else 'S' end from sis_roles_usuarios rus, sis_roles rol where rus.srol_ncorr = rol.srol_ncorr and rol.srol_tdesc in ('OTEC_CALLCENTER','CALLCENTER') and cast(rus.pers_ncorr as varchar) = '"&v_pers_ncorr&"'"

		'Para Asistente
		elseif tipo_usuario = "Asistente" then
			 c_permiso = "select case count(*) when 0 then 'N' else 'S' end from sis_roles_usuarios rus, sis_roles rol where rus.srol_ncorr = rol.srol_ncorr and rol.srol_tdesc in ('OTEC_ASISTADMISION','ASISTENTE OTEC') and cast(rus.pers_ncorr as varchar) = '"&v_pers_ncorr&"'"

		'Para Registro Curricular
		elseif tipo_usuario = "R.Curricular" then
			 c_permiso = "select case count(*) when 0 then 'N' else 'S' end from sis_roles_usuarios rus, sis_roles rol where rus.srol_ncorr = rol.srol_ncorr and rol.srol_tdesc in ('OTEC_REGCURRICULAR','REGISTRO CURRICULAR') and cast(rus.pers_ncorr as varchar) = '"&v_pers_ncorr&"'"

		'Para Escuela
		elseif tipo_usuario = "Escuela" then
			 c_permiso = "select case count(*) when 0 then 'N' else 'S' end from sis_roles_usuarios rus, sis_roles rol where rus.srol_ncorr = rol.srol_ncorr and rol.srol_tdesc in ('OTEC_ESCUELA','ESCUELA','DIR ESCUELA','COORDINADOR ACADEMICO','CAJERO - ADMISION','ESCUELA OTEC') and cast(rus.pers_ncorr as varchar) = '"&v_pers_ncorr&"'"

		'Para Personal
		elseif tipo_usuario = "Personal" then
			 c_permiso = "select case count(*) when 0 then 'N' else 'S' end from sis_roles_usuarios rus, sis_roles rol where rus.srol_ncorr = rol.srol_ncorr and rol.srol_tdesc in ('OTEC_RRHH','RRHH') and cast(rus.pers_ncorr as varchar) = '"&v_pers_ncorr&"'"
		...
		elseif tipo_usuario = "Cajero" then
			 c_permiso = "select case count(*) when 0 then 'N' else 'S' end from sis_roles_usuarios rus, sis_roles rol where rus.srol_ncorr = rol.srol_ncorr and rol.srol_tdesc in ('OTEC_CAJERO','CAJERO','CAJERO - MATRICULA','CAJERO - ADMISION') and cast(rus.pers_ncorr as varchar) = '"&v_pers_ncorr&"'"

		'Para Finanzas - Contabilidad
		elseif tipo_usuario = "Contabilidad" then
   			 c_permiso = "select case count(*) when 0 then 'N' else 'S' end from sis_roles_usuarios rus, sis_roles rol where rus.srol_ncorr = rol.srol_ncorr and rol.srol_tdesc in ('OTEC_FINANZAS','CONTABILIDAD_OTEC') and cast(rus.pers_ncorr as varchar) = '"&v_pers_ncorr&"'"

		'Para Títulos
		elseif tipo_usuario = "Títulos" then
   			 c_permiso = "select case count(*) when 0 then 'N' else 'S' end from sis_roles_usuarios rus, sis_roles rol where rus.srol_ncorr = rol.srol_ncorr and rol.srol_tdesc in ('OTEC_TITTULOSGRADOS','TITULOS Y GRADOS') and cast(rus.pers_ncorr as varchar) = '"&v_pers_ncorr&"'"

		'Para Muestra
		elseif tipo_usuario = "Muestra" then
   			 c_permiso = "select case count(*) when 0 then 'N' else 'S' end from sis_roles_usuarios rus, sis_roles rol where rus.srol_ncorr = rol.srol_ncorr and rol.srol_tdesc in ('OTEC_MUESTRA','MUESTRA_OTEC') and cast(rus.pers_ncorr as varchar) = '"&v_pers_ncorr&"'"

		'Para Externo
		elseif tipo_usuario = "Externo" then
   			 c_permiso = "select case count(*) when 0 then 'N' else 'S' end from sis_roles_usuarios rus, sis_roles rol where rus.srol_ncorr = rol.srol_ncorr and rol.srol_tdesc in ('OTEC_EXTERNO') and cast(rus.pers_ncorr as varchar) = '"&v_pers_ncorr&"'"
