...
dgso_ncorr = conectar.consultaUno("select dgso_ncorr from datos_generales_secciones_otec where cast(DCUR_NCORR as varchar)='"&dcur_ncorr&"' and cast(sede_ccod as varchar)='"&sede_ccod&"' and esot_ccod in (1,2)")
	...
	'response.Write("<br>esta_en_empresa="&esta_en_empresa)
	'response.Write("<br>esta_en_personas="&esta_en_personas)
	'response.Write("<br>empr_ncorr="&empr_ncorr)
'response.End()
...
'-----------------------------------------------------------------------------------------------------------------------------------------------------------------	
	otic_igual_empresa = 0
	if forma_pago = "4" then
		otic_igual_empresa = conectar.consultaUno("select isnull((select count(*) from ordenes_compras_otec where cast(dgso_ncorr as varchar)='"&dgso_ncorr&"' and cast(nord_compra as varchar)='"&nord_compra&"' and cast(empr_ncorr_2 as varchar)='"&empr_ncorr&"'),0)")
	end if 
	if otic_igual_empresa = "0" then 
		'response.Write(c_persona)
		...
		conexion.MensajeError "Imposible guardar esta empresa Otic a la postulación por que esta registrada como empresa para la misma orden de compra"
	end if	
