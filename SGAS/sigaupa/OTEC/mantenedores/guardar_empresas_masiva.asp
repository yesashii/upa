...
'---------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	orco_ncorr = conectar.consultaUno("select isnull((select isnull(orco_ncorr,0) from ordenes_compras_otec where cast(dgso_ncorr as varchar)='"&dgso_ncorr&"' and cast(nord_compra as varchar)='"&nord_compra&"'),0)")

	if esta_en_orden <> "0" and orco_ncorr = "0" then
		orco_ncorr = conectar.consultaUno("exec obtenerSecuencia 'ordenes_compras_otec'")
	end if

	if forma_pago = "2" or forma_pago="3" then
		if esta_en_orden = "0" then
		        orco_ncorr = conectar.consultaUno("exec obtenerSecuencia 'ordenes_compras_otec'")
				c_orden = " insert into ordenes_compras_otec (dgso_ncorr,empr_ncorr,fpot_ccod,nord_compra,AUDI_TUSUARIO,AUDI_FMODIFICACION,ORCO_NCORR)"&_
						  " values ("&dgso_ncorr&",'"&empr_ncorr&"',"&forma_pago&",'"&nord_compra&"','"&usuario&"',getDate(),"&orco_ncorr&")"
				...
				c_orden = " update ordenes_compras_otec  set empr_ncorr ="&empr_ncorr&",fpot_ccod="&forma_pago&",AUDI_TUSUARIO='"&usuario&"', AUDI_FMODIFICACION=getDate(),"&_
						  " orco_ncorr= "&orco_ncorr&" where cast(dgso_ncorr as varchar)='"&dgso_ncorr&"' and cast(nord_compra as varchar)='"&nord_compra&"'"
		end if 'if esta_en_orden = "0" then
	else

		if esta_en_orden = "0" then
		        orco_ncorr = conectar.consultaUno("exec obtenerSecuencia 'ordenes_compras_otec'")
				c_orden = " insert into ordenes_compras_otec (dgso_ncorr,empr_ncorr_2,fpot_ccod,nord_compra,AUDI_TUSUARIO,AUDI_FMODIFICACION, ORCO_NCORR)"&_
						  " values ("&dgso_ncorr&",'"&empr_ncorr&"',"&forma_pago&",'"&nord_compra&"','"&usuario&"',getDate(),"&orco_ncorr&")"
		else
...
'response.End()
auxCompra = int(nord_compra)
respuestaDir	= "postulacion_masiva_otec.asp?b[0][dcur_ncorr]="&dcur_ncorr&"&b[0][sede_ccod]="&sede_ccod&"&b[0][nord_compra]="&auxCompra&"&b[0][fpot_ccod]="&forma_pago
'---------------------debug>>
'esta_en_empresa 	= 1
'esta_en_personas = 1
'empr_ncorr				= 280957
'dgso_ncorr				= 1082
'pers_ncorr				= 280957
'c_persona				= update personas set pers_tnombre = 'FUNDACAO INIVERSIDADE DO OESTE DE SANTA CATARINA - UNOESC', audi_tusuario='15370707',audi_fmodificacion=getDate() where cast(pers_nrut as varchar)='84592369'
'c_empresa				= update empresas set empr_trazon_social='FUNDACAO INIVERSIDADE DO OESTE DE SANTA CATARINA - UNOESC',empr_tdireccion='RUA GETULIO VARGAS 2125, BARRIO FLOR DE SERRA CEP 8960-0000, JOACABA - SANTA CAT', ciud_ccod=1335,empr_tfono='5549331192636',empr_tfax='.',empr_tgiro='EDUCACION', empr_tejecutivo='KALINE ZENI',empr_temail_ejecutivo='kaline.zeni@unoesc.edu.br', audi_tusuario='15370707', audi_fmodificacion=getDate() where cast(empr_nrut as varchar)='84592369'
'esta_en_orden		= 0
'orco_ncorr				= 0
'forma_pago				= 2
'orco_ncorr				= 1391
...
'			response.Write(" Entró:  "&auxCompra)
'			response.end()
'		end if
'---------------------debug<<
response.Redirect(respuestaDir)
%>
