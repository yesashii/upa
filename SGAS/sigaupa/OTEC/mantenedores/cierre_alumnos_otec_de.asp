...
pagina.Titulo = "Cierre Alumnos por Programa"

set botonera =  new CFormulario
botonera.carga_parametros "cierre_alumnos_otec_de.xml", "botonera"
'response.End()
'---------------------------------------------------------------------------------------------------
...
'-----------------------------------------------------------------------
 ...
 f_busqueda.Carga_Parametros "cierre_alumnos_otec_de.xml", "f_busqueda"
 ...
 consulta = " select anio_admision,c.sede_ccod,c.sede_tdesc, b.dcur_ncorr,b.dcur_tdesc " & vbCrlf & _
			...
			" and a.dcur_ncorr not in (5,35) "& filtro_dcur & vbCrlf & _
...
dgso_ncorr = conexion.consultaUno("select dgso_ncorr from datos_generales_secciones_otec where cast(DCUR_NCORR as varchar)='"&DCUR_NCORR&"' and cast(sede_ccod as varchar)='"&sede_ccod&"'")
...
estado_programa = conexion.consultaUno("select esot_ccod from datos_generales_secciones_otec where cast(dgso_ncorr as varchar)='"&dgso_ncorr&"'")

'---------------------------------------------------------------------------------------------------
set datos_generales = new cformulario
datos_generales.carga_parametros "cierre_alumnos_otec_de.xml", "datos_generales"
...
listado_postulaciones.carga_parametros "cierre_alumnos_otec_de.xml", "f_listado"
...
consulta= " select a.pers_ncorr,cast(a.pers_nrut as varchar)+'-'+a.pers_xdv as rut,a.pers_nrut,a.pers_xdv, " & vbCrlf & _
		  ...
		  " protic.trunc(fecha_postulacion)as fecha_postulacion,replace(pote_nnota_final,',','.') as pote_nnota_final,pote_nasistencia, isnull(pote_nest_final,'') as pote_nest_final, " & vbCrlf & _
     	  " (select case count(*) when 0 then 'NO' else 'SI' end from certificados_emitidos_otec tt where tt.pers_ncorr=b.pers_ncorr and tt.dgso_ncorr=b.dgso_ncorr ) as solicitado, " & vbCrlf & _
		  " (select count(*) from certificados_emitidos_otec tt where tt.pers_ncorr=b.pers_ncorr and tt.dgso_ncorr=b.dgso_ncorr ) as bloqueado " & vbCrlf & _
		  " from personas a, postulacion_otec b,estados_postulacion_otec c " & vbCrlf & _
		  " where a.pers_ncorr=b.pers_ncorr and b.epot_ccod=c.epot_ccod and b.epot_ccod = 4 " & vbCrlf & _
		  " and cast(dgso_ncorr as varchar)='"&dgso_ncorr&"'  order by alumno"
...
set listado_postulaciones_asociadas = new cformulario
listado_postulaciones_asociadas.carga_parametros "cierre_alumnos_otec_de.xml", "f_listado_asociado"
listado_postulaciones_asociadas.inicializar conexion


consulta_a= " select a.pers_ncorr,cast(a.pers_nrut as varchar)+'-'+a.pers_xdv as rut,a.pers_nrut,a.pers_xdv, " & vbCrlf & _
		  " pers_tape_paterno + ' ' + pers_tape_materno +', '+ pers_tnombre  as alumno, " & vbCrlf & _
		  " c.epot_tdesc as estado_postulacion, " & vbCrlf & _
		  " case fpot_ccod when 1 then 'Persona Natural' when 2  then 'Empresa sin Sence' when 3 then 'Empresa con Sence' when 4 then 'Empresa y Otic' end as forma_pago, " & vbCrlf & _
		  " protic.trunc(fecha_asociacion)as fecha_postulacion,replace(pote_nnota_final,',','.') as pote_nnota_final,pote_nasistencia, isnull(pote_nest_final,'') as pote_nest_final, " & vbCrlf & _
     	  " (select case count(*) when 0 then 'NO' else 'SI' end from certificados_emitidos_otec tt where tt.pers_ncorr=b.pers_ncorr and tt.dgso_ncorr=b.dgso_ncorr ) as solicitado, " & vbCrlf & _
		  " (select count(*) from certificados_emitidos_otec tt where tt.pers_ncorr=b.pers_ncorr and tt.dgso_ncorr=b.dgso_ncorr ) as bloqueado " & vbCrlf & _
		  " from personas a, postulacion_asociada_otec b,estados_postulacion_otec c " & vbCrlf & _
		  " where a.pers_ncorr=b.pers_ncorr and b.epot_ccod=c.epot_ccod and b.epot_ccod = 4 " & vbCrlf & _
		  " and cast(dgso_ncorr as varchar)='"&dgso_ncorr&"' order by alumno"

'response.write("<pre>"&consulta&"</pre>")
listado_postulaciones_asociadas.consultar consulta_a
  ...
  <tr>
	...
	<table width="90%"  border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#D8D8DE">
      ...
      <tr>
        ...
        <td><table width="100%"  border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td><%pagina.DibujarLenguetas Array("Dirección de Extensión"), 1 %></td>
          </tr>
          <tr>
            <td height="2" background="../imagenes/top_r3_c2.gif"></td>
          </tr>
          <tr>
            <td><form name="edicion">
				...
				<input type="hidden" name="usua" value="<%=negocio.obtenerUsuario%>">
                <table width="95%"  border="0" align="center" cellpadding="0" cellspacing="0">
				  ...
				  <tr>
					  <td><strong>Matrículas del programa</strong></td>
				  </tr>
				  <tr>
				  ...
				  <tr>
				  	<td align="center">
									  ...
									  <tr>
									  	  <td align="center"><%
										                       if estado_programa = "3" then
															      response.Write("<font color=red>Programa Terminado</font>")
										                          botonera.agregaBotonParam "guardar_datos","deshabilitado", "true"
															   elseif estado_programa = "4" then
															      botonera.agregaBotonParam "guardar_datos","deshabilitado", "true"
										                       end if
										                         botonera.dibujaBoton "guardar_datos"%></td>
									  </tr>
									  </table>
					</td>
				  </tr>
				  ...
				  <tr>
					  <td><strong>Alumnos asociados</strong></td>
				  </tr>
				  <tr>
					  <td colspan="2"><div align="center">
									  <%listado_postulaciones_asociadas.dibujatabla()%>
					  </div></td>
				  </tr>
				  <tr>
					  <td>&nbsp;</td>
				  	...
				  	<td align="center">
									  ...
									  <tr>
									  	  <td align="center"><%
										                       if estado_programa = "3" then
															      response.Write("<font color=red>Programa Terminado</font>")
										                          botonera.agregaBotonParam "guardar_datos_asociados","deshabilitado", "true"
															   elseif estado_programa = "4" then
															      botonera.agregaBotonParam "guardar_datos_asociados","deshabilitado", "true"
										                       end if
										                         botonera.dibujaBoton "guardar_datos_asociados"%></td>
									  </tr>
									  </table>
					</td>
				  </tr>
