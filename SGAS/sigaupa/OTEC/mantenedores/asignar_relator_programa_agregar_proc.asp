...
set formulario = new cformulario
formulario.carga_parametros "asignar_relator_programa.xml", "asignar_relator"
...
for i=0 to formulario.cuentaPost - 1
	...
	seot_ncorr=conectar.consultaUno("select seot_ncorr from bloques_horarios_otec where cast(bhot_ccod as varchar)='"&bhot_ccod&"'")
	...
	if not EsVacio(bhot_ccod) and not EsVacio(pers_ncorr) and not EsVacio(sede_ccod) then
	  ...
	  consulta_cantidad_asignados= " select count(distinct pers_ncorr) from ( " & vbCrlf & _
                                   ...
                                   " and cast(a.seot_ncorr as varchar)='"&seot_ncorr&"' and trot_ccod=1 " & vbCrlf & _
                                   " union " & vbCrlf & _
	 ...
	 if cint(cantidad_asignados) <= cint(cantidad_relatores) then
	    if ya_grabado = "N" then
		    conectar.EstadoTransaccion conectar.EjecutaS("delete from bloques_relatores_otec where cast(bhot_ccod as varchar)='"&bhot_cod&"' and cast(trot_ccod as varchar)='"&trot_ccod&"'")
			SQL="insert into bloques_relatores_otec(bhot_ccod,pers_ncorr,sede_ccod,audi_tusuario,audi_fmodificacion,trot_ccod)"&_
				"values ("&bhot_ccod&","&pers_ncorr&","&sede_ccod&",'"&negocio.obtenerUsuario&"',getDate(),"&trot_ccod&")"
