...
session("url_actual")="../mantenedores/pago_relatores.asp?b[0][dcur_ncorr]="&dcur_ncorr&"&b[0][sede_ccod]="&sede_ccod&"&detalle=2&b[0][anio_admision]="&anio_admision&""
...
'response.Write("detalle = "&detalle&"</br>") 
'response.Write("anio_admision = "&anio_admision&"</br>") 
'response.Write("DCUR_NCORR = "&DCUR_NCORR&"</br>") 
'response.Write("sede_ccod = "&sede_ccod&"</br>") 
'response.Write("session = "&session("url_actual")&"</br>") 
end if

set pagina = new CPagina
pagina.Titulo = "Pago a relatores por programa"

set botonera =  new CFormulario
botonera.carga_parametros "pago_relatores.xml", "botonera"
'response.End()
'---------------------------------------------------------------------------------------------------
...
'----------------------------------------------------------------------- 
 ...
 f_busqueda.Carga_Parametros "pago_relatores.xml", "f_busqueda"
...
formulario_malla.carga_parametros "pago_relatores.xml", "f_secciones"
...
consulta ="  select '"&dgso_ncorr&"' as dgso_ncorr,cast(c.seot_tdesc as varchar)+'--'+cast(b.mote_ccod as varchar) as codigo, " & vbCrlf & _
          ...
          "  seot_ncantidad_relator as total_relatores, (select count(distinct pers_ncorr) from bloques_horarios_otec aa, bloques_relatores_otec bb where aa.bhot_ccod=bb.bhot_ccod and aa.seot_ncorr=c.seot_ncorr and trot_ccod=1) as asignados," &vbCrlf &_
		  "  '<a href=""javascript:agregar('+ cast(seot_ncorr as varchar)+','+ cast(seot_ncantidad_relator as varchar) + ',' + cast((select count(distinct pers_ncorr) from bloques_horarios_otec aa, bloques_relatores_otec bb where aa.bhot_ccod=bb.bhot_ccod and aa.seot_ncorr=c.seot_ncorr) as varchar) +',1)"">'+ 'Modificar' + '</a>'  as personalizar "& vbCrlf &_
...
consulta2 ="  select '"&dgso_ncorr&"' as dgso_ncorr,cast(c.seot_tdesc as varchar)+'--'+cast(b.mote_ccod as varchar) as codigo, " & vbCrlf & _
          "  b.mote_tdesc,c.seot_ncorr,isnull(MAOT_NHORAS_AYUDANTIA,0) as horas,protic.trunc(c.seot_finicio) as f_inicio, protic.trunc(c.seot_ftermino) as f_termino, " & vbCrlf & _
          "   (select count(distinct pers_ncorr) from bloques_horarios_otec aa, bloques_relatores_otec bb where aa.bhot_ccod=bb.bhot_ccod and aa.seot_ncorr=c.seot_ncorr and trot_ccod=2) as total_relatores," &vbCrlf &_
		  "  '<a href=""javascript:agregar('+ cast(seot_ncorr as varchar)+','+ cast(seot_ncantidad_relator as varchar) + ',' + cast((select count(distinct pers_ncorr) from bloques_horarios_otec aa, bloques_relatores_otec bb where aa.bhot_ccod=bb.bhot_ccod and aa.seot_ncorr=c.seot_ncorr) as varchar) +',2)"">'+ 'Modificar' + '</a>'  as personalizar "& vbCrlf &_
		  "  from mallas_otec a, modulos_otec b, secciones_otec c " & vbCrlf & _
          "  where a.mote_ccod=b.mote_ccod " & vbCrlf & _
          "  and a.maot_ncorr=c.maot_ncorr " & vbCrlf & _
          "  and c.dgso_ncorr='"&dgso_ncorr&"' " & vbCrlf & _
...
consulta2 = "select '' as maot_ncorr"
end if
...
set formulario_malla2 = new cformulario
formulario_malla2.carga_parametros "pago_relatores.xml", "f_secciones2"
formulario_malla2.inicializar conexion


formulario_malla2.consultar consulta2
...
function agregar(seot_ncorr,total,asignado,trot_ccod)
{
   if (asignado < total)
   	 {
	   alert("Debe asignar "+total+" relator(es) que esta(n) contemplado(s) en la propuesta del programa");
	 }
   else
     {
	   if(total == 0)
	   { 
	     alert("Se debe indicar a lo menos un relator para el programa");
	   }
	   else
	   {
	     direccion = "pago_relatores_agregar.asp?seot_ncorr=" + seot_ncorr+"&trot_ccod="+trot_ccod;
	     resultado=window.open(direccion, "ventana5","width=550,height=300,scrollbars=yes, left=380, top=100");
	   }
          ...
          <tr>
            <td><form name="edicion">
                <table width="95%"  border="0" align="center" cellpadding="0" cellspacing="0">
				  ...
				  <tr>
                    <td align="center"><strong>Relatores</strong></td>
                  </tr>
                  <tr>
				  ...
				  <tr>
				  	...
				  	<tr>
                    <td align="center"><strong>Ayudantes</strong></td>
                  </tr>

				  <tr>
                    <td><div align="right"><strong>P&aacute;ginas :</strong>                          
                      <%formulario_malla2.accesopagina%>
                    </div></td>
                  </tr>
                  <tr>
                    <td>&nbsp;</td>
