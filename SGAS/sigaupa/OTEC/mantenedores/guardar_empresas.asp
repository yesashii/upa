...
if pote_ncorr <> "" then 
	...
	empresa_igual_otic = conectar.consultaUno("select count(*) from postulacion_otec where cast(pote_ncorr as varchar)='"&pote_ncorr&"' and empr_ncorr_otic='"&pers_ncorr&"'")
	if empresa_igual_otic = "0" then 
		if norc_empresa = "0" then
				c_postulacion = "update postulacion_otec set empr_ncorr_empresa="&pers_ncorr&", audi_tusuario='"&usuario&"',audi_fmodificacion=getDate(), norc_empresa = null where cast(pote_ncorr as varchar)='"&pote_ncorr&"'"
		else
				c_postulacion = "update postulacion_otec set empr_ncorr_empresa="&pers_ncorr&", audi_tusuario='"&usuario&"',audi_fmodificacion=getDate(), norc_empresa = "&norc_empresa&" where cast(pote_ncorr as varchar)='"&pote_ncorr&"'"
		end if		    
	else
		conexion.MensajeError "Imposible guardar esta empresa a la postulación por que esta registrada como OTIC."
	end if		
end if
