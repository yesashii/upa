...
'		"f.aran_mcolegiatura, i.stpa_ccod, c.ofer_ncorr, p.jorn_tdesc, g.sede_tdesc,h.esde_ccod,sdes_mmatricula,h.sdes_mcolegiatura "  

		sql_informacion_postulante = " select  "  & vbCrLf &_
				"(select j.DIRE_TCALLE + ' ' + j.DIRE_TNRO + '  (' + k.CIUD_TDESC + ')' "  & vbCrLf &_
				" from direcciones_publica j "  & vbCrLf &_
				" INNER JOIN ciudades k "  & vbCrLf &_
				" ON j.ciud_ccod = k.ciud_ccod and j.tdir_ccod = 1 "  & vbCrLf &_
				" and j.pers_ncorr = a.pers_ncorr )  as direccion,  "  & vbCrLf &_
				" (select  k.CIUD_TDESC "  & vbCrLf &_
				" from direcciones_publica j INNER JOIN ciudades k "  & vbCrLf &_
				" ON j.ciud_ccod = k.ciud_ccod and j.tdir_ccod = 1 "  & vbCrLf &_
				" and j.pers_ncorr = a.pers_ncorr ) AS CIUD_TDESC , "  & vbCrLf &_
				" (select n.DIRE_TCALLE + ' ' + n.DIRE_TNRO + '  (' + o.CIUD_TDESC + ')' "  & vbCrLf &_
				" from direcciones_publica n INNER JOIN ciudades o "  & vbCrLf &_
				" ON n.ciud_ccod = o.ciud_ccod and n.tdir_ccod = 1 "  & vbCrLf &_
				" and n.pers_ncorr = m.pers_ncorr )  as direccion_codeudor , "  & vbCrLf &_
		" m.pers_tnombre + ' ' + m.pers_tape_paterno + ' ' + m.pers_tape_materno as nombre_codeudor, m.pers_tfono as pers_tfono_codeudor, "  & vbCrLf &_
		" a.pers_ncorr, b.post_ncorr, cast(a.pers_nrut as varchar(10))  + ' - ' + a.pers_xdv as rut, a.pers_tnombre + ' ' + a.pers_tape_paterno + ' ' + a.pers_tape_materno as nombre_completo, a.pers_nrut, a.pers_xdv, "  & vbCrLf &_
		" protic.obtener_nombre_carrera(c.ofer_ncorr, 'CE') as carrera,convert(datetime,getdate(),103) as fecha_actual,  g.sede_tdesc,  "  & vbCrLf &_
		" f.aran_mmatricula, f.aran_mcolegiatura, i.stpa_ccod,p.jorn_tdesc, "  & vbCrLf &_
		...
		" ON a.pers_ncorr = b.pers_ncorr AND b.tpos_ccod in (1,2)  "  & vbCrLf &_
		" and b.epos_ccod = 2  "  & vbCrLf &_
		" and cast(b.post_ncorr as varchar)= '"&v_post_ncorr&"'"  & vbCrLf &_
		...
		" ON c.ofer_ncorr = f.ofer_ncorr "  & vbCrLf &_
		...
		" INNER JOIN codeudor_postulacion l "  & vbCrLf &_
		" ON b.post_ncorr = l.post_ncorr  "  & vbCrLf &_
		" INNER JOIN personas_postulante m "  & vbCrLf &_
		" ON l.pers_ncorr = m.pers_ncorr "  & vbCrLf &_
		" INNER JOIN jornadas p "  & vbCrLf &_
		" ON c.jorn_ccod = p.jorn_ccod "  & vbCrLf &_
		" group by a.pers_ncorr, b.post_ncorr, a.pers_nrut, a.pers_xdv, a.pers_tnombre, a.pers_tape_paterno, a.pers_tape_materno, "  & vbCrLf &_
		" m.pers_tnombre, m.pers_tape_paterno, m.pers_tape_materno,m.pers_tfono,m.pers_ncorr,f.aran_mmatricula, "  & vbCrLf &_
		"f.aran_mcolegiatura, i.stpa_ccod, c.ofer_ncorr, p.jorn_tdesc, g.sede_tdesc,h.esde_ccod,sdes_mmatricula,h.sdes_mcolegiatura "  

		'response.Write("<pre>"&sql_informacion_postulante&"</pre>")
		'"select j.DIRE_TCALLE || ' ' || j.DIRE_TNRO || '  (' || k.CIUD_TDESC || ')'  as direccion, k.CIUD_TDESC, n.DIRE_TCALLE || ' ' || n.DIRE_TNRO  || '  (' || o.CIUD_TDESC || ')'  as direccion_codeudor,  " & vbCrLf &_
		                             '"     m.pers_tnombre || ' ' || m.pers_tape_paterno || ' ' || m.pers_tape_materno as nombre_codeudor, m.pers_tfono as pers_tfono_codeudor, " & vbCrLf &_
		                             '"     a.pers_ncorr, b.post_ncorr, a.pers_nrut || ' - ' || a.pers_xdv as rut, a.pers_tnombre || ' ' || a.pers_tape_paterno || ' ' || a.pers_tape_materno as nombre_completo, a.pers_nrut, a.pers_xdv, " & vbCrLf &_
		                             '"     obtener_nombre_carrera(c.ofer_ncorr, 'CE') as carrera, to_char(sysdate, 'dd/mm/yyyy') as fecha_actual, g.sede_tdesc, " & vbCrLf &_
									 '"	   f.aran_mmatricula, f.aran_mcolegiatura, i.stpa_ccod, p.jorn_tdesc, " & vbCrLf &_
									 '"	   nvl(f.aran_mmatricula, 0) + nvl(f.aran_mcolegiatura, 0) as subtotal, " & vbCrLf &_
									 '"	   sum(nvl(h.sdes_mmatricula, 0) + nvl(h.sdes_mcolegiatura, 0)) as total_descuentos, " & vbCrLf &_
									 '"	   nvl(f.aran_mmatricula, 0) + nvl(f.aran_mcolegiatura, 0) - sum(nvl(h.sdes_mmatricula, 0) + nvl(h.sdes_mcolegiatura, 0)) as total, " & vbCrLf &_
									 '"     sum(decode(h.esde_ccod, 1, nvl(h.sdes_mmatricula, 0) + nvl(h.sdes_mcolegiatura, 0), 0)) as total_descuentos_autorizados, " & vbCrLf &_
									 '"	   nvl(f.aran_mmatricula, 0) + nvl(f.aran_mcolegiatura, 0) - sum(decode(h.esde_ccod, 1, nvl(h.sdes_mmatricula, 0) + nvl(h.sdes_mcolegiatura, 0), 0)) as total_pagar, " & vbCrLf &_
									 '"     nvl(f.aran_mmatricula, 0) - sum(decode(h.esde_ccod, 1, nvl(h.sdes_mmatricula, 0), 0)) as total_pagar_matricula, " & vbCrLf &_
									 '"	   nvl(f.aran_mcolegiatura, 0) - sum(decode(h.esde_ccod, 1, nvl(h.sdes_mcolegiatura, 0), 0)) as total_pagar_colegiatura " & vbCrLf &_
									 '"from personas_postulante a, postulantes b, ofertas_academicas c, especialidades d, carreras e, aranceles f, sedes g, " & vbCrLf &_
									 '"     sdescuentos h, spagos i, direcciones_publica j, ciudades k, codeudor_postulacion l, personas_postulante m, direcciones_publica n, ciudades o, jornadas p " & vbCrLf &_
									 '"where a.pers_ncorr = b.pers_ncorr " & vbCrLf &_
									 '"  and b.ofer_ncorr = c.ofer_ncorr   " & vbCrLf &_
									 '"  and c.espe_ccod = d.espe_ccod " & vbCrLf &_
									 '"  and d.carr_ccod = e.carr_ccod " & vbCrLf &_
									 ...
									 '"  and j.CIUD_CCOD = k.ciud_ccod (+) " & vbCrLf &_
									 '"  and b.post_ncorr = l.post_ncorr " & vbCrLf &_
                                     '"  and l.pers_ncorr = m.pers_ncorr " & vbCrLf &_
									 '"  and m.pers_ncorr = n.pers_ncorr (+) " & vbCrLf &_
									 '"  and n.ciud_ccod = o.ciud_ccod (+) " & vbCrLf &_
									 '"  and c.jorn_ccod = p.jorn_ccod " & vbCrLf &_
									 '"  and n.tdir_ccod (+) = 1  " & vbCrLf &_
									 '"  and b.tpos_ccod = 1 " & vbCrLf &_
									 '"  and b.epos_ccod = 2 " & vbCrLf &_
									 '"  and b.post_ncorr = '" & v_post_ncorr & "' " & vbCrLf &_
									 '"group by a.pers_ncorr, b.post_ncorr, a.pers_nrut, a.pers_xdv, a.pers_tnombre, a.pers_tape_paterno, a.pers_tape_materno, " & vbCrLf &_
									 '"         e.carr_tdesc, d.espe_tdesc, g.sede_tdesc, f.aran_mmatricula, f.aran_mcolegiatura, i.stpa_ccod, c.ofer_ncorr, "  & vbCrLf &_
									 '"j.DIRE_TCALLE, j.DIRE_TNRO, k.CIUD_TDESC, m.pers_tnombre, m.pers_tape_paterno, m.pers_tape_materno, n.DIRE_TCALLE, n.DIRE_TNRO, m.pers_tfono, o.CIUD_TDESC, p.jorn_tdesc"
'			response.Write("<pre>"&sql_informacion_postulante&"</pre>")						 

		'sql_tabla_valores = "select b.tipo, b.ttipo, " & vbCrLf &_
		'                    "       decode(b.tipo, 1, a.aran_mmatricula, 2, -a.desc_matricula) as matricula, " & vbCrLf &_
		'					"	   decode(b.tipo, 1, a.aran_mcolegiatura, 2, -a.desc_colegiatura) as arancel, " & vbCrLf &_
		'					"	   decode(b.tipo, 1, a.total_arancel, 2, -a.total_descuentos) as total, " & vbCrLf &_
		'					"       decode(b.tipo, 1, a.aran_mmatricula, 2, -a.desc_matricula) as c_matricula, " & vbCrLf &_
		'					"	   decode(b.tipo, 1, a.aran_mcolegiatura, 2, -a.desc_colegiatura) as c_arancel, " & vbCrLf &_
		'					"	   decode(b.tipo, 1, a.total_arancel, 2, -a.total_descuentos) as c_total " & vbCrLf &_
		'					"from (select b.post_ncorr, b.ofer_ncorr, " & vbCrLf &_
		'					"             d.aran_mmatricula, d.aran_mcolegiatura, d.aran_mmatricula + d.aran_mcolegiatura as total_arancel, " & vbCrLf &_
		'					"             sum(nvl(f.sdes_mmatricula, 0)) as desc_matricula, sum(nvl(f.sdes_mcolegiatura, 0)) as desc_colegiatura, " & vbCrLf &_
		'					"			 sum(nvl(f.sdes_mmatricula, 0)) + sum(nvl(f.sdes_mcolegiatura, 0)) as total_descuentos " & vbCrLf &_
		...
		'					"        and cast(b.post_ncorr as varchar)= '" & v_post_ncorr & "' " & vbCrLf &_
		'					"      group by b.post_ncorr, b.ofer_ncorr, d.aran_mmatricula, d.aran_mcolegiatura " & vbCrLf &_
		'					"	) a, (select 1 as tipo, 'ARANCELES DE CARRERA' as ttipo from dual union " & vbCrLf &_
		'					"	      select 2 as tipo, 'DESCUENTOS Y CRÉDITOS AUTORIZADOS' as ttipo from dual) b " & vbCrLf &_
		'					"order by b.tipo asc"
...
sql_tabla_valores = "select b.tipo, b.ttipo, " & vbCrLf &_
					...
					"     sum(isnull(f.sdes_mmatricula, 0)) + sum(isnull(f.sdes_mcolegiatura, 0)) as total_descuentos" & vbCrLf &_
					"    from personas_postulante a INNER JOIN postulantes b " & vbCrLf &_
					"        ON a.pers_ncorr = b.pers_ncorr AND cast(b.post_ncorr as varchar)= '" & v_post_ncorr & "' " & vbCrLf &_
					"        INNER JOIN ofertas_academicas c " & vbCrLf &_
					"        ON b.ofer_ncorr = c.ofer_ncorr " & vbCrLf &_
					"        INNER JOIN aranceles d " & vbCrLf &_
					"        ON c.aran_ncorr = d.aran_ncorr " & vbCrLf &_
					"        LEFT OUTER JOIN spagos e " & vbCrLf &_
					"        ON b.post_ncorr = e.post_ncorr AND b.ofer_ncorr = e.ofer_ncorr " & vbCrLf &_
