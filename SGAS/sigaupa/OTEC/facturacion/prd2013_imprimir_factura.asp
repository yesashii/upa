...
if q_fact_ncorr=1272 or q_fact_ncorr=1273 then
	v_oc_asociada="Orden de compra N°: 142669"
end if
if q_fact_ncorr=1276 or q_fact_ncorr=1277 or q_fact_ncorr=1281 or q_fact_ncorr=1282 or q_fact_ncorr=1283 then
	v_oc_asociada="Orden de compra N°: 140385"
end if
...
'				"select top 1 ocot_nro_registro_sence from ordenes_compras_otec a, postulacion_otec b "& vbCrLf &_ 
'				   "	where cast(a.nord_compra as varchar)='"&v_oc_registro&"'  "& vbCrLf &_
'				   "	and a.dgso_ncorr=b.dgso_ncorr "& vbCrLf &_
'				"	and case when b.fpot_ccod=4 then a.empr_ncorr_2 else a.empr_ncorr end in (  "& vbCrLf &_
'				"		select empr_ncorr from facturas where fact_ncorr="&q_fact_ncorr&")  "& vbCrLf &_
'				"	and b.pote_ncorr in (select top 1 pote_ncorr from postulantes_cargos_factura where fact_ncorr="&q_fact_ncorr&") "
...
if v_correlativo="2" then
	sql_termino=" 'INICIO:02/01/'+cast(year(dgso_ftermino) as varchar)+' TERMINO:'+protic.trunc(dgso_ftermino) as duracion_programa, "
end if


if  q_fact_ncorr="910"  then
	sql_termino=" 'INICIO:15/09/2011 TERMINO:29/12/2011' as duracion_programa, "
end if

if q_fact_ncorr=1003 or q_fact_ncorr=1002 then
	sql_termino=" 'INICIO:02/01/2012 TERMINO:30/04/2012' as duracion_programa, "
end if

if q_fact_ncorr=1038  then
	sql_termino=" 'INICIO:19/04/2012 TERMINO:16/08/2012' as duracion_programa, "
end if

if  q_fact_ncorr="934" then
	v_dgso_ncorr="341"
end if

if  q_fact_ncorr="1121" or q_fact_ncorr="1122" or q_fact_ncorr="1123" or q_fact_ncorr="1124" then
	sql_termino=" 'INICIO:19/04/2012 TERMINO:16/08/2012' as duracion_programa, "
end if


if q_fact_ncorr="1155" or q_fact_ncorr="1156" or q_fact_ncorr="1157" or q_fact_ncorr="1158" or q_fact_ncorr="1159" or q_fact_ncorr="1160" then
	sql_termino=" 'INICIO:24/04/2012 TERMINO:29/09/2012' as duracion_programa, "
end if

'***********************************************

'case when len(dcur_nombre_sence)=0 then isnull(dcur_tdesc,dcur_nombre_sence) else isnull(dcur_nombre_sence,dcur_tdesc) end
sql_datos_otec =" select  case when dcur_nombre_sence is null or len(dcur_nombre_sence)=0 then dcur_tdesc else dcur_nombre_sence end as programa, "&_
				" sede_tdesc as sede,b.DCUR_NSENCE as cod_sense, b.dcur_nro_registro_sence as num_sence, "& vbCrLf &_
...
if q_fact_nfactura="6272" then
	dia		=	"30"
	mes		=	"DICIEMBRE"
	agno	=	"2011"
end if

if q_fact_nfactura="6329" or q_fact_nfactura="6330" then
	dia		=	"20"
	mes		=	"ENERO"
	agno	=	"2012"
end if

if q_fact_nfactura="7073" or q_fact_nfactura="7072" then
	dia		=	"28"
	mes		=	"FEBRERO"
	agno	=	"2013"
end if

if q_fact_nfactura="6331" then
	dia		=	"30"
	mes		=	"ENERO"
	agno	=	"2012"
end if

if q_fact_nfactura="6338" then
	dia		=	"09"
	mes		=	"MARZO"
	agno	=	"2012"
end if

if q_fact_nfactura="6344" then
	dia		=	"14"
	mes		=	"MARZO"
	agno	=	"2012"
end if
...
'	   archivo = archivo &space(80)&Ac1("",40,"I")
	...
	consulta_i= " Select isnull(cast(protic.documento_asociado_cuota(dc.tcom_ccod, dc.inst_ccod, dc.comp_ndocto, dc.dcom_ncompromiso, 'ding_ndocto') as varchar),'') as docto, "& vbCrLf &_
		...
		if  q_fact_ncorr="934" then
			v_registro_sence="3683289"
		end if

		if q_fact_ncorr=1012 then
			v_registro_sence="3930493"
		end if	
		
		if q_fact_ncorr=1069 then
			v_registro_sence="4006569"
		end if	
		
		if  q_fact_ncorr="1123" or q_fact_ncorr="1124" then
			v_registro_sence="3977870"
		end if



		if  q_fact_ncorr="1161" or q_fact_ncorr="1162" then
			v_registro_sence="4007344"
		end if


		if  q_fact_ncorr="1169" then
			v_registro_sence="4116602"
		end if
		
		if  q_fact_ncorr="1171" then
			v_registro_sence="4116914"
		end if
		
		if  q_fact_ncorr="1170" then
			v_registro_sence="4116912"
		end if
		if  q_fact_ncorr="1180" then
			v_registro_sence="4116914"
		end if
		if  q_fact_ncorr="1181" then
			v_registro_sence="4116914"
		end if
		
		if  q_fact_ncorr="1189" then
			v_registro_sence="3679524"
		end if
		if  q_fact_ncorr="1190" or q_fact_ncorr="1191" or q_fact_ncorr="1194" or q_fact_ncorr="1195" then
			v_registro_sence="4011722"
		end if

		if  q_fact_ncorr="1223" or q_fact_ncorr="1224" then
			v_registro_sence="4121234"
		end if
		
		if  q_fact_ncorr="1225" or q_fact_ncorr="1226" then
			v_registro_sence="4120625"
		end if	
		
		if  q_fact_ncorr="1227" or q_fact_ncorr="1229" then
			v_registro_sence="4120626"
		end if		

		if  q_fact_ncorr="1228" or q_fact_ncorr="1230" then
			v_registro_sence="4120624"
		end if	
		
		if  q_fact_ncorr="1251"then
			v_registro_sence="4090151"
		end if	
		if  q_fact_ncorr="1252" or q_fact_ncorr="1253" then
			v_registro_sence="4124534"
		end if	
		if  q_fact_ncorr="1254" or q_fact_ncorr="1255" then
			v_registro_sence="4105040"
		end if	
		if  q_fact_ncorr="1274" or q_fact_ncorr="1275" or q_fact_ncorr="1276" or q_fact_ncorr="1277" or q_fact_ncorr="1281" or q_fact_ncorr="1282" or q_fact_ncorr="1283" then
			v_registro_sence="4142664"
		end if
		if  q_fact_ncorr="1234"then
			v_registro_sence="4183955"
		end if	
		if  q_fact_ncorr="1272" or q_fact_ncorr="1273" then
			v_registro_sence="4203912"
		end if	
		if  q_fact_ncorr="1304"then
					v_registro_sence="4396859"
				end if
		if  q_fact_ncorr="1305"then
					v_registro_sence="4396993"
				end if
		if  q_fact_ncorr="1306"then
					v_registro_sence="4390274"
				end if		
		...
		if f_datos_otec.nroFilas > 0 then
			...
			if q_fact_ncorr=1039 then
				num_horas="125"
			end if	
			
			if q_fact_ncorr=1040 then
				num_horas="345"
			end if	
			
			if q_fact_ncorr=994 then
				programa="Curso en Técnicas de diseño de sitios web"
			end if
