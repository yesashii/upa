...
'--------------------------------------------------------------------------------------
'" ,'<a href=""javascript:ventana_modificar(1,case select top 1 isnull(empr_ncorr_otic,0) from postulacion_otec where comp_ndocto=a.comp_ndocto=a.pers_ncorr then 4 else 2 end,select top 1 dgso_ncorr from postulacion_otec where comp_ndocto=a.comp_ndocto,'+cast("&q_pers_nrut&" as varchar)+')"">Prefacturar</a>' as ventana "&vbcrlf&_



	sql_compromisos_pagar = " select a.pers_ncorr, b.comp_ndocto as c_comp_ndocto, "& vbCrLf &_
							"     case " & vbCrLf &_
							...
							"			b.tcom_ccod, b.inst_ccod, b.comp_ndocto, b.dcom_ncompromiso, b.tcom_ccod as c_tcom_ccod, " & vbCrLf &_
							...
							"			protic.documento_asociado_cuota(b.tcom_ccod, b.inst_ccod, b.comp_ndocto, b.dcom_ncompromiso, 'ding_ndocto') as num_oc,  "& vbCrLf &_ 
							...
							" ,'<a href=""javascript:ventana_modificar(2,' +cast((select case when(select top 1 isnull(empr_ncorr_otic,0) from postulacion_otec where comp_ndocto=a.comp_ndocto and pers_ncorr=a.pers_ncorr)>0 then 4 else 2 end) as varchar) +','+cast((select top 1 dgso_ncorr from postulacion_otec where comp_ndocto=a.comp_ndocto) as varchar) + ',' +cast('"&q_pers_nrut&"' as varchar)+ ')"">Prefacturar</a>' as ventana "&vbcrlf&_
							"	 from "& vbCrLf &_
...
'response.Write(sql_compromisos_pagar)
%>
...
/*function valida_pagos(miformulario) {
	...
	if ( tabla_c.CuentaSeleccionados('dcom_ncompromiso') > 0 ){
		...
		//alert(nreg);
		if (nreg >= 2){
			alert('Error: No puede facturar mas de una Orden de Compra a la vez.\nSeleccione solo una O.C. para facturar y luego siga con las siguientes.')
			return false
		}
...
function valida_pagos(miformulario) {
   ...
   if( num > 0 ) {
		...
		if(confirm('Ud. ha seleccionado '+ num +' registros para '+ msg_accion +'. ¿Desea continuar?')){
			resultado = window.open('','ventana','menubar = no; width=820;height=550; top = 0; left = 0; scrollbars= yes');
			return true;
		}
		else{
			return false;
