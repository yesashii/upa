...
v_limite_fac	=	15
v_mfac_ccod		= 	request.Form("mfac_ccod") 'Modalidad factura (Completa, Parcial)
v_monto_abono	= 	request.Form("monto_factura") 'Monto 
v_fact_ndivide	= 	request.Form("fact_ndivide") 'Divide la factura o solo se emite una sin detalle de alumnos
v_empr_ncorr 	= 	request.Form("empr_ncorr") 'empr_ncorr empresa u otic 
v_num_oc		= 	request.Form("num_oc") ' N° Orden de Compra

'(Logica inversa)
if v_fact_ndivide="" or EsVacio(v_fact_ndivide) then ' Si NO ha seleccionado la opcion, significa que se dividira si es necesario
	v_fact_ndivide="N"
end if
...
set formulario = new CFormulario
  ...
  formulario.ProcesaForm
  	...
  	for fila = 0 to formulario.CuentaPost - 1 ' --****( en rigor solo pasa una vez el ciclo)
...
'response.Write("<br><b>Estado Conexion 0: </b> "&conectar.obtenerEstadoTransaccion)
		...
		if v_dcom_ncompromiso <> "" then
				...
				'Factura por el monto ingresado (Facturacion Parcial)
				if v_monto_abono<>"" and v_mfac_ccod="2" then
					monto_saldo_cuota=v_monto_abono
				end if

					'suma = suma + monto_saldo_cuota  ' No tiene sentido hacer esta suma, pues no entra mas de una vez
					suma = monto_saldo_cuota
...
'v_cambio_anio=0 ' valor Cero no divide facturas
...
if v_fact_ndivide="S" then ' En caso que SI sea seleccionada la opcion para que no divida, solo se hace una factura
	fin_division=0 ' Cero significa que solo hara una factura
end if

'response.Write("<hr>")				
'response.Write("<b> Antes Suma Total:</b> "&suma_total&"<br>")		
'response.Write("<b> Antes Suma:</b> "&suma&"<br>")				
'response.Write("<b>Factura N°"&ind&" Valor Neto:</b> "&v_monto_neto)
'response.Write("<br><b>Factura N°"&ind&" Valor Iva:</b> "&v_monto_iva)

'#### EL CICLO FOR GENERARA TANTAS FACTURAS COMO DIVISIONES EXISTAN (CASOS DE ORDENES DE COMPRA CON MUCHOS ALUMNOS O QUE PASAN DE CURSO)  ########
fact_ncorrelativo=0
for ind = 0 to fin_division
	
	if v_fact_ndivide="N" then ' Si selecciono el boton no dividir, salta inmediato a calcular por el total del monto	
		...
		'#### segun tipos de facuras se calculan los montos			
			...
			else ' No se dividen las facturas
				if v_tfac_ccod=1 then
			...
			' No se dividen las facturas
				if v_tfac_ccod=1 then ' Afectas
					v_monto_neto=clng(suma*0.81)
					v_monto_iva=suma-v_monto_neto
				else ' Exentas
					v_monto_neto=suma
...
'response.Write("<hr>")				
'response.Write("<b> despues Suma Total:</b> "&suma_total&"<br>")		
'response.Write("<b> despues Suma:</b> "&suma&"<br>")			
'response.Write("<b>Factura N°"&ind&" Valor Neto:</b> "&v_monto_neto)
'response.Write("<br><b>Factura N°"&ind&" Valor Iva:</b> "&v_monto_iva)
	...
	'#####################################################################
		...
		sql_orco= 	" select top 1 orco_ncorr from ordenes_compras_otec  "& vbcrlf &_
					" where nord_compra="&v_num_oc&" "& vbcrlf &_
					" and empr_ncorr="&v_empr_ncorr&" or  empr_ncorr_2="&v_empr_ncorr&" "

'response.Write(sql_orco)			

		v_orco_ncorr=conectar.consultauno(sql_orco)
	
		
		v_fact_ncorr 	= conectar.consultauno("exec ObtenerSecuencia 'facturas'")
		
		SQL_INSERTA_FACTURA= 	" Insert into facturas (fact_ncorr,fact_nfactura,tfac_ccod,efac_ccod,fact_ffactura,pers_ncorr_alumno,fact_bdivide,fact_ncorrelativo, "& vbcrlf &_
								"INGR_NFOLIO_REFERENCIA,FOLIO_ABONO_FACTURA, empr_ncorr,mcaj_ncorr,audi_fmodificacion,audi_tusuario, sede_ccod,mfac_ccod,orco_ncorr) " & vbcrlf &_
								" Values("&v_fact_ncorr&","&v_fact_nfactura&","&v_tfac_ccod&",1,getdate(),"&empr_ncorr&",'"&v_fact_ndivide&"',"&fact_ncorrelativo&","& vbcrlf &_
								" "&v_folio_ref_fac&","&v_folio_abono&", "&empr_ncorr&","&v_mcaj_ncorr&",getdate(),"&v_usuario&","&v_sede&","&v_mfac_ccod&","&v_orco_ncorr&") "
	...
	'response.Write("<pre>"&sql_rango&"</pre>")
	
				v_rfca_ncorr 	= conectar.consultauno(sql_rango)
	
	'response.Write("<br>Estado 3.1: "&conectar.obtenerEstadoTransaccion)	
	...
	'response.Write("<br>Estado 4 dddd: "&conectar.obtenerEstadoTransaccion)
		...
		for fila = 0 to formulario.CuentaPost - 1
			...
			if v_dcom_ncompromiso <> "" then
				...
				' comentada porqu aumentaba el valor real del detalle_factura y del monto efectivamente pagado.
				...
				sql_inserta_detalle="insert into detalle_factura (fact_ncorr,comp_ndocto,tcom_ccod,inst_ccod,dcom_ncompromiso, " & vbcrlf &_
									...
									" "&suma&",'"&v_usuario&"',getdate()) "
				'response.Write("<pre>"&sql_inserta_detalle&"</pre>")
				conectar.EstadoTransaccion conectar.EjecutaS(sql_inserta_detalle)
				
			if v_mfac_ccod=1 or monto_detalle=0 then
...
' ******* FIN DIVISION DE FACTURAS POR AÑO O POR CANTIDAD DE ALUMNOS **************	
...
<%	
	...
	'***** actualiza el indice del correlativo de facturas para determinar cual es del año en curso y cual es del siguiente año
	if v_cambio_anio=0 then
		fact_ncorrelativo=0
	else
		fact_ncorrelativo=fact_ncorrelativo+1
	end if
