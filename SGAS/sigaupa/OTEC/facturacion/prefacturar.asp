...
if v_cambio_anio=1 and (q_tipo="2" or  q_tipo="3") then
...
if q_origen =2 then  'Despues de contratar (es decir en al Facturacion)
	...
	select case q_tipo
			...
			'Empresa  despues de contratado
				...
				sql_datos_postulante= 	" Select orco_ncorr,a.dgso_ncorr,count(a.pers_ncorr) as alumnos,a.empr_ncorr_empresa as pers_ncorr, "&vbcrlf&_
									...
									" group by orco_ncorr,a.dgso_ncorr,ocot_monto_empresa,ocot_monto_otic,a.empr_ncorr_empresa,d.dcur_ncorr,a.epot_ccod,d.dcur_tdesc,a.fpot_ccod,d.tdet_ccod,nord_compra "
		case "4"
			'Otic despues de contratado
				sql_extra=" and a.empr_ncorr_otic=e.pers_ncorr "
							
				sql_datos_postulante= 	" Select orco_ncorr,a.dgso_ncorr,count(a.pers_ncorr) as alumnos,a.empr_ncorr_otic as pers_ncorr, "&vbcrlf&_
									" d.dcur_ncorr,a.epot_ccod,d.dcur_tdesc,a.fpot_ccod, isnull(d.tdet_ccod,1281) as tdet_ccod, "&vbcrlf&_
									...
									" group by orco_ncorr,a.dgso_ncorr,ocot_monto_empresa,ocot_monto_otic,a.empr_ncorr_otic,d.dcur_ncorr,a.epot_ccod,d.dcur_tdesc,a.fpot_ccod,d.tdet_ccod,nord_compra "
	
		end select
else ' antes de hacer el contrato, es decir en la parte de la OC
...
v_orco_ncorr=	f_cargo.obtenerValor("orco_ncorr")
...
sql_datos_otec =" select case when dcur_nombre_sence is null or len(dcur_nombre_sence)=0 then dcur_tdesc else dcur_nombre_sence end as programa, "& vbCrLf &_
...
if v_cambio_anio=1 and (q_tipo="2" or  q_tipo="3") then
	...
	if q_tipo=2 then
		sql_institucion=" and tins_ccod=1"
	else
		sql_institucion=" and tins_ccod=2"
	end if

	sql_datos_otec = "select anos_ccod,b.dorc_mmonto, "& vbCrLf &_
					" (select case when dcur_nombre_sence is null or len(dcur_nombre_sence)=0 then dcur_tdesc else dcur_nombre_sence end  "& vbCrLf &_
					"	from datos_generales_secciones_otec a,  diplomados_cursos b "& vbCrLf &_
					"	where a.dcur_ncorr=b.dcur_ncorr and cast(a.dgso_ncorr as varchar)='"&q_dgso_ncorr&"') as programa, "& vbCrLf &_
					"   (select isnull(dcur_nsence,'') as  dcur_nsence "& vbCrLf &_
					"	from datos_generales_secciones_otec a,  diplomados_cursos b  "& vbCrLf &_
					"	where a.dcur_ncorr=b.dcur_ncorr and cast(a.dgso_ncorr as varchar)='"&q_dgso_ncorr&"') as cod_sence, "& vbCrLf &_
					"	'INICIO:'+protic.trunc(dorc_finicio)+' TERMINO:'+protic.trunc(dorc_ffin) as duracion_programa, "& vbCrLf &_
					"	dorc_naccion_sence as num_sence, dorc_nhoras as n_horas  "& vbCrLf &_
					"	from ordenes_compras_otec a ,detalle_ordenes_compras_otec  b "& vbCrLf &_
					"	where a.orco_ncorr= b.orco_ncorr "& vbCrLf &_
					" 	"&sql_institucion&" "& vbCrLf &_
					"	and a.orco_ncorr='"&v_orco_ncorr&"'"& vbCrLf &_
					" order by anos_ccod asc "
end if
...
'								" 	and a.ciud_ccod*=c.ciud_ccod "
		...
		sql_consulta_empresa= 	" Select a.empr_ncorr, a.empr_tnombre, a.empr_trazon_social, a.empr_nrut, a. empr_xdv, a.empr_tdireccion, a.ciud_ccod "& vbCrLf &_
								", a.empr_tfono, a.empr_tfax, a.empr_tgiro, a.empr_tejecutivo, a.empr_temail_ejecutivo, a.AUDI_TUSUARIO "& vbCrLf &_
								", a.AUDI_FMODIFICACION, c.ciud_tdesc as comuna, c.ciud_tcomuna as ciudad "& vbCrLf &_
								" from empresas a "& vbCrLf &_
								" LEFT OUTER JOIN ciudades c "& vbCrLf &_
								" ON a.ciud_ccod = c.ciud_ccod "& vbCrLf &_
								" Where empr_ncorr in "& vbCrLf &_
								"					( "& vbCrLf &_
								"						select top 1 pers_ncorr "& vbCrLf &_
								"						from personas "& vbCrLf &_
								"						where pers_nrut = "&q_pers_nrut&" "& vbCrLf &_
								"					)  "
...
'	   archivo = archivo &space(80)&Ac1("",40,"I")
	   ...
	   archivo = archivo & chr(13) & chr(10)
		...
		if f_datos_otec.nroFilas > 0 then
			...
			if v_cambio_anio=1 and (q_tipo="2" or  q_tipo="3") then
			' **** Cuando pasa de año, se muestran los valores ingresados *****
			v_anos=0
				while f_datos_otec.siguiente
					programa	= 	f_datos_otec.obtenerValor("programa")
					duracion	= 	f_datos_otec.obtenerValor("duracion_programa")
					cod_sence	= 	f_datos_otec.obtenerValor("cod_sence")
					num_horas	= 	f_datos_otec.obtenerValor("n_horas")
					num_sence	= 	f_datos_otec.obtenerValor("num_sence")
					anos_ccod	= 	f_datos_otec.obtenerValor("anos_ccod")
					v_monto		= 	f_datos_otec.obtenerValor("anos_ccod")
					
					if anos_ccod <> v_anos then
						archivo = archivo & space(30)&Ac1("Año:"&anos_ccod,9,"D")&space(5)& chr(13) &  chr(10)
					end if
					...
					archivo = archivo & space(10)&Ac1("N° Accion Sence:"&num_sence,35,"I")&space(5)& chr(13) &  chr(10)
					archivo = archivo & space(10)&Ac1("N° Horas:"&num_horas,35,"I")&space(5)& chr(13) &  chr(10)
					archivo = archivo & chr(13) &  chr(10)
					v_anos=anos_ccod
				wend
			else ' **** si no pasa de año *****
				f_datos_otec.siguiente
					...
					archivo = archivo & space(10)&Ac1("Duracion:"&duracion,50,"I")&space(5)& chr(13) &  chr(10)
					archivo = archivo & space(10)&Ac1("Cod Sence:"&cod_sence,35,"I")&space(5)& chr(13) &  chr(10)
					archivo = archivo & space(10)&Ac1("N° Sence:"&num_sence,35,"I")&space(5)& chr(13) &  chr(10)
					archivo = archivo & space(10)&Ac1("N° Horas:"&num_horas,35,"I")&space(5)& chr(13) &  chr(10)
			end if
		...
		if v_nro_alumnos > 0 then
			'v_monto_alumno	= Clng(total/v_nro_alumnos)' comentado porque no es necesario dividir el monto por alumno
			...
			for k=0 to f_listado_alumnos.nroFilas-1
				...
				archivo = 	archivo & space(5)&space(5)&Ac1(rut,10,"I")&space(2)&Ac1(nombre,35,"I")&space(10)&chr(13)&chr(10)
				filas	=	filas+1
			next
