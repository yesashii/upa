...
if q_tipo_persona=2 or q_tipo_persona=3 then
	v_pers_ncorr_empr=conexion.consultaUno("select pers_ncorr from personas where pers_nrut="&q_pers_nrut)
	v_existe_dir=conexion.consultaUno("select count(*) from personas where pers_ncorr="&v_pers_ncorr_empr)

if v_existe_dir=0 then
	...
	sql_inserta=" Insert into direcciones (pers_ncorr,dire_tdireccion,dire_tnro,dire_tfono,ciud_ccod) "&vbcrlf&_
				" values("&v_pers_ncorr_empr&", '"&v_direccion&"','','"&v_fono&"',"&v_ciudad&" ) "
...
if EsVacio(q_comp_ndocto) then
	...
	for fila = 0 to f_postulantes.CuentaPost - 1
	   ...
	   valor_dcur_ncorr	= f_postulantes.ObtenerValorPost (fila, "dcur_ncorr")
		...
		if valor_dcur_ncorr <> "" then
			...
			v_num_oc			= 	f_postulantes.ObtenerValorPost (fila, "nord_compra")
	...
	next
		...
		select case q_tipo_persona
			case "1"
				sql_datos_postulante= 	" Select a.pers_ncorr,d.dcur_ncorr,a.epot_ccod,d.dcur_tdesc,c.ofot_nmatricula+c.ofot_narancel as c_comp_mneto, "&vbcrlf&_
										"  0 as c_comp_mdescuento,c.ofot_nmatricula+c.ofot_narancel as c_comp_mdocumento "&vbcrlf&_
										" from postulacion_otec a, datos_generales_secciones_otec b ,  "&vbcrlf&_
										" ofertas_otec c , diplomados_cursos d "&vbcrlf&_
										" where a.pers_ncorr="&v_pers_ncorr &vbcrlf&_
										" and a.dgso_ncorr=a.dgso_ncorr "&vbcrlf&_
			...
			case "2"
				...
				sql_datos_postulante= 	" Select a.empr_ncorr_empresa as pers_ncorr,d.dcur_ncorr,a.epot_ccod,d.dcur_tdesc,sum(c.ofot_nmatricula)+sum(c.ofot_narancel) as c_comp_mneto, "&vbcrlf&_
									" 0 as c_comp_mdescuento,sum(isnull(c.ofot_nmatricula,0))+sum(isnull(c.ofot_narancel,0)) as c_comp_mdocumento "&vbcrlf&_
									" from postulacion_otec a, datos_generales_secciones_otec b ,  "&vbcrlf&_
									" ofertas_otec c , diplomados_cursos d "&vbcrlf&_
									" where a.empr_ncorr_empresa="&v_pers_ncorr &vbcrlf&_
									" and a.dgso_ncorr=b.dgso_ncorr "&vbcrlf&_
									" and b.dgso_ncorr=c.dgso_ncorr "&vbcrlf&_
									" and c.dcur_ncorr=d.dcur_ncorr "&vbcrlf&_
									" and d.dcur_ncorr="&v_dcur_ncorr&" "&vbcrlf&_ 
									" group by a.empr_ncorr_empresa,d.dcur_ncorr,a.epot_ccod,d.dcur_tdesc,a.fpot_ccod "
			
			case "3"
		
				sql_datos_postulante= 	" Select a.empr_ncorr_otic as pers_ncorr,d.dcur_ncorr,a.epot_ccod,d.dcur_tdesc,sum(c.ofot_nmatricula)+sum(c.ofot_narancel) as c_comp_mneto, "&vbcrlf&_
									" 0 as c_comp_mdescuento,sum(isnull(c.ofot_nmatricula,0))+sum(isnull(c.ofot_narancel,0)) as c_comp_mdocumento "&vbcrlf&_
									" from postulacion_otec a, datos_generales_secciones_otec b ,  "&vbcrlf&_
									" ofertas_otec c , diplomados_cursos d "&vbcrlf&_
									" where a.empr_ncorr_otic="&v_pers_ncorr &vbcrlf&_
									" and a.dgso_ncorr=b.dgso_ncorr "&vbcrlf&_
									" and b.dgso_ncorr=c.dgso_ncorr "&vbcrlf&_
									" and c.dcur_ncorr=d.dcur_ncorr "&vbcrlf&_
									" and d.dcur_ncorr="&v_dcur_ncorr&" "&vbcrlf&_ 
									" group by a.empr_ncorr_otic,d.dcur_ncorr,a.epot_ccod,d.dcur_tdesc,a.fpot_ccod "
			end select
'response.Write("<pre>"&sql_datos_postulante&"</pre>")

' validacion de empresa
if q_tipo_persona="2" then
...
'response.End()
	...
	f_cargo.AgregaCampoCons "spac_mneto", v_comp_mdocumento
	f_cargo.AgregaCampoCons "spac_mdescuento", 0
	...
	consulta = " select tcom_ccod, inst_ccod, comp_ndocto, pers_ncorr, tdet_ccod as c_tdet_ccod, spac_mneto as c_comp_mneto,tdet_ccod, "&vbcrlf&_
			   " spac_mdescuento as c_comp_mdescuento, spac_mpactacion as c_comp_mdocumento,  spac_mneto, spac_mdescuento, spac_mpactacion " & vbCrLf &_
	           " from sim_pactaciones where cast(comp_ndocto as varchar) = '" & q_comp_ndocto & "' "&vbcrlf&_
				" and cast(inst_ccod as varchar)= '" & q_inst_ccod & "' "&vbcrlf&_
				" and cast(tcom_ccod as varchar)= '" & q_tcom_ccod & "'"	
	'response.Write("<pre>" & consulta & "</pre>")		
	f_cargo.Consultar consulta
	
	v_comp_mdocumento = conexion.ConsultaUno("select spac_mpactacion from sim_pactaciones where cast(comp_ndocto as varchar)= '" & q_comp_ndocto & "' and tcom_ccod=7")   	
...
if isnull(v_comp_mdocumento) then
	v_comp_mdocumento=0
end if		   
...
function ValidarPactacion()
{
	...
	if (t_forma_pactacion.ObtenerValor(0, "sfpa_ncuotas")>1) {
		alert('El N° de cuotas no puede ser mayor a uno para la Orden de Compra');
