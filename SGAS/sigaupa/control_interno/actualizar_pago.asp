	...
	if ingr_mefectivo <> "" and ingr_mefectivo <> "0" then
		...
		for nA = 0 to nrAbonos - 1 'nA : numero de abono			
			if vAbono.obtenervalor("CC_COMPROMISOS_PENDIENTES",nA,"dcom_ncompromiso") <> "" and saldo_abono > 0 then
				...
				'response.Write(saldo_abono&"<- ** ->"&total_pagar&"ingreso involucrado: "&v_ingr_asociado)			
				...
				if  clng(v_total_saldo)=0 then
					...
					sql_actualiza_estado_doc=" update detalle_ingresos set edin_ccod=6 where cast(ingr_ncorr as varchar) ='"&v_ingr_asociado&"'"
...
'#################################################################################
		...
		for nI = 0 to nrDing - 1
			...
			'response.Write("<hr>ting_ccod "&ting_ccod&"<hr>")
			...
			if ting_ccod <> "6" then 'genera detalle cuando no es efectivo
				...
				if ting_ccod = "3" or ting_ccod = "14" or ting_ccod = "13" or ting_ccod = "51" then
					...
					sql = "select isnull(max(ding_ncorrelativo), 0) + 1 as nuevo_correlativo," & vbCrLf &_
						  ...
						  "where ting_ccod in(3,14,13,51) " & vbCrLf &_
...
'#################################################################################
	...
	for nA = 0 to nrAbonos - 1 'nA : numero de abono			
		...
		'==================================================================================================
					...
					'v_total_saldo_abonado 	= 	conectar.consultauno("select protic.total_recepcionar_cuota("&tcom_ccod&","&inst_ccod&","&comp_ndocto&","&dcom_ncompromiso&") ")
					...
					v_total_saldo 			= 	clng(dcom_mcompromiso) - ( clng(v_total_abonado) + clng(v_total_documentado) ) 
					
					'response.Write(" ->"& clng(dcom_mcompromiso)& " - ( "& clng(v_total_abonado) &" + "& clng(v_total_documentado) &" ) ) " )
					...
					'response.Write("Estado: "&conectar.ObtenerEstadoTransaccion)
					'conectar.EstadoTransaccion false
					'response.End()
		'==================================================================================================				
	...
	next	
		...
		'response.Write("154212221")
				if f_detalle_imprimir.ObtenerValor("ting_ccod") = "3" or f_detalle_imprimir.ObtenerValor("ting_ccod") = "14" then 'genera detalle solo para cheques              
