...
'				" "&sql_orden&" "
	...
	consulta = "select protic.obtener_rut(a.pers_ncorr) as rut, " & vbCrLf &_
				"  protic.obtener_rut (( " & vbCrLf &_
				"  SELECT top 1 pers_ncorr " & vbCrLf &_
				"  FROM codeudor_postulacion " & vbCrLf &_
				"  WHERE post_ncorr in ( SELECT pos.post_ncorr " & vbCrLf &_
				"						FROM postulantes pos INNER JOIN periodos_academicos per " & vbCrLf &_
				"						ON pos.pers_ncorr = a.pers_ncorr " & vbCrLf &_
				"						WHERE pos.peri_ccod = (select top 1 ab.peri_ccod from abonos ab where ab.ingr_ncorr = a.ingr_ncorr) " & vbCrLf &_
				"						and pos.peri_ccod = per.peri_ccod))) as rut_apoderado, " & vbCrLf &_
				"  protic.obtener_nombre_completo(a.pers_ncorr,'n') as nombre,b.ting_ccod, " & vbCrLf &_
				"    "&sql_extra&" " & vbCrLf &_
				"    a.ingr_fpago as ding_fdocto, a.ingr_mefectivo as ding_mdetalle," & vbCrLf &_
				"    a.ingr_mefectivo as ding_mdocto,b.plaz_ccod, " & vbCrLf &_
				"    protic.total_rebajado_ingreso(b.ingr_ncorr, '" & q_mcaj_ncorr & "') as anulado," & vbCrLf &_
				"    a.ingr_mefectivo - protic.total_rebajado_ingreso(b.ingr_ncorr, '" & q_mcaj_ncorr & "') as saldo" & vbCrLf &_
				"    from ingresos a LEFT OUTER JOIN detalle_ingresos b " & vbCrLf &_
				"		ON a.ingr_ncorr = b.ingr_ncorr and cast(b.ting_ccod as varchar) = '" & q_ting_ccod & "'" & vbCrLf &_
				"        WHERE a.eing_ccod not in (3,6) " & vbCrLf &_
				"        and a.ingr_mefectivo > 0 " & vbCrLf &_
				"        and cast(a.mcaj_ncorr as varchar) = '" & q_mcaj_ncorr & "' " & vbCrLf &_
				" "&sql_orden&" "
