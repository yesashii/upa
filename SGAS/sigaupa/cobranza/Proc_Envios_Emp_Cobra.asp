...
'------------------------------------------------------->>
tipoEmpresa	= request.Form("envios[0][tipo_empresa]")
empresa_envio = request.Form("envios[0][empresa_envio]")

'------------
if tipoEmpresa = 4 then
	v_edin_ccod = 100
elseif empresa_envio = "DICOM" then
	v_edin_ccod = 56
else
	v_edin_ccod = 10
end if
'response.Write("emp "&empresa_envio & " EDIN "&v_edin_ccod)
'response.End()
'-------------------------------------------------------<<
...
'response.end()					
			  ...
			  while f_documentos.Siguiente 
					...
					consulta_hmno = "UPDATE detalle_ingresos SET edin_ccod = "&v_edin_ccod&", AUDI_FMODIFICACION = getdate(), AUDI_TUSUARIO='" & audi_tusuario  & "'  WHERE edin_ccod not in (6,10) and cast(ingr_ncorr as varchar)='" & v_ingreso_hermano & "'"
...
'response.end
           ...
           consulta = "UPDATE detalle_ingresos SET edin_ccod = "&v_edin_ccod&" , AUDI_FMODIFICACION = getdate(), AUDI_TUSUARIO='" & audi_tusuario  & "' WHERE cast(envi_ncorr as varchar)='" & envio & "'"
'response.Write("<br>"&consulta)
'response.end
           conexion.EstadoTransaccion conexion.EjecutaS(consulta)	
	       consulta = 	"UPDATE detalle_envios SET edin_ccod = "&v_edin_ccod&" , AUDI_FMODIFICACION = getdate(), AUDI_TUSUARIO='" & audi_tusuario  & "' WHERE cast(envi_ncorr as varchar)='" & envio & "'"
