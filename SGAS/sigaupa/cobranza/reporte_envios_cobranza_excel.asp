...
f_detalle_envio.Carga_Parametros "tabla_vacia.xml", "tabla_vacia"
...
"	 ON ee.envi_ncorr = de.envi_ncorr "& vbCrLf 

					 if tipo_docto <> "" then					
					  consulta = consulta &"		and cast(de.ting_ccod as varchar)= '" & tipo_docto & "'"& vbCrLf
					 end if 

consulta = consulta &"	 INNER JOIN detalle_ingresos a "& vbCrLf &_
"	 ON de.ting_ccod = a.ting_ccod and de.ding_ndocto = a.ding_ndocto and de.ingr_ncorr = a.ingr_ncorr and a.ding_ncorrelativo = 1 "& vbCrLf 

					if inicio <> "" and termino <> "" then	
					  consulta = consulta & " AND convert(datetime,a.DING_FDOCTO,103) BETWEEN  convert(datetime,'" & inicio& " ',103) and convert(datetime,' " & termino & " ',103) "& vbCrLf
					end if

					if inicio = "" and termino <> "" then	
					  consulta = consulta & " AND convert(datetime,a.DING_FDOCTO,103) <= convert(datetime,'"&termino&"',103) "& vbCrLf
					end if

					if inicio <> "" and termino = "" then	
					  consulta = consulta & " AND convert(datetime,a.DING_FDOCTO,103) >= convert(datetime,'"&inicio&"',103) "& vbCrLf
					end if
...
consulta = consulta &"	 INNER JOIN ingresos b "& vbCrLf &_
"	 ON a.ingr_ncorr = b.ingr_ncorr "& vbCrLf &_
"	 INNER JOIN estados_detalle_ingresos a1 "& vbCrLf &_
"	 ON a.edin_ccod = a1.edin_ccod "& vbCrLf &_
"	 INNER JOIN familias_estados_detalle_ingr fe "& vbCrLf &_
"	 ON a1.fedi_ccod = fe.fedi_ccod "& vbCrLf 

					 if estado_docto <> "" then
					  consulta = consulta & "and cast(fe.fedi_ccod as varchar) = '" & estado_docto & "' "& vbCrLf
					end if

consulta = consulta &"	 INNER JOIN estados_detalle_ingresos h "& vbCrLf &_
"	 ON a.edin_ccod = h.edin_ccod "& vbCrLf &_
"	 INNER JOIN tipos_ingresos i "& vbCrLf &_
"	 ON a.ting_ccod = i.ting_ccod "& vbCrLf &_
"	 INNER JOIN personas j "& vbCrLf &_
"	 ON b.pers_ncorr = j.pers_ncorr "& vbCrLf 

					if rut_alumno <> "" then
					   consulta = consulta & "	   and cast(j.pers_nrut as varchar)= '" & rut_alumno & "' "& vbCrLf
					end if

consulta = consulta &"	 LEFT OUTER JOIN personas k "& vbCrLf &_
"	 ON a.pers_ncorr_codeudor = k.pers_ncorr "& vbCrLf 

					if rut_apoderado <> "" then
					   consulta = consulta & "	   and cast(k.pers_nrut as varchar)= '" & rut_apoderado & "' "& vbCrLf
					end if

consulta = consulta &"	 INNER JOIN abonos l "& vbCrLf &_
"	 ON b.ingr_ncorr = l.ingr_ncorr "& vbCrLf &_
"	 INNER JOIN detalle_compromisos m "& vbCrLf &_
...
"	 INNER JOIN postulantes n "& vbCrLf &_
"	 ON b.pers_ncorr = n.pers_ncorr and n.peri_ccod  = isnull(m.peri_ccod,n.peri_ccod) "& vbCrLf &_
"	 INNER JOIN ofertas_academicas o "& vbCrLf &_
"	 ON n.ofer_ncorr = o.ofer_ncorr "& vbCrLf 

					if sede <> "" then
					   consulta = consulta & "	  and cast(o.sede_ccod as varchar)='" & sede & "' "& vbCrLf
					end if

consulta = consulta &"	 INNER JOIN instituciones_envio hh "& vbCrLf &_
"	 ON ee.inen_ccod = hh.inen_ccod and hh.TINE_CCOD in (3,4)  "& vbCrLf 
