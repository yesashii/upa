<!-- #include file = "_controla_login.asp" -->
<%
Session.LCID=13322 ' Setea el  local ID para chile, con eso los valores numericos y de fecha se muestran en formato para chile
...
public sub print_r(arr, depth)
	...
	public sub comentario_desarrolladores(ip, texto, finalizar)
		ip_usuario = Request.ServerVariables("REMOTE_ADDR")
		ip_de_prueba = ip  '?aquí va tu IP
		'----------------------------------
...
class CConexion
    ...
    sub Inicializar(sistema)
		...
		objXMLDoc.load("C:\Sitios\SIGAUPA\biblioteca\conexion.xml")
		...
		for each node in root.childNodes
			...
			if atributo = sistema then
                ...
                dsn="sql_2008":usuario="protic":clave="pu7685bt,yet"
		...
		If con.Errors.Count > 0 then
			...
			For each error in con.errors
			  select case error.number
			  	case -2147217900
					salida = salida & "No se puede realizar la operación porque tiene elementos asociados"
			  end select
			next
...
class CFormulario
	...
	sub Carga_Parametros(nombre, idFormulario)
	    Set objXMLDoc = CreateObject("Microsoft.XMLDOM")
		objXMLDoc.async = False
		objXMLDoc.load("C:\Sitios\SIGAUPA\biblioteca\" & nombre )
	 ...
	 'formatolocal=setlocale(1033)
		...
		if nro_reg = 0 then
			...
			for i_=0 to nro_reg - 1
				...
				s = s & marca
				for j=0 to campos.count - 1
					if ucase(parametros.Item("editar")) = "TRUE"  and ucase(me.obtenerDescriptor(d(j),"permiso")) <> "LECTURAESCRITURA" and ucase(me.obtenerDescriptor(d(j),"NoIrA")) <> "N" then
...
'		next
 ...
 salida1 =" <input class=boton type=""button"" name=""" & nombre_boton & "f21"" value="""& v_texto &""" onClick=""" & str_funcion & """>"
	...
	Sub DibujaBoton_MVC(p_id_boton)
		Dim v_accion, v_texto, v_tipo_texto
		Dim salida, nombre_boton, str_funcion
		Dim v_class, v_funcion_over, v_funcion_out
		...
		str_funcion = Me.ObtenerFuncionBoton(p_id_boton)
		v_class = "click"
		v_funcion_over = "_OverBoton_MVC(this);"
		v_funcion_out = "_OutBoton_MVC(this);"
