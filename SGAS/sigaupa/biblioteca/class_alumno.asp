...
'--ACTUALIZACION--
'
'FECHA ACTUALIZACION 	:08/03/2013
'ACTUALIZADO POR	:JAIME PAINEMAL A.
'MOTIVO			:Corregir código, eliminar sentencia *=
'LINEA			:76 - 111 - 189 - 345
'********************************************************************
class CAlumno
...
'						   "  order by d.cont_ncorr desc " 
		...
		sql_datos_alumno = "select top 1 protic.obtener_nombre_completo(a.pers_ncorr,'n') as nombre_completo, " & vbCrLf &_
						   ...
						   " protic.ANO_INGRESO_UNIVERSIDAD(a.pers_ncorr) )as ano_ingreso, case d.econ_ccod when 1 then 'MATRICULADO' when 2 then 'PENDIENTE' else 'NO MATRICULADO' end as estado_matricula,  " & vbCrLf &_
						   " case isnull(e.emat_ccod,0) when 1 then 'ACTIVO' else e.emat_tdesc end  as estado_alumno " & vbCrLf &_
						   " From alumnos a INNER JOIN ofertas_academicas b " & vbCrLf &_
...
'							"  order by d.cont_ncorr desc "
		...
		sql_carrera_alumno = "select protic.obtener_nombre_completo(a.pers_ncorr,'n') as nombre_completo, " & vbCrLf &_ 
						   ...
						   " ON b.sede_ccod = c.sede_ccod " & vbCrLf &_
						   " LEFT OUTER JOIN contratos d " & vbCrLf &_
						   " ON a.matr_ncorr = d.matr_ncorr and d.econ_ccod<>3 " & vbCrLf &_
						   " INNER JOIN estados_matriculas e " & vbCrLf &_
						   " ON a.emat_ccod = e.emat_ccod " & vbCrLf &_
...
sql_carrera_alumno = "select nombre_completo, carrera, rut, sede_tdesc, ano_ingreso, estado_matricula, estado_alumno, ingreso_u,"& vbCrLf &_
							 "	   moroso," & vbCrLf &_
							 "peri_tdesc " & vbCrLf &_
							 " from (select protic.obtener_nombre_completo(a.pers_ncorr,'n') as nombre_completo, " & vbCrLf &_
						   ...
						   "	   		END AS moroso," & vbCrLf &_
							 ...
							 "  and a.emat_ccod<>9) as tabla" & vbCrLf &_
							 " union " & vbCrLf &_
							 " select nombre_completo, carrera, rut, sede_tdesc, ano_ingreso, estado_matricula, estado_alumno, ingreso_u," & vbCrLf &_
							 "moroso, " & vbCrLf &_
							 "peri_tdesc " & vbCrLf &_
							 " from  (" & vbCrLf &_
							 " select protic.obtener_nombre_completo(a.pers_ncorr,'n') as nombre_completo, " & vbCrLf &_
						   ...
						   "	   		END AS moroso," & vbCrLf &_
							 ...
							 " case isnull(a.emat_ccod,0) when 1 then 'ACTIVO' else c.emat_tdesc end  as estado_alumno, protic.ANO_INGRESO_UNIVERSIDAD(a.pers_ncorr) as ingreso_u,  " & vbCrLf &_
							 " d.peri_tdesc as peri_tdesc   " & vbCrLf &_
							 " from alumnos_salidas_intermedias a INNER JOIN salidas_carrera b " & vbCrLf &_
							 " ON a.saca_ncorr = b.saca_ncorr " & vbCrLf &_
							 " INNER JOIN estados_matriculas c " & vbCrLf &_
							 " ON a.emat_ccod = c.emat_ccod " & vbCrLf &_
							 " INNER JOIN periodos_academicos d " & vbCrLf &_
							 " ON a.peri_ccod = d.peri_ccod " & vbCrLf &_
							 " INNER JOIN carreras e " & vbCrLf &_
							 " ON b.carr_ccod = e.carr_ccod " & vbCrLf &_
							 " WHERE cast(a.pers_ncorr as varchar) = '"& pers_ncorr &"' " & vbCrLf &_
							 " order by a.emat_ccod desc  " & vbCrLf &_
							 " )as tabla "	
	...
	Sub DibujaDatos
		...
		if	not EsVacio(v_peri_ccod) then
			...
			formulario.agregacampoParam "moroso", "permiso","OCULTO"
		end if
		formulario.Consultar Me.ObtenerSql("INFORMACION_ALUMNO")
...
'			"order by nive_ccod  " & vbCrLf  
		...
		situacion_asignaturas_sql = "" & _ 
			...
			"		  UNION  " & vbCrLf  & _
			"		select distinct asig_ccod " & vbCrLf  & _
			"          FROM convalidaciones a INNER JOIN alumnos b " & vbCrLf  & _
			...
			"           and cast(b.pers_ncorr as varchar) = '" & pers_ncorr & "' " & vbCrLf  & _
			"		) a RIGHT OUTER JOIN malla_curricular b " & vbCrLf  & _
