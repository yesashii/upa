...
sql_tabla_valores = "select b.tipo, b.ttipo, " & vbCrLf &_
	...
	Sub DibujaDatos
		...
		if Me.EnvioPostulacion then						
			...
			'response.write Me.EstaMatriculado
			'if Me.EstaMatriculado then
			'	botonera.AgregaBotonParam "cambiar_info_codeudor", "deshabilitado", "TRUE"
			'else
			'	botonera.AgregaBotonParam "cambiar_info_codeudor", "deshabilitado", "FALSE"
			'end if
			botonera.AgregaBotonParam "cambiar_info_codeudor", "deshabilitado", Me.EstaMatriculado
	...
	Function EstaMatriculado
		...
		sql="select count(*) from alumnos where post_ncorr = '" & v_post_ncorr & "' and emat_ccod = 1"
		'response.write sql
		nmatriculas = CInt(conexion.ConsultaUno(sql))
