...
formbusqueda.carga_parametros "eval_asignaturas.xml", "busqueda"
formsecciones.carga_parametros "eval_asignaturas.xml", "secciones"
formprofesores.carga_parametros "eval_asignaturas.xml", "profesores"
...
set rec_secciones = conectar.ObtenerRS
	...
	'pruebas = "select a.cali_ncorr, a.secc_ccod, a.teva_ccod, a.cali_nevaluacion, a.cali_nevaluacion as c_cali_nevaluacion, cali_nponderacion, " & vbCrLf &_
	'          "       to_char(cali_fevaluacion,'dd/mm/yyyy') as cali_fevaluacion, b.teva_tdesc, " & vbCrLf &_
	'		  "	   count(d.cali_ncorr) as nnotas_ant, sum(case when e.emat_ccod = 1 then 1 else 0 end) as nnotas " & vbCrLf &_
	'		  "from calificaciones_seccion a, tipos_evaluacion b, secciones c, calificaciones_alumnos d, alumnos e  " & vbCrLf &_
	'		  "where a.teva_ccod = b.teva_ccod   " & vbCrLf &_
	'		  "  and a.secc_ccod = c.secc_ccod " & vbCrLf &_
	'		  "  and a.cali_ncorr = d.cali_ncorr (+) " & vbCrLf &_
	'		  "  and d.matr_ncorr = e.matr_ncorr (+) " & vbCrLf &_
	'		  "  and e.emat_ccod (+) = 1" & vbCrLf &_
	'		  "  and c.peri_ccod = '" & periodo & "' " & vbCrLf &_
	'		  "  and a.secc_ccod = '" & secc_ccod & "' " & vbCrLf &_
	'		  "group by a.cali_ncorr, a.secc_ccod, a.teva_ccod, a.cali_nevaluacion, cali_nponderacion, " & vbCrLf &_
	'		  "         to_char(cali_fevaluacion, 'dd/mm/yyyy'), b.teva_tdesc " & vbCrLf &_
	'		  "order by a.cali_nevaluacion"
'response.Write("<pre>"&pruebas&"</pre>")
...
if  ip_usuario="10.10.2.200" OR ip_usuario="10.10.2.27" OR ip_usuario="172.16.16.164" then'or ip_usuario="172.16.100.83"then
...
if anos_ccod = "2014" then  
	if secc_ccod <> "" and periodo <> ""  then
		...
		if sede_ccod <> "9" then 
			fecha_cierre= conectar.consultaUno("select case when convert(datetime,protic.trunc(getDate()),103) > convert(datetime,'14/07/2014',103) then 'S' else 'N' end ")
		else
			fecha_cierre= conectar.consultaUno("select case when convert(datetime,protic.trunc(getDate()),103) > convert(datetime,'26/07/2014',103) then 'S' else 'N' end ")
		end if
            ...
            <td><div align="center">
                    ...
                    <form action="" method="post" name="edicion" target="_blank" id="edicion">
                      ...
                      <table width="98%" border="0" align="center" cellpadding="5" cellspacing="0" >
						...
						<%end if%>
						<%if periodo >= "164" and secc_ccod <> "" then %>
						<tr> 
                          <td align="center">
						  <table width="90%" border="1" bordercolor="#333333">
						  	<tr>
								<td width="100%" align="center">
									<table width="100%">
										<tr><td colspan="2"><font size="2"><strong>Estimado Docente:</strong></font></td></tr>
										<tr><td colspan="2"><font size="2"><strong>A partir del año 2008, el Reglamento Académico establece, tanto para alumnos nuevos como antiguos, un mínimo de asistencia a clases como requisito para aprobar una asignatura.<br>
										                               Por esta razón, se solicita ingresar el porcentaje mínimo de asistencia correspondiente a su asignatura teniendo en consideración si se trata de una asignatura definida como teórica o práctica y lo que establece el artículo 31° del Reglamento del Alumno de Pregrado al respecto.<br>
																	   Si el porcentaje no es ingresado, el sistema exigirá por defecto un 90% de  asistencia para aprobar la asignatura.
																	   </strong></font>
										     </td>
									    <input type="hidden" name="secc_ccod_temporal" value="<%=secc_ccod%>">
										</tr>
										<tr>
											<td width="40%"><font size="2" color="#0033FF"><strong>Porcentaje de aprobación :</strong></font></td>
											<td align="left"><input type="text" name="secc_porce_asiste" size="10" maxlength="3" value="<%=porce_asignado%>" id="NU-N">%</td>
										</tr>
										<tr>
											<td width="40%">&nbsp;</td>
											<td align="left"><%botonera.dibujaboton "grabar_porcentaje"%></td>
										</tr>							   
									</table>
								</td>
							</tr>
					...
					<%if asig_cerrada <> "1" then%>
						<font color="#0000FF"><b>Observaci&oacute;n :</b></font>&nbsp;La 
                                  ...
                                  la escuela.
					<%else%>
                    &nbsp; Para editar una evaluación debe hacer click sobre el 
                    registro a editar 
					<%end if%>
					<!--<br><hr>
					<%'if tipo_asignatura ="1" then%>
					 - No olvide que ahora dispone de dos tipos de evaluaciones llamadas <strong>Pruebas Solemnes </strong>donde cada una de ellas corresponde entre el 20 y 30% de la nota final de la asignatura. Y además, el concepto de <strong>examen</strong> se debe definir como una nueva evaluación que obligadamente corresponde al 30% de la nota final.
					<%'end if%>-->
                    <p><br>
                    </p>
                  </div>
