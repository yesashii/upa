...
if ip_usuario = ip_de_prueba then
'response.Write("<pre>"&"secc_ccod = "&secc_ccod&"</br>") 
'response.Write("<pre>"&"asig_tdesc = "&asig_tdesc&"</br>") 
'response.Write("<pre>"&"ip_usuario = "&ip_usuario&"</br>") 
end if
...
'response.Write("<pre>"&"asig_tdesc = "&asig_tdesc&"</br>") 
'response.Write("<pre>"&"ip_usuario = "&ip_usuario&"</br>")
'response.Write("<pre>"&"periodo = "&periodo&"</br>") 
'response.Write("<pre>"&"sedeprofesor = "&sedeprofesor&"</br>") 
'response.Write("<pre>"&"consprofesor = "&consprofesor&"</br>") 
'response.Write("<pre>"&"consulta = "&consulta&"</br>") 
'response.Write("<pre>"&"consulta2 = "&consulta2&"</br>") 
'response.Write("<pre>"&"pers_ncorr = "&pers_ncorr&"</br>") 
end if
...
'response.Write("<pre>"&"consulta = "&consulta&"</br>") 
'response.Write("<pre>"&"consulta2 = "&consulta2&"</br>") 
'response.Write("<pre>"&"asignaturas = "&asignaturas&"</br>") 
end if
...
'response.Write("<pre>"&"consulta2 = "&consulta2&"</br>") 
'response.Write("<pre>"&"asignaturas = "&asignaturas&"</br>") 
'response.Write("<pre>"&"Secciones = "&Secciones&"</br>") 
end if
...
'response.Write("<pre>"&"asignaturas = "&asignaturas&"</br>") 
'response.Write("<pre>"&"Secciones = "&Secciones&"</br>") 
'response.Write("<pre>"&"dotos_asignatura = "&dotos_asignatura&"</br>") 
end if
...
'response.Write("<pre>"&"Secciones = "&Secciones&"</br>") 
'response.Write("<pre>"&"dotos_asignatura = "&dotos_asignatura&"</br>") 
'response.Write("<pre>"&"nombre = "&nombre&"</br>") 
end if
...
'response.Write("<pre>"&"Secciones = "&Secciones&"</br>") 
'response.Write("<pre>"&"dotos_asignatura = "&dotos_asignatura&"</br>") 
'response.Write("<pre>"&"nombre = "&nombre&"</pre>") 
'response.Write("<pre>"&"c_eva = "&c_eva&"</br>") 
end if
...
'response.Write("<pre>"&"nombre = "&nombre&"</br>") 
'response.Write("<pre>"&"c_eva = "&c_eva&"</br>") 
'response.Write("<pre>"&"total_ponderado = "&total_ponderado&"</br>") 
end if
...
'-------------------------------------------------------------------------------------------------
'se muestran todas la variables seteadas que sirven para llenar el formulario.....
if ip_usuario = ip_de_prueba then
'response.Write("secc_ccod = "&secc_ccod&"</br>") 
'response.Write("asig_tdesc = "&asig_tdesc&"</br>") 
'response.Write("ip_usuario = "&ip_usuario&"</br>")
'response.Write("periodo = "&periodo&"</br>") 
'response.Write("sedeprofesor = "&sedeprofesor&"</br>") 
'response.Write("consprofesor = "&consprofesor&"</br>") 
'response.Write("consulta = "&consulta&"</br>") 
'response.Write("consulta2 = "&consulta2&"</br>") 
'response.Write("asignaturas = "&asignaturas&"</br>") 
'response.Write("Secciones = "&Secciones&"</br>") 
'response.Write("dotos_asignatura = "&dotos_asignatura&"</br>") 
'response.Write("nombre = "&nombre&"</br>") 
'response.Write("c_eva = "&c_eva&"</br>") 
'response.Write("total_ponderado = "&total_ponderado&"</br>") 
'response.Write("c_alumnos = "&c_alumnos&"</br>") 
end if
'-------------------------------------------------------------------------------------------------
...
'--------------------------------------------seteo de asignatura cerrada: "asig_cerrada"
...
'-------------------------------------------------------------------------------------------------
'variables para habilitar ingreso de notas parciales
if ip_usuario = ip_de_prueba then
'response.Write("no_permite = "&no_permite&"</br>") 
'response.Write("asig_cerrada = "&asig_cerrada&"</br>") 
'response.Write("sys_cierra_notas = "&sys_cierra_notas&"</br>") 
'response.Write("bloqueado_cambio = "&bloqueado_cambio&"</br>") 
'response.Write("bloquear_botones = "&bloquear_botones&"</br>") 
'response.Write("autorizar = "&autorizar&"</br>") 
'response.Write("bloquear_todo = "&bloquear_todo&"</br>") 
end if
'-------------------------------------------------------------------------------------------------

'--------------------------------------------seteo de jornada y si es anual
if secc_ccod <> "" then
...
'--------------------------------------------seteo tipo de asignatura y los de periodos academico
...
'response.Write("autorizar = "&autorizar&"</br>") 
'response.Write("bloquear_todo = "&bloquear_todo&"</br>") 
'response.Write("jorn_ccod = "&jorn_ccod&"</br>") 
'response.Write("es_anual = "&es_anual&"</br>") 
'response.Write("tipo_asignatura = "&tipo_asignatura&"</br>") 
'response.Write("sql = "&sql&"</br>") 
'response.Write("anos_ccod = "&anos_ccod&"</br>") 
end if
'-------------------------------------------------------------------------------------------------

'-------------------------------------------------------------------------------------------------------------------------------------------
'--------------------------------------------seteo de bloqueado cambio, cierre de notas, autorizaciones (segun IP), y bloquear todo
...
if ip_usuario="10.10.2.27" OR ip_usuario="172.16.16.166" OR ip_usuario="172.16.100.83" then
	autorizar = true 'desbloqueado 10-07-2014
end if

'Bloqueado por personas
'---Docente: BUSTAMANTE PAVEZ GUILLERMO CARLOS
if pers_ncorr = "174203" then
	autorizar = true 
end if
...
'--------------------------------------------seteo de bloqueado y autorizacion: "bloquear_todo" y autorizar" segun resultado de query siguiente ------------
...
'response.Write("periodo = "&periodo&"</br>") 
'response.Write("secc_ccod = "&secc_ccod&"</br>") 
'response.Write("carr_seccion = "&carr_seccion&"</br>") 
'response.Write("tcar_ccod = "&tcar_ccod&"</br>") 
'response.Write("duracion_asignatura = "&duracion_asignatura&"</br>") 
end if
'-------------------------------------------------------------------------------------------------


'Periodos Lectivos
'PLEC_CCOD	PLEC_TDESC
'1			OTOÑO
'2			PRIMAVERA
'3			INVIERNO
'4			VERANO
...
'-----------------------------------------------------------------------------------------------------------------------------------------------------------
'-----------------------------------------------------------------------------------------------------------------------------------------------------------
'--------------------------------------------seteo de bloqueado y autorizacion: "bloquear_todo" y autorizar" segun PERIODO ACADEMICO -----------------------
'-----------------------------------------------------------------------------------------------------------------------------------------------------------
'-----------------------------------------------------------------------------------------------------------------------------------------------------------

'---------- para el año que se parametrice ----------
if anos_ccod = "2015" then  
	'---------- si hay seccion y periodo definido ----------
	if secc_ccod <> "" and periodo <> ""  then
		'---------- seteo de variables academicas ----------
		...
		'---------- bloquear todo si es periodo lectivo OTOÑO ----------
		if plec_ccod = "1" then
			bloquear_todo = "S"
		end if
		...
		'----------------------------------------------------------------------------------------
		'----------------------------------------------------------------------------------------
		'---------- AQUI HAY QUE COLOCAR LA NUEVA FECHA DE CIERRE DE PERIODO ACADEMICO ----------
		'----------------------------------------------------------------------------------------
		'----------------------------------------------------------------------------------------
		'---------- seteo fecha de cierre ----------
		if sede_ccod <> "9" then 
			'---------- para TODAS las sedes MENOS para LA ARAUCANA ----------
			fecha_cierre = conectar.consultaUno("select case when convert(datetime,protic.trunc(getDate()),103) > convert(datetime,'07/12/2015',103) then 'S' else 'N' end ")
		else
			'---------- para LA ARAUCANA ----------
			fecha_cierre = conectar.consultaUno("select case when convert(datetime,protic.trunc(getDate()),103) > convert(datetime,'07/12/2015',103) then 'S' else 'N' end ")
		end if
		...
		'---------- bloquear todo si fecha cierre es S, en caso contrario, no se bloquea ----------
		...
		'---------- bloquear todo si carrera es 900 ----------
		'900: CONTADOR AUDITOR (SANTIAGO)
		if carr_seccion = "900" then
			bloquear_todo = "N"
		end if
		
		'---------- Des/bloquear todo son carrera de LICENCIATURA ----------
		'10 : LICENCIATURA EN COMUNICACION VISUAL
		'35 : LICENCIATURA EN CONTABILIDAD Y AUDITORIA
		'37 : LICENCIATURA EN COMUNICACION CORPORATIVA
		'38 : LICENCIATURA EN COMUNICACION PERSUASIVA
		'39 : LICENCIATURA EN COMUNICACION SOCIAL
		'600: LICENCIATURA EN COMUNICACIONES
		if carr_seccion = "10" or carr_seccion = "35" or carr_seccion = "37" or carr_seccion = "38" or carr_seccion = "39" or carr_seccion = "600" then
			bloquear_todo = "N"
		end if
		...
		'---------- bloquear todo si tiempo de carrera es 2 o seccion es 600 o duracion de la carrera es trimenstral ----------
...
'response.Write("carr_seccion = "&carr_seccion&"</br>") 
'response.Write("duracion_asignatura = "&duracion_asignatura&"</br>") 
'response.Write("plec_ccod = "&plec_ccod&"</br>") 
'response.Write("sede_ccod = "&sede_ccod&"</br>") 
'response.Write("tcar_ccod = "&tcar_ccod&"</br>") 
end if
'-------------------------------------------------------------------------------------------------

'-------------------------------------------- seteo de vatiables academicas
...
'response.Write("autorizar = "&autorizar&"</br>") 
'response.Write("bloquear_todo = "&bloquear_todo&"</br>") 
'response.Write("fecha_cierre = "&fecha_cierre&"</br>") 
'response.Write("jorn_ccod = "&jorn_ccod&"</br>") 
'response.Write("es_anual = "&es_anual&"</br>") 
'response.Write("tipo_asignatura = "&tipo_asignatura&"</br>") 
'response.Write("sql = "&sql&"</br>") 
...
'response.Write("sede_ccod = "&sede_ccod&"</br>") 
'response.Write("tcar_ccod = "&tcar_ccod&"</br>") 
'response.Write("sede_tdesc_temp = "&sede_tdesc_temp&"</br>") 
'response.Write("carr_tdesc_temp = "&carr_tdesc_temp&"</br>") 
'response.Write("asig_ccod_temp = "&asig_ccod_temp&"</br>") 
'response.Write("asig_tdesc_temp = "&asig_tdesc_temp&"</br>") 
'response.Write("secc_tdesc_temp = "&secc_tdesc_temp&"</br>") 
'response.Write("profe_temp = "&profe_temp&"</br>") 
end if
'-------------------------------------------------------------------------------------------------
            ...
            <td><div align="center">
                    ...
                    <form action="" method="post" name="edicion" id="edicion">
                      ...
                      <table width="98%" border="0" align="center" cellpadding="5" cellspacing="0" >
						...
						<tr>
						   <td align="right">
								<!-- Habilitacion/Deshabilitar boton "Guardar" dependiendo de los parametros a considerar -->
								<%
								  
								  'variables para habilitar boton "Guardar"
								  if ip_usuario = ip_de_prueba then
								  'response.Write("<pre>"&"no_permite = "&no_permite&"</pre>") 
								  'response.Write("<pre>"&"asig_cerrada = "&asig_cerrada&"</pre>")
								  'response.Write("<pre>"&"sys_cierra_notas = "&sys_cierra_notas&"</pre>")
								  'response.Write("<pre>"&"bloqueado_cambio = "&bloqueado_cambio&"</pre>")
								  'response.Write("<pre>"&"bloquear_botones = "&bloquear_botones&"</pre>")
								  'response.Write("<pre>"&"autorizar = "&autorizar&"</pre>")
								  'response.Write("<pre>"&"bloquear_todo = "&bloquear_todo&"</pre>")
								  end if
