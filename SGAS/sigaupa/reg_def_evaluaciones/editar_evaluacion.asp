...
existe=CInt(conexion.consultauno(sql))
		'if existe <> "" then
		if existe > 0  and ip_usuario="172.16.11.127"  or ip_usuario="172.16.11.21" or ip_usuario="172.16.11.22" OR ip_usuario="172.16.16.164"  then 
		%>
...
f_evaluacion.Carga_Parametros "editar_evaluacion.xml", "f_datos_evaluacion"
...
if ip_usuario="10.10.2.200" OR ip_usuario="10.10.2.27" OR ip_usuario="172.16.16.164" then
	autorizar = false'desbloqueado 10-07-2014
end if
...
if anos_ccod < "2010" then
	if jorn_ccod="1" and autorizar=false and es_anual="0" then
		bloquear_todo= conexion.consultaUno("select case when convert(datetime,getDate(),103) >= convert(datetime,'01/01/2009',103) then 'S' else 'N' end ")
...
if anos_ccod = "2014" then
	if secc_ccod <> "" and periodo <> "" then
		...
		if sede_ccod <> "9" then 
			fecha_cierre= conexion.consultaUno("select case when convert(datetime,protic.trunc(getDate()),103) > convert(datetime,'14/07/2014',103) then 'S' else 'N' end ")
		else
			fecha_cierre= conexion.consultaUno("select case when convert(datetime,protic.trunc(getDate()),103) > convert(datetime,'26/07/2014',103) then 'S' else 'N' end ")
		end if				
...
b="<%=menordemayor%>"	
	if(preValidaFormulario(formulario)){
		...
		else{alert("No Pueden Existir Dos Evaluaciones Con La Misma Fecha")
            ...
            <td width="38%" height="20"><div align="center">
              <table width="90%"  border="0" cellspacing="0" cellpadding="0">
                <tr>
                  ...
                  <td><div align="center"><% if (asig_cerrada = "2")  or (bloqueado_cambio = TRUE) then
