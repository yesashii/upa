...
if ip_usuario="10.10.2.200" OR ip_usuario="172.16.16.166" then 'OR ip_usuario="10.10.2.27"
...
if anos_ccod = "2014" then  
	if secc_ccod <> "" and periodo <> ""  then
		...
		if sede_ccod <> "9" then 
			fecha_cierre= conectar.consultaUno("select case when convert(datetime,protic.trunc(getDate()),103) > convert(datetime,'31/01/2015',103) then 'S' else 'N' end ")
		else
			fecha_cierre= conectar.consultaUno("select case when convert(datetime,protic.trunc(getDate()),103) > convert(datetime,'31/01/2015',103) then 'S' else 'N' end ")
		end if
