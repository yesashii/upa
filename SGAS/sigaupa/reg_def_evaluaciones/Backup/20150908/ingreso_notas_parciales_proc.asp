...
'-------------------------------------seccion de asignatura seleccionada para ingresar calificaciones
secc_ccod =	request.form("secc_ccod")	


'if secc_ccod="62473" then
'	response.Write("audi_tusuario = "&audi_tusuario&" , secc_ccod = "&secc_ccod)
'	response.End()
'end if
...
while f_alumnos.siguiente
	...
	while f_eva.siguiente
	   ...
	   if calificacion_registrada = "" then calificacion_registrada = "NULL" end if
...
elseif conectar.ObtenerEstadoTransaccion = false then
	conectar.MensajeError "Ocurrio un error al grabar las calificaciones. \n\nIngresar las calificaciones de todos sus alumno, si no tiene de algun alumno colocar un 1.0 \nLa aplicacion permite modificarla a posterior. \n\nFavor vuelva a intentarlo."
end if

'response.Write("ingreso_notas_parciales_proc.asp")
'response.End()

'vuelve a pagina donde fue referenciada
