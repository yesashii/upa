...
session("url_actual")="../mantenedores/pago_relatores.asp?b[0][dcur_ncorr]="&dcur_ncorr&"&b[0][sede_ccod]="&sede_ccod&"&detalle=2"
...
consulta ="  select '"&dgso_ncorr&"' as dgso_ncorr,cast(c.seot_tdesc as varchar)+'--'+cast(b.mote_ccod as varchar) as codigo, " & vbCrlf & _
          ...
          "  seot_ncantidad_relator as total_relatores, (select count(distinct pers_ncorr) from bloques_horarios_otec aa, bloques_relatores_otec bb where aa.bhot_ccod=bb.bhot_ccod and aa.seot_ncorr=c.seot_ncorr) as asignados," &vbCrlf &_
		  "  '<a href=""javascript:agregar('+ cast(seot_ncorr as varchar)+','+ cast(seot_ncantidad_relator as varchar) + ',' + cast((select count(distinct pers_ncorr) from bloques_horarios_otec aa, bloques_relatores_otec bb where aa.bhot_ccod=bb.bhot_ccod and aa.seot_ncorr=c.seot_ncorr) as varchar) +')"">'+ 'Modificar' + '</a>'  as personalizar "& vbCrlf &_
...
function agregar(seot_ncorr,total,asignado)
{
   if (asignado < total)
   	 {
   ...
   else
     {
	   ...
	   {
	     direccion = "pago_relatores_agregar.asp?seot_ncorr=" + seot_ncorr;
