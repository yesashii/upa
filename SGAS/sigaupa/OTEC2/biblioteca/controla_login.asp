...
Class CLogin
	...
	sub Inicializa 
                dsn="sigaupa_sbd02":usuario="protic":clave=",.protic"
	...
	function CantidadAlumnosActivos ()
			sql_alumnos_activos="select count(*) from login_usuarios where datediff(mi,lusu_factualiza,getdate()) <=20 and elog_ccod=1 and lusu_tusuario='A'"
			CantidadAlumnosActivos =  me.ConsultaUnoLogin(sql_alumnos_activos)
	end function

	function CantidadActivosModulo (modulo)
			sql_activos_modulo="select count(*) from login_usuarios where datediff(mi,lusu_factualiza,getdate()) <=20 and elog_ccod=1 and lusu_tmodulo='"&modulo&"' "
