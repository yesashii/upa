	 ...
	 if RUT <> "" then
	   ...
	   if es_alumno <> "0" then
		'############################################################################################
			...
			sql_carr_ccod = "SELECT distinct(ltrim(rtrim(carr_ccod))) as carr_ccod FROM alumnos a, ofertas_academicas b, especialidades c WHERE a.ofer_ncorr=b.ofer_ncorr and b.espe_ccod=c.espe_ccod and cast(a.pers_ncorr as varchar)='"&v_pers_ncorr&"' and b.peri_ccod in ("&peri_ccod&") and a.emat_ccod in (1,4)"
			carr_ccod = conexion2.consultaUno(sql_carr_ccod)
			
			sql_sede_ccod = "SELECT sede_ccod FROM alumnos a, ofertas_academicas b WHERE a.ofer_ncorr=b.ofer_ncorr and cast(a.pers_ncorr as varchar)='"&v_pers_ncorr&"' and b.peri_ccod in ("&peri_ccod&") and a.emat_ccod=1"
...
'*****************  XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX ***************	
				...
				'end if
'*****************  XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX ***************	
				if v_mes_actual = 11 and  v_dia_actual >= 1  then
						cerrar_proceso  = false
				end if
				if v_mes_actual = 11 and  v_dia_actual <= 15  then
						cerrar_proceso  = false
				end if
			'else
...
'se habilita para mes y dia en que sera aplicada la encuesta
				if v_mes_actual = 6 and v_dia_actual >= 01 then 'a partir del 01/06
						cerrar_proceso  = false
				end if
				if v_mes_actual = 7 and v_dia_actual <= 31 then 'todo el mes de 07
						cerrar_proceso  = false
				end if
				if v_mes_actual = 8 and v_dia_actual <= 14 then 'hasta el 14/08
						cerrar_proceso  = false
				end if
