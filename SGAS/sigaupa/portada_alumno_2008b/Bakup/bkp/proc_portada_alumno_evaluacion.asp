	 ...
	 if RUT <> "" then
	   ...
	   if es_alumno <> "0" then
		'############################################################################################
			...
			sql_carr_ccod = "SELECT ltrim(rtrim(carr_ccod)) FROM alumnos a, ofertas_academicas b, especialidades c WHERE a.ofer_ncorr=b.ofer_ncorr and b.espe_ccod=c.espe_ccod and cast(a.pers_ncorr as varchar)='"&v_pers_ncorr&"' and b.peri_ccod in (234) and a.emat_ccod in (1,4)"
            carr_ccod = conexion2.consultaUno(sql_carr_ccod)
			sql_sede_ccod = "SELECT sede_ccod FROM alumnos a, ofertas_academicas b WHERE a.ofer_ncorr=b.ofer_ncorr and cast(a.pers_ncorr as varchar)='"&v_pers_ncorr&"' and b.peri_ccod in (234) and a.emat_ccod=1"
			...
			'if sede_ccod <> "4" then 
				...
				'if sede_ccod = "9" and v_mes_actual = 6 and v_dia_actual >= 9 and v_dia_actual <= 14 then
				'		cerrar_proceso  = false
				'end if
			...
			'else
				...
				'		cerrar_proceso  = false
				'end if
'*****************  PRORROGA LA EVALUACION DOCENTE 1ER SEMESTRE 2014 POR PARES EVALUADORES. SOL.: MA. TERESA MERINO ***************	
				if v_mes_actual = 6 and  v_dia_actual >= 29  then
						cerrar_proceso  = false
				end if
				if v_mes_actual = 7 then
						cerrar_proceso  = false
				end if
				if v_mes_actual = 8 and v_dia_actual >= 1 and v_dia_actual <= 14 then
						cerrar_proceso  = false
				end if
