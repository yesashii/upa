	 ...
	 if RUT <> "" then
	   ...
	   if es_alumno <> "0" then
		 ...
		 c_carr_ccod = " select top 1 ltrim(rtrim(carr_ccod)) from alumnos a, ofertas_academicas b, especialidades c "&_
					   ...
					   " and cast(a.pers_ncorr as varchar)='"&pers_ncorr&"' and b.peri_ccod in (234) and a.emat_ccod in (1,4,8) order by a.alum_fmatricula desc "'solo matricula 1er semestre 2014
		 ...
		 promedio     = conexion.consultaUno("select promedio from PROMEDIOS_ALUMNOS_CARRERA where cast(pers_ncorr as varchar)='"&pers_ncorr&"' and carr_ccod='"&carr_ccod&"' and peri_ccod=232")
		 crear_acceso=false
		 es_carrera = false
		 ...
		 if carr_ccod ="45" and v_mes_actual=1 and not esVacio(promedio) then 'Para la Carrera de Publicidad
			es_carrera = true
			crear_acceso = false
			if  v_dia_actual=20 and cDbl(promedio) >= 5.3 then 
		  		    es_carrera = true
					crear_acceso = true
					'RESPONSE.Write("9")		
			elseif anio_ingreso <= "2011" and v_dia_actual=21 then 
			...
			elseif anio_ingreso = "2012" and v_dia_actual=22 then 
				if v_hora_sys < 12 and cDbl(promedio) >= 5.0 then
					es_carrera = true
					crear_acceso = true
					'RESPONSE.Write("12")
			...
			elseif anio_ingreso = "2013" and v_dia_actual=23 then 
				if v_hora_sys < 12 and cDbl(promedio) >= 5.0 then
					es_carrera = true
					crear_acceso = true
					'RESPONSE.Write("14")
		 ...
		 elseif carr_ccod ="45" and v_mes_actual=1 and esVacio(promedio) then
		    if anio_ingreso <= "2011" and v_dia_actual=21  and v_hora_sys >= 12 then 
				es_carrera = true
				crear_acceso = true
				'RESPONSE.Write("11")			   
			elseif anio_ingreso = "2012" and v_dia_actual=22 and v_hora_sys >= 12 then 
				es_carrera = true
				crear_acceso = true			   
				'RESPONSE.Write("13")
			elseif anio_ingreso = "2013" and v_dia_actual=23 and v_hora_sys >= 12 then 
				es_carrera = true
				crear_acceso = true
				'RESPONSE.Write("15")
			end if
		 ...
		 if carr_ccod ="45" then
			es_carrera = true	
			if ((v_mes_actual=1 and v_dia_actual >= 24)) then
				crear_acceso = true
			end if
			if ((v_mes_actual=2 and v_dia_actual <= 28)) then
				crear_acceso = true
			end if
		 end if 
		 ...
		 if carr_ccod ="43" then
			es_carrera = true	
			if v_mes_actual=1 and (v_dia_actual = 13 or v_dia_actual = 14  or v_dia_actual = 15) and  anio_ingreso <= "2010" then
				crear_acceso = true
			elseif v_mes_actual=1 and (v_dia_actual = 16 or v_dia_actual = 17 or v_dia_actual = 18 or v_dia_actual = 19) and  anio_ingreso = "2011" then
				crear_acceso = true
			elseif v_mes_actual=1 and (v_dia_actual = 20 or v_dia_actual = 21 or v_dia_actual = 22) and  anio_ingreso = "2012" then
				crear_acceso = true	
			elseif v_mes_actual=1 and (v_dia_actual = 23 or v_dia_actual = 24 or v_dia_actual = 25 or v_dia_actual = 26) and  anio_ingreso = "2013" then
				crear_acceso = true	
			end if
		 end if 
		 ...
		 if carr_ccod ="47" then
			es_carrera = true	
			if v_mes_actual=1 and (v_dia_actual = 14  or v_dia_actual = 15) and  anio_ingreso <= "2011" then
				crear_acceso = true
			elseif v_mes_actual=1 and (v_dia_actual = 16) and  anio_ingreso = "2012" then
				crear_acceso = true
			elseif v_mes_actual=1 and (v_dia_actual = 17) and  anio_ingreso = "2013" then
				crear_acceso = true	
			end if
		 end if 
		 ...
		 if carr_ccod ="47" then
			es_carrera = true	
			if v_mes_actual=1 and v_dia_actual >= 18 then
				crear_acceso = true
			end if
			if v_mes_actual=2 and v_dia_actual <= 28 then
				crear_acceso = true
		 ...
		 if carr_ccod ="830" or carr_ccod ="104" or carr_ccod ="800" or carr_ccod ="8"   or carr_ccod ="99"  or carr_ccod ="14" or carr_ccod ="17" or carr_ccod ="16" or carr_ccod ="32" or carr_ccod ="103" or carr_ccod ="51" or carr_ccod ="110" or carr_ccod ="106" or carr_ccod ="850" or carr_ccod ="970" or carr_ccod ="860" or carr_ccod ="880" or carr_ccod ="940" or carr_ccod ="41"  or carr_ccod ="100" or carr_ccod ="101" or carr_ccod ="102" or carr_ccod ="113" or carr_ccod ="112" or carr_ccod ="49" OR carr_ccod = "840"   then
			es_carrera = true	
			if v_mes_actual=1 and (v_dia_actual >= 13) then
				crear_acceso = true
		 ...
		 if sede_ccod = "9" then
			es_carrera = true	
			if (v_mes_actual=4 and v_dia_actual >= 7 and v_dia_actual <= 27 ) then
				crear_acceso = true
			end if
		 end if
		...
		if not es_carrera then
			session("autorizacion_carga_2009") = false
			session("mensajeerror")= "Consulta sobre el proceso de toma de carga directamente en tu escuela a partir del día 13 de enero."
            response.Redirect("portada_alumno.asp") 
		end if
