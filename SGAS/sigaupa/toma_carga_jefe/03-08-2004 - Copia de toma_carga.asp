...
if not EsVacio(pers_nrut) then
	v_plec_ccod = conectar.ConsultaUno("select plec_ccod from periodos_academicos where peri_ccod = '" & peri_ccod & "'")
	if v_plec_ccod = "2" then
		sentencia = "crear_matricula_seg_semestre('" & sede_ccod & "', '" & pers_nrut & "')"
		conectar.EjecutaP(sentencia)
	end if
