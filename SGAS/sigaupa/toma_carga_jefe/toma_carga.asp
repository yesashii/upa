...
formulario.carga_parametros "parametros.xml", "toma_carga_modificar"
...
if not EsVacio(pers_nrut) then
	...
	bloqueado = false
		...
		if not EsVacio(msj_bloqueo) and msj_bloqueo <> "El postulante se encuentra moroso.\n\n" then
			bloqueado = true
...
'end if 
 ...
 alumno.construyeSituacionAsignaturas
		 'response.Write("<pre>"&sql_espe_ccod&"</pre>")
...
'"select distinct a.asig_ccod,c.asig_ccod,c.asig_ccod asig_ccod_paso, c.asig_tdesc as asignatura, asig_nhoras, b.secc_ccod, '" & matr_ncorr & "' as matr_ncorr " & _
...
'"  order by a.nive_ccod, a.asig_ccod " & vbCrLf

'--------------------------------------------------------------------------------------------INICIO CONSULTA SQLServer 2008
asignaturas_disponibles_cons = "" & vbCrlf & _ 
"select distinct a.asig_ccod,                                                                                                " & vbCrlf & _
"                c.asig_ccod,                                                                                                " & vbCrlf & _
"                c.asig_ccod                  as asig_ccod_paso,                                                             " & vbCrlf & _
...
"from   (                                                                                                                    " & vbCrlf & _
"select distinct b.asig_ccod,                                                                                                " & vbCrlf & _
...
"                                                                                 and sf.sitf_baprueba = 'S') as apro        " & vbCrlf & _
"                                                              ---PONER PERS_NCORR                                           " & vbCrlf & _
"                                                              on hom.asig_ccod_fuente = apro.asig_ccod                      " & vbCrlf & _
"                                          group  by homo_ccod,                                                              " & vbCrlf & _
"                                                    asig_ccod_destino) as prueba                                            " & vbCrlf & _
"                                      on mc.asig_ccod = prueba.asig_ccod_destino -- se agrega prueba. 19_03_2013            " & vbCrlf & _
...
"                  and f.pers_ncorr = '"& pers_ncorr &"'                                                                     " & vbCrlf & _
") as d                                                                                                                      " & vbCrlf & _
"                    on a.asig_ccod = d.asig_ccod                                                                            " & vbCrlf & _
"order  by a.nive_ccod,                                                                                                      " & vbCrlf & _
"          a.asig_ccod                                                                                                       " & vbCrlf 
...
'destino = "(select a.carr_ccod,a.asig_ccod, a.secc_tdesc, a.secc_ccod," & vbCrLf & _
'		"    case a.carr_ccod  when '"&v_carr_ccod&"' then '(*)' + cast(a.asig_ccod as varchar) + '-' + a.secc_tdesc + ' -> ' + protic.horario(a.secc_ccod)  " & vbCrLf & _
'		"	        else cast(a.asig_ccod as varchar) + '-' + a.secc_tdesc + ' -> ' + protic.horario(a.secc_ccod)  " & vbCrLf & _
'		"	  end horario" & vbCrLf & _
'		"     " & vbCrLf & _
'		"    from secciones a,cargas_academicas c" & vbCrLf & _
'		"    where a.secc_ccod *= c.secc_ccod    " & vbCrLf & _
...
'			"    c.secc_ccod,e.asig_ccod ||' '|| e.asig_tdesc as a_plan ,    " & vbCrLf  & _
'			"    a.asig_ccod||' '|| a.asig_tdesc as a_destino, " & vbCrLf  & _
'			"    'Secc. ' || b.secc_tdesc||' -> '|| horario (c.secc_ccod) seccion, " & vbCrLf  & _
...
'			"	 and matr_ncorr='"&matr_ncorr&"'" & vbCrLf  & _
...
cons_resumen = "select " & vbCrLf  & _
				"    c.secc_ccod,e.asig_ccod + ' ' + e.asig_tdesc as a_plan ,    " & vbCrLf  & _
				"    a.asig_ccod + ' ' + a.asig_tdesc as a_destino, " & vbCrLf  & _
				"    'Secc. ' + b.secc_tdesc + ' -> ' + protic.horario(c.secc_ccod) seccion, " & vbCrLf  & _
				"    c.audi_fmodificacion " & vbCrLf  & _
				" from asignaturas a,secciones b,equivalencias c,malla_curricular d,asignaturas e " & vbCrLf  & _
				" where a.asig_ccod=b.asig_ccod " & vbCrLf  & _
				"    and b.secc_ccod=c.secc_ccod " & vbCrLf  & _
				"    and c.mall_ccod=d.mall_ccod " & vbCrLf  & _
				"	 and e.asig_ccod=d.asig_ccod " & vbCrLf  & _
				...
				"	 and cast(sede_ccod as varchar)='"&sede_ccod&"'" & vbCrLf  & _
				"	 and cast(b.peri_ccod as varchar)='"&peri_ccod&"'" & vbCrLf

				
registros=conectar.consultauno("select count(*) from ("&cons_resumen&") f")
	
tresumen.consultar cons_resumen


'Secciones =	" SELECT 	NVL(C.ASIG_CCOD,A.ASIG_CCOD) asig_ccod, b.secc_ccod,case  when a.carr_ccod = '"&v_carr_ccod&"' then " & vbCrLf  & _
'	 	   	"	 	  '(*)'||a.asig_ccod || '-' || a.secc_tdesc || ' -> ' || horario (a.secc_ccod) " & vbCrLf  & _
'		   	" else  " & vbCrLf  & _
'		   	" 	   a.asig_ccod || '-' || a.secc_tdesc || ' -> ' || horario (a.secc_ccod)   " & vbCrLf  & _
'		 	" end    horario " & vbCrLf  & _
'			" FROM secciones  a, cargas_academicas b, ELECTIVOS C " & vbCrLf  & _
'			" WHERE b.MATR_NCORR = '"&matr_ncorr&"' " & vbCrLf  & _
'			"  AND B.SECC_CCOD = C.SECC_CCOD(+)" & vbCrLf  & _
'			" and a.secc_ccod = b.secc_ccod " 
			
'Secciones = "select isnull(C.ASIG_CCOD,A.ASIG_CCOD) asig_ccod, b.secc_ccod," & vbCrLf  & _
'			"        case  a.carr_ccod when '' then '(*)' + cast(a.asig_ccod as varchar)+ '-' + a.secc_tdesc + ' -> ' + protic.horario(a.secc_ccod) " & vbCrLf  & _
'			"                        else cast(a.asig_ccod as varchar)+ '-' + a.secc_tdesc + ' -> ' + protic.horario (a.secc_ccod) end horario" & vbCrLf  & _
'			"    from secciones a,cargas_academicas b,electivos c" & vbCrLf  & _
'			"    where a.secc_ccod = b.secc_ccod " & vbCrLf  & _
'			"    and b.secc_ccod *= c.secc_ccod" & vbCrLf  & _
'			"    and cast(b.matr_ncorr as varchar) = '"&matr_ncorr&"'"

'--------------------------------------------------------------------------------------------INICIO CONSULTA SQLServer 2008
Secciones = " select isnull(c.asig_ccod, a.asig_ccod) asig_ccod,        " & vbCrlf & _
"       b.secc_ccod,                                                    " & vbCrlf & _
"       case a.carr_ccod                                                " & vbCrlf & _
"         when '' then '(*)' + cast(a.asig_ccod as varchar) + '-'       " & vbCrlf & _
"                      + a.secc_tdesc + ' -> '                          " & vbCrlf & _
"                      + protic.horario(a.secc_ccod)                    " & vbCrlf & _
"         else cast(a.asig_ccod as varchar) + '-'                       " & vbCrlf & _
"              + a.secc_tdesc + ' -> '                                  " & vbCrlf & _
...
"       left outer join electivos as c                                  " & vbCrlf & _
...
%>		
          ...
          <tr>
            <td align="right">  <%if autorizado_carga =0 or sys_cierra_toma_carga = true then 
			                         f_botonera.AgregaBotonParam "GUARDAR2", "deshabilitado", "TRUE"
								end if%>
			                  <%f_botonera.DibujaBoton "GUARDAR2"%></td>
          </tr>
          <tr>
            <td align="right">&nbsp;</td>
          </tr>
