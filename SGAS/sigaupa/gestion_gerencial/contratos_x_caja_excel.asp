...
sql="select top 1 peri_ccod from PERIODOS_ACADEMICOS where PERI_FINICIO_PERIODO >= '"&inicio&"' order by PERI_CCOD asc"
'response.Write(sql)
v_periodo = conexion.consultaUno(sql)

'response.Write("<pre>"&inicio&"</pre>")
'response.Write("<pre>"&termino&"</pre>")
'response.Write("<pre>"&v_sede_ccod&"</pre>")
'response.Write("<pre>"&v_pers_ncorr&"</pre>")
'response.Write("<pre>"&v_periodo&"</pre>")
'response.End()
...
'-------postgrado y pregado
postgrado_pregrado = 0
pregrado_lascondes = 0
postgrado_lascondes = 0
pregrado_melipilla = 0
postgrado_melipilla = 0
total_suma_pregrado = 0
total_suma_postgrado = 0

'--------Extension--------- 
extension_cursos_otec = 0
extension_cursos_otec_lascondes = 0
extension_cursos_otec_melipilla = 0
total_suma_extension = 0
...
' Ocupando las tablas sdescuentos como pase de matricula, (no estaba bien definido el pase matricula)
 consulta ="Select distinct p.sede_tdesc as sede_caja,j.sede_ccod,protic.trunc(j.mcaj_finicio) as fecha_apertura, isnull(a.mcaj_ncorr,0) as mcaj_ncorr,d.econ_ccod,d.contrato as n_contrato,g.sede_tdesc as sede, " & vbCrLf &_
			...
			"	 Case (select count(*) from alumno_credito where post_ncorr=d.post_ncorr and tdet_ccod not in (1402)) when 1 then 'SI' else 'NO' end as beca_mineduc, " & vbCrLf &_
 			" (select isnull(max(tdet_tdesc),' No registra información') from alumno_credito ac, " & vbCrLf &_
			" tipos_detalle td where ac.post_ncorr=d.post_ncorr and ac.tdet_ccod=td.tdet_ccod and ac.tdet_ccod not in (1402)) nombre_beca_mineduc , " & vbCrLf &_
 			" carr.TCAR_CCOD, 0 as V_TCOM_CCOD --(select TCOM_CCOD from compromisos c where  c.tcom_ccod = 7 and b.comp_ndocto=c.comp_ndocto and b.inst_ccod=c.inst_ccod )as V_TCOM_CCOD " & vbCrLf &_
 			" From  " & vbCrLf &_
 			" ingresos a  " & vbCrLf &_
    		...
    		" and b.inst_ccod=c.inst_ccod " & vbCrLf &_
 			" and c.tcom_ccod in (1,2,7) " & vbCrLf &_
    		...
    		" and m.esde_ccod =1 " & vbCrLf &_
 			" join ESPECIALIDADES esp  on f.ESPE_CCOD = esp.ESPE_CCOD " & vbCrLf &_
 			" join CARRERAS carr   on esp.CARR_CCOD = carr.CARR_CCOD " & vbCrLf &_
 			" join TIPOS_CARRERA tc on carr.TCAR_CCOD = tc.TCAR_CCOD " & vbCrLf &_
 			" where a.ting_ccod=7 " & vbCrLf &_
 			" " &filtro&" "& vbCrLf &_
 			" group by  f.ofer_ncorr,p.sede_tdesc,d.peri_ccod,j.sede_ccod,m.sdes_nporc_colegiatura,e.post_bnuevo,j.mcaj_finicio,e.pers_ncorr,k.pers_ncorr,d.cont_fcontrato, " & vbCrLf &_
			" i.econ_tdesc,a.mcaj_ncorr,d.econ_ccod,d.post_ncorr,d.cont_ncorr,d.contrato,g.sede_tdesc,h.jorn_tdesc,protic.obtener_nombre_carrera(f.ofer_ncorr,'C'), " & vbCrLf &_
			" protic.obtener_nombre_completo(e.pers_ncorr,'n')," & vbCrLf &_
 			" carr.TCAR_CCOD, b.COMP_NDOCTO, b.INST_CCOD " & vbCrLf &_
...
<table width="100%" border="0">
 <tr> 
  ...
  <tr> 
    ...
    <td width="85%" colspan="2"><strong>:</strong> <%=inicio %> al <%=termino%></td>
  </tr>
  <tr>
...
<p>&nbsp;</p><table width="100%" border="1">
  ...
  postgrado_pregrado = lista.ObtenerValor("TCAR_CCOD")
  extension_cursos_otec = lista.ObtenerValor("V_TCOM_CCOD")
  'response.Write(postgrado_pregrado)
  'response.Write(extension_cursos_otec)
  
 '**************pregrado las condes************
   if v_sede="1" and postgrado_pregrado = "1" then
	  'response.Write("entro")
		' if v_nuevo="Nuevo" then
			pregrado_lascondes=pregrado_lascondes+ 1
		 'else
			'v_antiguos_central=v_antiguos_central+ 1
		 end if
		 
'*************fin consulta pregrado las condes*********

'**************postgrado las condes************
   if v_sede="1" and postgrado_pregrado = "2" then
	  'response.Write("entro")
		' if v_nuevo="Nuevo" then
			postgrado_lascondes=postgrado_lascondes+ 1
...
'*************fin consulta pregrado las condes******
'**************extension las condes************
   if v_sede="1" and extension_cursos_otec = "7" then
	  'response.Write("entro")
		' if v_nuevo="Nuevo" then
			extension_cursos_otec_lascondes=extension_cursos_otec_lascondes+ 1
			
		 'else
			'v_antiguos_central=v_antiguos_central+ 1
		 end if
		 
'*************fin consulta Extension las condes******


 '**************pregrado Melipilla************
   if v_sede="4" and postgrado_pregrado = "1" then
	  'response.Write("entro")
		' if v_nuevo="Nuevo" then
			pregrado_melipilla=pregrado_melipilla+ 1
		 'else
			'v_antiguos_central=v_antiguos_central+ 1
		 end if
...
'**************postgrado las condes************
   if v_sede="4" and postgrado_pregrado = "2" then
	  'response.Write("entro")
		' if v_nuevo="Nuevo" then
			postgrado_melipilla=postgrado_melipilla+ 1
		 'else
			'v_antiguos_central=v_antiguos_central+ 1
		 end if
...
'**************extension melipilla************
   if v_sede="1" and extension_cursos_otec = "7" then
	  'response.Write("entro")
		' if v_nuevo="Nuevo" then
			extension_cursos_otec_melipilla=extension_cursos_otec_melipilla+ 1
			
		 'else
			'v_antiguos_central=v_antiguos_central+ 1
		 end if
		 
'*************fin consulta Extension melipilla******
...
'******* SEDE BUSTAMANTE ************************************
 ...
 total_suma_pregrado = pregrado_lascondes+pregrado_melipilla
 
 total_suma_postgrado = postgrado_lascondes+postgrado_melipilla
 
 total_suma_extension = extension_cursos_otec_lascondes+extension_cursos_otec_melipilla
...
<table width="100%" border="1">
  ...
  <tr>
    ...
    <td align="center"><%=pregrado_lascondes%></td>
    <td align="center"><%=postgrado_lascondes%></td>
    <td align="center"><%=extension_cursos_otec_lascondes%></td>
  ...
  <tr >
    ...
    <td align="center"><%=pregrado_melipilla%></td>
    <td align="center"><%=postgrado_melipilla%></td>
    <td align="center"><%=extension_cursos_otec_melipilla%></td>
  </tr>
  <!--
  <tr >
  ...
  <tr bgcolor="#66FFFF">
    ...
    <td align="center"><%=total_suma_pregrado%></td>
    <td align="center"><%=total_suma_postgrado%></td>
    <td align="center"><%=total_suma_extension%></td>
