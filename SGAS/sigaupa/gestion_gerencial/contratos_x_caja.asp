...
v_periodo 		= request.querystring("busqueda[0][periodo]") '// si es que se tikea el check periodo. 0 no tikea 1 tikea
...
'response.Write("ip_usuario = "&ip_usuario&"</br>") 
'response.Write("v_timer = "&v_timer&"</br>") 
'response.Write("inicio = "&inicio&"</br>") 
'response.Write("termino = "&termino&"</br>") 
...
'Si v_sede_ccod <> "" -->> agrgar filtro: and f.sede_ccod = v_sede_ccod
if v_sede_ccod <> "" then
	filtro = " and f.sede_ccod= " & v_sede_ccod
end if

'Si v_pers_ncorr <> "" -->> agregar filtro: and k.pers_ncorr = v_pers_ncorr
if v_pers_ncorr <> "" then
	filtro = filtro& " and k.pers_ncorr = " & v_pers_ncorr
end if

'Si inicio <> "" y termino <> "" -->> agregar filtro
if inicio <> "" then
	if termino <> "" then
		...
		'Cambiado por LTORRES el 20-10-2015
		filtro =filtro&" and convert(varchar(8),j.mcaj_finicio,112) BETWEEN convert(varchar(8),convert(datetime,'"&inicio&"',103),112) AND convert(varchar(8),convert(datetime,'"&termino&"',103),112) "
	else
		'filtro =filtro&" and protic.trunc(convert(datetime,j.mcaj_finicio,103)) = protic.trunc(convert(datetime,'"&inicio&"',103)) "
		'Cambiado por LTORRES el 20-10-2015
		filtro =filtro&" and convert(varchar(8),j.mcaj_finicio,112) = convert(varchar(8),convert(datetime,'"&inicio&"',103),112) "
	end if
end if

'Si v_periodo <> "0" -->> agregar filtro: and cast(d.peri_ccod as varchar) = v_peri_ccod
if v_periodo <> "0" then
	filtro = filtro & " and cast(d.peri_ccod as varchar) = '" & v_peri_ccod& "' "
end if

'-----------------------------------------------------------------------------------------------------------------------------------------
' Ocupando las tablas descuentos como pase de matricula, (porque no estaba bien definido el pase matricula)
'-----------------------------------------------------------------------------------------------------------------------------------------
consulta = " Select distinct isnull(a.mcaj_ncorr,0) as mcaj_ncorr,d.econ_ccod,d.contrato as n_contrato,g.sede_tdesc, "& vbCrLf &_
