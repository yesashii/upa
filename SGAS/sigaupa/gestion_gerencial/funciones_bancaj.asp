...
sql_bancaj_sede=	" select facu_ccod,jorn_ccod,sede_ccod, carr_ccod,facultad,cast(carrera+ case jorn_ccod when 1 then '- (D)' else '- (V)' end as varchar) as carrera, "& vbCrLf &_ 
				...
				"	From ( "& vbCrLf &_ 
					...
					" case tipo_ingreso when 2 then sum(monto_recaudado) end as total_titulacion "& vbCrLf &_ 
						"  from (      "& vbCrLf &_ 
						"   select case when rtrim(protic.obtener_carrera_ingreso(a.mcaj_ncorr,a.ting_ccod,ingr_nfolio_referencia,a.pers_ncorr))=12 and g.espe_ccod in(186,187,63,286) then rtrim(protic.obtener_carrera_ingreso(a.mcaj_ncorr,a.ting_ccod,ingr_nfolio_referencia,a.pers_ncorr))+cast(g.espe_ccod as varchar) else protic.obtener_carrera_ingreso(a.mcaj_ncorr,a.ting_ccod,ingr_nfolio_referencia,a.pers_ncorr) end as carr_ccod, "& vbCrLf &_ 
						...
						" 	  and b.ting_ccod in (3,4,6,13,14,51,52)     "& vbCrLf &_ 
						...
						"   select  case when rtrim(protic.obtener_carrera_cargo(f.post_ncorr))=12 and g.espe_ccod in(186,187,63,286) then rtrim(protic.obtener_carrera_cargo(f.post_ncorr))+cast(g.espe_ccod as varchar) else protic.obtener_carrera_cargo(f.post_ncorr) end as carr_ccod, "& vbCrLf &_ 
						...
						" 		on f.ofer_ncorr=g.ofer_ncorr  "& vbCrLf &_ 
						" 		and g.sede_ccod='"&p_sede&"'	   "& vbCrLf &_ 
						" ) as a, carreras_bancaj b, areas_academicas c,facultades d   "& vbCrLf &_ 
...
sql_bancaj_sede_pareo=	" select c.facu_tdesc as facultad,b.presc_carrera_desc as carrera,b.presc_aranceles,b.presc_titulaciones,b.presc_total,"& vbCrLf &_ 
				...
				" (select facu_ccod,jorn_ccod,sede_ccod, carr_ccod,facultad,cast(carrera+ case jorn_ccod when 1 then '- (D)' else '- (V)' end as varchar) as carrera, "& vbCrLf &_ 
				" cast(isnull(max(total_arancel),0) as numeric) as arancel,cast(isnull(max(total_titulacion),0) as numeric) as titulacion, cast(isnull(max(total_arancel),0)+isnull(max(total_titulacion),0) as numeric) as total  "& vbCrLf &_ 
				"	From ( "& vbCrLf &_ 
					" select d.facu_ccod,d.facu_tdesc as facultad ,b.carr_tdesc as carrera, a.carr_ccod,a.tipo_ingreso,a.jorn_ccod,a.sede_ccod, "& vbCrLf &_ 
					" case tipo_ingreso when 1 then sum(monto_recaudado) end as total_arancel, "& vbCrLf &_ 
	...
	sql_facultad =	" select b.sede_tdesc,a.facu_ccod,a.sede_ccod,a.facultad , "& vbCrLf &_ 
						...
						"	From ( "& vbCrLf &_ 
							...
							" case tipo_ingreso when 2 then sum(monto_recaudado) end as total_titulacion "& vbCrLf &_ 
								...
								" 		  and a.eing_ccod not in (3,6) "& vbCrLf &_ 
								" 		  and e.econ_ccod not in (3) "& vbCrLf &_ 
								" 		  and g.sede_ccod in (1,2,4) "& vbCrLf &_ 
								"  UNION	  "& vbCrLf &_ 
								"   -- Titulaciones pagadas directamente   "& vbCrLf &_ 
...
Function ObtenerConsultaFacultadPareo(p_facu, v_anos)
	...
	sql_facultad =	" select a.sede_tdesc,isnull(b.presc_sede,a.sede_ccod)as presc_sede, isnull(b.presc_aranceles,0) as presc_aranceles, "& vbCrLf &_
					...
					" 		 from sedes where sede_ccod  in (1,2,4) "& vbCrLf &_
						" ) a "& vbCrLf &_
						" left outer join  "& vbCrLf &_
