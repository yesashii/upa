...
'response.Write("total "&v_total_pagar_colegiatura)
...
consulta = "select a.stde_ccod, a.post_ncorr, a.ofer_ncorr, a.sdes_nporc_matricula  as sdes_nporc_matricula, a.sdes_nporc_colegiatura as sdes_nporc_colegiatura, a.esde_ccod, " & vbCrLf &_
           "       b.stde_tdesc, a.sdes_mmatricula  as sdes_mmatricula, a.sdes_mcolegiatura  as sdes_mcolegiatura, isnull(a.sdes_mmatricula, 0) + isnull(a.sdes_mcolegiatura, 0) as subtotal, a.sdes_tobservaciones " & vbCrLf &_
...
consulta =  "select b.post_ncorr, b.ofer_ncorr, b.sdpa_ncuota, b.sdpa_ccod, " & vbcrlf & _
			...
			" b.sdpa_femision, b.sdpa_fvencimiento, b.sdpa_mmonto as sdpa_mmonto, " & vbCrLf &_
...
consulta = "select max(ncuotas_doc_matricula) as max_cuotas_doc_matricula, " & vbCrLf &_
...
"             case  " & vbcrlf & _
"                 when a.fecha  >= getdate() then year(getdate()) " & vbcrlf & _
"                 else  year(getdate())+1 " & vbcrlf & _
...
{		
	...
	for (var i = 0; i < tabla_fp_matricula.filas.length; i++) {
		if (tabla_fp_matricula.filas[i].campos["butiliza"].objeto.value == 'S') {	
			if (tabla_fp_matricula.filas[i].campos["ting_ccod"].objeto.value != '6') {		
	...
	ncuotas_matricula = parseInt(ncuotas_doc_matricula) + parseInt(ncuotas_efec_matricula);
	...
	for (var i = 0; i < tabla_fp_colegiatura.filas.length; i++) {
		if (tabla_fp_colegiatura.filas[i].campos["butiliza"].objeto.value == 'S') {
			if (tabla_fp_colegiatura.filas[i].campos["ting_ccod"].objeto.value != '6') {		
	...
	ncuotas_colegiatura = parseInt(ncuotas_doc_colegiatura) + parseInt(ncuotas_efec_colegiatura);
	...
	for (var i = 0; i < <%=f_forma_pago_colegiatura.NroFilas%>; i++) {
		...
		if ((tabla_fp_colegiatura.filas[i].campos["butiliza"].objeto.value == 'S') && (v_sdfp_mmonto == 0)) {
			eleemnto=tabla_fp_matricula.filas[i].campos["butiliza"].objeto.name;
			alert('Si va a utilizar esta forma de pago, monto a pagar debe ser mayor que $0.'+eleemnto);
...
var indice=1;
	...
	//alert(variable);
	var v_indice='';
	//&&(variable=="fp_colegiatura"))
	if (nombre=="_fp_colegiatura[0][butiliza]"){
		
		//---------------------------------------------------------------------------------------------
		// DESHABILITA LAS OTRAS FORMAS DE PAGO QUE NO SEAN TRANSBANK 
	...
	//if(variable=="fp_colegiatura")
		if(variable=="fp_colegiatura"){
			//---------------------------------------------------------------------------------------------
			// DESHABILITA  EL PAGO CON TRANSBANK
...
function convierte_codigo(cadena,indice){
	...
	var v_sede='<%=sede%>';
	// codigos sbif de las plazas correspondientes a sedes upacifico
	var melipilla 	='0376';
	var talagante 	='0368';
	var san_bernardo='0344';
	var santiago	='0320';
	...
	switch(v_banco){
	// traduccion de codigo sbif para los bancos
	...
	document.forma_pago.elements[v_variable+"["+v_indice+"][sdfp_ndocto_inicial]"].value=n_cheque;
	document.forma_pago.elements[v_variable+"["+v_indice+"][sdfp_tctacte]"].value=cta_cte;
	document.forma_pago.elements[v_variable+"["+v_indice+"][banc_ccod]"].value=banco;
	document.forma_pago.elements[v_variable+"["+v_indice+"][plaz_ccod]"].value=plaza;
	document.forma_pago.elements[v_variable+"["+v_indice+"][sdfp_tplaza_sbif]"].value=v_plaza;
                  ...
                  <tr>
                    <td><%pagina.DibujarSubtitulo "Datos del postulante"%>
						...
						<form name="forma_pago">
					          ...
					          <table width="98%"  border="0" align="center" cellpadding="0" cellspacing="0">
                                <tr>
                                  <td width="80%"><%f_forma_pago_matricula.DibujaTabla%></td></tr>
                                <tr>
                                  <td><br>
                          ...
                          <% f_fecha_inicio.DibujaCampo ("fecha_inicio_pago")%>
                              <table width="98%"  border="0" align="center" cellpadding="0" cellspacing="0">
                                <tr>
                                  <td width="80%">
                                    <%f_forma_pago_colegiatura.DibujaTabla%></td>
                                </tr>
                                <tr>
                                  <td><br>
                              ...
                              <form name="detalle_pagos">
                                ...
                                <table width="98%"  border="0" align="center" cellpadding="0" cellspacing="0">
                                  <tr>
                                    <td><div align="center"><%f_detalle_pagos.DibujaTabla%></div></td>
                                  </tr>
                                </table>
                              </form>                              <br>
   					    <div align="left">                        </div>					
