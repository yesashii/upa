...
'LINEA			:115,117 - 225,227 - 296,297,298,306,310,311,312,313 - 492,493
...
'LINEA					:1296 A 1339  -  1998 A 2007
'********************************************************************
'******************************************************************************
'--ACTUALIZACION--
'
'FECHA ACTUALIZACION		: 	10/03/2014
'ACTUALIZADO POR			: 	Luis Herrera G.
'MOTIVO						: 	Corregir código, reemplazar personas__postulante 
'								por personas y direcciones__publica por
'								direcciones. 
'******************************************************************************
...
sql = "SELECT anos_ccod FROM periodos_academicos WHERE peri_ccod = (SELECT PERI_CCOD FROM postulantes WHERE post_ncorr = "&q_post_ncorr&")"
'response.write sql
anos_matricula = conexion.ConsultaUno(sql)
'response.write anos_matricula
...
'response.write q_post_ncorr & " 2222"

consulta_personas = "SELECT * FROM personas WHERE pers_ncorr= (SELECT pers_ncorr FROM postulantes WHERE post_ncorr=" & q_post_ncorr &")"
'response.write consulta_personas
set control_personas = new CFormulario
control_personas.Carga_Parametros "tabla_vacia.xml", "tabla_vacia"
control_personas.Inicializar conexion
control_personas.Consultar consulta_personas
control_personas.Siguiente

'response.write "<br>"&control_personas.ObtenerValor("pers_ncorr") &"<---"

if control_personas.ObtenerValor("pers_ncorr") = "" then  
	'response.write "OK ENTRE"
	'''''''''''''''''''''''''''''''''''''''''''
	''ESTO SE REALIZÓ DEBIDO A PROBLEMAS CON ''
	''  EL INGRESO DE LOS POSTULANTES A LA   ''
	''            TABLA PERSONAS             ''
	'''''''''''''''''''''''''''''''''''''''''''
	consulta_ingreso = "insert into personas SELECT PERS_NCORR,TVIS_CCOD,SEXO_CCOD,TENS_CCOD,COLE_CCOD,ECIV_CCOD,PAIS_CCOD,PERS_BDOBLE_NACIONALIDAD,PERS_NRUT,PERS_XDV,PERS_TAPE_PATERNO,PERS_TAPE_MATERNO," & vbCrLf &_
		" PERS_TNOMBRE,PERS_FNACIMIENTO,CIUD_CCOD_NACIMIENTO,PERS_FDEFUNCION,PERS_TEMPRESA,PERS_TFONO_EMPRESA,PERS_TCARGO,PERS_TPROFESION,PERS_TFONO,PERS_TFAX,PERS_TCELULAR," & vbCrLf &_
		" PERS_TEMAIL,PERS_TPASAPORTE,PERS_FEMISION_PAS,PERS_FVENCIMIENTO_PAS,PERS_FTERMINO_VISA,PERS_NNOTA_ENS_MEDIA,PERS_TCOLE_EGRESO,PERS_NANO_EGR_MEDIA,PERS_TRAZON_SOCIAL,PERS_TGIRO," & vbCrLf &_
		" PERS_TEMAIL_UAS as PERS_TEMAIL_INTERNO,NEDU_CCOD,IFAM_CCOD,ALAB_CCOD,ISAP_CCOD,FFAA_CCOD,PERS_TTIPO_ENSENANZA,PERS_TENFERMEDADES,PERS_TMEDICAMENTOS_ALERGIA,AUDI_TUSUARIO,AUDI_FMODIFICACION,ciud_nacimiento,regi_particular,ciud_particular," & vbCrLf &_
		" null as pers_bmorosidad, null as sicupadre_ccod, null as sitocup_ccod, null as tenfer_ccod, null as descrip_tenfer, null as trabaja, null as pers_temail2" & vbCrLf &_
		" FROM personas_postulante WHERE pers_ncorr= (SELECT pers_ncorr FROM postulantes WHERE post_ncorr=" & q_post_ncorr &")"
	'response.write consulta_ingreso

	conexion.ejecutaS consulta_ingreso
end if 
...
consulta_datos = "select a.pers_ncorr, b.post_ncorr, cast(a.pers_nrut as varchar)+ ' - ' + cast(a.pers_xdv as varchar) as rut," & vbCrLf &_
    ...
    " from personas a INNER JOIN postulantes b " & vbCrLf &_
    ...
    " INNER JOIN aranceles f " & vbCrLf &_
    " ON c.aran_ncorr = f.aran_ncorr " & vbCrLf &_
    " INNER JOIN sedes g " & vbCrLf &_
    " ON c.sede_ccod = g.sede_ccod " & vbCrLf &_
    " LEFT OUTER JOIN sdescuentos h " & vbCrLf &_
    " ON b.post_ncorr = h.post_ncorr and b.ofer_ncorr = h.ofer_ncorr " & vbCrLf &_
    " LEFT OUTER JOIN spagos i " & vbCrLf &_
    " ON b.post_ncorr = i.post_ncorr and b.ofer_ncorr = i.ofer_ncorr " & vbCrLf &_
    " WHERE b.tpos_ccod in (1,2) " & vbCrLf &_
...
'response.Write("<pre>" & fc_datos.ObtenerValor("pers_ncorr")&"<--")				 
'response.End()
...
consulta = " select b.tipo, b.ttipo,   " & vbcrlf & _
	...
	"         INNER JOIN ofertas_academicas c " & vbcrlf & _
	"         ON b.ofer_ncorr = c.ofer_ncorr " & vbcrlf & _
	"         INNER JOIN aranceles d " & vbcrlf & _
	"         ON c.aran_ncorr = d.aran_ncorr " & vbcrlf & _
	"         LEFT OUTER JOIN spagos e " & vbcrlf & _
	"         ON b.post_ncorr = e.post_ncorr " & vbcrlf & _
	"         LEFT OUTER JOIN sdescuentos f " & vbcrlf & _
	"         ON b.post_ncorr = f.post_ncorr and b.ofer_ncorr = f.ofer_ncorr " & vbcrlf & _
	"         and f.esde_ccod = 1 " & vbcrlf & _
	"         WHERE b.post_ncorr = '"&q_post_ncorr&"' " & vbcrlf & _
...
"  from (select a.post_ncorr, a.ofer_ncorr, b.stpa_ccod, " & vbcrlf & _
"        c.tcom_ccod, c.ting_ccod " & vbcrlf & _
"        from postulantes a LEFT OUTER JOIN spagos b " & vbcrlf & _
"        ON a.post_ncorr = b.post_ncorr and a.ofer_ncorr = b.ofer_ncorr " & vbcrlf & _
"        INNER JOIN  sformas_pactacion_contrato c " & vbcrlf & _
"        ON a.post_ncorr ='"&q_post_ncorr&"') a " & vbcrlf & _
"        LEFT OUTER JOIN sdetalles_forma_pago b " & vbcrlf & _
...
'response.Write("<pre>"&sql_fp_matricula&"</pre>")
...
if not b_contrato_generado then
	...
	while f_consulta.Siguiente
		...
		if f_consulta.ObtenerValor("ting_ccod") ="52" then
			...
			f_detalle_pagos.AgregaCampoFilaParam i_, "sdpa_fvencimiento","permiso", "LECTURA"
			f_detalle_pagos.AgregaCampoFilaParam i_, "sdpa_mmonto","permiso","LECTURAESCRITURA"
			f_detalle_pagos.AgregaCampoFilaParam i_, "sdpa_fvencimiento","id","FE-N"
		end if
...
''sql ="(select cast(b.fini_ndia as varchar(2))+'/' + cast(b.fini_nmes as varchar)+ '/' +cast(fini_anio as varchar) as fecha " & vbcrlf & _
''"         from                     " & vbcrlf & _
...
''"              from             " & vbcrlf & _
''"              fechas_inicio_pagos) a ) b) c " & vbcrlf 

sql = "(SELECT case when convert(datetime, GETDATE(),103) < convert(datetime, c.fecha,103) then CAST(DAY(c.fecha) AS VARCHAR)+'/'+CAST(MONTH(c.fecha) AS VARCHAR)+'/'+CAST(YEAR(c.fecha) AS VARCHAR) else CAST(DAY(GETDATE())AS varchar)+'/'+cast(MONTH(GETDATE())AS varchar)+'/'+cast(YEAR(GETDATE())AS varchar) end  AS fecha " & vbcrlf &_
" from                     " & vbcrlf &_
" (select cast(b.fini_ndia as varchar(2))+'/' + cast(b.fini_nmes as varchar)+ '/' +cast(fini_anio as varchar) as fecha " & vbcrlf &_
"          from                     " & vbcrlf &_
"          (   select  a.fini_ndia, a.fini_nmes, " & vbcrlf &_
"              case  " & vbcrlf &_
"                  when  getdate() > '30/10/'+cast(year(getdate()) as varchar)  " & vbcrlf &_
...
"and esca_ccod not in (2) "& vbcrlf & _ 
...
'response.write(sql_tubo_cae_upa)
...
'-----------------------------------------debug>>
'response.write("sql_tuvo_cae_upa = "&tubo_cae_upa)
'-----------------------------------------debug<<
...
v_pers_nrut = fc_datos.ObtenerValor("pers_nrut")

sql_titulacion="select texto_titulacion from sd_titulacion_diciembre_2013 where pers_nrut ="& v_pers_nrut
'response.Write(sql_titulacion)
'response.End()
'-----------------------------------------debug>>
'response.write(sql_titulacion)
'-----------------------------------------debug<<
msg_titulacion	=	conexion.consultaUno(sql_titulacion) 

if msg_titulacion <> "" then
	msg_titulacion	=	msg_titulacion&" "'"El alumno es Renovante del Credito CAE, debe pasar por el modulo de admision y llenar la solicitud de credito CAE para matricularse."
	espacios		=	msg_titulacion&"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"
	msg_titulacion	=	espacios&msg_titulacion
end if

'response.Write("prueba"&titulacion)
%>
...
for (var i = 0; i < <%=f_detalle_pagos.NroFilas%>; i++) {
		...
		//formulario.elemets["detalle_pagos["+i+"][sdpa_fvencimiento]"].readOnly = true;
		if(v_objeto_fecha){
...
function HabilitarFila(p_variable, p_fila, p_habilitado)
{
	...
	//diaactual = momentoActual.getDate();
	//mesactual = momentoActual.getMonth();
	//anhoactual = momentoActual.getYear();
	...
	if (p_habilitado) {
		...
		var fecha_efectivo = document.forma_pago.elements["fecha_inicio[0][fecha_inicio_pago]"].value;
		var cuotas_efectivo =  tabla.filas[3].campos["stpa_ncuotas"].objeto.value;
		//alert(fecha_efectivo);
		if (fecha_efectivo != ""){
			if (estado == "matricula") {
		...
		if (frecuencia_anterior == 0 )
		{
			//alert("frecuencia 0 ");
		    //if (p_fila != 3) 
			if(v_tipo_ingreso.value !=66) // SEA DISTINTO A PAGARE UPA
			{
				//alert(p_fila+"!=3");
				if (estado == "matricula")
				{
					...
					//alert(document.forma_pago.elements["fecha_inicio[0][fecha_inicio_pago]"].value);
					f_ini = document.forma_pago.elements["fecha_inicio[0][fecha_inicio_pago]"].value;
					//alert(v_tipo_ingreso.value);
					if (v_tipo_ingreso.value==52){
						...
						document.forma_pago.elements["fp_colegiatura[5][sdfp_finicio_pago]"].readOnly = false;
						...
						var f_inicio = "31/12/"+<% =anos_matricula %>
						document.forma_pago.elements["fp_colegiatura[2][sdfp_finicio_pago]"].readOnly = true;
						...
						var f_inicio = "30/11/"+<% =anos_matricula %>
						document.forma_pago.elements["fp_colegiatura[1][sdfp_finicio_pago]"].readOnly = true;
						...
						var f_inicio = "30/11/"+<% =anos_matricula %>
						document.forma_pago.elements["fp_colegiatura[0][sdfp_finicio_pago]"].readOnly = true;
						...
						var f_inicio = dia_inicio+"/"+mes+"/"+anio
						document.forma_pago.elements["fp_colegiatura[4][sdfp_finicio_pago]"].readOnly = false;
				...
				var f_inicio = document.forma_pago.elements["fecha_inicio[0][fecha_inicio_pago]"].value;
				if (v_tipo_ingreso.value==52){
					diap=25;
					mesp='02';
					//var f_inicio = document.forma_pago.elements["fecha_inicio[0][fecha_inicio_pago]"].value;
		...
		else
		{
		  //alert("otra frecuencia");
				...
				{
					tabla.filas[p_fila].campos["sdfp_finicio_pago"].objeto.value = '31/12/'+ <% =anos_matricula %>;
				}
				else if((v_tipo_ingreso.value==174) || (v_tipo_ingreso.value==175)){
					
					tabla.filas[p_fila].campos["sdfp_finicio_pago"].objeto.value = '30/11/'+ <% =anos_matricula %>; 
				}
				else{					
		...
		//alert(p_fila + " "+ tabla.filas[p_fila].campos["sdfp_mmonto"].objeto.value + " <-"+(-1) * (tabla_suma.filas[0].campos["diferencia"].objeto.value)); 
		// ACA ESTA EL PROBLEMA
		if(tabla.filas[p_fila].campos["sdfp_mmonto"].objeto.value == 0 )
		{
...
{	
	...
	var v_indice='';
	if ((nombre=="_fp_colegiatura[3][butiliza]")||(nombre=="_fp_colegiatura[4][butiliza]")||(nombre=="_fp_colegiatura[5][butiliza]"))
	{
		// alert(" Indice: "+v_indice_docto);
		//---------------------------------------------------------------------------------------------
		// DESHABILITA LAS OTRAS FORMAS DE PAGO QUE NO SEAN PAGARE UPA, TRANSBANK  O MULTIDEBITO
				...
				elemento7=document.forma_pago.elements["_fp_colegiatura[7][butiliza]"];
				elemento7.checked=false;
				cambiaOculto(elemento7, 'S', 'N')
				HabilitarFila(_ObtenerVariableCampo(elemento7), _FilaCampo(elemento7), !elemento7.checked);				
				
				elemento8=document.forma_pago.elements["_fp_colegiatura[8][butiliza]"];
				elemento8.checked=false;
				cambiaOculto(elemento8, 'S', 'N')
				HabilitarFila(_ObtenerVariableCampo(elemento8), _FilaCampo(elemento8), !elemento8.checked);	
				
				/*elemento9=document.forma_pago.elements["_fp_colegiatura[9][butiliza]"];
				elemento9.checked=false;
				cambiaOculto(elemento9, 'S', 'N')
				HabilitarFila(_ObtenerVariableCampo(elemento9), _FilaCampo(elemento9), !elemento9.checked);	
		*///---------------------------------------------------------------------------------------------
		...
		if(variable=="fp_colegiatura"){
			...
			// DESHABILITA  EL PAGO CON TRANSBANK Y CON MULTIDEBITO
				elemento3=document.forma_pago.elements["_fp_colegiatura[3][butiliza]"];
				if ((nombre_variable=="_fp_colegiatura[4][butiliza]")||(nombre_variable=="_fp_colegiatura[5][butiliza]")||(nombre_variable=="_fp_colegiatura[6][butiliza]")||(nombre_variable=="_fp_colegiatura[7][butiliza]")||(nombre_variable=="_fp_colegiatura[8][butiliza]")||(nombre_variable=="_fp_colegiatura[9][butiliza]")){ 
					elemento3.checked=false;
					//alert(3);
				}
				cambiaOculto(elemento3, 'S', 'N');
				...
				if ((nombre_variable=="_fp_colegiatura[3][butiliza]")||(nombre_variable=="_fp_colegiatura[5][butiliza]")||(nombre_variable=="_fp_colegiatura[6][butiliza]")||(nombre_variable=="_fp_colegiatura[7][butiliza]")||(nombre_variable=="_fp_colegiatura[8][butiliza]")||(nombre_variable=="_fp_colegiatura[9][butiliza]")){ 
					elemento4.checked=false;
					//alert(4);
				}
				cambiaOculto(elemento4, 'S', 'N');
				...
				elemento5=document.forma_pago.elements["_fp_colegiatura[5][butiliza]"];
				if ((nombre_variable=="_fp_colegiatura[3][butiliza]")||(nombre_variable=="_fp_colegiatura[4][butiliza]")||(nombre_variable=="_fp_colegiatura[6][butiliza]")||(nombre_variable=="_fp_colegiatura[7][butiliza]")||(nombre_variable=="_fp_colegiatura[8][butiliza]")||(nombre_variable=="_fp_colegiatura[9][butiliza]")){ 
					elemento5.checked=false;
					//alert(5);
				}
				cambiaOculto(elemento5, 'S', 'N');
...
function verifica_lectura_(objeto){
valor=objeto.value;
conteo=0;
                          ...
                          <div align="center">						
						    ...
						    <table width="98%"  border="0" align="center" cellpadding="0" cellspacing="0">
						     ...
						     <marquee><font color="#FF0000" size="3" style="font-style:italic; font-weight:bold; text-decoration:blink"><%=msg_fuas%><%=otro_msg%></font></marquee>
					  		 <br/>
                             <marquee><font color="#FF0000" size="3" style="font-style:italic; font-weight:bold; text-decoration:blink"><%=msg_titulacion%></font></marquee>	    
                                  ...
                                  <tr>
								    ...
								    <table class=v1 width='100%' border='1' cellpadding='0' cellspacing='0' bordercolor='#999999' bgcolor='#ADADAD'>
									  ...
									  <th><font color='#333333'>Plaza</font></th>
										  ...
										  <tr> <td ><%f_forma_pago_matricula.DibujaCampo("ting_ccod")%>
												  ...
												  <%f_forma_pago_matricula.DibujaCampo("sdfp_tplaza_sbif")%><%f_forma_pago_matricula.DibujaCampo("butiliza")%></td>
...
<script>
	...
	if(document.forma_pago.elements["fp_colegiatura[2][sdfp_finicio_pago]"].value != ""){
		document.forma_pago.elements["fp_colegiatura[2][sdfp_finicio_pago]"].readOnly = true;
	}
	
	if(document.forma_pago.elements["fp_colegiatura[1][sdfp_finicio_pago]"].value != ""){ 
		document.forma_pago.elements["fp_colegiatura[1][sdfp_finicio_pago]"].readOnly = true;
	}
</script>