...
'response.Write(q_post_ncorr)
v_peri_ccod=negocio.obtenerPeriodoAcademico("Postulacion")
'*********************************************************
'****** Propone descuento por Matricula Anticipada *******
for fila = 0 to f_postulantes.CuentaPost - 1
   ...
   '*****************************
    ...
    sql_existe_compromiso="select count(*) from compromisos "& vbCrLf &_ 
						  " where post_ncorr="&v_post_ncorr&" "& vbCrLf &_ 
							" and ofer_ncorr="&v_ofer_ncorr&" "& vbCrLf &_ 
							" and tcom_ccod=37 "& vbCrLf &_ 
							" and ecom_ccod=1"
   v_existe= conexion.consultaUno(sql_existe_compromiso)
  
	   if v_existe > 0 then
	    'response.Write("Existe:<br> "&sql_existe_compromiso&"<hr>")
			'#######	VERICAR SI YA A SIDO PAGADO	############
			sql_pendiente_de_pago ="Select cast(protic.total_recepcionar_cuota(37,dcom.inst_ccod,dcom.comp_ndocto,dcom.dcom_ncompromiso) as varchar)  "& vbCrLf &_
								" From compromisos a, detalle_compromisos dcom "& vbCrLf &_
								...
								"	and a.ofer_ncorr="&v_ofer_ncorr&" "& vbCrLf &_
								"	and a.tcom_ccod=37 "& vbCrLf &_ 
								"	and a.ecom_ccod=1"
								
			v_pendiente= conexion.consultaUno(sql_pendiente_de_pago)
			
			sql_existe_descuento="Select count(*) from sdescuentos where post_ncorr="&v_post_ncorr&" and ofer_ncorr="&v_ofer_ncorr&" and stde_ccod=1386"
			v_existe_descuento= conexion.consultaUno(sql_existe_descuento)
			
			if v_pendiente=0 and v_existe_descuento=0 then
			'response.Write("Pendiente:<br> "&sql_pendiente_de_pago&"<hr>")
			
				'########	crear descuento		#######
									
				sql_monto_matricula="select  aran_mmatricula from aranceles a,ofertas_academicas b "& vbCrLf &_
									"where a.ofer_ncorr="&v_ofer_ncorr&" "& vbCrLf &_
									"and a.aran_ncorr=b.aran_ncorr"
																
				v_monto_matricula= conexion.consultaUno(sql_monto_matricula)
				
				  tipo_descuento=1386 ' pago anticipado matricula
				  consulta_insercion="INSERT INTO sdescuentos "& vbCrLf &_ 
									 " (stde_ccod,post_ncorr,ofer_ncorr,esde_ccod,sdes_mmatricula,sdes_mcolegiatura,sdes_nporc_matricula,sdes_nporc_colegiatura,sdes_tobservaciones, audi_tusuario, audi_fmodificacion) "& vbCrLf  &_  
									 " Values ("&tipo_descuento&","&v_post_ncorr&","&v_ofer_ncorr&",2,"&v_monto_matricula&",0,100,0,'pago anticipado','descuento sistema',getDate() ) "
...
next
'*********** Fin descuento Matricula Anticipada **********
'*********************************************************

f_postulantes.MantieneTablas false
