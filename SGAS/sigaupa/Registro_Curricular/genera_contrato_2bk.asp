...
{		
	...
	formulario.elements["fp_matricula[" + i + "][stpa_ncuotas]"].value
	
	for (var i = 0; i < tabla_fp_matricula.filas.length; i++) {
		if (formulario.elements["fp_matricula[" + i + "][butiliza]"].value == 'S') {
			//ting_ccod=51 debito o pago redcompra, o similar (efectivo)
			if ((formulario.elements["fp_matricula[" + i + "][ting_ccod]"].value == '51')||(formulario.elements["fp_matricula[" + i + "][ting_ccod]"].value == '13')) {		
				if	(!VerificaNumerosTarjeta(formulario.elements["fp_matricula[" + i + "][sdfp_tctacte]"]))
					return false;
				
			}
			if ((formulario.elements["fp_matricula[" + i + "][ting_ccod]"].value != '6')&&(formulario.elements["fp_matricula[" + i + "][ting_ccod]"].value != '51')) {		
				...
				if(formulario.elements["fp_matricula[" + i + "][ting_ccod]"].value == '13'){
					ncuotas_doc_matricula = ncuotas_doc_matricula +1; 
				}else{
					ncuotas_doc_matricula += parseInt(formulario.elements["fp_matricula[" + i + "][stpa_ncuotas]"].value);
				}
				
			}
			else {
					ncuotas_efec_matricula += parseInt(formulario.elements["fp_matricula[" + i + "][stpa_ncuotas]"].value);
			}
		}
	}
	...
	for (var i = 0; i < <%=f_forma_pago_matricula.NroFilas%>; i++) {
		...
		if ((tabla_fp_matricula.filas[i].campos["butiliza"].objeto.value == 'S') && (v_sdfp_mmonto == 0)) {
			...
			formulario.elements["_fp_matricula[" + i + "][sdfp_mmonto]"].select();
			//tabla_alt_fp_matricula.filas[i].campos["sdfp_mmonto"].objeto.select();			
			return false;
		}
		
		v_fecha_pago = formulario.elements["_fp_matricula[" + i + "][sdfp_finicio_pago]"].value;
		//v_fecha_pago=tabla_fp_matricula.filas[i].campos["sdfp_finicio_pago"].objeto.value;
		if (FechaMayorHoy(v_fecha_pago)==false){
			formulario.elements["_fp_matricula[" + i + "][sdfp_finicio_pago]"].select();
	...
	for (var i = 0; i < <%=f_forma_pago_colegiatura.NroFilas%>; i++) {
		...
		v_fecha_pago = formulario.elements["_fp_colegiatura[" + i + "][sdfp_finicio_pago]"].value;
		//v_fecha_pago=tabla_fp_colegiatura.filas[i].campos["sdfp_finicio_pago"].objeto.value;
		if (FechaMayorHoy(v_fecha_pago)==false){
			formulario.elements["_fp_colegiatura[" + i + "][sdfp_finicio_pago]"].select();
...
function HabilitarFila(p_variable, p_fila, p_habilitado)
{
	...
	if (p_habilitado) {
		...
		document.forma_pago.elemets["fp_matricula["+ p_fila +"][sdfp_mmonto]"].value = '0';
		document.forma_pago.elements["fp_matricula["+ p_fila +"][sdfp_finicio_pago]"].value = '';
	...
	else
	 {
		...
		v_tipo_ingreso =document.forma_pago.elements["fp_matricula[" + p_fila + "][ting_ccod]"];
	...
	//	var cuotas_efectivo =  tabla.filas[0].campos["stpa_ncuotas"].objeto.value;
		...
		var fecha_efectivo =  document.forma_pago.elements["fp_matricula[0][sdfp_finicio_pago]"].value;
		var cuotas_efectivo =  document.forma_pago.elements["fp_matricula[0][stpa_ncuotas]"].value;
		...
		for (var i = p_fila; i >= 0; i--)
	    {
		  if (tabla.filas[i].ExisteCampo("sdfp_nfrecuencia") == true)
		    if (tabla.filas[i].campos["sdfp_nfrecuencia"].objeto.getAttribute("disabled") == false )
			    {
				  frecuencia_anterior = document.elemets["["+ i +"][sdfp_nfrecuencia]"].value;			   
				  cant_cuotas =  document.elemets["["+ i +"][stpa_ncuotas]"].value;
				  fecha_anterior =  document.elemets["["+ i +"][sdfp_finicio_pago]"].value;
	   ...
	   if (frecuencia_anterior == 0 )
		  {
			  ...
			  if (estado == "matricula")
 			    {
				  ...
				  if ((fecha_efectivo == "")||(v_tipo_ingreso.value==6)||(v_tipo_ingreso.value==13)||(v_tipo_ingreso.value==51))
				    document.forma_pago.elements["fp_matricula["+p_fila+"][sdfp_finicio_pago]"].value = '<%=fc_datos.ObtenerValor("fecha_actual")%>'; 
					//tabla.filas[p_fila].campos["sdfp_finicio_pago"].objeto.value = '<%=fc_datos.ObtenerValor("fecha_actual")%>';  
					
			      else
					document.forma_pago.elements["fp_matricula["+p_fila+"][sdfp_finicio_pago]"].value = fecha_anterior = dia1 + "/" + mes1 + "/" + anio1;
			  ...
			  else
			   {
                 ...
                 document.forma_pago.elements["fp_matricula["+p_fila+"][sdfp_finicio_pago]"].value =f_inicio;
			...
			//	tabla.filas[p_fila].campos["sdfp_finicio_pago"].objeto.value = '<%=fc_datos.ObtenerValor("fecha_actual")%>';
			  ...
			  if (v_tipo_ingreso.value==52){
					...
					document.forma_pago.elements["fp_matricula["+p_fila+"][sdfp_finicio_pago]"].value = diap + "/" + mesp + "/" + aniop;
		...
		document.forma_pago.elements["fp_matricula["+p_fila+"][sdfp_mmonto]"].value = -1 * (document.elements["fp_matricula[0][diferencia]"].value)    
