	...
	if (p_habilitado) {
		...
		if (frecuencia_anterior == 0 )
		{
		    ...
		    if (p_fila != 0) 
			{
				...
				if (estado == "matricula")
				{
					...
					if (v_tipo_ingreso.value==52){
						...
						document.forma_pago.elements["fp_colegiatura[5][sdfp_finicio_pago]"].readOnly = true;
						...
						var f_inicio = dia_inicio+"/"+mes+"/"+anio
						document.forma_pago.elements["fp_colegiatura[4][sdfp_finicio_pago]"].readOnly = true;
				...
				if (v_tipo_ingreso.value==52){
					...
					diap=30;
					mesp='11';
					var f_inicio = document.forma_pago.elements["fecha_inicio[0][fecha_inicio_pago]"].value; // ACA ESTOY
					var arr_fecha_pag = f_inicio.split(/\//); 
					var aniop =  arr_fecha_pag[2];
					tabla.filas[p_fila].campos["sdfp_finicio_pago"].objeto.value = diap + "/" + mesp + "/" + aniop;
				}
		  ...
		  //alert("otra frecuencia");
				...
				{
					tabla.filas[p_fila].campos["sdfp_finicio_pago"].objeto.value = '31/12/'+ anio; 
				}
				else if((v_tipo_ingreso.value==174) || (v_tipo_ingreso.value==175)){
					
					tabla.filas[p_fila].campos["sdfp_finicio_pago"].objeto.value = '30/11/'+ anio; 
				}
				else{					
	...
	if ((nombre=="_fp_colegiatura[3][butiliza]")||(nombre=="_fp_colegiatura[4][butiliza]")||(nombre=="_fp_colegiatura[5][butiliza]")){ 
		...
		if(variable=="fp_colegiatura"){
			...
			// DESHABILITA  EL PAGO CON TRANSBANK Y CON MULTIDEBITO
				...
				elemento3=document.forma_pago.elements["_fp_colegiatura[3][butiliza]"];
				if (!((nombre!="_fp_colegiatura[0][butiliza]")&&(nombre!="_fp_colegiatura[1][butiliza]")&&(nombre!="_fp_colegiatura[2][butiliza]"))){ 
					elemento3.checked=false;
				}
				cambiaOculto(elemento3, 'S', 'N')
				HabilitarFila(_ObtenerVariableCampo(elemento3), _FilaCampo(elemento3), !elemento3.checked);
