...
if v_esde_ccod = "1" then 'Autorizado
...
function Validar()
{
	...
	if (parseFloat(o_sdes_mmatricula.value) > (parseFloat(porc_max_matricula) * parseFloat(o_aran_mmatricula.value) / 100) ) {
	...
	if (parseFloat(o_sdes_mcolegiatura.value) > (parseFloat(por_max_colegiatura) * parseFloat(o_aran_mcolegiatura.value) ) / 100 ) {
	    alert('El valor ingresado supera el máximo permitido.')
