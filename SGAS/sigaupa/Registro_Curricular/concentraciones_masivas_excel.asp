...
<%
Response.AddHeader "Content-Disposition", "attachment;filename=historicos_notas.xls"
...
consulta="select * "& vbCrLf &_ 
		 ...
		 "      (select top 1 plan_tdesc from alumnos a1, ofertas_academicas o1,planes_estudio p1 where a1.pers_ncorr=d.pers_ncorr and a1.ofer_ncorr=o1.ofer_ncorr and o1.espe_ccod = c.espe_ccod and a1.emat_ccod <> 9  and a1.plan_ccod = p1.plan_ccod order by o1.peri_ccod desc, convert(datetime,a1.audi_fmodificacion) desc)   "& vbCrLf &_ 
		 "	   as plan_tdesc  "& vbCrLf &_ 
		 ...
		 "	        on c.peri_ccod = pea.peri_ccod and pea.anos_ccod = 2015  "& vbCrLf &_ 
		 ...
		 "	 and d.emat_ccod not in (9) and f.carr_ccod='51' "& vbCrLf &_ 
		 "	 and protic.ano_ingreso_carrera(a.pers_ncorr,'51') <= 2015 "& vbCrLf &_ 
...
<tr borderColor="#999999" bgColor="#c4d7ff">
	<TH><FONT color="#333333" face='Courier New, Courier, mono' size='1'>N°</FONT></TH>
	<TH><FONT color="#333333" face='Courier New, Courier, mono' size='1'>RUT</FONT></TH>
	<TH><FONT color="#333333" face='Courier New, Courier, mono' size='1'>ALUMNO</FONT></TH>
	<TH><FONT color="#333333" face='Courier New, Courier, mono' size='1'>NIVEL</FONT></TH>
    <TH><FONT color="#333333" face='Courier New, Courier, mono' size='1'>PLAN</FONT></TH>
	<TH><FONT color="#333333" face='Courier New, Courier, mono' size='1'>ASIGNATURA</FONT></TH>
	<%for o_ = 1 to oportunidades
			response.Write("<TH><FONT color=#333333 face='Courier New, Courier, mono' size='1'>"&o_&"a OP.</FONT></TH>")
	  next%>
	<TH><FONT color="#333333" face='Courier New, Courier, mono' size='1'>REPROBADOS</FONT></TH>  
</tr>
<%  contador_alumnos = 1
	...
	plan_tdesc = alumnos.obtenerValor("plan_tdesc")
  ...
  <%
    ...
    cons_historico="select a.nive_ccod,ltrim(rtrim(a.asig_ccod)) as asig_ccod, ltrim(rtrim(a.asig_ccod)) + ' -- ' + protic.initcap(asig.asig_tdesc) as asig_tdesc,a.mall_ccod, " & vbCrLf  & _
				   ...
				   "     when '(-)' then ' '" & vbCrLf  & _
				   "     else ('('+ cast(pa.anos_ccod as varchar) + '-' + case cast(b.sitf_ccod as varchar) when 'A' then 'A' when 'R' then 'R' when 'C' then 'C' when 'SP' then 'SP' when 'H' then 'HM' when 'S' then 'Su' when 'RS' then 'RS' when 'RI' then 'RI' when 'CR' then 'ECR' end +')') end ,'' ) as anos_ccod  " & vbCrLf  & _
				   "	 from (  " & vbCrLf  & _
...
'response.End()					
  ...
  <!--<tr> 
    <td width="100%" align="left">-->
		<%reprobados  = 0
		if plan_ccod <> "" then
		  response.Write("<table class='v1' width='100%' border='1' bordercolor='#CCCCFF' bgcolor='#CCCCFF' cellspacing='0' cellpadding='0'>")
		    historico.siguiente
			sit_final22	= historico.obtenervalor("sitf_ccod")
			if sit_final22 = "R " or sit_final22 = "RI" then
			...
			if historico.obtenervalor("asig_ccod") <> "" then
				...
				if aux = historico.obtenervalor("asig_ccod") then
				    ...
				    'response.Write("<br>contador "&contador_alumnos&" -->entre")
					sit_final22	= historico.obtenervalor("sitf_ccod")
					if sit_final22 = "R " or sit_final22 = "RI" then
						reprobados = reprobados + 1
					end if
					...
					filas = filas + 1
					response.Write("<td class=click><FONT color=#333333 face='Courier New, Courier, mono' size='1'>"&contador_alumnos&"</font></td>")
					response.Write("<td class=click><FONT color=#333333 face='Courier New, Courier, mono' size='1'>"&rut&"</font></td>")
					response.Write("<td class=click><FONT color=#333333 face='Courier New, Courier, mono' size='1'>"&nombre&"</font></td>")
					response.Write("<td class=click><FONT color=#333333 face='Courier New, Courier, mono' size='1'>"&nivel&"</font></td>")
					response.Write("<td class=click><FONT color=#333333 face='Courier New, Courier, mono' size='1'>"&plan_tdesc&"</font></td>")
					...
					next
					response.Write("<td class=click><FONT color=#333333 face='Courier New, Courier, mono' size='1'>"&reprobados&"</font></td>")
					reprobados = 0
					response.Write("</tr>")
					col	=	1
					contador = 2
