...
'ENTRADA				          : NA
'SALIDA				            : NA
'MODULO QUE ES UTILIZADO	: TOMA DE CARGA ACADEMICA(Archivo de proceso)
'
'--ACTUALIZACION--
'
...
& "group  by secc_ncupo "
'Nota: Se realiza el cambio de Right Outer Join a Left Outer Join (linea 59)
...
if cInt(topones) > 0  or cInt(cupo_disponible) < 1 then
	...
	consulta_insercion = " insert into cargas_Academicas (matr_ncorr,secc_ccod,acse_ncorr, audi_tusuario, audi_fmodificacion,fecha_ingreso_carga,usuario,tipo_carga)"&_
	                     " values ("&matr_ncorr&","&seccion&",2,'"&v_usuario&" (Online)',getdate(),getDate(),'Alumno','DAE')"
