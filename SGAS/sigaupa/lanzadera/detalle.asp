...
if  (pers_ncorr ="" ) then response.Redirect("blanco.asp")
	...
	'fsel.agregacampoparam "sede_ccod","destino","(select sede_ccod, sede_tdesc from sedes where sede_ccod in (1,4,9) and sede_ccod in (select sede_ccod from sis_sedes_usuarios where pers_ncorr = '"&pers_ncorr&"') ) a order by sede_ccod"
	...
	fc_periodos.Consultar ("select tape_ccod, a.peri_ccod, peri_tdesc from periodos_academicos a, actividades_periodos b where a.peri_ccod = b.peri_ccod and b.acpe_bvigente = 'S' ORDER BY peri_ccod DESC")


' ***** validacion para agregar planificacion por defecto al cargar pagina la primera vez *******
' ***** Vreado por mriffo.**********
' ***** Fecha: 16/03/2015 **********
	if smod_ccod=157 then
	v_planificacion=	conectar.ConsultaUno ("select max(a.peri_ccod) as valor from periodos_academicos a, actividades_periodos b where a.plec_ccod=1 and a.peri_ccod = b.peri_ccod and b.acpe_bvigente = 'S' and b.TAPE_CCOD=5")
		if isnull(v_planificacion) then
			v_planificacion= Session("_periodo")
		end if
		fsel.agregacampocons "sede_ccod",sede_ccod
		fsel.agregacampocons "tape_ccod","6"
		fsel.agregacampocons "peri_ccod",Session("_periodo")

	end if
' ***** fin agregacion de validacion x mriffo *******************
