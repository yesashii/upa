...
'ENTRADA		:NA
'SALIDA			:NA
'MODULO QUE ES UTILIZADO: sin acceso
'
'--ACTUALIZACION--
'
...
consulta = " select d.carr_tdesc as carrera, "& vbCrLf &_
		   ...
		   " protic.obtener_titulos_docente(a.pers_ncorr) as titulos_docente,protic.obtener_asignaturas_docente_carrera_anuales (1,d.carr_ccod,a.pers_ncorr,2006) as asignaturas,protic.trunc(fecha_grabado)as fecha, "& vbCrLf &_
		   " fortalesas_carrera"& vbCrLf &_
		   " from encuestas_docentes a INNER JOIN personas b "& vbCrLf &_
		   " ON a.pers_ncorr = b.pers_ncorr and  isnull(antiguos,'N')='N' "& vbCrLf &_
		   " LEFT OUTER JOIN sexos c "& vbCrLf &_
		   " ON b.sexo_ccod = c.sexo_ccod "& vbCrLf &_
		   " INNER JOIN carreras d "& vbCrLf &_
		   " ON a.carr_ccod = d.carr_ccod WHERE a.carr_ccod ='"&carr_ccod&"' and isnull(a.pers_ncorr,0)<>0 "& vbCrLf &_
		   " union all "& vbCrLf &_
		   " select d.carr_tdesc as carrera, "& vbCrLf &_
		   " 'NO INGRESADO' as sexo_tdesc,'--' as edad,anos_universidad,'--' as grados_docente, "& vbCrLf &_
		   " '--' as titulos_docente,'--' as asignaturas,protic.trunc(fecha_grabado)as fecha, "& vbCrLf &_
		   " fortalesas_carrera"& vbCrLf &_
		   " from encuestas_docentes a INNER JOIN carreras d "& vbCrLf &_
		   " ON a.carr_ccod = d.carr_ccod and a.carr_ccod = '"&carr_ccod&"' and  isnull(a.pers_ncorr,0)=0 "& vbCrLf &_
		   " where isnull(antiguos,'N') = 'N' "
