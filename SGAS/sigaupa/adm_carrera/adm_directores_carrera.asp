...
'--Query antigua antes del 24-06-2015
...
'--Creada por LUIS TORRES desde el 24-06-2015
consulta = " select distinct isnull(cast(c.pers_nrut as varchar),' ') as pers_nrut, " & vbCrLf &_
		   "        x.sede_ccod, " & vbCrLf &_
		   "        z.carr_ccod, " & vbCrLf &_
		   "        ltrim(rtrim(z.carr_tdesc)) as carr_tdesc, " & vbCrLf &_
		   "        (ltrim(rtrim(c.pers_tnombre))+' '+ltrim(rtrim(c.pers_tape_paterno))+' '+ltrim(rtrim(c.pers_tape_materno))) as nombre_director, " & vbCrLf &_
		   "        (ltrim(rtrim(convert(varchar,c.pers_nrut)))+'-'+ltrim(rtrim(c.pers_xdv))) as rut_director, " & vbCrLf &_
		   "        s.sede_tdesc as sede, " & vbCrLf &_
		   "        x.jorn_ccod, " & vbCrLf &_
		   "        j.jorn_tdesc as jornada " & vbCrLf &_
		   " from            ofertas_academicas  x " & vbCrLf &_
		   " left outer join especialidades      y on y.espe_ccod = x.espe_ccod " & vbCrLf &_
		   " left outer join carreras            z on z.carr_ccod = y.carr_ccod " & vbCrLf &_
		   " left outer join cargos_carrera      b on b.carr_ccod  = z.carr_ccod and b.sede_ccod = x.sede_ccod and b.jorn_ccod = x.jorn_ccod " & vbCrLf &_
		   " left outer join personas            c on c.pers_ncorr = b.pers_ncorr " & vbCrLf &_
		   " left outer join tipos_carrera       t on t.tcar_ccod  = b.tcar_ccod and t.tcar_tdesc in ('POSTGRADO','POSTGRADO') " & vbCrLf &_
		   " left outer join sedes               s on s.sede_ccod  = x.sede_ccod " & vbCrLf &_
		   " left outer join jornadas            j on j.jorn_ccod  = x.jorn_ccod " & vbCrLf &_
		   " where convert(varchar,x.sede_ccod) = '"&v_sede_ccod&"' " & vbCrLf &_
		   " and   z.carr_ccod is not null " & vbCrLf &_
