...
if isnull(etca_ccod) or etca_ccod=1 then
	...
	sin_restriccion = conexion.consultaUno("select count(*) from personas a, sis_roles_usuarios b where a.pers_ncorr=b.pers_ncorr and b.srol_ncorr in (2,143) and cast(a.pers_nrut as varchar)='"&usuario_sesion&"'")
	...
	for i = 0 to nroVars - 1
		...
		if secc_ccod <> "" and not isnull(homo_ccod) and tiene_parciales = "0" then
			topones_cons = "select isnull(sum(protic.topones_alumno_nuevo_2('" & secc_ccod & "','" & matr_ncorr & "')),0)"
			...
			'******************* usuario que puede tomar carga sin topones **************
			 ...
			 if tipo_plan <> "0"  and matr_ncorr <> "" then 
				...
				talu_ccod = conexion.consultaUno("select talu_ccod from alumnos where cast(matr_ncorr as varchar)='"&matr_ncorr&"'")
				response.Write("<br>"&talu_ccod)
				if talu_ccod = "2" or talu_ccod = "3" then
					if cint(total_a_asignar) > 15 then
						'response.Write("asigna mensaje")
						msje_creditos = "Imposible realizar la toma de carga solicitada, con ello se excedería el límite de 15 créditos."
					end if
					topones = "0"
					sin_restriccion = "0"
				else
...
'topones="0"
			...
			elseif tipo_plan <> "0" and msje_creditos <> "" and sin_restriccion = "0" then
				'conexion.estadoTransaccion false
			...
			elseif  msj_parciales <> "" then
				...
				if completo_requisitos = "0" then
				  inserta_carga_cons = "insert into cargas_academicas (matr_ncorr, secc_ccod, audi_tusuario, audi_fmodificacion,fecha_ingreso_carga,usuario,tipo_carga) " & vbCrLf &_
				                       "select '" & matr_ncorr & "','" & secc_ccod & "','" & negocio.obtenerusuario & "', getDate(),getDate(),'Directivo','Carga Plan'  " & vbCrLf &_
				  ...
				  inserta_carga_cons = " insert into cargas_academicas (matr_ncorr, secc_ccod,acse_ncorr, audi_tusuario, audi_fmodificacion,fecha_ingreso_carga,usuario,tipo_carga) " & vbCrLf &_
				                       " select '" & matr_ncorr & "','" & secc_ccod & "',3,'" & negocio.obtenerusuario & "', getDate(),getDate(),'Directivo','Carga Plan'  " & vbCrLf &_
									   " where not exists (select 1 from cargas_academicas a2 where cast(a2.matr_ncorr as varchar)= '" & matr_ncorr & "' and cast(secc_ccod as varchar) = '" & secc_ccod & "')"
				end if					
