...
& "group  by secc_ncupo "
'Nota: Se realiza el cambio de Right Outer Join a Left Outer Join (linea 90)
...
'****************** debemos revisar la suma de los creditos que tiene asignado el alumno mas los de la nueva asignatura
			 ...
			 if tipo_plan <> "0"  and matricula <> "" then 
				...
				if plan_asignatura = "378" or plan_asignatura = "527" then 
				    cred_asignatura =  "0.0"
				end if
				...
				talu_ccod = conectar.consultaUno("select talu_ccod from alumnos where cast(matr_ncorr as varchar)='"&matricula&"'")
...
if msj_errores <> "" then	
	...
	session("mensajeError") = "Equivalencia Guardada correctamente"
end if	
'response.End()	
response.Redirect("equivalencias.asp")
'response.End()
%>