...
topones_cons = "select isnull(sum(protic.topones_alumno_nuevo_2('" & seccion & "','" & matr_ncorr & "')),0)"
...
& "group  by secc_ncupo "
'Nota: Se realiza el cambio de Right Outer Join a Left Outer Join 
'debido a que de otra forma el sistema no entregaba los mismos resultados.
...
if cInt(topones) > 0  or cInt(cupo_disponible) < 1 then
	...
	consulta_insercion = " insert into cargas_Academicas (matr_ncorr,secc_ccod,acse_ncorr, audi_tusuario, audi_fmodificacion,fecha_ingreso_carga,usuario,tipo_carga)"&_
	                     " values ("&matr_ncorr&","&seccion&",2,'"&v_usuario&"',getdate(),getDate(),'Directivo','DAE')"
	conectar.ejecutas consulta_insercion
	session("mensajeError") = "Asignatura Artístico-Cultural guardada exitosamente"

end if ' fin del if por que el alumno no tiene topones y la sección aún tiene cupos

response.Redirect("ingreso_cursos_dae.asp")
