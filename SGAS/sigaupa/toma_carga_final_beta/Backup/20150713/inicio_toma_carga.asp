	...
	if cdbl(v_peri_ccod) < 218 then  'para años inferiores a 2009
		...
		v_mes_actual	= 	Month(now())
		 ...
		 crear_acceso=false
			...
			if v_peri_ccod = "240"  and  v_mes_actual = 7 and v_dia_actual >= 17 then
			    crear_acceso = true
			end if
			if v_peri_ccod = "240"  and  v_mes_actual = 8 and v_dia_actual >= 14 then
			    crear_acceso = true
			end if
...
promedio_semestral = conexion.consultaUno("select cast(promedio as varchar) + ' Al día ' + protic.trunc(audi_fmodificacion) from PROMEDIOS_ALUMNOS_CARRERA where cast(pers_ncorr as varchar)='"&pers_ncorr_temporal&"' and carr_ccod='"&carrera_prueba&"' and peri_ccod=238")
