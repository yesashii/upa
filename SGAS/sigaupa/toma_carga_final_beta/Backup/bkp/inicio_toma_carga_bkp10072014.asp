...
if pers_ncorr_temporal <> "" then
	...
	if cdbl(v_peri_ccod) < 218 then  'para años inferiores a 2009
		...
		v_mes_actual	= 	Month(now())
		 ...
		 crear_acceso=false
			if v_peri_ccod = "234"  and  v_mes_actual = 1 and v_dia_actual >= 13 then
			    crear_acceso = true
			end if
			
			if v_peri_ccod = "234"  and  v_mes_actual = 2 and v_dia_actual <= 28 then
			    crear_acceso = true
			end if
			
			if v_peri_ccod = "234"  and  v_mes_actual = 3 and v_dia_actual <= 28 then
			    crear_acceso = true
			end if
			...
			if sede_prueba = "9" and ((v_mes_actual = 4 and v_dia_actual >= 07) or (v_mes_actual = 5 and v_dia_actual <= 05))  then
			     'response.write ("entro")
			    crear_acceso = true
			end if	
...
c_encuestas = "select cantidad_carga - con_evaluacion_docente as diferencia "& vbCrLf &_
			...
			if v_plec_ccod = "2" and v_peri_ccod = "232" then
			c_encuestas = " select cantidad_carga - con_evaluacion_docente as diferencia  "& vbCrLf &_
						  "	 from  "& vbCrLf &_
						  "	  (  "& vbCrLf &_
						  "		 select cast(d.pers_nrut as varchar)+'-'+d.pers_xdv as rut, d.pers_tnombre + ' ' + d.pers_tape_paterno + ' ' +  d.pers_tape_materno as alumno,  "& vbCrLf &_
						  ...
						  "      and convert(datetime,protic.trunc(isnull(cc.fecha_ingreso_carga,'30-04-2013')),103) < convert(datetime,'29-04-2013',103) "& vbCrLf &_
						  "		 and not exists (select 1 from secciones sec,convalidaciones conv  "& vbCrLf &_
						  "						 where sec.secc_ccod=cc.secc_ccod and cc.matr_ncorr=conv.matr_ncorr and sec.asig_ccod=conv.asig_ccod)  "& vbCrLf &_
						  "		 and exists (select 1 from secciones aaa, bloques_horarios bbb, bloques_profesores ccc  "& vbCrLf &_
						  "					 where aaa.secc_ccod=cc.secc_ccod and aaa.secc_ccod=bbb.secc_ccod   "& vbCrLf &_
						  "					 and bbb.bloq_ccod=ccc.bloq_ccod and ccc.tpro_ccod=1  "& vbCrLf &_
						  "					 and convert(datetime,protic.trunc(ccc.audi_fmodificacion),103) < convert(datetime,'29-04-2013',103)))  "& vbCrLf &_
...
promedio_semestral = conexion.consultaUno("select cast(promedio as varchar) + ' Al día ' + protic.trunc(audi_fmodificacion) from PROMEDIOS_ALUMNOS_CARRERA where cast(pers_ncorr as varchar)='"&pers_ncorr_temporal&"' and carr_ccod='"&carrera_prueba&"' and peri_ccod=230")
