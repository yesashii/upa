...
if pers_ncorr_temporal <> "" then
	...
	if cdbl(v_peri_ccod) < 218 then  'para años inferiores a 2009
		...
		if sede_prueba = "7" then
			bloquear_toma_diurnos = "N" 'habilitado para siempre según solicitud jefa registro curricular
		end if
		...
		v_mes_actual	= 	Month(now())
		 anio_ingreso = conexion.consultaUno("select isnull(protic.ano_ingreso_carrera("&pers_ncorr_temporal&",'"&carrera_prueba&"'),2014)")
		 ...
		 crear_acceso=false
			...
			if v_peri_ccod = "236"  and  v_mes_actual = 7 and v_dia_actual >= 18 then
			    crear_acceso = true
			end if
			
			if v_peri_ccod = "236"  and  v_mes_actual = 7 and v_dia_actual >= 14 and carrera_prueba = "45" then
			    crear_acceso = true
			end if
			
			if v_peri_ccod = "236"  and  v_mes_actual = 8 and v_dia_actual <= 14 then
			    crear_acceso = true
			end if
			
			'if v_peri_ccod = "230"  and  v_mes_actual = 5 and v_dia_actual >= 13 and v_dia_actual <= 17 and carrera_prueba= "110" then
			 '   crear_acceso = true
			'end if
			
			'if v_peri_ccod = "232"  and  v_mes_actual = 7 and v_dia_actual >= 23 and carrera_prueba = "45" then
			'    crear_acceso = true
			'end if
			'if v_peri_ccod = "232"  and  v_mes_actual = 7 and v_dia_actual >= 28 and carrera_prueba <> "45" then
			'    crear_acceso = true
			'end if
			
			'if v_peri_ccod = "232"  and  v_mes_actual = 8 and v_dia_actual <= 23 then
			'    crear_acceso = true
			'end if
			
			'if v_peri_ccod = "232" and sede_prueba= "9"  and  v_mes_actual = 8 and v_dia_actual >= 13 and v_dia_actual <= 31 then
			'    crear_acceso = true
			'end if
			
			'MAGISTER Y LICENCIATURA
			...
			if sede_prueba = "9" and ((v_mes_actual = 7 and v_dia_actual >= 18) or (v_mes_actual = 8 and v_dia_actual <= 14))  then
			     'response.write ("entro")
			    crear_acceso = true
			end if	
...
c_encuestas = "select cantidad_carga - con_evaluacion_docente as diferencia "& vbCrLf &_
			...
			if v_plec_ccod = "2" and v_peri_ccod = "236" then
			c_encuestas = " select cantidad_carga - con_evaluacion_docente as diferencia  "& vbCrLf &_
						  "	 from  "& vbCrLf &_
						  "	  (  "& vbCrLf &_
						  "		 select cast(d.pers_nrut as varchar)+'-'+d.pers_xdv as rut, d.pers_tnombre + ' ' + d.pers_tape_paterno + ' ' +  d.pers_tape_materno as alumno,  "& vbCrLf &_
						  ...
						  "      and convert(datetime,protic.trunc(isnull(cc.fecha_ingreso_carga,'29-04-2014')),103) < convert(datetime,'28-04-2014',103) "& vbCrLf &_
						  "		 and not exists (select 1 from secciones sec,convalidaciones conv  "& vbCrLf &_
						  "						 where sec.secc_ccod=cc.secc_ccod and cc.matr_ncorr=conv.matr_ncorr and sec.asig_ccod=conv.asig_ccod)  "& vbCrLf &_
						  "		 and exists (select 1 from secciones aaa, bloques_horarios bbb, bloques_profesores ccc  "& vbCrLf &_
						  "					 where aaa.secc_ccod=cc.secc_ccod and aaa.secc_ccod=bbb.secc_ccod   "& vbCrLf &_
						  "					 and bbb.bloq_ccod=ccc.bloq_ccod and ccc.tpro_ccod=1  "& vbCrLf &_
						  "					 and convert(datetime,protic.trunc(ccc.audi_fmodificacion),103) < convert(datetime,'29-04-2014',103)))  "& vbCrLf &_
...
'			  if diferencia_encuestas > "0" or justificado_encuesta = "0" then 
  			  if diferencia_encuestas > "0" then 
			  	mensaje_encuesta = "El alumno no contestó todas las evaluaciones docentes correspondientes al año 2013-01, le restan "&diferencia_encuestas&" encuestas por evaluar"
			  end if
			  
			  '/////////////////////////Dirección de docencia pide cancelar requisito Ev.Docente 10-08-2009---------------------------
			  '/////////////////////////Dirección de docencia pide cancelar requisito Ev.Docente 16-06-2014---------------------------
			  mensaje_encuesta = ""
			  diferencia_encuestas = "0"
...
'			  end if
			 ...
			 if bloqueo_notas = "Bloqueado" then
			 	 mensaje_bloqueo_notas = "El alumno presenta un bloqueo académico en el sistema, lo que impide la toma de carga, haga el favor de comunicarse con su escuela para solucionar la situación."
			 end if
end if
...
'habilitación de encargados de deportes las condes, lyon y baquedano
...
'habilitación encargados de deportes melipilla
if  rut_usu="11404850" or rut_usu="7037083" or rut_usu="10536399" then
	if v_mes_actual = 1 and v_dia_actual <= 31 and v_peri_ccod="226" then
		crear_acceso=true					
	end if
...
promedio_semestral = conexion.consultaUno("select cast(promedio as varchar) + ' Al día ' + protic.trunc(audi_fmodificacion) from PROMEDIOS_ALUMNOS_CARRERA where cast(pers_ncorr as varchar)='"&pers_ncorr_temporal&"' and carr_ccod='"&carrera_prueba&"' and peri_ccod=234")
							...
							<tr>
								...
								<td width="15%" align="center">
								    <table width="100%" cellpadding="0" cellspacing="0">
										...
										<tr title="Acceder a información del Alumno" onClick="abrir_info_persona(<%=q_pers_nrut%>)" style="CURSOR: hand">
											<td width="33" height="31" align="center"><img width="31" height="31" src="../imagenes/magic-1.png" border="0"></td>
											<td align="left"><font color="#000066"><strong>Botón Mágico</strong></font></td>
										</tr>
              ...
              <table width="90%"  border="0" cellspacing="0" cellpadding="0">
                <tr>
                  ...
                  <td><div align="center"><% if cerrar_carga_diurno then
											 ...
											 if sede_prueba= "9" and cint(carrera_prueba)=110  then
