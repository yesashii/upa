...
'response.End()
if 	carr_ccod <> "" then
	filtro = " and b.carr_ccod in  ('" & carr_ccod & "')"
else
	filtro = ""
end if
...
pers_ncorr_encargado=conexion.consultaUno("Select pers_ncorr from personas where cast(pers_nrut as varchar)='"&usuario&"'")
	...
	if pers_ncorr_encargado="112627" then
	filtro2=filtro2&"b.sede_ccod=8 and "
	end if

sql_detalles_mate = "  SET ARITHABORT OFF " & vbcrlf & _
" SET ANSI_WARNINGS OFF " & vbcrlf & _
" select facu_tdesc,za.ccos_tcompuesto,c.jorn_tdesc,(select plan_tdesc from planes_estudio where plan_ccod= a.plan_ccod) as plan_tdesc, plan_ccod, nive_ccod, " & vbcrlf & _
					...
					" a.secc_ccod, cast(a.asig_ccod as varchar)+ '-' + cast(a.secc_tdesc as varchar) as seccion, asig_tdesc,duas_tdesc, a.sede_ccod,(Select sede_tdesc from sedes where sede_ccod=a.sede_ccod) as sede_tdesc, peri_ccod," & vbcrlf & _
 					" protic.horario_con_sala_hora(b.secc_ccod) AS horario, isnull(secc_nhoras_pagar,asig_nhoras) as asig_nhoras,secc_ncupo," & vbcrlf & _
" COUNT (distinct bloq_ccod) AS horas" & vbcrlf & _
" , isnull(round(cast(100/isnull(cast(isnull(secc_nhoras_pagar,case when asig_nhoras=0 then 999999999 else asig_nhoras end) as numeric),999999999)as decimal(5,2))*sum(protic.dias_habiles(dias_ccod,bloq_finicio_modulo,bloq_ftermino_modulo)),2),0) as porc " & vbcrlf & _
" , case when isnull(cast(isnull(secc_nhoras_pagar,case when asig_nhoras=0 then 999999999 else asig_nhoras end) as int),999999999) > isnull(sum(protic.dias_habiles(dias_ccod,bloq_finicio_modulo,bloq_ftermino_modulo)),0) then 1 else 2 end as estado " & vbcrlf & _
" , cast(isnull(round(sum(protic.dias_habiles(dias_ccod,bloq_finicio_modulo,bloq_ftermino_modulo)),2),0)*2 as varchar)" & vbcrlf & _
" + ' ( ' + cast( isnull(round(cast(100/isnull(cast(isnull(secc_nhoras_pagar,case when asig_nhoras=0 then 999999999 else asig_nhoras end) as numeric),999999999)as decimal(5,2))*sum(protic.dias_habiles(dias_ccod,bloq_finicio_modulo,bloq_ftermino_modulo)),0),0)*2 as varchar) + '% )' as hrs_asignadas" & vbcrlf & _
" ,  cast(sum(protic.dias_habiles(dias_ccod,bloq_finicio_modulo,bloq_ftermino_modulo)) as varchar)+' ('+cast(isnull(round(cast(100/isnull(cast(isnull(secc_nhoras_pagar,case when asig_nhoras=0 then 999999999 else asig_nhoras end) as int),999999999)as decimal(5,2))*sum(protic.dias_habiles(dias_ccod ,bloq_finicio_modulo,bloq_ftermino_modulo)),2),0) as varchar)+'%)' as horas_plan, " & vbcrlf & _
" protic.retorna_rut_profesor(a.secc_ccod) as rut_profesor,protic.retorna_profesor(a.secc_ccod) as profesor,protic.retorna_ayudante(a.secc_ccod) as Ayudante,(select count(*) from cargas_Academicas carg where carg.secc_ccod=a.secc_ccod) as num_alumnos, "& vbcrlf &_
...
" (select protic.trunc(max(bloq_ftermino_modulo)) as f_termino from bloques_horarios bh where bh.secc_ccod=a.secc_ccod) as f_termino, "& vbcrlf &_
...
" protic.profesores_seccion(a.secc_ccod) as listado_profesores, "& vbcrlf &_
" (select count(distinct t2.pers_ncorr) from bloques_horarios tt, bloques_profesores t2 "& vbcrlf &_
" where tt.secc_ccod=a.secc_ccod and tt.bloq_ccod=t2.bloq_ccod and t2.tpro_ccod=1) as total_profesores "& vbcrlf &_
" from ( " & vbcrlf & _
" select facu_tdesc,b.jorn_ccod,b.carr_ccod,secc_nhoras_pagar,secc_ccod, secc_tdesc,b.secc_ncupo,c.asig_ccod, asig_tdesc,duas_tdesc, asig_nhoras, sede_ccod, b.peri_ccod,d.plan_ccod,d.nive_ccod " & vbcrlf & _
" from ( " & vbcrlf & _
...
" , asignaturas c , malla_curricular d,duracion_asignatura e, periodos_academicos pea, carreras ca, areas_academicas aa, facultades fa " & vbcrlf & _
" where " & vbcrlf & _
...
" and b.carr_ccod=ca.carr_ccod " & vbcrlf & _
" and ca.area_ccod=aa.area_ccod " & vbcrlf & _
" and aa.facu_ccod=fa.facu_ccod " & vbcrlf & _
""&filtro&"" & vbcrlf & _
...
" a.sede_ccod,peri_ccod,secc_ncupo, asig_nhoras,  protic.horario (b.secc_ccod),secc_nhoras_pagar,facu_tdesc " & vbcrlf & _ 
...
'LA ULTIMA MODIFICACION FUE EL 08-09-2009 POR JUAN PABLO DURAN Y LA MODIFICACION FUE PARA QUE CUANDO EL asig_nhoras FUERAN IGUAL  A 0 EN LA DIVISIONES ESTE SE REEMPLAZARA POR  999999999 
'------------------------------------------------------------------------------
%>
<html>
<head>
...
<table width="75%" border="1">
  <tr> 
	...
	<td><div align="center"><strong>Profesores</strong></div></td>
	<td><div align="center"><strong>Total Profesores</strong></div></td>
  ...
  <tr> 
	...
	<td><div align="center"><%=f_detalle_mat.ObtenerValor("facu_tdesc")%></div></td>
	...
	<td><div align="left"><%=f_detalle_mat.ObtenerValor("listado_profesores")%></div></td>
	<td><div align="left"><%=f_detalle_mat.ObtenerValor("total_profesores")%></div></td>
