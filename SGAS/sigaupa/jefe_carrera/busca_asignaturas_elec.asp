...
pagina.Titulo = "Asignaturas Planificadas"
...
response.Write("asig_ccod_2 "&asig_ccod_2 &" asig_tdesc "&asig_tdesc)
if asig_ccod_2="" and asig_tdesc ="" then
	asig_ccod_2 ="NADA"
	asig_tdesc ="NADA"
end if
consulta =" select b.secc_ccod,isnull(b.secc_ccod,1) as seccion,cast(rtrim(ltrim(a.asig_ccod)) as varchar)+'-'+cast(a.asig_tdesc as varchar)+' seccion'+' '+cast(secc_tdesc as varchar) as asignatura,"& vbCrlf & _
		  "	isnull(protic.retorna_horario(isnull(cast(b.secc_ccod as varchar),'')),'NO TIENE HORARIO') as hor, "& vbCrlf & _
		  "	isnull(protic.retorna_profesor(isnull(cast(b.secc_ccod as varchar),'')),'NO TIENE PROFESOR') profesor"& vbCrlf & _
		  ...
		  " and (cast(a.asig_ccod as varchar) = '"&asig_ccod_2&"' or '"&asig_ccod_2&"' is null )"& vbCrlf & _
		  " and (cast(a.asig_tdesc as varchar) like '%"&asig_tdesc&"%' or '%"&asig_tdesc&"%' is null )"

          ' response.Write("<pre>"&consulta&"</pre> ==> 2")
           'response.End()
...
sql_malla = "select mall_ccod " & vbCrlf & _
			" from malla_curricular " & vbCrlf & _
			" where cast(asig_ccod as varchar)  = '"&asig_ccod&"'  " & vbCrlf & _
...
sql_electivos = " select a.secc_ccod, cast(rtrim(ltrim(c.asig_ccod)) as varchar)+'-'+cast(c.asig_tdesc as varchar)+' seccion'+' '+cast(secc_tdesc as varchar) as asignatura, " & vbCrlf & _
				" isnull(protic.retorna_horario(isnull(cast(b.secc_ccod as varchar),'')),'NO TIENE HORARIO') as hor, " & vbCrlf & _
				" isnull(protic.retorna_profesor(isnull(cast(b.secc_ccod as varchar),'')),'NO TIENE PROFESOR') profesor " & vbCrlf & _
				" from electivos a,secciones b,asignaturas c " & vbCrlf & _
				" where a.secc_ccod = b.secc_ccod " & vbCrlf & _
				" and b.asig_ccod = c.asig_ccod "  & vbCrlf & _
				" and cast(a.mall_ccod as varchar)= '"&mall_ccod&"'" & vbCrlf & _
				" and cast(a.asig_ccod as varchar) = '"&asig_ccod&"'"
'           response.Write("<pre>"&sql_electivos&"</pre> ==> 2")
 '          response.End()
...
function buscar(formulario){
	...
	formulario.target="_self";
	formulario.action = 'busca_asignaturas_elec.asp';
	formulario.submit();
          ...
          <tr>
			...
			<input type="hidden" name="sede_ccod" value="<%= sede_ccod%>">
                ...
                <table width="98%"  border="0" align="center" cellpadding="0" cellspacing="0">
                  ...
                  <tr>
                    <td><div align="left"><strong>De la lista de asignaturas, seleccione las que quiere como electivos para la asignatura <strong>
