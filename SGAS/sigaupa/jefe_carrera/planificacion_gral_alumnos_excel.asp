...
" , isnull(round(cast(100/isnull(cast(isnull(secc_nhoras_pagar,asig_nhoras) as numeric),999999999)as decimal(5,2))*sum(protic.dias_habiles(dias_ccod,bloq_finicio_modulo,bloq_ftermino_modulo)),2),0) as porc " & vbcrlf & _
" , case when isnull(cast(isnull(secc_nhoras_pagar,asig_nhoras) as int),999999999) > isnull(sum(protic.dias_habiles(dias_ccod,bloq_finicio_modulo,bloq_ftermino_modulo)),0) then 1 else 2 end as estado " & vbcrlf & _
" , cast(isnull(round(sum(protic.dias_habiles(dias_ccod,bloq_finicio_modulo,bloq_ftermino_modulo)),2),0)*2 as varchar)" & vbcrlf & _
" + ' ( ' + cast( isnull(round(cast(100/isnull(cast(isnull(secc_nhoras_pagar,asig_nhoras) as numeric),999999999)as decimal(5,2))*sum(protic.dias_habiles(dias_ccod,bloq_finicio_modulo,bloq_ftermino_modulo)),0),0)*2 as varchar) + '% )' as hrs_asignadas" & vbcrlf & _
" ,  cast(sum(protic.dias_habiles(dias_ccod,bloq_finicio_modulo,bloq_ftermino_modulo)) as varchar)+' ('+cast(isnull(round(cast(100/isnull(cast(isnull(secc_nhoras_pagar,asig_nhoras) as int),999999999)as decimal(5,2))*sum(protic.dias_habiles(dias_ccod ,bloq_finicio_modulo,bloq_ftermino_modulo)),2),0) as varchar)+'%)' as horas_plan, " & vbcrlf & _
...
" from ( " & vbcrlf & _
" select b.jorn_ccod,b.carr_ccod,secc_nhoras_pagar,secc_ccod, secc_tdesc,b.secc_ncupo,c.asig_ccod, asig_tdesc,duas_tdesc, asig_nhoras, sede_ccod, b.peri_ccod,d.plan_ccod,d.nive_ccod " & vbcrlf & _
" from ( " & vbcrlf & _
...
" a.sede_ccod,peri_ccod,secc_ncupo, asig_nhoras,  protic.horario (b.secc_ccod),secc_nhoras_pagar " & vbcrlf & _ 
