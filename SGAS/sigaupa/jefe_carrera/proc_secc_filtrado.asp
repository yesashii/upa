...
if (No_repetida)  then 
		...
		if insertar <> "" then
			...
			fsecc_asig.carga_parametros "adm_secciones_filtradas.xml", "4.1"
			...
			if request.Form("insertar_electivo")= "1" then
						...
						formulario.carga_parametros "adm_secciones_filtradas.xml", "f_electivos"
		...
		if actualizar <> "" then
			...
			for i=0 to fsecc_asig.cuentaPost - 1
				...
				'response.Write("<hr>secc_tdesc antiguo "&secc_tdesc)
				secc_tdesc = sub_cadena_seccion & jorn_ccod_corta & ")"
				'response.Write(" secc_tdesc nuevo "&secc_tdesc&"<hr>")
	 ...
	 if eliminar <> "" then
			...
			fsecc_asig.carga_parametros "adm_secciones_filtradas.xml", "4.2"
			fsecc_asig.inicializar conexion1
			fsecc_asig.procesaForm
