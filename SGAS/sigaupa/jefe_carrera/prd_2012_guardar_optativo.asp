...
topones_cons = "select sum(protic.topones_alumno('" & seccion & "','" & matr_ncorr & "'))"
...
if cInt(topones) > 0  or cInt(cupo_disponible) < 1 then
	...
	if cInt(topones) > 0 then
		msj_topones = conectar.ConsultaUno("select protic.detalle_topones_alumno('" & seccion & "','" & matr_ncorr & "')")
...
if afecta_promedio="" or afecta_promedio="N" then
	...
	cantidad_optativos_alumno = " select d.asig_ccod " & vbCrLf & _
    ...
    cantidad_optativos_alumno = conectar.consultaUno( "select isnull(count(*),0) from (" & cantidad_optativos_alumno & ")a")
	...
	'	en este caso la asignatura no afectará al promedio del alumno ya que se encuentra con todos sus optativos realizados
		...
		optativo_disponible = " select top 1 cast(a.mall_ccod as varchar) as mall_ccod, a.asig_ccod " & vbCrLf & _
							  ...
							  " where a. asig_ccod = b.asig_ccod and b.clas_ccod=2 asig_tdesc not like '%especialidad%'" & vbCrLf & _
							  " and cast(plan_ccod as varchar)='"&plan_alumno&"'" & vbCrLf & _
							  " and asig_ccod not in ("&cantidad_optativos_alumno&")"
	   ...
	   consulta_insercion = " insert into equivalencias (MATR_NCORR,SECC_CCOD,MALL_CCOD,ASIG_CCOD,AUDI_TUSUARIO,AUDI_FMODIFICACION) "&_
		                    " values("&matr_ncorr&","&seccion&","&mall_plan&",'"&asignatura_plan&"','traspaso optativos deportivos toma_carga',getdate()) "
	   conectar.ejecutaS consulta_insercion
