...
'				"	 )  as tabla"& vbCrLf
 ...
 ' CONSULTA PARA SABER LA CANTIDAD MAXIMA DE NOTAS PLANIFICADAS PARA LAS ASIGNATURAS DE UN ALUMNO X   
		sql	 =  "select isnull(max(nro_calificaciones),0) as maximo "& vbCrLf &_
				...
				"		  ON a.pers_ncorr = b.pers_ncorr AND b.EMAT_CCOD  <> 9 "& vbCrLf &_
				"		  INNER JOIN postulantes c "& vbCrLf &_
				"		  ON a.pers_ncorr = c.pers_ncorr and b.ofer_ncorr = c.ofer_ncorr "& vbCrLf &_
				"		  INNER JOIN cargas_academicas d "& vbCrLf &_
				"		  ON b.matr_ncorr = d.matr_ncorr and cast(c.peri_ccod as varchar) = '" & Periodo & "' "& vbCrLf &_
				"		  INNER JOIN secciones e "& vbCrLf &_
				"		  ON d.secc_ccod = e.secc_ccod "& vbCrLf &_
				"		  LEFT OUTER JOIN calificaciones_seccion f "& vbCrLf &_
				"		  ON e.secc_ccod = f.secc_ccod "& vbCrLf &_
				"		  WHERE cast(a.pers_nrut as varchar) = '" & rut_alumno & "' "& vbCrLf &_
				"		group by a.pers_nrut, e.asig_ccod,c.peri_ccod, e.secc_ccod, f.secc_ccod "& vbCrLf &_
				"	 )  as tabla"
