...
botonera.carga_parametros "plan_academica_laboratorio.xml", "btn_edicion_plan_acad"
...
if b_bloque_creado then
  tiene_asignado_laboratorio =  conec_resul.consultaUno("select count(*) from bloques_horarios where cast(bloq_ccod as varchar)='"&a&"' and sala_ccod in (30,32,31,29,43,274,65,25,175,176,102,161,85,133,167,266,79,80,35,76,280,281,298,299,306,302,336)")
  'response.Write(tiene_asignado_laboratorio)
  if tiene_asignado_laboratorio = "0" then
  		mensaje_cambio = "Bloque asignado a otra sala!, se cambiará por la que seleccione en ésta pantalla. ¿está seguro(a) de continuar?"
  end if
  
  if usuario_iniciado = "8516097" or usuario_iniciado = "15740666" then 'Habilitación de permisos para Regina
...
formu_resul.carga_parametros "plan_academica_laboratorio.xml", "edicion_bloque"
...
fc_salas.Carga_Parametros "plan_academica_laboratorio.xml", "consulta"
fc_salas.Inicializar conec_resul

consulta = " select a.sala_ccod, a.equipamiento as equipamiento " &_
           " from salas a, tipos_sala b " &_
		   " where a.tsal_ccod=b.tsal_ccod  and a.sala_ccod in (30,32,31,29,43,274,65,25,175,176,102,161,85,133,167,266,336) " &_ 
		   " "
...
f_botonera_profesor.Carga_Parametros "plan_academica_laboratorio.xml", "botonera"
...
arr_salas[<%=i_%>]["equipamiento"] = '<%=fc_salas.ObtenerValor("equipamiento")%>';
	<%
	i_ = i_ + 1
wend
%>
...
function MostrarCupos(p_sala_ccod)
{
	...
	for (i = 0; i < arr_salas.length; i++) {
		if (arr_salas[i]["sala_ccod"] == p_sala_ccod) {			
			v_equipamiento = arr_salas[i]["equipamiento"];
			break;
		}
	}
	if (v_equipamiento !='' )
	{
		alert(v_equipamiento);
		//desc_cupos.innerText = "Esta sala tiene " + v_equipamiento;
	}	
}

function CrearBloque(formulario)
{
	var fini = formulario.elements['pl[0][bloq_finicio_modulo]'].value;
	var ffin = formulario.elements['pl[0][bloq_ftermino_modulo]'].value;
	var sala = formulario.elements['pl[0][sala_ccod]'].value;
	
	if((fini !="")&&(ffin != "")&&(sala !=""))
	{
		formulario.action = 'actualizar_bloque_laboratorio.asp?accion=A';
		formulario.submit();
	}
	else
	{
	  alert("Debe ingresar todos los datos solicitados antes de continuar");	
	}	
}

function enviar(formulario){
	formulario.action ='actualizar_bloque_laboratorio.asp';
	var fini = formulario.elements['pl[0][bloq_finicio_modulo]'].value;
	var ffin = formulario.elements['pl[0][bloq_ftermino_modulo]'].value;
	var sala = formulario.elements['pl[0][sala_ccod]'].value;
	if((fini !="")&&(ffin != "")&&(sala !=""))
				   ...
				   <%
					...
					<%if mensaje_cambio = "" then%>
                      ...
                      <tr> 
						...
						<div align="left">  </div>
                          <div align="right"><% if usuario_iniciado = "8516097" or usuario_iniciado = "13483051"  or usuario_iniciado ="8474919" then
					  ...
					  <%else%>
					   <tr>
					   	  <td colspan="7" align="center">
						  	 <font face="Georgia, Times New Roman, Times, serif" size="+1" color="#0033FF"><br>El bloque horario que desea modificar, se encuentra asignado a una sala de clases o taller, es necesario que sea liberado por dirección de docencia para poder asignar el laboratorio correspondiente.<br></font>
						  </td>
					   </tr>
					  <%end if%>
                    </table>
                  ...
                  <td width="202" bgcolor="#D8D8DE"><table width="95%"  border="0" align="center" cellpadding="0" cellspacing="0">
					...
					<%if sys_cierra_planificacion=false and mensaje_cambio = "" then%>
		                 <td>
						      <div align="center">
                               <%
							     if usuario_iniciado = "8516097" or usuario_iniciado = "13483051" or usuario_iniciado = "13813352"  or usuario_iniciado ="8474919" then
