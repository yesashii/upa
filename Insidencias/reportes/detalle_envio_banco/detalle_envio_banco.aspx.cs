...
namespace detalle_envio_banco
{
	...
	public class WebForm1 : System.Web.UI.Page
	{
		 ...
		 private string Crear_Consulta_Listado_Letras(string envio, string periodo)
		{
			string sql;
			 sql= "";
			sql = " SELECT a.ENVI_NCORR, b.TING_CCOD, b.DING_NDOCTO AS c_ding_ndocto, b.INGR_NCORR, b.DING_NDOCTO, c.DING_MDOCTO,  \n";
            sql = sql +" c.DING_MDETALLE,a.ENVI_FENVIO, d.INGR_FPAGO, CONVERT(varchar, d.INGR_FPAGO, 103) AS ingr_fpago,  \n";
            ...
            sql = sql +"              (SELECT     CCTE_TDESC \n";
            sql = sql +"                FROM           CUENTAS_CORRIENTES \n";
            sql = sql +"                WHERE      (CCTE_CCOD = a.CCTE_CCOD)) AS ccte_tdesc, \n";
            sql = sql +"              (SELECT     INEN_TDESC \n";
            sql = sql +"                FROM           INSTITUCIONES_ENVIO \n";
            sql = sql +"                WHERE      (INEN_CCOD = a.INEN_CCOD)) AS inen_tdesc,  \n";
			sql = sql +" protic.obtener_rut(f.PERS_NCORR) AS rut_apoderado \n";
			sql = sql +" from   envios as a \n";
			sql = sql +"       join detalle_envios as b \n";
			sql = sql +"               on a.envi_ncorr = b.envi_ncorr \n";
			sql = sql +"       join detalle_ingresos as c \n";
			sql = sql +"               on b.ting_ccod = c.ting_ccod \n";
			sql = sql +"                  and b.ding_ndocto = c.ding_ndocto \n";
			sql = sql +"                  and b.ingr_ncorr = c.ingr_ncorr \n";
			sql = sql +"                  and c.ding_ncorrelativo = 1 \n";
			sql = sql +"       join estados_detalle_ingresos as c1 \n";
			sql = sql +"               on b.edin_ccod = c1.edin_ccod \n";
			sql = sql +"       join ingresos as d \n";
			sql = sql +"               on c.ingr_ncorr = d.ingr_ncorr \n";
			sql = sql +"       join personas as e \n";
			sql = sql +"               on d.pers_ncorr = e.pers_ncorr \n";
			sql = sql +"       left outer join personas as f \n";
			sql = sql +"                    on c.pers_ncorr_codeudor = f.pers_ncorr \n";
			sql = sql +"where  cast(a.envi_ncorr as varchar) = '" + envio + "' ";
			return (sql);
		}
