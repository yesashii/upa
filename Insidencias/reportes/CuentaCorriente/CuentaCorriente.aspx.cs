		...
		*/
		...
		string generar_sql_datos_alumno(string pers_ncorr, string periodo, string persona,string peri_sel)
		{
			...
			else
			{
    			...
    			sql = "select * "+"\n";
				sql = sql +"from   (select a.pers_ncorr, "+"\n";
				sql = sql +"               protic.obtener_nombre_completo(a.pers_ncorr, 'n')                                                                                                                               as nombre "+"\n";
				sql = sql +"               , "+"\n";
				sql = sql +"               protic.obtener_rut(a.pers_ncorr) "+"\n";
				sql = sql +"               as rut, "+"\n";
				sql = sql +"               c.sede_tdesc, "+"\n";
				sql = sql +"               protic.obtener_nombre_carrera(a.ofer_ncorr, 'CE')                                                                                                                               as "+"\n";
				sql = sql +"               carr_tdesc, "+"\n";
				sql = sql +"               isnull(protic.ano_ingreso_carrera(a.pers_ncorr, (select protic.obtener_nombre_carrera((select top 1 ofer_ncorr "+"\n";
				sql = sql +"                                                                                                      from   alumnos "+"\n";
				sql = sql +"                                                                                                      where  matr_ncorr = a.matr_ncorr), 'CC'))), protic.ano_ingreso_universidad(a.pers_ncorr))as "+"\n";
				sql = sql +"               ano_ingreso, "+"\n";
				sql = sql +"               case "+"\n";
				sql = sql +"                 when protic.tiene_contrato_periodo('" +peri_sel+"', a.pers_ncorr) >= 1 then 'MATRICULADO' "+"\n";
				sql = sql +"                 else 'NO MATRICULADO' "+"\n";
				sql = sql +"               end                                                                                                                                                                             as "+"\n";
				sql = sql +"                      estado_matricula, "+"\n";
				sql = sql +"               case isnull(e.emat_ccod, 0) "+"\n";
				sql = sql +"                 when 1 then 'ACTIVO' "+"\n";
				sql = sql +"                 else e.emat_tdesc "+"\n";
				sql = sql +"               end                                                                                                                                                                             as "+"\n";
				sql = sql +"               estado_alumno "+"\n";
				sql = sql +"                      , "+"\n";
				sql = sql +"               protic.ano_ingreso_universidad(a.pers_ncorr) "+"\n";
				sql = sql +"                      as ingreso_u, "+"\n";
				sql = sql +"               (select peri_tdesc "+"\n";
				sql = sql +"                from   periodos_academicos "+"\n";
				sql = sql +"                where  peri_ccod = '" +peri_sel+"')                                                                                                                                             as "+"\n";
				sql = sql +"               peri_tdesc "+"\n";
				sql = sql +"        from   alumnos as a "+"\n";
				sql = sql +"               inner join ofertas_academicas as b "+"\n";
				sql = sql +"                       on a.ofer_ncorr = b.ofer_ncorr "+"\n";
				sql = sql +"                          and b.peri_ccod = '"  + periodo + "' "+"\n";
				sql = sql +"               inner join sedes as c "+"\n";
				sql = sql +"                       on b.sede_ccod = c.sede_ccod "+"\n";
				sql = sql +"               left outer join contratos as d "+"\n";
				sql = sql +"                            on a.matr_ncorr = d.matr_ncorr "+"\n";
				sql = sql +"                               and d.econ_ccod <> 3 "+"\n";
				sql = sql +"               inner join estados_matriculas as e "+"\n";
				sql = sql +"                       on a.emat_ccod = e.emat_ccod "+"\n";
				sql = sql +"        where  cast(a.pers_ncorr as varchar) = '"  + pers_ncorr + "' "+"\n";
				sql = sql +"               and a.emat_ccod <> 9) as tabla "+"\n";
				sql = sql +"union "+"\n";
				sql = sql +"select * "+"\n";
				sql = sql +"from   (select a.pers_ncorr, "+"\n";
				...
				sql = sql +"               inner join estados_matriculas as e "+"\n";
				sql = sql +"                       on a.emat_ccod = e.emat_ccod "+"\n";
				sql = sql +"        where  cast(a.pers_ncorr as varchar) = '"  + pers_ncorr + "') as tabla ";
...
*/
		string generar_sql_comentarios(string pers_ncorr)
		{
			...
			sql = "select a.come_ncorr, "+"\n";
			sql = sql +"       a.come_tcomentario, "+"\n";
			sql = sql +"       protic.trunc(a.come_fcomentario) as come_fcomentario, "+"\n";
			sql = sql +"       b.tico_tdesc, "+"\n";
			sql = sql +"       substring(c.pers_tnombre, 1, 1) + '' "+"\n";
			sql = sql +"       + c.pers_tape_paterno            as pers_ncorr_autor "+"\n";
			sql = sql +"from   comentarios as a "+"\n";
			sql = sql +"       inner join tipos_comentarios as b "+"\n";
			sql = sql +"               on a.tico_ccod = b.tico_ccod "+"\n";
			sql = sql +"       left outer join personas as c "+"\n";
			sql = sql +"               on a.pers_ncorr_autor = c.pers_ncorr "+"\n";
			sql = sql +"where  cast(a.pers_ncorr as varchar) = '"  + pers_ncorr + "' ";
