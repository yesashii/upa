	...
	public class WebForm1 : System.Web.UI.Page
	{
		...
		private string EscribirCodigo( string post_ncorr)
		{
			...
			sql =  "SELECT con.cont_ncorr AS ciudad_codeudor1, pag.paga_ncorr nro_pagare, \n ";
					sql = sql + " (select SUM(di.DING_MDOCTO) from contratos cc, pagares pag, compromisos com, ABONOS a, INGRESOS i, detalle_ingresos di WHERE cc.cont_ncorr = com.COMP_NDOCTO AND cc.cont_ncorr = pag.cont_ncorr AND com.tcom_ccod = 2 AND cc.POST_NCORR = p.post_ncorr AND di.TING_CCOD=dii.TING_CCOD AND ISNULL(pag.opag_ccod, 1) NOT IN (2) AND a.COMP_NDOCTO = com.COMP_NDOCTO AND i.INGR_NCORR = a.INGR_NCORR AND di.INGR_NCORR = a.INGR_NCORR and com.ecom_ccod <> 3  and di.DING_NDOCTO=dii.DING_NDOCTO) AS valor_pagar, \n ";
					sql = sql + " (select COUNT(*) from contratos cc, pagares pag, compromisos com, ABONOS a, INGRESOS i, detalle_ingresos di WHERE cc.cont_ncorr = com.COMP_NDOCTO AND cc.cont_ncorr = pag.cont_ncorr AND com.tcom_ccod = 2 AND cc.POST_NCORR = p.post_ncorr AND di.TING_CCOD=dii.TING_CCOD AND ISNULL(pag.opag_ccod, 1) NOT IN (2) AND a.COMP_NDOCTO = com.COMP_NDOCTO AND i.INGR_NCORR = a.INGR_NCORR AND di.INGR_NCORR = a.INGR_NCORR and com.ecom_ccod <> 3  and di.DING_NDOCTO=dii.DING_NDOCTO)  AS NUMERO_CUOTAS, \n ";
...
//Response.Flush();
		...
		private string EscribirCodigoMultidebito( string post_ncorr)
		{
			...
			sql = sql + " (select SUM(s.SDFP_MMONTO) FROM SDETALLES_FORMA_PAGO s WHERE POST_NCORR = p.post_ncorr AND TING_CCOD=dii.ting_ccod) AS valor_pagar, \n ";
			sql = sql + " (select COUNT(*) FROM SDETALLES_PAGOS s WHERE POST_NCORR = p.post_ncorr AND TING_CCOD=dii.ting_ccod) AS NUMERO_CUOTAS, \n ";
			sql = sql + " cast(DATEPART(dd, GETDATE()) AS varchar) dd_hoy,protic.trunc(dii.ding_fdocto) as fecha_pago, \n ";
			sql = sql + " (SELECT mes_tdesc FROM meses WHERE mes_ccod = DATEPART(mm, GETDATE()))AS mm_hoy, \n ";
			sql = sql + " cast(DATEPART(mm, GETDATE()) AS varchar) mm_antiguo,\n "; 
			sql = sql + " dii.ding_tcuenta_corriente cuenta_cte,(select banc_tdesc from bancos where banc_ccod=dii.banc_ccod) as banco ,\n";
		...
		private string EscribirCodigoPagaCae( string post_ncorr, int valor)
		{
			string sql;

			sql =  "SELECT dii.ting_ccod, con.cont_ncorr AS ciudad_codeudor1,\n ";
			sql = sql + " (select SUM(s.SDFP_MMONTO) FROM SDETALLES_FORMA_PAGO s WHERE POST_NCORR = p.post_ncorr AND TING_CCOD=dii.ting_ccod) AS valor_pagar,\n ";
			sql = sql + " (select COUNT(*)  FROM SDETALLES_FORMA_PAGO s WHERE POST_NCORR = p.post_ncorr AND TING_CCOD=dii.ting_ccod) AS NUMERO_CUOTAS, \n ";
			sql = sql + " cast(DATEPART(dd,GETDATE()) AS varchar) dd_hoy,\n ";
			sql = sql + " protic.trunc(dii.ding_fdocto) as fecha_pago, \n ";
			sql = sql + " (SELECT mes_tdesc FROM meses WHERE mes_ccod = DATEPART(mm, GETDATE()))AS mm_hoy, \n ";
			sql = sql + " cast(DATEPART(mm, GETDATE()) AS varchar) mm_antiguo,\n ";
			sql = sql + " cast(DATEPART(yy, GETDATE())AS varchar) yy_hoy, ciu.ciud_tdesc \n ";
			sql = sql + " ciudad_sede, pac.anos_ccod periodo_academico, (pac.anos_ccod + 1) AS inicio_vencimiento, \n ";
			...
			sql = sql + "  AND ddc.tdir_ccod = 1  \n ";
			sql = sql + " AND ddp.tdir_ccod = 1  \n ";
			sql = sql + "  and isnull(dii.ting_ccod, 0) in (" + valor + ") \n ";
			sql = sql + "  and cps.ecom_ccod <> 3   \n ";
			sql = sql + " and con.econ_ccod in (1, 2)   \n ";
			sql = sql + " and cps.tcom_ccod in (2)   \n ";
			sql = sql + " and dc.tcom_ccod in (2)   \n ";
		...
		private string EscribirCodigorepactacionPagaCae(string repa_ncorr, string post_ncorr)
		{
			string sql;

			sql = "SELECT distinct \n ";
				sql = sql + " rp.repa_ncorr AS ciudad_codeudor1, dii.DING_NDOCTO AS nro_pagare,    \n ";
				sql = sql + " (SELECT SUM(DING_MDETALLE) FROM detalle_ingresos r WHERE r.repa_ncorr =rp.repa_ncorr and r.TING_CCOD=dii.TING_CCOD and r.INGR_NCORR=ii.INGR_NCORR GROUP BY DING_MDETALLE ) AS valor_pagar,  \n ";
				sql = sql + " (SELECT COUNT(*) FROM detalle_ingresos r WHERE r.repa_ncorr=rp.repa_ncorr and r.TING_CCOD=dii.TING_CCOD and r.INGR_NCORR=ii.INGR_NCORR GROUP BY DING_MDETALLE ) AS NUMERO_CUOTAS,      \n ";
				sql = sql + " cast(DATEPART(dd, GETDATE()) AS varchar) dd_hoy,\n ";
				sql = sql + " protic.trunc(dii.ding_fdocto) as fecha_pago,    \n ";
				sql = sql + " (SELECT mes_tdesc FROM meses WHERE mes_ccod = DATEPART(mm, GETDATE()))AS mm_hoy,    \n ";
				sql = sql + " cast(DATEPART(mm, GETDATE()) AS varchar) mm_antiguo,    \n ";
				...
				sql = sql + " and cps.inst_ccod=dc.inst_ccod      \n ";
				sql = sql + " and cps.tcom_ccod=dc.tcom_ccod       \n ";
				sql = sql + " join detalle_ingresos dii     \n ";
				sql = sql + " ON rp.repa_ncorr = dii.repa_ncorr\n ";
				sql = sql + " left outer join ingresos ii     \n ";
				sql = sql + " on dii.ingr_ncorr = ii.ingr_ncorr    \n ";
				sql = sql + " left outer join tipos_ingresos tii    \n ";
				sql = sql + " on dii.ting_ccod =tii.ting_ccod    \n ";
				...
				sql = sql + " and dii.DING_NDOCTO=173 \n ";
				sql = sql + " and ddc.tdir_ccod = 1     \n ";
				sql = sql + " AND ddp.tdir_ccod = 1     \n ";
				sql = sql + " and cps.ecom_ccod <> 3      \n ";
				sql = sql + " and cps.tcom_ccod in (3)    \n ";
				sql = sql + " and dc.tcom_ccod in (3) \n ";
				sql = sql + " and ii.ting_ccod in (15) \n ";
		...
		private string EscribirCodigorepactacionPagabeca(string repa_ncorr, string post_ncorr)
		{
			string sql;

			sql = "SELECT  distinct \n ";
			sql = sql + " rp.repa_ncorr AS ciudad_codeudor1, dii.DING_NDOCTO AS nro_pagare,    \n ";
			sql = sql + " (SELECT SUM(DING_MDETALLE) FROM detalle_ingresos r WHERE r.repa_ncorr =rp.repa_ncorr and r.TING_CCOD=dii.TING_CCOD GROUP BY DING_MDETALLE ) AS valor_pagar,  \n ";
			sql = sql + " (SELECT COUNT(*) FROM detalle_ingresos r WHERE r.repa_ncorr=rp.repa_ncorr and r.TING_CCOD=dii.TING_CCOD GROUP BY DING_MDETALLE ) AS NUMERO_CUOTAS,      \n ";
			sql = sql + " cast(DATEPART(dd, GETDATE()) AS varchar) dd_hoy,\n ";
			sql = sql + " protic.trunc(dii.ding_fdocto) as fecha_pago,    \n ";
			sql = sql + " (SELECT mes_tdesc FROM meses WHERE mes_ccod = DATEPART(mm, GETDATE()))AS mm_hoy,    \n ";
			sql = sql + " cast(DATEPART(mm, GETDATE()) AS varchar) mm_antiguo,    \n ";
			...
			sql = sql + " on dii.banc_ccod = bn.banc_ccod\n ";    
			sql = sql + " WHERE  cast(rp.repa_ncorr as varchar) = '"+repa_ncorr+"'  \n ";
			sql = sql + " and dii.DING_NDOCTO IN(174,175) \n ";
			sql = sql + " and ddc.tdir_ccod = 1     \n ";
			sql = sql + " AND ddp.tdir_ccod = 1     \n ";
			sql = sql + " and cps.ecom_ccod <> 3      \n ";
			sql = sql + " and cps.tcom_ccod in (3)    \n ";
...
//Response.Flush();
		...
		private string EscribirPagareRepactacionCurso( string repa_ncorr)
		{
			string sql;
				sql = "SELECT  \n ";
				sql = sql + " rp.repa_ncorr AS ciudad_codeudor1, pag.paga_ncorr nro_pagare,    \n ";
				...
				sql = sql + "cast(DATEPART(mm, GETDATE()) AS varchar) mm_antiguo,  \n ";  
				sql = sql + "dii.ding_tcuenta_corriente cuenta_cte,(select banc_tdesc from bancos where banc_ccod=dii.banc_ccod) as banco , \n "; 
				sql = sql + "cast(DATEPART(yy, GETDATE())AS varchar) yy_hoy, ccp.ciud_tdesc   \n "; 
				sql = sql + "ciudad_sede, year(getdate()) periodo_academico, (year(getdate()) + 1) AS inicio_vencimiento,   \n "; 
				sql = sql + "(year(getdate()) + 2) AS final_vencimiento, cast(pp.pers_nrut AS varchar) + '-' + cast(pp.pers_xdv AS varchar) AS rut_post,   \n "; 
				sql = sql + "pp.pers_tnombre + ' ' + pp.pers_tape_paterno + ' ' + pp.pers_tape_materno AS nombre_alumno,    \n ";
				sql = sql + "'' AS carrera, cast(pp.pers_nrut AS varchar) + '-' + cast(pp.pers_xdv AS varchar) AS rut_codeudor,   \n "; 
				sql = sql + "pp.pers_tnombre + ' ' + pp.pers_tape_paterno + ' ' + pp.pers_tape_materno    \n ";
				sql = sql + "AS nombre_codeudor, ddp.dire_tcalle + ' ' + cast(ddp.dire_tnro AS varchar)+' '+ case ddp.DIRE_TBLOCK when '' then '' else 'Depto '+cast(ddp.DIRE_TBLOCK as varchar) end AS direccion_codeudor,    \n ";
				sql = sql + "ccp.ciud_tcomuna ciudad_codeudor, ddp.dire_tcalle + ' ' + cast(ddp.dire_tnro AS varchar) AS direccion_postulante,   \n ";
				sql = sql + "ccp.ciud_tdesc ciudad_codeudor1_x_contrato,dii.ding_mdetalle as valor_cuota,ccp.ciud_tdesc comuna_codeudor  \n ";
				sql = sql + " FROM personas pp   \n ";  
				sql = sql + "left outer join direcciones ddp    \n ";
				sql = sql + "    on pp.pers_ncorr = ddp.pers_ncorr   \n "; 
				sql = sql + "left outer join ciudades ccp    \n ";
				sql = sql + "    on ddp.ciud_ccod =ccp.ciud_ccod    \n ";
				sql = sql + " join compromisos cps     \n "; 
				sql = sql + "    on pp.pers_ncorr=cps.pers_ncorr  \n ";
				sql = sql + "    and cps.tcom_ccod=3     \n "; 
				sql = sql + "join repactaciones rp  \n ";
				sql = sql + "    on cps.comp_ndocto=rp.repa_ncorr  \n ";
				sql = sql + "join pagares pag    \n ";
				...
				sql = sql + "WHERE  cast(rp.repa_ncorr as varchar) = '"+repa_ncorr+"' \n "; 
				sql = sql + "AND ddp.tdir_ccod = 1     \n ";
				sql = sql + "and isnull(dii.ting_ccod, 0) in (52,59,66)   \n "; 
				sql = sql + "and cps.ecom_ccod <> 3   \n ";   
				sql = sql + "and cps.tcom_ccod in (3) \n ";  
				sql = sql + "and dc.tcom_ccod in (3)  \n ";
	...
	return (sql);
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			//Response.Write(imprimirFinanza);
			...
			if (imprimirFinanza=="S")
			{
				...
				if(repa_ncorr !="") 
				{
					...
					else if (tipo_pagare=="C" )
					{
						sql = EscribirCodigorepactacionPagaCae(repa_ncorr,post_ncorr);
					}
					else if (tipo_pagare=="B1" || tipo_pagare=="B2")
					{ 
						sql = EscribirCodigorepactacionPagabeca(repa_ncorr,post_ncorr);
					}
				...
				else if (tipo_pagare=="C")// Pagare Cae
				{ 
					sql = EscribirCodigoPagaCae(post_ncorr,173);
					oleDbDataAdapter1.SelectCommand.CommandText = sql;
					oleDbDataAdapter1.Fill(datosPagare1);

				}
				else if (tipo_pagare=="B1")// Pagare BECA 1
				{ 
					sql = EscribirCodigoPagaCae(post_ncorr,174);
					oleDbDataAdapter1.SelectCommand.CommandText = sql;
					oleDbDataAdapter1.Fill(datosPagare1);

				}
				else if (tipo_pagare=="B2")// Pagare BECA 2
				{ 
					sql = EscribirCodigoPagaCae(post_ncorr,175);
					oleDbDataAdapter1.SelectCommand.CommandText = sql;
					oleDbDataAdapter1.Fill(datosPagare1);

				}
				else if(tipo_pagare=="M")// Pagare de Multidebito
				{
					sql = EscribirCodigoMultidebito(post_ncorr);
					oleDbDataAdapter1.SelectCommand.CommandText = sql;
					oleDbDataAdapter1.Fill(datosPagare1);
				}
				else if(tipo_pagare=="CU" || tipo_pagare=="CM" || tipo_pagare=="CT") // Pagare de cursos (todos, upa, multidebito y transbank)
				{
					sql = EscribirPagareRepactacionCurso(repa_ncorr);
					oleDbDataAdapter1.SelectCommand.CommandText = sql;
					oleDbDataAdapter1.Fill(datosPagare1);
				}
				else
			...
			CrystalReportPagaUpa2 reporteUpa = new CrystalReportPagaUpa2();
			CrystalReportPagaCae reporteCae = new CrystalReportPagaCae();
			CrystalReportPagaBeca reporteBeca = new CrystalReportPagaBeca();


			if(tipo_pagare=="U" || tipo_pagare=="CU")
			{
			...
			{
				reporteCae.SetDataSource(datosPagare1);
				VerPagare.ReportSource = reporteCae;
				ExportarPDF(reporteCae);
			}
			else if(tipo_pagare=="B1" || tipo_pagare=="B2")
			{
				reporteBeca.SetDataSource(datosPagare1);
				VerPagare.ReportSource = reporteBeca;
				ExportarPDF(reporteBeca);
			}
			else if(tipo_pagare=="M" || tipo_pagare=="CM")
			{
		...
		{    
			...
			// oleDbConnection1
			// 
			//this.oleDbConnection1.ConnectionString = "Provider=SQLOLEDB;server=sbd03;OLE DB Services = -2;uid=protic;pwd=,.protic;initi" +"al catalog=sigaupa";
			// CONECTOR ADMISION
			...
			this.datosPagare1.Namespace = "http://www.tempuri.org/datosContrato.xsd";
