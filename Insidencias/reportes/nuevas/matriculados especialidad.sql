  select a.pers_ncorr, e.carr_ccod, c.peri_ccod, cast(pers_nrut as varchar)+'-'+cast(pers_xdv as varchar) as rut,   
			 ...
			 and c.peri_ccod=protic.retorna_max_periodo_matricula(a.pers_ncorr,'202',e.carr_ccod)   
			 ...
			 and a.pers_ncorr not in (
                    select pers_ncorr from alumnos where ofer_ncorr=12997
             )
              group by a.pers_ncorr, e.carr_ccod, c.peri_ccod,pers_nrut, pers_xdv, pers_tnombre,pers_tape_paterno, pers_tape_materno,pers_fnacimiento,d.matr_ncorr   



select * from especialidades where carr_ccod=49		
select * from carreras


select * from alumnos where ofer_ncorr=12997 and emat_ccod in (1,4,8,2)   