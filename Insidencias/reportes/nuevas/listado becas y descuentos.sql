select count(*) as cantidad, beneficio from (
    select distinct protic.obtener_rut(d.pers_ncorr) as rut,tdet_tdesc as beneficio,a.esde_ccod, protic.obtener_nombre_carrera(b.ofer_ncorr,'CJ') as carrera
    from sdescuentos a, postulantes b, tipos_detalle c, personas d, alumnos e
    where a.post_ncorr=b.post_ncorr
    and a.stde_ccod=c.tdet_ccod
    and b.pers_ncorr=d.pers_ncorr
    and b.peri_ccod in (214,216)
    and a.esde_ccod =1
...
select count(rut) as alumnos, beneficio, carrera
 from (
            ...
            where b.peri_ccod in (214,216)
            and c.peri_ccod in (214,216)
            and c.econ_ccod not in (2,3)
            and g.esde_ccod in (1)
            and convert(datetime,cont_fcontrato,103) between  convert(datetime,'01/10/2008',103) and convert(datetime,'01/10/209',103)
