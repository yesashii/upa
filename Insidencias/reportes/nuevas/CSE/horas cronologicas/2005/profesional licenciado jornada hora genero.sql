...
and aa.pers_ncorr not in ( select distinct pers_ncorr from administrativos_docentes)
and aa.grado in ('LICENCIADO','PROFESIONAL')
...
select distinct case d.sede_tdesc when 'MELIPILLA' then 'MELIPILLA' else 'SANTIAGO' end as sede,a.pers_ncorr,
    ...
    protic.obtener_grado_docente(a.pers_ncorr,'D') as descripcion_grado,23 as horas_semanales
    ...
    where a.pers_ncorr      =   b.pers_ncorr
        ...
        and datepart(year,a.cdoc_fcontrato_ini)=2005
        and convert(datetime,a.cdoc_fcontrato_ini,103)<=convert(datetime,'30/04/2005',103)
        and protic.obtener_grado_docente(a.pers_ncorr,'G') in ('LICENCIADO','PROFESIONAL')
        and bloq_ccod in (
