select genero, cantidad, sede,carrera,especialidad,jornada,entre_300_325,minimo, máximo,
...
(select count(*) from postulantes po,personas pe where po.ofer_ncorr=b.ofer_ncorr 
 and po.pers_ncorr=pe.pers_ncorr and pe.sexo_ccod=c.sexo_ccod and po.post_nano_paa = 2005
 ...
 and alu.ofer_ncorr=po.ofer_ncorr and alu.emat_ccod in (1,2,4,8,13) and alu.alum_fmatricula <=convert(datetime,'30/04/2006',103) )
...
(select count(*) from postulantes po,personas pe
 ...
 and pe.sexo_ccod=c.sexo_ccod and po.post_nano_paa = 2005 and isnull(post_npaa_verbal,0) <> 0 and isnull(post_npaa_matematicas,0) <> 0
and exists (select 1 from alumnos alu where alu.post_ncorr=po.post_ncorr 
 and alu.ofer_ncorr=po.ofer_ncorr and alu.emat_ccod in (1,2,4,8,13) and alu.alum_fmatricula <=convert(datetime,'30/04/2006',103) )
...
(select count(*)
 from postulantes po,personas pe
 where po.ofer_ncorr = b.ofer_ncorr and po.pers_ncorr = pe.pers_ncorr
 and pe.sexo_ccod=c.sexo_ccod and po.post_nano_paa = 2005
 and exists (select 1 from alumnos alu where alu.post_ncorr=po.post_ncorr 
 and alu.ofer_ncorr=po.ofer_ncorr and alu.emat_ccod in (1,2,4,8,13) and alu.alum_fmatricula <=convert(datetime,'30/04/2006',103) )
...
(select count(*)
 from postulantes po,personas pe
 where po.ofer_ncorr = b.ofer_ncorr and po.pers_ncorr = pe.pers_ncorr
 and pe.sexo_ccod=c.sexo_ccod and (isnull(po.post_nano_paa,0) <> 2005 or isnull(post_npaa_verbal,0) = 0 or isnull(post_npaa_matematicas,0) = 0)
 and exists (select 1 from alumnos alu where alu.post_ncorr=po.post_ncorr 
 and alu.ofer_ncorr=po.ofer_ncorr and alu.emat_ccod in (1,2,4,8,13) and alu.alum_fmatricula <=convert(datetime,'30/04/2006',103) )
 ) as no_rindieron_psu,
(select count(*)
...
select po.*,pers_nnota_ens_media,c.*
from alumnos a, ofertas_academicas b, personas c, sexos d,especialidades e, carreras f ,sedes g, jornadas h,postulantes po
where a.ofer_ncorr=b.ofer_ncorr
and a.pers_ncorr=c.pers_ncorr
and a.post_ncorr=po.post_ncorr
and c.sexo_ccod=d.sexo_ccod
and b.sede_ccod=g.sede_ccod
and b.jorn_ccod=h.jorn_ccod
and b.espe_ccod=e.espe_ccod
...
select top 10 cast(((isnull(post_npaa_verbal,0) + isnull(post_npaa_matematicas,0))/ 2) as decimal(4,1)),* from postulantes 
select top 10 * from personas 



select * from tipos_pruebas_admision


select * from postulantes where post_nano_paa in (2004) 
