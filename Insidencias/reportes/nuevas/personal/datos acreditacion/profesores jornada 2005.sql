select carrera,aa.sede,aa.pers_ncorr,aa.rut,aa.nombre_docente,cc.sexo_tdesc as genero,aa.tipo_profesor,
aa.grado,aa.descripcion_grado, sum(hora_semana) as horas_semanales
from  (
    select b.carr_tdesc as carrera,aa.carr_ccod,regimen,max(coordinacion) as horas_coordinacion,
        ...
        ((max(coordinacion)*2)+(sum(horas)*2))/case regimen when 'ANUAL'then 36 
            ...
            and b.tcar_ccod=1
        group by b.carr_tdesc,aa.pers_ncorr,aa.horas,aa.carr_ccod,aa.sede,aa.regimen,aa.nombre_docente,aa.tipo_profesor
) aa, personas bb, sexos cc
where aa.pers_ncorr=bb.pers_ncorr
and bb.sexo_ccod=cc.sexo_ccod
--and aa.grado in ('DOCTORADO')
group by carrera,aa.sede,aa.pers_ncorr,aa.rut,aa.nombre_docente,cc.sexo_tdesc,aa.tipo_profesor,aa.grado,aa.descripcion_grado
--having sum(hora_semana) >= 33

UNION 

select distinct k.carr_tdesc as carrera,case d.sede_tdesc when 'MELIPILLA' then 'MELIPILLA' else 'SANTIAGO' end as sede,a.pers_ncorr,
    ...
    sedes d,profesores e,tipos_profesores f,personas g,sexos h,bloques_horarios i,secciones j, carreras k 
