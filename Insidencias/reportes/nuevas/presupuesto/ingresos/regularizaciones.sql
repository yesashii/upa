...
fecha as fecha_asignacion,rut,nombre_alumno,max(arancel) as arancel, carrera,centro_costo,detalle_gasto
from (
...
ccos_tcompuesto as centro_costo,(select ting_detalle_gasto from tipos_ingresos where ting_ccod=c.ting_ccod) as detalle_gasto
From ingresos a
   ...
   left outer join centros_costos_asignados z    	
		on z.cenc_ccod_carrera  =g.carr_ccod       
		and z.cenc_ccod_sede    =e.sede_ccod       
		and z.cenc_ccod_jornada =e.jorn_ccod    	
    ...
    Where b.ting_ccod in (
        ...
        and convert(datetime,a.ingr_fpago,103) between  convert(datetime,'01/02/2009',103) and convert(datetime,getdate(),103)
       group by m.tcom_tdesc,l.tcom_ccod,a.mcaj_ncorr,d.matr_ncorr,a.ingr_nfolio_referencia,a.ingr_fpago,b.ting_ccod,c.ting_tdesc,c.ting_ccod,a.pers_ncorr,ccos_tcompuesto
)  as tabla
group by aplicado_sobre,monto_descontado,sobre_monto,beneficio,
fecha,nombre_alumno, rut,carrera, caja,comprobante,centro_costo,detalle_gasto
