...
*/
		...
		********************************************************************/
		...
		private string ObtenerSql(string p_repa_ncorr)
		{
			...
			SQL = " SELECT 1 AS ORDEN, 'Se cambian documentos :' AS ENCABEZADO, A.TING_CCOD, A.INGR_NCORR,\n";
			SQL = SQL +  " CASE G.TING_CCOD WHEN 52 THEN CAST(A.DING_NDOCTO AS VARCHAR) ELSE CASE G.TING_CCOD WHEN 173 THEN null ELSE CAST(A.DING_NDOCTO AS VARCHAR)END END AS DING_NDOCTO, \n";
			SQL = SQL +  " D.BANC_TDESC, A.DING_FDOCTO, D.BANC_CCOD, CASE A.TING_CCOD WHEN 52 THEN 'PAG. TRANS.' else G.TING_TDESC end + ' ' + H.EDIN_TDESC AS TING_TDESC, A.DING_MDETALLE AS MONTO1, 0 as MONTO2, E.ABON_MABONO AS MONTO3 \n";
			SQL = SQL +  " FROM DETALLE_INGRESOS A \n"; 
			SQL = SQL +  " INNER JOIN INGRESOS B  \n"; 
			SQL = SQL +  " ON A.INGR_NCORR = B.INGR_NCORR AND (cast(A.REPA_NCORR as varchar)= '" + p_repa_ncorr + "' ) AND (B.EING_CCOD <> 3) \n"; 
			SQL = SQL +  " INNER JOIN ABONOS C  \n"; 
			SQL = SQL +  " ON B.INGR_NCORR = C.INGR_NCORR  \n"; 
			SQL = SQL +  " INNER JOIN TIPOS_INGRESOS G  \n"; 
			SQL = SQL +  " ON A.TING_CCOD = G.TING_CCOD   \n"; 
			SQL = SQL +  " INNER JOIN ABONOS E  \n"; 
			SQL = SQL +  " ON C.TCOM_CCOD = E.TCOM_CCOD AND C.INST_CCOD = E.INST_CCOD AND C.COMP_NDOCTO = E.COMP_NDOCTO AND C.DCOM_NCOMPROMISO = E.DCOM_NCOMPROMISO   \n"; 
			SQL = SQL +  " INNER JOIN INGRESOS F  \n"; 
			SQL = SQL +  " ON E.INGR_NCORR = F.INGR_NCORR AND A.REPA_NCORR = F.INGR_NFOLIO_REFERENCIA  \n"; 
			SQL = SQL +  "AND (F.TING_CCOD = 9) AND (F.EING_CCOD = 5) AND (isnull(F.INGR_MTOTAL, 0) > 0)  \n"; 
			SQL = SQL +  " LEFT OUTER JOIN BANCOS D  \n"; 
			SQL = SQL +  " ON A.BANC_CCOD = D.BANC_CCOD    \n"; 
			SQL = SQL +  " LEFT OUTER JOIN ESTADOS_DETALLE_INGRESOS H  \n"; 
			SQL = SQL +  " ON A.EDIN_CCOD = H.EDIN_CCOD  \n"; 
			SQL = SQL +  " WHERE CASE A.TING_CCOD WHEN 52 THEN 'PAG. TRANS.' else G.TING_TDESC end + ' ' + H.EDIN_TDESC IS NOT NULL\n"; 
			SQL = SQL +  " UNION \n";
			SQL = SQL +  " SELECT 2 AS ORDEN, 'Nuevos documentos :' AS ENCABEZADO, F.TING_CCOD, F.INGR_NCORR, \n";
			SQL = SQL +  " CASE F.TING_CCOD WHEN 52 THEN CAST(F.DING_NDOCTO AS VARCHAR) ELSE CASE F.TING_CCOD WHEN 173 THEN null ELSE CAST(F.DING_NDOCTO AS VARCHAR)END END AS DING_NDOCTO, \n";
			SQL = SQL +  " G.BANC_TDESC, F.DING_FDOCTO, G.BANC_CCOD, CASE F.TING_CCOD WHEN 52 THEN 'PAG. TRANS.' else H.TING_TDESC end AS TING_TDESC, C.DCOM_MNETO AS MONTO1, C.DCOM_MINTERESES AS MONTO2, F.DING_MDETALLE AS MONTO3 \n";
			SQL = SQL +  " FROM REPACTACIONES A \n";
			SQL = SQL +  " INNER JOIN COMPROMISOS B  \n";
			SQL = SQL +  " ON A.REPA_NCORR = B.COMP_NDOCTO AND (A.REPA_NCORR = " + p_repa_ncorr + ") AND (B.TCOM_CCOD = 3) \n";
			SQL = SQL +  " INNER JOIN DETALLE_COMPROMISOS C  \n";
			SQL = SQL +  " ON B.TCOM_CCOD = C.TCOM_CCOD AND B.INST_CCOD = C.INST_CCOD AND B.COMP_NDOCTO = C.COMP_NDOCTO  \n";
			SQL = SQL +  " INNER JOIN ABONOS D  \n";
			SQL = SQL +  " ON C.TCOM_CCOD = D.TCOM_CCOD AND C.INST_CCOD = D.INST_CCOD AND C.COMP_NDOCTO = D.COMP_NDOCTO AND C.DCOM_NCOMPROMISO = D.DCOM_NCOMPROMISO   \n";
			SQL = SQL +  " INNER JOIN INGRESOS E  \n";
			SQL = SQL +  " ON D.INGR_NCORR = E.INGR_NCORR AND A.REPA_NCORR = E.INGR_NFOLIO_REFERENCIA AND (E.EING_CCOD = 4) AND (E.TING_CCOD = 15)  \n";
			SQL = SQL +  " INNER JOIN DETALLE_INGRESOS F  \n";
			SQL = SQL +  " ON E.INGR_NCORR = F.INGR_NCORR   \n";
			SQL = SQL +  " INNER JOIN TIPOS_INGRESOS H  \n";
			SQL = SQL +  " ON F.TING_CCOD = H.TING_CCOD   \n";
			SQL = SQL +  " LEFT OUTER JOIN BANCOS G  \n";
			SQL = SQL +  " ON F.BANC_CCOD = G.BANC_CCOD \n";
			SQL = SQL +  " ORDER BY ORDEN ASC, A.TING_CCOD ASC, A.DING_FDOCTO ASC \n";
			//Response.Write(SQL);
			//Response.Flush();
			return SQL;
