...
namespace imprimir_letra
{
	...
	public class WebForm1 : System.Web.UI.Page
	{
		...
		private string EscribirCodigo(string v_post_ncorr)
		{
			...
			sql = sql + " cc.carr_tdesc + ' ('+ substring(jorn_tdesc,1,1) + ')' as carrera,ciu.ciud_tdesc ciudad_sede, ";
			sql = sql + " convert (varchar,dii.DING_FDOCTO,103) as fecha_entera_v, ";
			sql = sql + " (select mes_tdesc from meses Where mes_ccod=datepart (month,dii.DING_FDOCTO)) as mes_v, ";
			...
			sql = sql + " protic.obtener_nombre(ppc.pers_ncorr,'c')  as nombre_codeudor, ";
			sql = sql + " protic.obtener_direccion_letra(ppc.pers_ncorr, 1,'CNPB') as direccion, ";
	        ...
	        sql = sql + " abonos bb, ingresos ii, detalle_ingresos dii, sedes ss, ciudades ciu, jornadas ";
			sql = sql + "  where p.pers_ncorr=pp.pers_ncorr  ";
            sql = sql + " and con.post_ncorr=p.post_ncorr and ";  
	        sql = sql + " cps.ecom_ccod <> 3 and "; 
	        sql = sql + " con.econ_ccod not in (3,4) and     "; 
		...
		private string EscribirCodigo_Pact() 
		{
			...
			// query para cursos y otras pactaciones por cajas

			SQL = " select protic.nro_letra_pactada(dii.ding_ndocto, dii.ingr_ncorr, e.comp_ndocto,7) as valores, protic.obtener_rut(p.pers_ncorr) as rut_post, g.tdet_tdesc as carrera, n.ciud_tdesc as ciudad_sede,";
			SQL = SQL +  "        convert (varchar,dii.DING_FDOCTO,103) as fecha_entera_v,";
			SQL = SQL +  " 	   (select mes_tdesc from meses Where mes_ccod=datepart (month,dii.DING_FDOCTO)) as mes_v,";
			SQL = SQL +  " 	   datepart (dd,dii.DING_FDOCTO) as dd_v,";
			SQL = SQL +  " 	   datepart (yyyy,dii.DING_FDOCTO) as ano_v,";
			...
			SQL = SQL +  " 	   protic.obtener_rut(j.pers_ncorr) as rut_codeudor, j.pers_tfono as fono_codeudor, protic.obtener_nombre(j.pers_ncorr,'c') as nombre_codeudor, protic.obtener_direccion(j.pers_ncorr, 1,'CN') as direccion, l.ciud_tdesc as ciudad, l.ciud_tcomuna as comuna";
		...
		private string EscribirCodigo_Repactaciones() {
			...
			// Consulta para las Repactaciones
			
			sql = " select protic.nro_letra_pactada(dii.ding_ndocto,ii.ingr_ncorr,cps.comp_ndocto,3)  as valores,";
			sql = sql + " convert(varchar,pp.PERS_NRUT) +'-'+pp.PERS_XDV as rut_post, ";
			sql = sql + " cc.carr_tdesc + ' ('+ substring(jorn_tdesc,1,1) + ')' as carrera,ciu.ciud_tdesc ciudad_sede, ";
			sql = sql + " convert (varchar,dii.DING_FDOCTO,103) as fecha_entera_v, ";
			sql = sql + " (select mes_tdesc from meses Where mes_ccod=datepart (month,dii.DING_FDOCTO)) as mes_v, ";
			...
			sql = sql + " especialidades ee, carreras cc, ";
			sql = sql + " direcciones_publica ddp, ciudades c, ";
			sql = sql + " compromisos cps , detalle_compromisos dc, ";
			sql = sql + " abonos bb, ingresos ii, detalle_ingresos dii, sedes ss, ciudades ciu, jornadas ";
			sql = sql + "  where p.pers_ncorr=pp.pers_ncorr  ";
			sql = sql + " and cps.ecom_ccod <> 3  "; 
			sql = sql + " and cps.comp_ndocto=dc.comp_ndocto and   ";  
			sql = sql + " cps.tcom_ccod=dc.tcom_ccod and    "; 
			sql = sql + " bb.comp_ndocto=dc.comp_ndocto and    "; 
			sql = sql + " bb.tcom_ccod=dc.tcom_ccod and     "; 
			sql = sql + " bb.dcom_ncompromiso=dc.dcom_ncompromiso and  ";    
			...
			for (int i = 0; i < Request.Form.Count; i++)
			{
				...
				if (pase!=0)
				{
					...
					if ((Request.Form.GetKey(i)==post_ncorr) && (tipo_impresion != "2"))
					{
						...
						//datos = datos +  " and p.post_ncorr=56491";
					...
					if (Request.Form.GetKey(i)==ingr_ncorr)
					{
						...
						switch (tipo_impresion) {
							case "2" :
								...
								break;
							case "3" :
								sql = EscribirCodigo_Repactaciones();
								//Response.Write(sql);
								//Response.Flush();
								break;
							default:
