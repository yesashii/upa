...
namespace CartaGuia
{
	...
	public class WebForm1 : System.Web.UI.Page
	{
		...
		// listado de letras por apoderado antigua
		private string Crear_Consulta_Listado_Letras_Agrupado_Ant(string envio, string periodo)
		{
			string sql;
		  
			sql = "SELECT  0 as carta, a.envi_ncorr, a.envi_ncorr as c_envi_ncorr, c.pers_ncorr, ";
			sql = sql +		" f.pers_nrut as r_alumno, f.pers_xdv,  ";
			sql = sql +" cast(f.pers_nrut as varchar) + '-' + f.pers_xdv as rut_alumno,  ";
			sql = sql +		" cast(f.pers_nrut as varchar) + '-' + f.pers_xdv as c_rut_alumno,  ";
			sql = sql +		" isnull(cast(g.pers_nrut as varchar),' ') as code_nrut , isnull(g.pers_xdv,' ') as code_xdv,  ";
			sql = sql +		" isnull(cast(g.pers_nrut as varchar),' ') as  r_apoderado,  ";
			sql = sql +		" isnull(cast(g.pers_nrut as varchar),' ') + '-' + isnull(g.pers_xdv,' ') as rut_apoderado,  ";
			sql = sql +		" isnull(cast(g.pers_nrut as varchar),' ') + '-' + isnull(g.pers_xdv,' ') as c_rut_apoderado,  ";
			sql = sql +		" isnull(cast(g.pers_tnombre as varchar), ' ') + ' ' + isnull(cast(g.pers_tape_paterno as varchar),' ') as nombre_apoderado,  ";
			sql = sql +		"count(a.envi_ncorr) as cantDOC, sum(b.ding_mdocto) as totalDOC  ";
			sql = sql +  "FROM  detalle_envios a,  detalle_ingresos b,  ingresos c,  ";
			sql = sql +	  " personas f,   personas g  ";
			sql = sql +  "WHERE b.DING_NCORRELATIVO = 1  ";
			sql = sql +  "and a.ting_ccod = b.ting_ccod ";
			sql = sql +  "and a.ding_ndocto = b.ding_ndocto ";
			sql = sql +  "and a.ingr_ncorr = b.ingr_ncorr ";;
			sql = sql +  "and b.ingr_ncorr = c.ingr_ncorr ";
			sql = sql +  "and c.pers_ncorr = f.pers_ncorr ";
			sql = sql +  "and b.PERS_NCORR_CODEUDOR *= g.pers_ncorr "; 
			sql = sql +  "AND a.envi_ncorr='"+ envio + "' "; 
			sql = sql +  "GROUP BY a.envi_ncorr, c.pers_ncorr, f.pers_xdv, g.pers_xdv,   ";
			sql = sql +	  "g.pers_tape_paterno, g.pers_tnombre, f.pers_nrut ,g.pers_nrut ";
