...
'--------------------------------------debug>>
response.Write("<br>bloq: "&bloq&"<br/>")
'response.end()
'--------------------------------------debug<<
if bloq = "" then
	bloq = "NULL"
...
elseif bloq_ayudantia="4" then
	consulta = "select isnull(isnull(asig_nhoras_elearning,isnull(b.secc_nhoras_pagar,a.asig_nhoras)),0) from asignaturas a, secciones b, sub_secciones c where a.asig_ccod=b.asig_ccod and b.secc_ccod=c.secc_ccod and c.ssec_ncorr = " & ssec
    consulta2 = "select isnull(sum(protic.dias_habiles(dias_ccod,bloq_finicio_modulo,bloq_ftermino_modulo)),0) as a from bloques_horarios where bloq_ayudantia=4 and  ssec_ncorr = " & ssec
end if
...
if hora + horas <= 22 then
	...
	mensajeError = ""
	if bloq_ayudantia <> "4" then 'No considera topones cuando es un bloque elearning
...
'-------------------------------------------------------- debug >>
				'response.write("v_bloq_ccod: "&v_bloq_ccod&"<br/>")
				'response.end()
'-------------------------------------------------------- debug <<
