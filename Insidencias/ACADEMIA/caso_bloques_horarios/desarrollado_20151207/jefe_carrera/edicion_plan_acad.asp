...
va_bloq_ccod 	= request("bloq_ccod")
vb_ssec_ncorr = request("ssec_ncorr")
vc_pers_ncorr = request("pers_ncorr")
bloque_prueba = request("bloq_ccod")
subseccion		= request.QueryString("ssec_ncorr")
bloque				= request.QueryString("bloque")
profesor			= "select isnull(convert(varchar(20),protic.profesor_subseccion("&subseccion&")),'') as docente"
...
'/****************************************/
'/****************************************/
'/* DETERMINANDO EL valor de bloq_ccod */
'/************************************/
bloq_ccod = va_bloq_ccod
If Not IsNull(subseccion) Then
	if bloquesHorarios > 0 and subseccion <> "" then

consulta = "" & vbCrLf &_
"select top 1 a.bloq_ccod                            					" & vbCrLf &_
...
"                  and f.sede_ccod = isnull("&sede_ccod&",1) 	" & vbCrLf &_
"       inner join asignaturas as g                  					" & vbCrLf &_
"               on f.asig_ccod = g.asig_ccod         					" & vbCrLf &_
"       inner join dias_semana as h                  					" & vbCrLf &_
"               on a.dias_ccod = h.dias_ccod         					"

		if ip_usuario = ip_de_prueba then response.Write("<pre>"&consulta&"</pre>") end if' DEBUG

	bloq_ccod = conec_resul.ConsultaUno(consulta)
end if'If Not IsNull(subseccion) Then

'Preguntamos si el bloque ya esta creado.....
	If Not IsNull(bloq_ccod) Then
...
sql = "select distinct isnull(b.srol_npermiso,2) from sis_roles_usuarios a, sis_roles b where cast(a.pers_ncorr as varchar)='"&pers_ncorr_temporal&"' and a.srol_ncorr=b.srol_ncorr "
...
if tipo_permiso="1" and bloque = "1" then
...
consulta = "select pers_ncorr, bloq_ccod, protic.obtener_nombre_completo(pers_ncorr, 'PM,N') as nombre_profesor, sede_ccod, a.tpro_ccod,bloq_anexo," & vbCrLf &_
		   ...
		   "where cast(bloq_ccod as varchar) = '" & bloq_ccod & "' and a.tpro_ccod=b.tpro_ccod"
...
'	if ip_usuario = ip_de_prueba then response.Write("<pre>b_bloque_creado: "&b_bloque_creado&"</pre>") end if' DEBUG
...
'	f_botonera_profesor.AgregaBotonParam "crear_bloque", "deshabilitado", "TRUE" ' se deshabilita por ticket 1263
end if
  ...
  <%if tipo_permiso="1" and bloque = "1" then%>
	...
	<%if tipo_permiso="1" and bloque = "1" then%>
		<%if mensaje_cambio = "" then %>
				...
				<%else%>
					  					<tr>
												<td colspan="7" align="center"><font face="Georgia, Times New Roman, Times, serif" size="+1" color="#0033FF"><br>El bloque horario que desea modificar, se encuentra asignado a un laboratorio o auditorium, es necesario que sea liberado por el departamento de desarrollo y tecnología para poder asignar la sala o taller correspondiente.<br></font></td>
					   					</tr>
				<%end if%>
			<%end if%>
                      <tr>
...
<%end if%>
<%if tipo_permiso="1" and bloque="0" then%>
    ...
    <tr>
    	<td><br>
			...
			<table width="20%"  border="0" align="right" cellpadding="0" cellspacing="0">
				<tr>
					<td><div align="center"><%f_botonera_profesor.DibujaBoton("agregar_profesor")%></div></td>
					<td><div align="center"><%f_botonera_profesor.DibujaBoton("eliminar_profesor")%></div></td>
				</tr>
			</table>
			<%end if%>
<%end if%>
...
<%
end if ' tipo_permiso="2"
%>

<font face="Verdana, Arial, Helvetica, sans-serif" size="1"><br></font></td>
</tr>
                  ...
                  <td width="202" bgcolor="#D8D8DE"><table width="95%"  border="0" align="center" cellpadding="0" cellspacing="0">
					...
					<%if sys_cierra_planificacion=false  then%>
					   <% 'response.Write(tipo_permiso)
					      if tipo_permiso="1" and bloque="1" and mensaje_cambio="" then%>
							   <td>
							      <div align="center">
