...
/****** Object:  StoredProcedure [dbo].[GENERA_CONTRATO_DOCENTE]    Script Date: 03/01/2016 13:09:03 ******/
SET ANSI_NULLS ON
GO
        ...
        begin
            ...
            if @v_num_anexo is null
             ...
             if @v_mes_actual=3 -- si es Marzo (1er Semestre- Inicio año academico)
                 begin
                     set @v_inicio_contrato='07-03-'+cast(@v_ano_actual as varchar)
                ...
                begin   
                    if @v_mes_actual=8 -- si es Agosto (2do Semestre)
                        ...
                        begin
                            set @v_inicio_contrato='07-'+cast(@v_mes_actual as varchar)+'-'+cast(@v_ano_actual as varchar)
     end 
end
...
/*
		                                ...
		                                isnull(Y.hopr_nhoras , case isnull(b.bloq_ayudantia,0) when 0 then protic.retorna_horas_seccion1(d.secc_ccod,@v_tipo_profe,e.pers_ncorr) else protic.retorna_horas_tipo_bloque(d.secc_ccod,b.bloq_ayudantia) end)
		                                 ...
		                                 B.BLOQ_CCOD
			                           FROM BLOQUES_PROFESORES A, BLOQUES_HORARIOS B, SECCIONES D, CARRERAS_DOCENTE E, asignaturas F,horas_profesores Y
			                            WHERE A.PERS_NCORR = @p_pers_ncorr
		                                        and E.CARR_CCOD = @p_carr_ccod
					                   AND E.SEDE_CCOD = @p_sede_ccod
					                            AND E.JORN_CCOD = @p_jorn_ccod
