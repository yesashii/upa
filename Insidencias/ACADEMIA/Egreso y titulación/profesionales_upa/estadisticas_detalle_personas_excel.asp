...
'Response.AddHeader "Content-Disposition", "attachment;filename=estadisticas_totales_sede_personas.xls"
...
query_UEG = "select distinct cast(f.pers_nrut as varchar)+'-'+f.pers_xdv as rut, f.pers_tape_paterno + ' ' + f.pers_tape_materno + ', ' + f.pers_tnombre as nombre,"& vbCrLf &_
	        " (select sexo_tdesc from sexos ttt where ttt.sexo_ccod=f.sexo_ccod) as sexo, protic.trunc(f.pers_fnacimiento) as nacimiento, 'Universidad' as institu, "& vbCrLf &_
            ...
            "                    and isnull(protic.trunc(tt.fecha_egreso),'') <> '') "

query_UTI= "select distinct cast(f.pers_nrut as varchar)+'-'+f.pers_xdv as rut, f.pers_tape_paterno + ' ' + f.pers_tape_materno + ', ' + f.pers_tnombre as nombre,"& vbCrLf &_
	       " (select sexo_tdesc from sexos ttt where ttt.sexo_ccod=f.sexo_ccod) as sexo, protic.trunc(f.pers_fnacimiento) as nacimiento,'Universidad' as institu,   "& vbCrLf &_
		   " '"&sede_tdesc&"' as sede, e.facu_tdesc as facultad, c.carr_tdesc as carrera,  "& vbCrLf &_
		   " (select top 1 t3.espe_tdesc  "& vbCrLf &_
           "         from alumnos tt (nolock), ofertas_academicas t2, especialidades t3  "& vbCrLf &_
           ...
           "                 and t2.tsca_ccod in (1,2,5)) "


query_PRG= " select distinct cast(f.pers_nrut as varchar)+'-'+f.pers_xdv as rut, f.pers_tape_paterno + ' ' + f.pers_tape_materno + ', ' + f.pers_tnombre as nombre,"& vbCrLf &_
	       " (select sexo_tdesc from sexos ttt where ttt.sexo_ccod=f.sexo_ccod) as sexo, protic.trunc(f.pers_fnacimiento) as nacimiento,'Universidad' as institu,   "& vbCrLf &_
		   " '"&sede_tdesc&"' as sede,  e.facu_tdesc as facultad, c.carr_tdesc as carrera, ' ' as especialidad, "& vbCrLf &_
		   " (select top 1 t4.jorn_tdesc  "& vbCrLf &_
           "         from alumnos tt (nolock), ofertas_academicas t2, especialidades t3, jornadas t4  "& vbCrLf &_
...
query_SIE= " select distinct cast(f.pers_nrut as varchar)+'-'+f.pers_xdv as rut, f.pers_tape_paterno + ' ' + f.pers_tape_materno + ', ' + f.pers_tnombre as nombre,"& vbCrLf &_
	       " (select sexo_tdesc from sexos ttt where ttt.sexo_ccod=f.sexo_ccod) as sexo, protic.trunc(f.pers_fnacimiento) as nacimiento,'Universidad' as institu,   "& vbCrLf &_
		   " '"&sede_tdesc&"' as sede,  e.facu_tdesc as facultad, c.carr_tdesc as carrera, ' '  as especialidad,  "& vbCrLf &_
		   " (select top 1 t4.jorn_tdesc  "& vbCrLf &_
           "         from alumnos tt (nolock), ofertas_academicas t2, especialidades t3, jornadas t4  "& vbCrLf &_
...
query_SIT= "select distinct cast(f.pers_nrut as varchar)+'-'+f.pers_xdv as rut, f.pers_tape_paterno + ' ' + f.pers_tape_materno + ', ' + f.pers_tnombre as nombre,"& vbCrLf &_
	       " (select sexo_tdesc from sexos ttt where ttt.sexo_ccod=f.sexo_ccod) as sexo, protic.trunc(f.pers_fnacimiento) as nacimiento,'Universidad' as institu,   "& vbCrLf &_
		   " '"&sede_tdesc&"' as sede, e.facu_tdesc as facultad, c.carr_tdesc as carrera, ' '  as especialidad,   "& vbCrLf &_
		   " (select top 1 t4.jorn_tdesc  "& vbCrLf &_
           "         from alumnos tt (nolock), ofertas_academicas t2, especialidades t3, jornadas t4  "& vbCrLf &_
...
query_IEG ="select distinct cast(f.pers_nrut as varchar)+'-'+f.pers_xdv as rut, f.pers_tape_paterno + ' ' + f.pers_tape_materno + ', ' + f.pers_tnombre as nombre,"& vbCrLf &_
	       " (select sexo_tdesc from sexos ttt where ttt.sexo_ccod=f.sexo_ccod) as sexo, protic.trunc(f.pers_fnacimiento) as nacimiento,'Instituto' as institu,   "& vbCrLf &_
		   " '"&sede_tdesc&"' as sede,  e.facu_tdesc as facultad, c.carr_tdesc as carrera, ' '  as especialidad, "& vbCrLf &_
		   " (select jorn_tdesc from jornadas ttt where ttt.jorn_ccod=a.jorn_ccod) as jornada,    "& vbCrLf &_
		   "    (select case count(*) when 0 then 'NO' else 'SI' end  "& vbCrLf &_
		   "     from egresados_upa2 ttt where ttt.pers_ncorr=a.pers_ncorr and ttt.carr_ccod=a.carr_ccod  "& vbCrLf &_
...
query_ITI= " select distinct cast(f.pers_nrut as varchar)+'-'+f.pers_xdv as rut, f.pers_tape_paterno + ' ' + f.pers_tape_materno + ', ' + f.pers_tnombre as nombre,"& vbCrLf &_
	       " (select sexo_tdesc from sexos ttt where ttt.sexo_ccod=f.sexo_ccod) as sexo, protic.trunc(f.pers_fnacimiento) as nacimiento,'Instituto' as institu,   "& vbCrLf &_
		   " '"&sede_tdesc&"' as sede,  e.facu_tdesc as facultad, c.carr_tdesc as carrera, ' '  as especialidad, "& vbCrLf &_
		   " (select jorn_tdesc from jornadas ttt where ttt.jorn_ccod=a.jorn_ccod) as jornada,    "& vbCrLf &_
		   "    (select case count(*) when 0 then 'NO' else 'SI' end  "& vbCrLf &_
...
query_POG= " select distinct cast(f.pers_nrut as varchar)+'-'+f.pers_xdv as rut, f.pers_tape_paterno + ' ' + f.pers_tape_materno + ', ' + f.pers_tnombre as nombre,"& vbCrLf &_
	       " (select sexo_tdesc from sexos ttt where ttt.sexo_ccod=f.sexo_ccod) as sexo, protic.trunc(f.pers_fnacimiento) as nacimiento,'Universidad' as institu,   "& vbCrLf &_
		   " '"&sede_tdesc&"' as sede, e.facu_tdesc as facultad, c.carr_tdesc as carrera, ' '  as especialidad,  "& vbCrLf &_
		   " (select top 1 t4.jorn_tdesc  "& vbCrLf &_
           "         from alumnos tt (nolock), ofertas_academicas t2, especialidades t3, jornadas t4  "& vbCrLf &_
...
query = query_UEG & " UNION "& vbCrLf &_ 
		query_UTI & " UNION "& vbCrLf &_ 
		query_PRG & " UNION "& vbCrLf &_ 
		query_SIE & " UNION "& vbCrLf &_ 
		query_SIT & " UNION "& vbCrLf &_ 
		query_IEG & " UNION "& vbCrLf &_ 
		query_ITI & " UNION "& vbCrLf &_ 
		query_POG & " ORDER BY nombre asc"
