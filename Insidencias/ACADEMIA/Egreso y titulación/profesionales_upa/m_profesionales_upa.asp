...
pagina.Titulo = "Mantenedor ALUMNI"
'-------------------------------------------------------------------------------
...
f_alumnos.Inicializar conexion
 ...
 consulta = " select cast(a.pers_nrut as varchar) +'-'+ b.pers_xdv as rut,"& vbCrLf &_
            ...
            "                  where a.pers_nrut = aa.pers_nrut and aa.pers_ncorr=ba.pers_ncorr  "& vbCrLf &_
			"                  and ba.ofer_ncorr=ca.ofer_ncorr and ca.espe_ccod=da.espe_ccod  "& vbCrLf &_
            "                  and ba.emat_ccod in (4,8) and da.carr_ccod=a.carr_ccod )  "
 ...
 consulta = consulta &	" union                 "& vbCrLf &_
						" select cast(d.pers_nrut as varchar)+'-'+d.pers_xdv as rut,d.pers_tape_paterno + ' ' + d.pers_tape_materno + ' ' + d.pers_tnombre as alumno,  "& vbCrLf &_
						" f.emat_tdesc as estado,e.anos_ccod as realizado,protic.initCap(isnull(case pers_temail when '' then 'No Registra' else pers_temail end,'No registra')) as email, isnull(protic.trunc(pers_fnacimiento),'No Registra') as fecha_nacimiento,  "& vbCrLf &_
						" a.pers_ncorr,c.carr_ccod, 'U' as letra,  "& vbCrLf &_
						" (select top 1 jorn_ccod from alumnos tt (nolock), ofertas_academicas t2, especialidades t3  "& vbCrLf &_
						"  where tt.pers_ncorr=a.pers_ncorr and tt.ofer_ncorr=t2.ofer_ncorr and t2.espe_ccod=t3.espe_ccod  "& vbCrLf &_
						"  and t3.carr_ccod=c.carr_ccod order by t2.peri_ccod desc) as jorn_ccod,  "& vbCrLf &_
						"  protic.ultima_modificacion_cpp(d.pers_ncorr,1) as modificado   "& vbCrLf &_
						" from alumnos_salidas_intermedias a, alumni_personas d, estados_matriculas f,periodos_academicos e,  "& vbCrLf &_
						" salidas_carrera c, alumnos_salidas_carrera tt  "& vbCrLf &_
						" where a.pers_ncorr=d.pers_ncorr and a.emat_ccod=f.emat_ccod  "& vbCrLf &_
						" and a.peri_ccod=e.peri_ccod and a.saca_ncorr=c.saca_ncorr and a.pers_ncorr=tt.pers_ncorr and a.saca_ncorr=tt.saca_ncorr  "& vbCrLf &_
						" and a.emat_ccod in (4,8) " 
						if sin_carrera = "" or sin_carrera = "0" then 
							  if sin_jornada = "" or sin_jornada = "0" then 
								consulta = consulta & " and  carr_ccod='"&carr_ccod&"' "& vbCrLf &_
								                      " and cast((select top 1 jorn_ccod from alumnos tt (nolock), ofertas_academicas t2, especialidades t3  "& vbCrLf &_
													  "           where tt.pers_ncorr=a.pers_ncorr and tt.ofer_ncorr=t2.ofer_ncorr and t2.espe_ccod=t3.espe_ccod  "& vbCrLf &_
													  "           and t3.carr_ccod=c.carr_ccod order by t2.peri_ccod desc) as varchar) = '"&jorn_ccod&"'"
							  else
...
f_resumen.Inicializar conexion
 ...
 consulta = consulta &	" union                 "& vbCrLf &_
						"  select distinct a.pers_ncorr,c.carr_ccod, 'U' as entidad, "& vbCrLf &_
						" (select case count(*) when 0 then 0 else 1 end from alumnos_salidas_intermedias tt, salidas_carrera t2  "& vbCrLf &_
						"		where tt.pers_ncorr=a.pers_ncorr and tt.saca_ncorr=t2.saca_ncorr and t2.carr_ccod=c.carr_ccod and tt.emat_ccod=4) as egresado,  "& vbCrLf &_ 
						" (select case count(*) when 0 then 0 else 1 end from alumnos_salidas_intermedias tt, salidas_carrera t2 "& vbCrLf &_
						"		where tt.pers_ncorr=a.pers_ncorr and tt.saca_ncorr=t2.saca_ncorr and t2.carr_ccod=c.carr_ccod and tt.emat_ccod=8) as titulado  "& vbCrLf &_
						" from alumnos_salidas_intermedias a, alumni_personas d (nolock), estados_matriculas f,periodos_academicos e, "& vbCrLf &_
						"	  salidas_carrera c, alumnos_salidas_carrera tt "& vbCrLf &_
						" where a.pers_ncorr=d.pers_ncorr and a.emat_ccod=f.emat_ccod "& vbCrLf &_
						" and a.peri_ccod=e.peri_ccod and a.saca_ncorr=c.saca_ncorr "& vbCrLf &_
						" and a.emat_ccod in (4,8) and a.pers_ncorr=tt.pers_ncorr and a.saca_ncorr=tt.saca_ncorr "
						if sin_carrera = "" or sin_carrera = "0" then 
							  if sin_jornada = "" or sin_jornada = "0" then 
								consulta = consulta & " and  c.carr_ccod='"&carr_ccod&"' "& vbCrLf &_
								                      " and cast((select top 1 jorn_ccod from alumnos tt (nolock), ofertas_academicas t2, especialidades t3  "& vbCrLf &_
													  "           where tt.pers_ncorr=a.pers_ncorr and tt.ofer_ncorr=t2.ofer_ncorr and t2.espe_ccod=t3.espe_ccod  "& vbCrLf &_
													  "           and t3.carr_ccod=c.carr_ccod order by t2.peri_ccod desc) as varchar) = '"&jorn_ccod&"'"
...
function enviar(formulario){
           	formulario.action ="m_profesionales_upa.asp";//?matr_ncorr="+matricula+"&pers_ncorr="+pers+"&sede_ccod="+sede+"&plan_ccod="+plan+"&peri_ccod="+periodo+"&asig_ccod="+asignatura;
			formulario.submit();
}
</script>
<% f_busqueda.generaJS %>
